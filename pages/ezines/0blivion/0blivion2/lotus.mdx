...
server (that is not the CA that signed the serveró s certificate).
...
After authentication is done, the useró s session holds the username as the
...
Web useró s name from REMOTE_USER.
...
is not used in Domino authentication the form wonó t be saved without it.
...
so you cannot extract a useró s password if it is forgotten, only replace it.
...
- Server Access List Doesnó t Work with Domino
...
in each databaseó s ACL called Anonymous and grant it No Access.
...
If Anonymous isnó t listed in the ACL, Domino grants the
unregistered Web user access based on the databaseó s default access level.
