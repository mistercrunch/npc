...
@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%
     ...
     í~~~~~\
   //\\     \ 
  ô    ô     \ Enciendo la mecha...               
 ô kSh  ô     ~~~~~~~~~~~~~~~~~~~~~~*" 
  ô    ô       
   ôôôô


Hace unos años atras todo me parecia igual, al ver como personas influian
en la vida de los demas; controlados por la mentira y el engaño, al ver como
...
Maldito chiquillo estupido. Maldita rata que hace daño solo con la finalidad
de sentirse bien.  
...
enteras (y no precisamente pequeñas).
...
realidad no tenian acceso de modo oficial colßndose en ella por las noches;
...
profesor del MIT, les diera acceso prßcticamente ilimitado a esa maquina.
...
con el que ni sus diseñadores hubieran creido posibles; fue en este entorno
...
lenguajes de programacion, trabajan extensivamente con las entra╧as y el
...
hackers ya que siempre los confunden .(die cracker maldito)
 ...
 CALLING CARD--> Tarjeta de credito emitida por una compañia telefonica
 ...
 CORTAFUEGO (FIREWALL)--> Computadora que registra todos los paquetes de
                          informacion que entran en una compañia para,
...
por ofrecerle seguridad llamesele viçtimas/çlientes, bueno la teçnica ke vamos
usar es instalado keyloggers en las makina de la viçtima.
...
Existen muchos Keyloggers en Internet, estos difieren en tamaño, efectividad,
...
Bien senCillo dismuyes lo mas que se pueda el tamaño de la ventana, luego 
...
milenios de años espero que la clase de historia ke te dieron en la escuela,
...
Y sabiendo ke se trata de un virus el cual no hace daño y lo puedes ejecutar
...
ordenador Windows95 o WindowsNT, y permite compartir una sola conexión (RDSI,
Modem, Ethernet, etc.) en toda tu red local. Aunque WinGate se ejecuta en una
sola máquina Win95 o NT, se puede acceder a él desde cualquier estación con el
...
escondidas por tras de un aparente simple comando de grabaci≤n y ke en este
...
usuarios no autorizados ejecutar operaciones de privilegio en la máquina
afectada.
...
UDP. Se puede cambiar el n·mero del puerto por el que atiende el servidor en
ambos casos. Tambien se puede proteger el acceso al servidor mediante una clave.

<<<<<<¿Sinteticamente, qué es?>>>>>>
...
<<<<<<¿Como se tramsmite?>>>>>>
        ...
        Principalmente como un ejecutable de cualquier tipo con una extensión
aproximada de 122 Kb. que uno generalmente recibe a través de Internet bajo
cualquier nombre, pero no necesariamente por esta vía.


<<<<<<¿Como pueden acceder a mi makina?>>>>>>

        Puede realizarse únicamente si la computadora se conecta a una red.
Esta red puede ser del tipo Internet o una red LAN (Red de Area Local) sin
conexión a Internet.


                Precisiones técnicas de su funcionamiento
                ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

<<<<<<¿En que lenguaje de programacion fue escrito y compilado?>>>>>>

        Todos los programas del troyano fueron escritos en C++ y compilados
con el Microsoft Visual C++ Compiler. Todas las fechas de compilación del
...
Este es en sí mismo un troyano. Este programa es la parte del troyano que funciona
...
puede configurar el Servidor como así también "pegarlo" (infectar) a otros
ejecutables tal como lo hacen todos los virus.

        Cuando el programa anfitrión es infectado su código fuente es movido
"hacia abajo" y el código del troyano es colocado en la cabecera del programa.
Cuando los programas infectados son ejecutados, el troyano extracta la imagen
del programa original y engendra una nueva copia sin ningún tipo de efecto
colateral.
...
ejecutables del paquete del troyano son utilizados por éste mientras comprime/
...
un programa infectado que hizo de anfitrión. El troyano ubica las opciones
...
Crea un canal TCP/IP, le asigna (por defecto) un número de port 313337 y lo
abre para quedar "a la escucha". El troyano ejecuta entonces un bucle estandar
de Windows Dispatch Message (envío de mensaje) y de esta forma se mantiene en
la memoria de Windows como un proceso con atributo de invisible (este hace que
no aparezca una ventana en la lista de programas en Ejecución -ALT+TAB- y que
...
en forma encriptada y comienzan con la cadena de identificación "*!*QWTY?"
(sin las comillas).


        El troyano es capaz de expandir sus habilidades a través de plug-ins
...
mayor sobre el ciudadano que el que se hubiese podido soñar mientras se le 
...
búsqueda o visitas paginas, ELLOS SABEN CUANDO, COMO, CUANTO TIEMPO y
...
solo obtiene el nº de accesos sino que por lo general tambien sabe de
...
aficiones, trabajo, direccion) pero no mienten todo es según sus propias
...
mejor es tener una direccion única desde la cual la redirijas a tu
verdadero buzon.
...
ñºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºñ
ñ								       ñ	
ñ          	   http://www.anonymizer.com:8080/		       ñ 	
ñ								       ñ	
ñºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºñ
...
el puedes efectuar búsquedas simples o mucho mas complejas (mediante filtros).
...
Los buscadores ganan dinero, son los únicos que lo hacen en Inet y estan
...
Hace algún tiempo mucha gente desperto y supo que su browser (Netscape por
...
¿Pero que son las 'galletitas'? Las cookies son informacion, diseñada por
el site que visitas, util para el, pero que TU almacenas. Es una pequeña
...
interese al diseñador del site como: Tu direccion IP, la fecha y hora, un
...
diseñadas las 'cookies' hace que solo el que las origino pueda consultarlas
...
al servidor mandaras tu ID automaticamente y cada nueva búsqueda sera
almacenada con lo que al cabo de un tiempo podran saber muy fiablemente
que te interesa, asi que cuando la compañia Platano Loco quiera poner algún
...
responsabilidad, ¡yo no me hago responsable de los daños que estos scripts
puedan causar!.
...
--------------------------------------------------------------------------------
        En esta entrega les voy a hablar sobre los protocolos de comunicación,
lo que me animó a escribir estas líneas, es al poco interes mostrado a
...
Y esa es la razón de mi iniciativa para enseñar un poco de lo aprendido,
espero que les sea de su agrado. ;-)
-------------------------------------------------------------------------------
   TCP/IP (Transmisión Control Protocol/Internet Protocol) es el nombre común
que identifica a más de 100 protocolos de comunicación, usados para entrelazar
las computadoras dentro de una red.
--------------------------------------------------------------------------------
   TCP/IP se ha convertido en el estándar de Internet por su interopera-
bilidad entre diferentes tipos de computadoras, además de que la gran
mayoría de las redes públicas o privadas soportan TCP/IP como protocolo de
comunicación, permitiendo así la comunicación entre ellas.
--------------------------------------------------------------------------------
   HTTP se soporta sobre TCP, que es el protocolo de transporte de TCP/IP para
comunicaciones orientadas a conexión. Como otros protocolos de la familia
TCP/IP, utiliza el esquema cliente-servidor. El servidor permanace activo
esperando conexiones de los clientes HTTP. Una vez es solicitada una conexión,
...
con otros protocolos como SMTP(Correo Electrónico) y FTP(transferencia de
archivos), su información debe distinguirse de alguna manera de la de los demás.
Para ellos se usa un elemento de TCP/IP llamado puerto IP.
VER DOMAIN NAME SYSTEM
--------------------------------------------------------------------------------
    Se trata de un número que identifica a cada protocolo y genera una especie
...
--------------------------------------------------------------------------------
Los números reservados para las diferentes aplicaciones en Internet se llaman
"Puertos" (well known ports).
Los paquetes o tramas de cada protocolo son etiquetados con una pareja de estos
números llamados "Puerto Origen" y "Puerto Destino".
Generalmente se usa el número 80 como Puerto Origen de los paquetes procedentes
de servidor, y como Puerto Destino de los paquetes procedentes del cliente.
El otro número puede ser definido por el cliente siempre que no coinicida con
uno de los puertos bien conocidos. Por supuesto, todo esto ocurre de forma
inadvertida para el usuario final.
--------------------------------------------------------------------------------
     La información que viaje dentro de los paquetes HTTP no es otra que la
...
solo viajan paquetes de confirmación de que se ha recibido un bloque
de información y/o que se esperan más bloques.
--------------------------------------------------------------------------------
     Todas las computadoras en Internet tienen una dirección IP (Internet Protocol)
única. Esa dirección IP es un conjunto de cuatro números que pueden ser del 0
al 255 : Por Ejemplo, una dirección IP es 200.13.16.38.
--------------------------------------------------------------------------------
     Ya que ese conjunto de números es más difícil de recordar que un nombre,
claro está. Por tal razón fue creado el Domain Name System (DNS), el cual es
un sistema que relaciona la dirección IP de números con un nombre de dominio
y que nos dice donde está localizada dicha computadora. Por ejemplo,
en la dirección http://www.listindiario.com.do , por consiguiente este es el
nombre del dominio.
--------------------------------------------------------------------------------

     Para entender la estructura y tener una idea
     más clara de un dominio se debe tener en cuenta :


1) Leer de derecha a izquierda.

2) Los últimos caracteres en el ejemplo indican donde se localiza la compu-
tadora , que en este caso es República Dominicana(do).

3) Siguiendo en el orden señalado , las siguientes letras indican el tipo de
dominio, que en este caso es comercial(com).

4) A continuación está el nombre de la organización (Dominicana).
...
mayoría de las veces se usa FTP para copiar un archivo de un ordenador
...
Un médico recibe a un paciente, que se queja de estar muy agotado y ojeroso.
...
- Pero hombre -dijo el médico ya molesto- yo ya tengo la causa de su problema.
Es más que obvio. Ud., mi amigo, tiene demasiadas relaciones sexuales!
- ¿En serio, por eso? Menos mal, Doctor, me saca un peso de encima. Yo pensé
que era por la paja.
...
¿Qué hace un tipo vestido de vampiro conduciendo un tractor?
...
través del correo electrónico. Si les llega un e-mail con el
siguiente subject: "Alerta de VIRUS!!!" no lo abran. Si lo hacen el
virus alterará la segunda mitad de cualquier texto en su sistema.
   ...
   Cristo durante la crucificción:
   - Dimas, Dimas, acercate hijo mío.
   - No puedo señor, estoy atado a la cruz.
   - Gestas, Gestas, acercate hijo mío.
   - No puedo señor, tambien estoy atado.
...
relacionados con Linux. Además, los conocidos Linus Torvald y Bill Gates
harán conferencias en sus respectivos stands.

Esta sera la primera vez que el publico verá a los dos grandes personajes
hablando de sus respectivos productos al mismo tiempo. Este duelo se presenta
de forma más interesante que nunca, ya que el avance de Linux se hace cada vez
...
Ciertamente, "Melissa"no era un virus especialmente dañino. No destruia datos
...
extension inédita en la historia de los virus -con el antecedente light del "
Happy 98"-; ese fue el motivo de preocupacion.
...
empresa que invento originalmente el ICQ 99, aún ha confirmado el problema.
...
como ╙xpression Evaluator", cuyo proposito es permitir a los
...
archivo en la maquina, mientras que el último lee el archivo enviado,
...
Basta sustituir el ".\archivo_enviado.txt"por ╙xprCalc.cfm", para que
se borre este último archivo del servidor web. A partir de este momento
se podra usar öpenfile.cfm"para subir ficheros al servidor sin que
...
Humor
.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-..-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.
La llegada de un nuevo colaborador mi amigo y maestro "DarkLinux" 
.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-..-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.
Y mucho mas...
.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-..-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.
