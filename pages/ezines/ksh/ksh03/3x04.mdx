...
----------
 ...
 3.3.1       cuentas con contraseñas debiles.
 3.3.2       cuentas sin contraseñas.
 3.3.3       cuentas del sistema.
 3.3.4       los UID.
 3.3.5       los permisos.
 3.4       Escogiendo una Contraseña Segura.
 ...
 7.       CONTROL DE LAS CONTRASEÑAS
 7.1       Valores de Caducidad de las Contraseñas.
...
- El 1 es el Dueño del Archivo (UID).

- El 2 es el grupo al cual pertenece el dueño del archivo (GID).
...
| |
| |__ Estos 3 primeros, son los permisos del dueño del archivo.
...
dueño del archivo, y ningun permiso para el grupo y el resto de usuarios del
sistema.
...
Para solucionar este problema, se creo un metodo de encriptacion de contraseñas
...
contraseña, esta no se descifra sino que se compara con la que se encuentra en
el fichero /etc/passwd, si las contraseñas comparadas son iguales, entonces
...
caracteres imprimibles y despues se guardan en el archivo de contraseñas.

Por tal motivo NO se conoce ningun metodo por el cual la contraseña encriptada
pueda verse en solo texto.

Al ver que la contraseña encriptada podria llegar a ser amenazada con los
...
que se implementa con la contraseña encriptada, por tal motivo hay 4,096 formas
de que una contraseña quede encriptada. Esto quiere decir que pueden haber muchos
usuarios con la misma contraseña, pero al encriptarla el fichero que de distinto.

El salt esta basado a la hora en la cual se encripta la contraseña, ademas
podemos reconocer cual es el salt en una contraseña encriptada, ya que son los
...
Los caracteres p7 es el salt de la contraseña encriptada, asi que cuando un
usuario accede a un sistema, lo que se hace es que la contraseña que ingresa
el usuario se le aplica el salt de la contraseña que esta en el fichero /etc/passwd
...
3.3.1   Cuentas con contraseñas debiles
     ...
     contraseñas que son faciles de adivinar, hagalo lo mas pronto posible, antes
     de que un Cracker lo haga por usted y comprometa la seguridad de su sistema.


3.3.2   Cuentas sin Contraseñas
        ------------------------
     Esto es verdaderamente peligroso para su sistema, una cuenta sin contraseña
     es el camino para que cualquier persona entre a su sistema.
     Verifique en el fichero /etc/passwd si alguna cuenta NO tiene contraseña.
...
3.3.3   Cuentas del Sistema
     ...
     Es importante que verifique que estas contraseñas tenga un (*) en el lugar
     de la contraseña, si no es asi edite el fichero /etc/passwd y hagalo cuanto
     antes. Si alguna persona se entera de que estas cuentas no tienen contraseñas
...
Es muy importante para su makina que usted escoja una buena contraseña como
root, y ademas usted deberia de capacitar a sus usuarios para que igualmente
escojan una buena contraseña y no se vea afectada la seguridad de su sistema.

Asi que le dare 5 indicaciones para una correcta eleccion de su contraseña.


1.   Escoja una contraseña bastante larga, cuanto mas larga sera mas dificil
     de adivinar.

2.   NO anote la contraseña en ningun medio, ya sea electronico o impreso. Lo
     mejor es que escoja una que la pueda recordar, pero NO la ande divulgando.

3.   Escoja una contraseña con letras minusculas y mayusculas, ademas incluya
     caracteres y numeros, que vayan en una posicion intermedia.

     Ej:  piL{Xo7[

4.   NO escoja contraseñas que tengan que ver con su nombre, apellido, identificacion
...
Cuando las contraseñas estan shadow, es porque en el archivo /etc/passwd hay
una (x) o una (*) en la parte de las contraseñas. Asi que para ver las contraseñas
cifradas nos tenemos que dirigir al fichero /etc/shadow, donde realmente estan
las contraseñas de los usuarios pero logicamente encriptadas.
...
el nombre del usuario, la contraseña encriptada y la informacion de caducidad de
las contraseñas.
...
La proteccion de contraseña de acceso telefonico se controla con los archivos:
/etc/dialups y /etc/d_passwd.

El archivo /etc/dialups contiene una lista de los puertos seriales protegidos por
la contraseña de acceso telefonico.
...
al shell, y el segundo campo la contraseña. En este ejemplo ninguno de ellos
tiene contraseña, asi que no le pedira contraseña al momento de registrarce.

La contraseña para acceso telefonico se agrega mediante la opcion (-m) en el
comando passwd. La sintaxis seria asi:

passwd -m shell

Donde shell, es una shell especifica en el archivo de contraseñas de acceso telefonico.
...
Como podemos ver, el administrador le asigno una contraseña de acceso telefonico
al shell /bin/sh, asi que todos los usuarios que tengan este shell y vallan a
ingresar al sistema telefonicamente, tendran que digitar la contraseña que introdujo
...
El shell /bin/sh, ya tiene contraseña...
...
La informacion de la caducidad de las contraseñas se almacena junto con la
contraseña encriptada, como una serie de caracteres imprimibles. Los controles
se incluyen despues de la contraseña, precedidos por una coma. Los caracteres
despues de la coma representan la siguiente informacion:

- El numero de semanas en que es valida la contraseña.

- El numero minimo de semanas en las cuales el usuario no puede cambiar su
  contraseña.

- La ultima modificacion de la contraseña.
...
semanas antes de que dicha contraseña expire. y tengo 7 semanas minimas antes
...
la contraseña la proxima vez que ingrese al sistema, o puede hacer de que el
sistema no le permita cambiar la contraseña, para eso, hacemos lo siguiente:


-> Para que cambie la contraseña la proxima vez que ingrese:

kralj:UvIJn4sAv89X9z,./:302:120:Kralj Killer:/users/kralj:/bin/sh


-> Para que NO pueda cambiar la contraseña:
