...
ºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññºññº
    kSh	      kSh       kSh       kSh	    kSh       kSh	kSh	  kSh
╕,°ñ░`░ñ°,╕,°ñ░`░ñ°╕╕,°ñ░`░ñ°,╕,°ñ░`░ñ°╕╕,°ñ░`░ñ°,╕,°ñ░`░ñ°╕╕,°ñ░`░ñ°,╕,°ñ░`░
...
@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%
ñññññññññññññññññññññññññ KNOWLEDGE's SLAVES HACKER ññññññññññññññññññññññññ
@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%
...
A solo unos dias de cumplir un a±o ya de la salida de la primera entrega, ha 
...
dirigido por Θl; pero como el mismo lo reconoce, su labor es mas la de un "router" del grupo 
...
desarrollo y se encuentra soportado por una multitud de programadores comunicados a travΘs de 
...
En el 2000
~~~~~~~~~~~     
Linus Torvalds anuncia a travΘs de la lista de correo del Kernel que este fin de semana sera 
...
- Soporta memoria virtual, librerφas compartidas, carga en demanda de ejecutables, memoria 
compartida con 'copy-on-write', multiprocesamiento simΘtrico(SMP), etc.
...
en el mercado. Los drivers para hardware nuevo aparecen en cuestian de dφas.

- Soporta la mayorφa de las tarjetas de red conocidas.

- Soporta la mayorφa de las tarjetas de sonido y CD-ROMs en el mercado.
...
que es una versian gratuita de X. XFree86 soporta la mayorφa de las tarjetas de video en el 
...
Windows(Wine, todavφa en desarrollo, aunque corre casi todo el MS Office con algunos pequenos 
problemas), Macintosh(Executor), etc. TambiΘn puede correr directamente los programas de SCO 
...
simΘtrico. Incluso se estan desarrollando versiones de Linux para maquinas masivamente 
...
el cadigo fuente si no queremos; existen muchφsimos usuarios de Linux que no saben programar 
y a·n asφ estan muy contentos con el sistema.

- La comunidad de Linux nos ofrece un excelente soporte tΘcnico, muchφsimo mejor que las 
companφas de sistemas comerciales. Por poner un ejemplo... al hablar al soporte tΘcnico de 
Microsoft, una de las primeras cosas que nos preguntan es el n·mero de nuestra tarjeta de 
crΘdito. Al hablar al soporte tΘcnico de SCO, nos dicen que mejor nos venden un tutorial para 
el sistema operativo. El soporte tΘcnico de IBM frecuentemente no sabe de lo que le estamos 
hablando.
...
lφnea e impresos sobre el sistema. Mucha de la mejor documentacian para Linux forma parte del 
Linux Documentation Project(LDP), que son libros en lφnea gratuitos.
...
Debido al Θxito de Linux, respaldado por su funcionalidad y disponibilidad en todo el mundo, 
las empresas mas importantes de software han lanzado versiones de sus productos para Linux,
asφ, por ejemplo, tenemos a Oracle, Informix, Corel, Netscape, IBM, HP, etc. TambiΘn las 
...
MΘtodos de instalacion

Seg·n su eleccian del mΘtodo de instalacion necesitara de uno o dos disquettes de 3.5"HD. 
...
PCMCIA requiere el uso del diso adicional tambiΘn.

CD-ROM

Si usted tiene un CD de Red Hat Linux y un disco de arranque entonces necesitara un 
drive para disquettes de 3.5" HD o una instalacian de DOS en su maquina, asφ como un 
...
Si usted desea instalar a travΘs de una red, necesitara montar el CD-ROM de Red Hat 
Linux en una maquina que soporte el sistema de archivos ISO-9660 con extensiones de 
Rock Ridge. Dicha maquina tambiΘn debera soportar NFS. Tiene que exportar el sistema 
de archivos del CD-ROM por NFS. Necesitara tener configurado el servidor de nombres 
o conocer la direccian IP del servidor NFS, asφ como la ruta del CD-ROM exportado.
...
que utilizara. TambiΘn necesitara la ruta del directorio principal de Red Hat en el 
sitio FTP.
...
que serφa utilizada para almacenamiento de datos en el sistema instalado).

Si esta utilizando un sistema de archivos de DOS no podra utilizar los nombres largos 
de archivos de Linux para los paquetes rpm. La utilierφa RPM no tiene problemas con los 
nombres de los archivos, pero es buena idea que usted los recuerde para que sepa quΘ es 
...
Su distribucian puede incluir disco de arranque y adicional. Si no es asφ, lo primero que 
tiene que hacer es crear el disco de arranque, que utilizara durante la instalacian. 
TambiΘn necesitara un disco adicional sino esta instalando desde CD-ROM o NFS. Estos deben 
...
tiene DOS instalado en su computadora, puede arrancar la utilerφa de instalacian desde el 
CD-ROM con el comando:

\dosutils\autoboot

Tome en cuenta que a la hora de la instalacian hay ciertos lφmites acerca de los sistemas de 
archivos que soporta el kernel. Sin embargo, despuΘs de la instalacian hay soporte para todos 
...
de red. TambiΘn todos los dispositivos de mouse, SLIP, CSLIP, PPP, PLIP, emulacian de FPU, 
...
de 3.5". despuΘs tiene que ejecutar el siguiente comando (asumiendo que su drive de CD-ROM 
es d:):
...
de nuevo. Cuando Θste le pergunte por el nombre de archivo, escriba \images\supp.img. 
...
El sistema utilizara la imagen en el disquette para arrancar. Al principio vera la lφnea de 
...
parametros, anatelos. Los va a necesitar despuΘs.

DespuΘs de escribir cualesquiera parametros sean necesarios, presione ENTER para comenzar 
...
algo asφ como la memoria virtual de windows 95. Si tu computador tiene mucha memoria no 
...
se puede configurar una vez va con Linux. Lo ·nico indispensable es el arranque del sistema.
Antes de instalar LILO debes tener en cuenta que este solo se instala en discos duros con 
menos de 1024 cilindros. Con la mayorφa de los discos nuevos no es posible su instalacian a
menos que se haga una partician antes del cilindro 1024 y se instale ahi el lilo, nunca en
el MBR (Master Boot Record). Si tu disco duro tiene menos de 1024 cilindros no tendras ning·n
...
Especificarlos es tarea facil, es basicamente introducir la etiqueta que serφa linux para 
...
Deberφa ver la pregunta login: de nuevo. Esta viendo la segunda consola virtual ha entrado
...
esto deberφa darle una idea de las capacidades multiusuario del sistema.  Mientras esta 
...
Por ejemplo, para montar el CD-ROM podrφamos hacerlo de esta forma: 
...
Donde: 
ò	dispositivo: el dispositivo que se quiere montar. (p.e. /dev/cdrom ) 
ò       punto_de_montaje: Lugar en el arbol de directorios donde se quiere montar (p.e. /mnt/cdrom ) 
ò	sistema_de_ficheros:  Sistema de ficheros del dispositivo ( p.e. iso9660 ) 
ò       opciones_mnt: Opciones para montar el sistema de ficheros. Las mas importantes: 
...
noauto : salo puede ser montado explφcitamente. 
...
CONCEPTO debes ejecutar boserve.exe, pues te harφa sensible al ataque.
...
Resource: '*Rna\Codetel\huascar.f'  Password: 'ilmcmd1(É╖1@reg.msn.comregister'
...
te funciona el comando dir, íííesta infectado!!! O sea, que estas dentro.
...
Apendice - ¿Como puedo escribir mi propio plugin para Back Orifice?
   (Traduccion del original que acompaña al programa)
...
electranico. El que el uso de criptografφa permita comunicaciones secretas, que no 
...
Estos controles, ademas de ser con frecuencia in·tiles, ponen en  peligro la proteccian 
...
~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^~^			
  ...
  - ¿Que ocurre si alguien intercepta el mensaje PGP que me envφa 
     otra persona?
  - ¿Que ocurre si alguien intercepta el mensaje PGP que yo envφo 
     a otra persona?
...
¿QUE ES PGP?
  ...
  desencriptarla. Por ejemplo, un texto que diga "Hola, soy yo" quedarφa 
...
¿QUE UTILIDAD TIENE PGP?
  ...
  electronico. Cuando se envφa un mensaje de correo electronico, ese 
  ...
  podrφa tener acceso a los mensajes. Usando PGP los mensajes no viajan 
...
¿ES FIABLE PGP?
  ...
  usuarios lo usan dφa a dφa para sus comunicaciones. Con respecto a su 
  ...
  que continuamente estan al dφa de todo lo relacionado con PGP y la 
  criptografφa.
...
¿DONDE SE PUEDE CONSEGUIR?
  ...
  existe una de las mejores paginas sobre criptografφa desde donde se 
...
¿COMO FUNCIONA PGP?
  ...
  encriptarlo CON NUESTRA CLAVE P┌BLICA que le habremos proporcionado 
...
¿COMO ENVIAR UN MENSAJE ENCRIPTADO?
  ...
  pagina WEB si la tiene allφ publicada. Una vez con su archivo de clave 
...
¿COMO DESENCRIPTAR UN MENSAJE QUE ME HAN ENVIADO?
  ...
  procedemos a copiarlo φntegro al portapapeles: 
...
¿QUE SON LAS FIRMAS DIGITALES?
  ...
  garantizar que realmente proceden de la persona que lo envφa y no un 
...
archivos ocultos, aunque no asφ el directorio mismo de Front
...
Existe un potencial ataque de denegacian de servicio (de aquφ en
...
autoritario. Cuando un servidor recibe una consulta, tφpicamente
...
servidor a la escucha en el host remoto, cada reenvφo tratara de
resolver el servidor de nombres. Esto se intenta tφpicamente 3
veces, a los 0, 12 y 24 segundos. En este caso, el trafico se ve
significativamente multiplicado. Al abusar de m·ltiples
...
especifica sobre versiones de firmware o correcciones) envφe un email 
...
podrφa manipular archivos en la jerarquφa /proc/&lt;pid>/,
especφficamente el archivo 'mem', para hacer que un programa
...
mensajerφa vφa web. Debido a errores en expresiones regulares que
...
Cualquier usuario con acceso de escritura al directorio raφz de
...
MsDos. De los cerca de 30.000 virus conocidos hoy en dφa el 90% aproximadamente son virus de MsDos.
La mayoria de estos virus de MsDos que fueron tan famosos hace años dejaron de funcionar con la
...
alguno en el tamaño de los ejecutables, pese a que algunos han crecido cerca de 6Kb. 
                
Retro-virus: 
El virus fue diseñado para atacar directamente al software antivirus. Un ejemplo: Si estando el virus
activo en memoria ejecutamos el antes conocidisimo ThunderByte Antivirus, ciertos parametros son
añadidos automaticamente a la linea de comandos, de forma que el escaneo de la memoria
...
aquel entonces, realizaban la b·squeda de ficheros infectados mediante una sencilla
...
Multipartito: 
El virus infecta m·ltiples objetivos en un mismo sistema. En este caso se trata del MBR (Master
...
incompetentes dieron con lo que pudo haber sido la solucion definitiva. Aparecio la b·squeda heuristica,
...
en aparecer tecnicas *anti-heuristicas*. Pero veamos un poco mas sobre la b·squeda heuristica. Se trata
de examinar un programa en busca de signos de infeccion. Un virus, cuando infecta un ejecutable,
necesita realizar algunos cambios en el, siendo estos cambios sospechosos el objeto de la b·squeda por
...
pudiendo aplicar entonces la b·squeda por cadenas tradicional. El problema de la b·squeda heuristica
...
En mas de una ocasion alg·n conocido antivirus ha dado por infectado alg·n fichero del sistema, con la
...
compatibles con el nuevo formato de nombres largos y las nuevas funciones de b·squeda de ficheros del
sistema. 
...
memoria para el kernel y sus funciones), pero suponia un primer paso que seguramente alento a muchos
...
Los desarrolladores de anti-virus aun andaban pensando como portar sus motores de b·squeda heuristica
...
Esta claro que 16bits no son lo mismo que 32bits, ┐pero que diferencia supone esto a la hora de diseñar
...
mejora sobre un motor de polimorfismo de 16bits seria añadir los 16bits que le faltan para llegar a 32.
...
posibles instrucciones daremos con una bonita explosion combinatoria, a la que aun nos queda por añadir
...
y a referencias a BYTE, si no al resto de las instruciones y al resto de los tamaños del operador ( WORD
...
a primera vista parece, puesto que si durante esta b·squeda accedemos a una direccion protegida
...
con una de las caracteristicas mas ·tiles de Win32, que abordaremos con detalle mas adelante,
...
observar como uno de los parametros de llamada a estas API's es el tamaño del fichero. Pues bien,
tendremos que abrir el fichero indicando su tamaño mas el tamaño requerido por el virus. Pero la primera
vez que abrimos el fichero aun no sabemos si se trata de una victima valida, algunas soluciones a este
problema pasan por: 

Abrir el fichero con acceso solo de lectura y respetando su tamaño original. Una vez comprobamos
que se trata de un fichero apto para la infeccion cerramos el *FileMapping* y lo volvemos a crear,
especificando esta vez el tamaño original mas el tamaño necesario para albergar al virus. 
Otra posibilidad seria abrir el fichero y crear su imagen en memoria una sola vez, incluyendo el
tamaño del virus. Si tras inspeccionar el ejecutable descubrimos que no es apto para la infeccion,
restauramos el tamaño original utilizando la API SetEndOfFile. 
...
enormemente las labores de b·squeda, analisis y modificacion de los campos que nos interesan. 
...
Podremos verla si abrimos alg·n ejecutable con un editor hexadecimal, se trata de los caracteres "MZ". De
...
Microsoft, donde aparecen junto a detallada informacion sobre su utilizacion. El ·nico inconveniente es que
tenemos que portar todo a ensamblador, pero tambien se trata de una ventaja, si no prueba a escribir la
misma funcion en C y compara el tamaño de los ejecutables resultantes. 
...
Junto con el paquete que acompaña a este articulo encontraras los ficheros necesarios para manejar
...
Cryptography )Criptografía= Término formado a partir del griego kruptos,
[oculto[ ... significa, según el diccionario académico, [Arte de escribir con
clave secreta o de un modo enigmático[ ... Es criptográfico cualquier
procedimiento que permita a un emisor ocultar el contenido de un mensaje de
modo que sólo personas en posesión de determinada clave puedan leerlo, tras
haberlo descifrado. 


Cryptology )Criptología= Es la parte de la Criptografía que tiene por objeto
el descifrado de criptogramas cuando se ignora la clave. 


encryption )cifrado, encriptación= El cifrado es el tratamiento de un conjunto
...
Buenas= Conocido programa de libre distribución, escrito por Phil
Zimmermann, que impide, mediante técnicas de criptografía, que ficheros y
mensajes de correo electrónico puedan ser leídos por otros. Puede también
utilizarse para firmar electrónicamente un documento o un mensaje,
realizando así la autentificación del autor. 
...
pagar las facturas telefónicas.


cipherpunk )revientaclaves, revientacifrados= Persona especializada en romper
criptográficas y superar sistemas de seguridad. No confundir con cyberpunk.
...
tradicionales de usar la tecnología para controlar la sociedad.
...
de correo electrónico o servir una página web. [Daemon[ es una palabra latina
que significa [espíritu[ )bueno o malo= o [demonio[.


dialup )conexión por línea conmutada= Conexión temporal, en oposición a
conexión dedicada o permanente, establecida entre ordenadores por línea
telefónica normal. Dícese también del hecho de marcar un número de teléfono


download )bajar, descargar= En Internet proceso de transferir información
desde un servidor de información al propio ordenador personal.


hoax )bulo, camelo= Término utilizado para denominar a rumores falsos,
especialmente sobre virus inexistentes,  que se difunden por la red, a veces
con mucho éxito causando al final casi tanto daño como si se tratase de un
virus real.

key )clave= Código de signos convenidos para la transmisión de mensajes
secretos o privados.
...
killer app )aplicación asesina, aplicación rompedora= Aplicaciones Internet 
que han producido los mayores avances de la red a lo largo de sus historia. 
A finales de los años ochenta fue el correo electrónicoñ a primeros de los 
noventa la World Wide Web. _Cuál será la killer app del cambio de siglo
y de milenio_. 
...
Seg·n informaciones recibidas de personas que ya
...
de sitios web de descarga de software para Linux, y que señala
que el sitio mas popular es ftp.linux.tucows.com.

El pasado diciembre, el n·mero total de archivos descargados del
sitio ftp.linux.tucows.com era casi de 1,7 millones para todas las
distribuciones de Linux. Seg·n Tucows, Corel alcanzo el
...
Seg·n AMD, el Athlon a 1.1 Ghz sera comercializado hacia  fines del segundo 
trimestre de 2000.
...
Esta ·ltima incursion de los "Hackers" solamente afecto la pagina de esta prestigiosa
institucion de seguridad informatica, se deduce de ello que quizas fue asi por el nivel de
seguridad que deben tener estos ·ltimos que se decidio ese tipo de agresion y no se continuo
...
"RSA Security inc. hackeada. íConfienos sus datos! íRece a Ala!" o "Las chicas son
est·pidas y simples".
...
El ·ltimo compilador ANSI C/C++ y sus utilidades de linea de comandos ahora 
disponible. Gratis en el Sitio Web de Borland. 

Inprise/Borland (Nasdaq:INPR) anuncio la disponiblidad de su ·ltimo compilador 
...
"En los ·ltimos 11 años millones de programadores han reconocido la velocidad y 
calidad de la  tecnologia en compiladores Borland C/C++. Comenzando este mes de 
febrero, todos los desarrolladores tienen acceso gratuito al ·ltimo Borland C/C++ 
...
esta optimizado para 32-bit, que incluye los ·ltimos estandares ANSI/ISO C++ incluyendo
...
y pequeñas empresas, mismo nicho de M0k0$0ft y sus productos NT. 
...
NT, supo sacar ventaja del aparente descuido de Microsoft en los ·ltimos años. Entonces,
...
Sistemas Mainframe 
^^^^^^^^^^^^^^^^^^^
Enormes y masivos sistemas operativos, diseñados para correr aplicaciones corporativas de
...
retornaron impulsados por los grandes vol·menes de informacion y las necesidades
...
años en informatica integrada y sistemas operativos.
...
IBM es la primera y ·nica empresa que ofrece herramientas y
...
Corel solucionara el fallo de seguridad de su Linux
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
La companφa Corel esta trabajando en un parche para su ·ltima versian de Linux, con vistas a 
...
Solucionar este fallo es en estos momentos la principal prioridad de Corel, seg·n ha declarado la
propia companφa. Se espera que la companφa ponga pronto un parche en la pagina Web.
...
lo mejor de todo es GRATISíííí...
...
Que no desesperen ya solo faltan poco más de 364.000 dias para el [Efecto 3000]...
...
Pero ademas de esto, UN DICTADOR, ASESINO, alguien que tuvo por mas de 12 años
...
vol·menes y subdirectorios, pero NO USES WINDOWS. 
...
faciles de usar. Y el Usuario vio que todo conocimiento era in·til, ya
...
FUNERAL
^^^^^^^
Hay un funeral y en eso va pasando por la calle principal la carroza fúnebre y
detras un niño, que lloraba:
...
Solo me resta decir; F3L¡Z hAcK! y aquellas mentes inquietas, hackers, wannabes, 
