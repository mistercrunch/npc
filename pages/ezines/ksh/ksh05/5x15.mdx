...
tienen exito   debido a   las contrase±as   debiles, hay   todavia un   numero
...
archivos  de  contrase±a.  Ademas,  mientras en los sistemas de  ventanas  (X,
...
para  capturar   las  contrase±as).  Finalmente,  mientras   los  gusanos, los
...
íBueno!  Un solo  usuario ocioso  -- es  probable que  nadie notara  si usted
...
cuenta "guest" en  su archivo local  de contrase±as. Como  usuario guest usted
puede poner un .rhosts en el directorio home remoto del usuario guest, que  le
permitira loguearce en la maquina sin tener que proporcionar una contrase±a.
...
Si el blanco tiene  "+" comodφn en su  /etc/hosts.equiv (valor por defecto  en
...
login del blanco puede utilizar el rlogin e ingresar al blanco sin contrase±a.
...
principal del blanco y modificar el fichero de contrase±a para tener acceso de
root.
...
para arreglar la mayorφa de los problemas en los permisos de los archivos.  En
...
caso, mandando por correo el archivo de contrase±a a si mismo -- por el metodo
...
Ahora usted  espera simplemente  el fichero  de contrase±as  que se enviara de
nuevo a usted. 
...
archivo de contrase±a, o cualquier cosa que usted desee. Si su sistema  exhibe
...
file transfer  program. Este  daemon no  requiere ninguna  contrase±a para  la
...
el ejemplo, usted consigue  el archivo de contrase±as  remoto y lo pone  en su
...
archivados en ftp.uu.net). Ademas, muchos adivinan las contrase±as facilmente,
...
principal, para no mencionar las contrase±as para crackear.
...
tener ninguna contrase±a en absoluto: toda la seguridad esta en el cliente, no
...
arbitrarias del NIS (tales como  el fichero de contrase±as) cuando  usted sabe
...
fichero de contrase±as de  nuevo a maligno.com simplemente  enviando cualquier
mensaje a el: 
...
de dominio. Si NIS se usa para propagar los mapas de contrase±a, entonces  las
contrase±as shadow no dan proteccion  adicional porque el mapa shadow  todavia
...
conectar a el, vφa la funcion XOpenDisplay(); si  la funci≤n devuelve un  NULL
...
mßquinas). Usted puede determinar  a menudo el OS,  a veces bajo el  numero de
...
buena, puesto que ha habido  probablemente mas bugs de seguridad  se±alados en
...
a±adirce al final  del fichero, a  pesar de los  mensajes de error,  junto con
...
ataque tipico para conseguir el archivo de contrase±a puede parecerce a esto: 
...
En el  momento en  que escribiamos,  la versi≤n  8.6.4 del  sendmail (vease el
...
este papel nosotros usamos la palabra confianza siempre que hay una  situaci≤n
...
autenticacion de  la  contrase±a, cuando  la  autenticacion  de la  contrase±a
...
hosts.equiv  que permiten  el acceso  sin la  comprobacion de  la contrase±a;
...
Cualquier  forma  de  confianza puede  ser  enga±ada,  estafada, o  derribada,
...
victima.com, victima.com creera que la conexi≤n viene de gran.victima.com,  y,
si se  asume que  gran.victim.com esta  en /etc/hosts.equiv  o en los archivos
/.rhosts, usted podra realizar la conexion sin proveer una contrase±a. Con  el
NIS, es algo facil de corregir la base de datos del host en el NIS maestro (si
esta es  controlada por  el intruso)  o enga±ada  o forzada  el NIS  (vease la
...
confianza, usted  no sera  vulnerable por  host enga±ozos  (spoofing). La otra
...
-   Use  contrase±as shadow  y  un comando  en  el passwd  que  desapruebe las
contrase±as pobres.  Desactive o anule cuentas de usuario sin usar/inactivas.
...
seguridad tales como ejecutar el Kerberos o usar contrase±as de poco tiempo  o
...
el  conocimiento.  El  intentar  restringir  el  acceso  a  la  informaci≤n de
...
Originalmente concebido hace algunos a±os, SATAN es realmente el prototipo  de
una vision mucho mas grande y mas comprensiva de una herramienta de seguridad.
En su encarnaci≤n actual, SATAN  sondea e informa remotamente varios  fallos y
...
extensi≤n ".sat"; el programa lo ejecutara automaticamente. El programa genera
...
cien entradas  en los  rhosts de  una cuenta,  y en  unas ocasiones, el n·mero
...
ficheros rhosts,  pero el  tama±o de  los archivos  del rhosts aumentaron, asi
...
poner comentarios de estilo-shell en sus ficheros de rhosts, que la mayorφa de
...
riegos a muchos sitios (por lo menos un vendedor importante envφa sus sistemas
...
eran de hecho vulnerables al robo del archivo de contrase±as. De los 656 hosts
...
contrase±as, usando nada mas que una estacion de trabajo Sun de  bajo-extremo.
Otras 40 contrase±as se encontraron dentro  de los proximos 20 minutos; y  una
contrase±a de root se encontro durante solo una hora. El resultado despues  de
unos  dias  de cracking:  cinco  contrase±as de  root  encontradas, 19  de  24
archivos de contrase±a  (ochenta por ciento)  con por lo  menos una contrase±a
conocida, y 259 de 1594 (uno en seis) contrase±as se supusieron. 
