...
al di lα del velo di Maya, oltre Matrix,
arrivare nel mondo reale.
Quello vero.
Non per tutti.
Per noi lo Φ.
Lo Φ perchΦ Φ pi∙ affascinante, coinvolgente...nostro.
...
che ci porta sempre pi∙ in lα, oltre il buio,
imprigionati nella rete.
Scelta nostra.
Come ogni cosa facciamo.

Cerchiamo la veritα, e la troveremo.
...
Abbiamo lavorato tanto ma ci sentiamo ben appagati.Ci sono state molte perplessitα 
...
nel realizzare questo nostro sogno. Questo sarα l'ultimo numero della noflyzone in 
vekkio stile...dal prossimo numero noterete la differenza di qualitα..ci impegneremo 
ancora di + e principalemente STUDIEREMO di + .. xkΦ ricordate ke c'Φ sempre da 
...
tutti nello scrivere x raggiungere una libertα di informazione sempre maggiore ...
...
Oggi parleremo di come hackare i pub. Il nostro scopo sarα quello di installare un
ftp server privato per mezzo di un demone, chiamato Serv-u ftp Daemon, che ci potrα
...
Prima per≥ spiegher≥ un po la questione dell'unicode e di cosa possiamo riuscire a
...
Purtroppo da quando Φ uscito il magnifico virus Nimda tanti server hanno patchato
...
tramite la seguente stringa possiamo fare certe cosette:
 - vi dar≥ i pressuposti per capire come scaricare files dal server quali persino
   account ecc. ecc...
...
comando copy nella cartella del web che Φ solitamente \inetpub\ftproot\. Poi
...
ed http che trovate in /winnt/system32/Logfiles/ per gli ftp il nome della DIR Φ
questa: MSFTPSV per il web il nome della DIR Φ questa: W3svc
...
Φ la stringa di default io ne ho provati parecchi e la maggior parte circa un 70%
...
Questo Φ un altro baco che risiede prevalentemente su server non recenti, ma come ben
sappiamo sono ancora tanti quelli bacati, colpa dei SysAdmin stupidi che non patchano
il sistema. Tramite un client Tftp che potete trovare nei siti che sapete voi Φ
possibile uppare e downloadare files senza essere loggati ;) figo no, con questo
immaginate cosa Φ possibile fare, praticamente quasi tutto, per≥ c'Φ anche l'altra
faccia della medaglia.
Non Φ supportato da tutti i server :( percui l'importante Φ provare provare e
riprovare.
...
vulnerabilitα unicode sui i server microsoft scannati, o lo facciamo manualmente cosa
...
Questo Φ unpiccolo tutorial bonus che spiega come trovare gli ftp pubblici nei quali
...
Ora impostiamo l'ip di partenza dal quale il nostro Dr. Grim inizerα. F6 e mettiamo
...
altri IP ADD ranges e mettete tipo 1000 ogni ranges corrisponde a 254 ip cioΦ da
...
ci sono i log degli IP sul quale Φ installato un ftp ma non Φ anonimo o senza
...
Questo Φ un esempio naturalmente cercate di cammuffare pure i nomi dei files.
...
path dove andarα a pescare i nostri file ad es: c:\yes\ poi OK (cacchi oche palle
forse sono troppo dettagliato ;) scusate, vabbΦ portate pazienza ho quasi finito :P )
Usiamo questa stringa per trasferire il nostro file dal nostro c:\yes ad una cartella
del server remoto ricordate che GET (Φ il nostro upload) e PUT (il nostro download),
...
svolto per voi avete le basi, il resto Φ solo frutto della vostra immaginazione e
fantasia ;)
...
Nn so xkΦ mi sono deciso a scrivere tale lamerata .. fra
...
magari xkΦ il webmaster o magari l'admin lo vedi troppo
...
Questa giuda si dividerα in 2 parti... la prima analizzerα
l'hacking da windows,la seconda l'hacking da macchine unix.

La prima cosa da fare in un attakko Φ il FootPrinting!!
...
Direi che il primissimo passo nel footprinting Φ fare un
...
/dns www.sitodelpalestrato.it vi darα il corrispettivo ip
...
Scanner consigliati :
SSS: ( nn Φ un semplice portscanner , usa dei plugins nn vi
...
scansioni, che varia da o.s. a o.s. ed Φ causato dalla diversa
implementazione delle rfc, ma non sempre Φ affidabile, esistono patch per
simulare la presenza di un altro sistema operativo... anche i banner possono
essere cambiati ;), n.d.pex3)
Questo Φ ci≥ che accade con l'opzione -O di nmap.

Bene... dopo queste delucidazioni potete iniziare a scaldare
il nostro scanner ...il nostro target sarα :
...
Φ tutto stupendo ...in quanto il vecchio Bill non sebra dare
...
Ma xkΦ ti stai fossilizzando sulla 21 ?

La porta 21 Φ il servizio FTP , lo stiamo controllando x bene
xkΦ il 50% dei deface sono causa dei bug DElla 21 ... xke una
...
abbiamo visto ke Gira SU IIS 5.0 , ke Φ scritto in PHP ,
...
nn esiste la piena anonimitα .. Φ tutto al 50% di fortuna e
...
i proxy li potete settare dalle propietα di connessione e ne
...
registrarvi fatelo con dati falsi e sotto wingate mentre se giα
l'avete connettevi alla vostra shell sempre sotto un bel Sock,Kiaro ?


Qua elencher≥ diversi tutorial dove trovare informazioni a riguardo :
...
verrα visualizzata la pagina di 'login' al pannello di amministrazione,
...
[che non viene interpretato, in quanto Φ un file txt]...
...
il sysadmin li va a guardare subito se trova qualcosa di sospetto quindi Φ
...
C'Φ da dire inoltre che ogni tipo di attacco non lascia le sue tracce
...
PREMESSA : cancellare i log , nn Φ sempre una impresa facile
...
Cos∞ cerchiamo le righe contenenti il nostro ip

2 del c:\WINNT\System32\W3SVC3\in000732.log
cos∞ con il comando < del > cancelliamo il seguente log
...
Parlando di sistemi windows ( 2 p*ll* ) il file dei log Φ in esecuzione da
...
1. probabilmente, anzi di sicuro, non ti lascia modificarlo se Φ giα in uso
   da un altro processo;
2. cos∞ facendo siamo sicuri, si spera :-P, che non saremo loggati durante
   questa azione.
n.d.pex3)
...
Nn voglio essere ripetivo , la teoria Φ la stessa ; passiamo
subito alla pratica !!!
...
(non sempre Φ attendibile, ovvero: non fidatevi ciecamente di nmap, esistono
lkm che modificano lo stack tcp/ip... a proposito: anche i banner sono
modificabili cos∞ come possono essere "simulate" vulnerabilitα inesistenti..
n.d.pex3)
...
SATAN = ( o SANTA ) Φ un programma che gira sotto macchine Unix-like e richiede 
perl5.0, un browser (netscape vα bene),32 MB di ram;
questo programma consente l'analisi di reti e sottoreti di un sistema
per trovare falle e aperture


La ditta consiglia : Indubbiamente il nessus... questo pu≥ essere
un p≥ rognoso nell'installazione ma vi darα dei risultati certi!
...
Iniziamo con un p≥ di pratica : anche nel nostro amato linux direi che
lo spoofing Φ essenziale quindi...

Scanniamo un sito con il nostro fido nmap: il risultato sarα...
...
12345/tcp  open        Altro servizio ftp(No!! nn Φ il NetBus, dudes!:)

Nmap run completed -- 1 IP address (1 host up) scanned in 24 seconds

Bene... fatto ci≥ abbiamo il materiale su cui iniziare... sappiamo che
...
wu-ftpd 2.6.1?? bhΦ mica male calcolando che ultimamente Φ stato rilasciato
...
compiliamo e.... l'admin Φ fottuto!!
(pex3 consiglia www.packetstormsecurity.com ...ovviamente ;-)

Vediamo un p≥ la cosa da pi∙ vicino!!!

|---------------------------------------------|
| Wu-ftpd .... benedetto chi l'ha inventato :P|
|---------------------------------------------|

Allora.... la prima versione che intendo analizzare Φ la 2.6.0... le
...
su in qualsiasi versione di linux.... Φ da remoto...e necessita di anonymous:(
Se lo chiamate expl.c da shell basterα fare(ormai dovreste averlo capito):

gcc expl.c -o expl
./expl &lt;e i vari argomenti>

e con un p≥ di culo... siete root... ora attenti a non farvi beccare!!
...
vedere un bel casino(un miracolo per noi) nello stack!!... La shell Φ NOSTRA!
Anzi... vostra!
...
a questa) pi∙ tempo... ma la teoria Φ questa:
...
2) Cerco document.  ----> Trovo exploit per accedere al sistema e/o documentazione
                          sulle vulnerabilitα
3)ENTRO!!           -----> Facendo attenzione all'anonimitα
4)Mantengo          -----> che Φ la procedura pi∙ difficile! Quindi sotto con la
 l'accesso                 prossima parte del tutorial!! (Φ anche la pi∙ utile, a
                                                          meno che non volete 
...
Allora... la backdoor pi∙ semplice Φ aggiungere un utente con pieni
privilegi al file delle passwd... cosa che nn mi soffermo neanche ad analizzare
in quanto 1) sapete sicuramente come farlo da voi e 2) il file delle passwd Φ la
prima cosa (dopo i log:) che un admin va a guardare dopo che vi ha beccati nella sua
macchina quindi...

L'arternativa + semplice Φ il NetCat: con questo bel programmino potete restare in
ascolto su una determinata porta del sistema(consiglio una abb alta) e appena vi
ci connettete eseguire un programma... ad esempio un bel /bin/sh/ che vi ritornerα il
...
cos∞ il NetCat resterα in background in ascolto sulla 31345... e voi vi connetterete ad
esso sempre utilizzando il NC vi darα la shell bella e pronta :) semplice no??

Altra cosa importante Φ l'uso degli sniffers... cos∞, se l'admin cazzuto cambia le passwd
voi le troverete tutte nel log del tool... pronte per essere utilizzate!
Per sniffare SOLO le passwd di linux direi che il miglior tool Φ senza dubbio LinSniff
che potete trovare su packetstorm(e dove senn≥:)... mentre se volete uno sniff + "massiccio"
che cattura tutti i pacchetti che passano sulla rete potete usare lo sniffer di Clearhout
che trovate sempre(non lo dir≥ pi∙) su packetstorm! A voi la scelta... anche se secondo me
pi∙ semplici sono le cose meglio Φ... poi dipende da cosa volete fare!! ;)
...
Arrivo... arrivo... wow... ero andato un attimo a... hemm... fare pip∞
...
visto che su unix/linux possiamo utilizzare i rootkit... perchΦ non farlo??
...
santa pace... comuque non dimenticatevi MAI che non si pu≥ mai essere anonimi al
...
------------------Parliamo un p≥ degli Exploit------------------------

Come abbiamo giα detto e ripetitamo gli expoit sono :
...
mentre per gli script in perl e/o in sh non Φ neccessaria la
...
Lanciare un exploit sulla vostra makkina  Φ rischioso allora
o eseguiteli da un altra makkina in cui avete accesso oppure potrete
eseguirli da una vostra shell ...

Dove trovarli:

Nei soliti siti che giα abbiamo nominato fino alla nausea! :)
Ed ovviamente in tutti i vari siti che riguardano la sicurezza

----------------- TELNET ------------------------

Telnet nn Φ altro ke un semplicissimo client ...
quello ke fa exploit lo esegue telnet in backdoor

Potrete trovare qualke difficoltα ad usarlo bene xkΦ
usa comandi unix ... smanettate un p≥ :
...
se vi domande xkΦ dovreste usare una shell o a cosa vi serve
vi kiarisco subito il concetto ....

La shell nn Φ altro ke l'intermediario fra noi ed il kernel ...
Φ un semplice programma ke si occupa ti accettare il nostro input, e
...
Nn pensate ke vi basti avere una shell x essere tranmquilli xkΦ nn Φ cos∞
...
* Controllare se il vostro provider vi ha giα dato una shell account.
(Vari provider offrono shell account gratis)
(dove lo hai letto? sulla guida di lord shinva?
con tutto il rispetto per LordShinva, ma Φ un po' vecchiotta :-P
...
... In qiesto articolo non Φ mia intenzione addentrarmi nei meandri dell'argomento
sia perchΦ sarebbe troppo lungo, ma anche perchΦ al momento la mia competenza sui
BO non Φ delle migliori... mi sto documentando... quindi se rimanete sintonizzati
su #noflyzone (whoooo) potrete molto presto trovare un nuovo lungo tutorial in merito
...ora vi spieghero≥ solo la teoria...
...
questo, come Φ comprensibile viene allocato in memoria a partire da un indirizzo.
E verrα trattato come una normale variabile...
ma ovviamente avrα anche un indirizzo di fine... ha uno spazio limitato una variabile
!! E provate ad immaginare cosa succederebbe se questa variabile che per esempio Φ della
...
(bhΦ... non proprio un attimo) pi∙ grande del normale a sovrascrivere le istruzioni che
stanno sullo stack???? Certo che si! Se ci riusciamo... bhΦ... eseguire un bel /bin/sh
come root mi sembra il minimo, no??

OK...Ok.... non Φ cos∞ semplice... la parte pratica Φ moooooolto pi∙ complicata...ma come
vi ho detto presto troverete nuove info in merito! Quindi... occhio a
www.noflyzone-crew.cjb.net!!!!!
(naaa.. occhio a BFi, che Φ meglio.... ,n.d.pex3)

P.S. cmq per non lasciarvi al secco proprio del tutto vi anticipo qualche shellcode
faticosamente(seee come no!!) ricavato!! Lo shell code non Φ altro quello che dovete
...
BhΦ direi di iniziare da un semplice /bin/sh... sempre utile!!
...
Come ricavarli??? Vi spiegher≥ anche questo nel tutorial promesso! per ora vi basti
sapere che bisogna scrivere il codice in c... debuggarlo... vedere il corrispettivo
in assembly...e tradurlo in linguaggio macchina(che Φ praticamente uguale all'assebly
...
dal processore della macchina quindi ad esempio "int"(interrupt) diventerα \xcd...
push %esp diventerα \x54 e cos∞ via... Φ un lavoraccio ma a noi piacciono i lavoracci, no?)
...
[pex3 off the hook]
 ...
 <~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~>
 |Meditate Gente.... Meditate... Φ la cosa pi∙ utile che potete fare per il  |
...
Nessus Security Scanner Φ:

        http://www.nessus.org/

Nessus Φ a mio parere il miglior "security scanner" free (opensource e non
commerciale, cioΦ gratuito e di libero utilizzo) in circolazione.
Un security scanner Φ un programma che scansiona uno (o pi∙) host/ip alla
...
--------------------------------------------------- About this Tutorial: --
Mi sono deciso di scrivere questo tutorial perchΦ avendolo installato pi∙
...
------------------------------------------------------------ Disclaimer: --
Premettendo che ogni cosa che qui Φ scritta non Φ nulla di illegale, ma
...
Ci≥ che Φ scritto qui sopra va integrato con i disclaimer presenti all'url:

  http://www.pex3.com/disclaimer.php

in quanto da considerarsi parte integrante del sito www.pex3.com e, in caso
di contraddizion tra ci≥ di cui scritto nei disclaimer specifici di questo
testo e ci≥ di cui nei disclaimer generali del sito www.pex3.com, avrα
valore inequivocabilmente pi∙ rilevante ci≥ di cui Φ riportato all'interno
...
precompilati del sito FreeshRpms http://www.freshrpms.net/ perchΦ trovate
...
sito di nessus cos∞ siete sicuri che vada e non abbia "strane" backdoor ;-)
Io uso SuSE e consiglio comunque di installare i tar.gz, che danno pi∙
sicurezza ;-) anche se tra le distro che ho provato questa Φ quella che da
pi∙ difficoltα.

Siccome questa vuole essere una guida per i newbie sottolineo che il numero
1.0.10 potrebbe essere differente: questo non Φ un problema, probabilmente
...
--------------------------------------------------------- Installazione: --
Se avete scaricato i pacchetti precompilati (rpm) siete giα arrivati alla
fine dell'installazione (o quasi :P ), basterα infatti recarsi nella
...
e il gioco Φ fatto: il vostro nessus Φ pronto a runnare ;-) quindi passate
al prossimo paragrafo (per≥ non scordatevi di passare dal via e ritirare le
20.000 prima ;-)
...
camicie:
(quello che scrivo sulla riga dove c'Φ # sono commenti... non scriveteli!)
...
preoccupatevi se ci≥ Φ successo, succede quasi a tutti :P e proprio qui
...
- Se usate una delle distribuzioni linux: slackware, red hat e mandrake
  ...
  *Questo consiglio per risolvere il problema delle librerie Φ stato
...
- Se usate la distribuzione Linux SuSE le cose si complicano un attimo:
  ...
  nel men∙ di sinistra, dopodichΦ nella parte destra della finestra
  ...
  altrimenti fate doppio click sul nome del pacchetto finchΦ non compare,
  dopo averli selezionati tutti e quattro cliccate su "OK" e inserite i cd
  che vi domanda, riavviate il sistema (per precauzione, ma non Φ
...
- Se ottenete l'errore "undefined reference to yylex" durante
  ...
  problema con SuSE perchΦ la conosco bene, ma analogamente dovreste
  ...
  disposizione e installate i pacchetti che vi mancano direttamente da l∞.

- Se vi esce uno (strano?) errore di librerie (non trova qualcosa che ha a
  che fare con libnasl) questo Φ dovuto al fatto che le librerie sono messe
...
- I pi∙ frequenti problemi di installazione sono stati affrontati, ma
  ...
  sicuramente ci sarα qualcuno in grado di aiutarvi ;-) e magari verrα
  inserito il vostro problema nella nuova release delle F.A.Q. :o)

-------------------------------------------------- Prima Configurazione: --
La parte pi∙ difficile Φ stata superata, ora viene quella pi∙ bella :-)
Per prima cosa spiego un po' il principio di funzionamento di nessus:
- Il pacchetto nessus Φ costituito dal lato server (nessus deamon) e dal
  lato client (nessus client).
- Il server Φ un programma che permette al client di consultare il database
  dei buchi (plugins) e pu≥ essere avviato solo dall'utente root
  ...
  paragrafo, ora mi limito a spiegare come Φ composto il pacchetto nessus.
- Il client (eseguibile sia da terminale che sotto X-Windows) Φ il
  programma che l'utente utilizza per effettuare le scansioni
  interfacciandosi al nessus deamon e sarα quindi avviabile da qualunque
...
- Ovviamente visto che abbiamo un lato server e un lato client indipendenti
  esisterα un "meccanismo" che ci consentirα di loggarci al server dal
...
- Il nessus possiede un database interno con degli "user" e delle "pass
  ...
  viene creato vengono associati dei "rules", cioΦ l'elenco degli host che
  questo user pu≥ scansionare, possono essere quindi diniegati dei server o
  permessi solo degli altri, come anche si pu≥ settare (come probabilmente
  farete voi :-) di permettere la scansione di ogni server locale e remoto.

L'intento di questo tutorial Φ di insegnarvi come settare ogni opzione del
...
Come sempre il nostro poco prolisso linux non ci darα output con i
complimenti per aver aggiunto correttamente un utente, quindi accertiamoci
di ci≥ manualmente con il comando:   (eseguito sempre dall'utente root)
...
Se cos∞ non fosse avremmo certamente sbagliato qualcosa, ma supponendo che
...
"/usr/local/sbin", infatti Φ eseguibile solo da "root".
...
"chiper" Φ infatti la scelta di default:
...
vi chiede se l'utente si connetterα al server dalla macchina locale
...
Se invece inserite un username che non Φ usato da nessun utente per fare il
login al sistema vi chiede da che ip (non Φ valido l'host name) si pu≥
collegare:
...
specificate da quale/quali ip/classe di ip pu≥ connettersi.
Ora dobbiamo settare i "rules" (spiegati sopra),la sintassi corretta Φ la
seguente:
...
comune) dovremmo settare i "rules" semplicemente cos∞:
...
tasto "invio" e inserite la "pass phrase", cioΦ la password:
...
e l'utente Φ stato aggiunto correttamente! ;-)
...
Se vi dice che il file/comando Φ sconosciuto inserite tutta la path:

  /usr/local/sbin/nessusd -D

ed ora da utente "pex3" se avete scelto che il nessus user Φ un utente
...
e vi si aprirα la prima volta una finestra con una riga dove vedete dei
...
chiave per voi, mentre le volte successive che eseguirete il client uscirα
semplicemente una finestrella che vi chiede di inserire la vostra "pass
phrase", inseritela e vi si aprirα magicamente il tanto sospirato nessus
...
perchΦ sarebbe quantomeno inutile: infatti vengono costantemente potenziate
e/o aggiornate, quindi seguite il mio consiglio: provatele tutte, prima o
poi le imparerete a conoscere :-)
Se vi dice che Φ impossibile connettersi al server probabilmente il server
...
- Se hai installato il nessus da un pacchetto rpm il file con le chiavi di
  ...
  "/etc/nessus/"
  anzichΦ quella citata appena sopra.

Errore:
- La finestra dei plugins del client Φ vuota, o cos∞ sembra :-)
Soluzione:
- Connettiti al nessus server prima, poi vedrai che non sarα pi∙ vuota ;-)

Problema:
- La scansione alla ricerca dei buchi Φ molto lenta.
Soluzione:
- Nelle ultime versioni si pu≥ settare a piacimento il tempo di attesa per
  le chiamate recv(). Di default nelle ultime versioni questo timeout Φ
  settato a 15 (dalla 0.99.4 in poi, prima era settato a 5), ma per
  connessioni veloci Φ buona norma riportarlo a 5, settando il file:
  ...
  Questa modifica alla configurazione di default Φ stata apportata perchΦ
  ...
  consiglia solo ai collegamenti a velocitα (o meglio a larghezza di banda)
  ...
  molto   probabilmente vi dirα che l'host non esiste, ma solo perchΦ non ha
...
bash Φ malconfigurata e non trova il comando ricordatevi di inserire tutto
il percorso "/usr/local/bin/" prima del comando, cos∞:
...
dei bugs trovati, di default se non mettete questo parametro Φ selezionato
"nsr", quello che io preferisco Φ html_graph.

--config-file= file di configurazione del nessus per settare le varie
opzioni, pi∙ avanti Φ spiegato come crearne uno.

--batch-mode|-q mi Φ stato detto che bisogna esprimerlo, non ho ancora avuto
modo di testare nessus senza questa opzione, cmq per "sentito dire" vi
consiglio di metterlo sempre ;-D

&lt;host> Φ l'ip del nessus server a cui ci si vuole connettere (o l'hostname).

&lt;port> Φ la porta in cui Φ in ascolto il nessus server, la porta storica di
nessusd Φ 3001, ma Φ meglio usare la porta 1241, assegnata da IANA.

&lt;password> serve solo con nessus 1.1.x e 1.2.x e non si deve usare con
nessus 1.0.x!

&lt;target file> Φ il file contenenti gli ip dei server da scansionare (pu≥
contenerne anche pi∙ di uno)

&lt;results file or directory> il percorso (meglio se assoluto) e il nome del
file dove salvare l'ouput, cioΦ i risultati della scansione, il frutto di
...
immagini png o jpg, francamente non ricordo, ma non Φ molto rilevante... cmq
dovrebbero essere jpg :-D

queste sono solo le opzioni essenziali e siccome non Φ molto utile (per voi)
...
cose Φ e come crearne uno:
- innanzitutto Φ il file "centrale" della sessione di scanning, contiene
...
- crearlo Φ semplicissimo: attivando il nessus client in modalitα grafica
...
------------------------------------------------------ Upgrading Nessus: --
Ovviamente, siccome Φ uno strumento che viene aggiornato con rapiditα, in
quanto legato alla internet-security, Φ spesso doveroso fare un upgrade,
giusto per non rendere inutile la scansione, che non sarebbe in grado di
trovare bug e/o vulnerabilitα varie rese note solo dopo dell'uscita della
...
Per aggiornarlo Φ cmq molto facile: io vi consiglio di disinstallarlo e
...
- tra una release e l'altra non cambiano solo il numero di plugins, cioΦ i
  tests delle vulnerabilitα, ma tutto il sistema nessus viene reso pi∙
...
- se viene rilasciato un nuovo nessus tutto al completo ci sarα un motivo
  valido, non vi pare? :-)
...
scriveteli! :-)
  ...
  #il seguente comando Φ pericoloso: eliminerα tutto ci≥ che si trova in
  #questa directory: cioΦ tutto il contenuto di "/home/pex3/nessus/*"
  #NB. ->> Φ irreversibile <<- cioΦ i dati non saranno pi∙ recuperabili
  #quindi usate cautela!!!
  rm -r *
  #ora Φ tempo di reinstallazione:
...
sembrerα pi∙ semplice e sarete pi∙ sciolti ;-) ovviamente fate ci≥ solo se
disponete di una nuova versione di nessus! ;-D

--------------------------------- Come interpretare i risulatati: --
A questo punto l'unica cosa che manca nel tutorial Φ come interpretare gli
...
buchi, allora vi spiegherα (in inglese :-( ) di cosa si tratta e dove
...
- se da dei "warning" (avvertimenti) significa che il particolare Φ
abbastanza rilevante, ma non comporta necessariamente un bug,
- se da delle "info" non Φ nulla di speciale: versione del software
installato, fingerprinting e cose del genere: comunque utile, perchΦ se
...
Per quanto riguarda le info che nessus vi mette nel report scansione vi d≥
un suggerimento forse un po' paranoico: se il server target della scansione
(bersaglio) Φ serio, non fidatevi delle info del nessus (e nemmeno di
quelle di nmap, in particolare dello stack fingerprinting), infatti Φ
...
realtα non Φ quello che sta girando, o che venga spoofata la versione del
software in uso dal server. Molto spesso capita anche che venga simulata la
presenza di un servizio (per es. netbios su *nix) che di solito non Φ
presente su quel sistema operativo, quindi nessus in questi casi sparerα
cazzate a casaccio o, pi∙ semplicemente, vi dirα che non riesce ad eseguire
un footprinting/fingerprinting del server target.

----------------------------------------------------------- Conclusioni: --
Beh che dire: Φ finita! ;-) spero vi sia piaciuto e che continuate a
leggere i miei tutorial... devo dire che mi Φ molto piaciuto scrivere questo
tutorial (non solo per la musica GgDag rules di sottofondo :-) e quindi
presto ne scriver≥ un altro, ma ora devo pensare di cosa parlare nel
prossimo! ;-)

------------------------------------------------------------Links Utili: --
I migliori siti dove reperire informazioni su vulnerabilitα, bugs, exploit,
...
linux (kuht.it forse? :-) dicendo (a mio avviso "giustamente") che RedHat Φ
un gruviera (non proprio con questi termini :-), ora dimostro ci≥ che
sostenevo:
...
- solita installazione di default workstation/office, ma questa volta,
  come giα detto, di RedHat 7.2 (uscita addirittuta DOPO di SuSE 7.2),
- pochi minuti dopo l'installazione all-default ecco i risultati di Nessus:

7 buchi (!!!) e svariati (9) warning, nonchΦ fingerprinting esatto,
...
posso obiettare nulla: si Φ comportata banissimo, mentre SuSE ha rivelato 1
...
presentava vulnerabilitα degne di nota.
...
un "gruviera"?
(testuali parole. gruviera: qualitα di formaggio pieno di buchi :-)
oppure sarα stato il caldo di gennaio (!?) a far s∞ che RedHat abbia
...
connetterα mai (spero...) una RedHat all-default cos∞ come non
...
Forse vi chiedete perchΦ non rispondo nel suddetto forum e invece srivo la
risposta in questo tutorial... intanto il forum Φ chiuso, cos∞ come il sito
che verrα trasferito al url:

  http://www.esoh.it/

e poi per un altro buon motivo:
perchΦ questo tutorial Φ ad hoc per questo tipo di confronti, in modo da
...
- gli insistenti, ricordate: domandare e' legittimo,
  ...
  quindi evitare dei "domande-flood" (?) in chan e/o in query Φ la prima
  regolare per non ritrovarsi +b (+b Φ un chan mode non molto bello :-)
...
- noflyzone crew members, ovvero:
  ...
  [Evil],
  R|Ppy,
  BIGAlex,      (non ditegli che Φ messo malex altrimenti si arrabia: Φ solo
...
- ava666, (the best one)
- lo0ker, (vecchio nemico... hihihi ma Φ un grande ;-)
- Cujo, (all'inizio mi bannava, ora si Φ abituato a me :-)
- SuperAlbert,
- bitflesh, (amico di vecchia data)
- cavallocaruso,
- ErMaK, (no comment: e non aggiungo altro :-)
- CobWebX,
- Lottan, (che si Φ iscritto alla newsletter per accedere all'area xxx :-)
...
- Cioda, (non Φ proprio il suo nome... :-)
- IceAgitter^One,
...
Φ gratuito e consiste in un semplice redirect verso la vostra casella di
email vera e propria ;-) per intenderci Φ tipo un cjb.net delle email, ma
con il vantaggio che non viene codata nessuna pubblicitα e poi.... volete
...
vabbΦ anche se non vi fate questo account fuckate allo stesso microsoft:
infondo Φ questo ci≥ che conta ;-) ci≥ che conta Φ liberarsi da un monopolio
...
\\\\\\\\\\\\\\\\\\\\\\\\\\
|-------------------------------------------------------------------------------
| Questo articolo Φ stato scritto per puri scopi informativi e per PREVENIRE   |
| attacchi informatici. L'autore nn prende alcuna responsabilitα per l'uso     |
| sconsiderato od improprio delle informazioni contenute in queso file. Il     |
| possesso di questo documento nn Φ illegale. Utilizzarlo per scopi illegali...|
| lo Φ(bana-gelato)                                                            |
|-------------------------------------------------------------------------------
...
INTRODUZIONE
///////////////////////
Ciao a tutti!!!! Vedo con piacere che vi interessano le vulnerabilitα del PHP-NUKE
...fate bene... fate bene.... Infatti codesto (wow che paroloni) tool Φ molto diffuso
nel web! Per nostra fortuna Φ pure particolarmente buggato! UAZ UAZ! In questo tute
capirete come sfruttare le vulnerabilitα per avere la wwwroot... o cmq passwd di
...
///////////////////////////////
                                           /---------\
Allora... da dove posso iniziare?? Ma si..| index.php | andrα benissimo!!!!!
                                           \ --------/
Dovete sapere che l'exploit che and≥ ad analizzare:
...
ad un utente di "includere" un file. Grazie all'irresponsabilitα dei programmatori
Φ possibile "includere" file che provengono anche da un server remoto! E cosa pensate
che succederebbe se includessimo uno scriptuccio carino carino? Ovviameente verrebbe
eseguito con i permessi del wwwroot! Questo nn ci permettarα certo di avere una shell
(o meglio: nn subito;) ma un buon deface(urk! nn fatelo mai) nn ce lo leva nessuno!!
COSA COSA? Non conoscete il php e quindi nn sapete fare lo script? BhΦ basta creare
...
E lo upplodate sul vostro server preferito. DopodichΦ da browser basterα fare:
...
Il prossimo Φ un bug alquanto vecchio in quanto funziona esclusivamente sulla
...
Desclaimer:
Questo tutorial Φ da utilizzarsi unicamente a scopo didattico e teorico nei paesi
dove Φ previsto il pagamento per vedere la televisione via satellite... l'italia
Φ fra uno di questi quindi o andate a vivere in Alaska o vi comprate un abbonamento!
;)
...
5 a sto punto: |device->16f84|, |options->ludipipo->com1->TestHardware| e premete
  invio,|picsettings->osctype| le opzioni nella nuova finestra devono essere cos∞:
...
7 ora estraete la w@fer e mettete nel ludipipo solo l'eeprom(quello ke avete tolto
  ...
  facendo attenzione al verso (capite il verso grazie ad una piccola cavitα nel chip
  ke va messa in corrispondenza a quella dello slot)... ora passiamo alle operazioni
  con il software

8 |device->24c16| e poi |File->loadBin|(in alcuni casi Φ anche questo .hex...nn
...
Bene ora avete finito.... siete solo meno ignoranti e nn con + soldi perchΦ ricordate
che in italia Φ illegale fare h@cks@t!!!!!!!!! se nn avete capito qualcosa nn esitate
chiedermelo in chan!
...
partiamo subito ma prima mettiamo su un p≥ di musica okz, dehehe, k, si parte.....
				*/\* NoflyZone Crew */\*
allora, partiamo subito subito, senza perdere tempo, questa Φ la parte ke mancava
ed Φ anke la + significativa, il plugin elemnto essenziale per far funzionare sta
benedetta Backdoor......
-----------------------------------++++++++-------------------------------------
...
Chiaro Φ, ke il PLugIn, pu≥ essere modificato a Vs piacimento, ma l'importante Φ
...
Io Ov3rSyS affermo con sinceritα che questa guida Φ a puro scopo informativo, per
...
responsabile di ci≥ che farai dopo aver appreso queste informazioni.
...
ci≥ che bisongnerebbe fare quando si Φ all'interno di un server linux appena bucato
e si Φ root.
Diciamo per assurdo che abbiamo bucato il server fa-un.freddocane.it ed ora ci
troviamo in questa situazione
bash#
La prima cosa da fare Φ unsettare alcune proprietα (se cos∞ possono essere chiamate)
...
invisibile, perchΦ non Φ assolutamente vero, infatti le precauzioni da prendere per
...
bash#
*Nota: se siete l'user ov3r, tutto ci≥ che digitate in shell finirα nel file
...
scrivere]
bash#
in questo caso saremmo stati l'user ov3r. Ma c'Φ anche un altro comando per vedere
chi siamo, ed Φ il "whoami"
bash# whoami
root
bash#
e se siamo ov3r
...
si Φ root, quindi se lo siete bene, altrimenti trovatevi un exploit locale per
divenire da user a root.
Dopo ci≥ digitiamo "w", che ci fa vedere se ci sono degli utenti sul sistema
bash# w
7:30pm  up  1:57,  1 user,  load average: 1.14, 1.00, 0.67
USER     TTY      FROM              LOGIN@   IDLE   JCPU   PCPU  WHAT
pippo    :0       console           5:34pm   ?     0.00s   ?     -     [utente pippo]
bash#
in questo caso l'utente presente sul sistema Φ solo 1 ed Φ pippo, e voi direte :-
Come mai Φ solo 1 se ci sono anche io qui dentro? Non dovrebbe vedersi anche
l'utente root cioΦ io? - semplicemente possono capitare
entrambi i casi, ovvero questo dipende dall'exploit che avete utilizzato per bucare,
poichΦ alcuni vi nascondono al w altri no, quindi in questo caso l'exploit ci ha
nascosti, e nel caso contrario oltre a pippo si vedrebbe anche l'utente root cioΦ
noi. Quindi voi dovete controllare chi Φ loggato nella macchina, e potrebbe
capitare (dovete essere sfigati) che quando avete bucato il root sia presente quindi
potrebbe risultare ci≥:
...
pippo    :0       console           5:34pm   ?     0.00s   ?     -      [utente pippo]
bash#
quindi voi dovete controllare l'ora del login per vedere se il root che Φ loggato
dentro Φ il vero root oppure se siete voi. Ricordatevi che spesso l'admin non entra
come utente root sulla macchina, ma bens∞ come utente
normalissimo (come per esempio nei casi citati sopra potrebbe essere pippo!). Quindi
fate attenzione.
Se c'Φ il root la cosa migliore da fare sarebbe andarsene al pi∙ presto, ma non dopo
aver cancellato i vostri log (ora vi dir≥ come). Considerando che il root non ci sia,
...
per vedere la net di server in cui Φ situato il sistema su cui siamo, poi
bash# df -h
per vedere gli hardisk montati e lo spazio disponibile su di essi (Φ giα capitato di
bucare un server con gli hardisk pieni zeppi che manco un rootkit ci stava :P) poi
potete fare bash# ps -aux
per vedere i processi attivi e capire che tipo di server Φ, poi
bash# netstat -autn
per vedere le porte aperte ma soprattutto per vedere se la vostra connessione al
server Φ visibile (il 100% dei casi). Ora bisogna crearsi una backdoor sul server
bucato, e questo lo si pu≥ fare in un infinitα di modi
...
perchΦ senn≥ dovrei scrivere un libro intero!
...
/log ) queste sono le principali, ma non Φ detto che i log siano solo qui, potrebbe
...
siate nel server della Nasa o dell' Fbi (la nasa dicono che non sia poi cos∞ sicura
...
cancellarsi dai log...il pi∙ semplice Φ quello di usare un programmino come illusion
o come il t0rnsb che potete trovare facilmente nel web, per≥ questi programmini
...
da alcuni file.log giα predefiniti, e non da tutti, anzi potrebbe essere che li
cancellino da tutti ma solamente perchΦ avete avuto culo e lo avete eseguito
...
semplicemente via l'ip da ciascun file, e si pu≥ fare facendo come segue
...
bash#
questo Φ quello che c'Φ su una "mia" Red Hat (a buon intenditore poche parole :P)...
voi direte :-minkiaaaa quanti- eggiα,sono parecchi, e pensate che non sono neanche
...
essa sia presente (anche se Φ presente pi∙ volte), e con "> freddocane.tmp" gli
...
"mv freddocane.tmp cron", quindi ora il file cron Φ stato truccato, ed al suo interno
...
che l'admin sia abbastanza sveglio). Questo processo a mano pu≥ diventare
parecchio lungo e abbastanza snervante, quindi io ho preparato un bello script bash
che vi aiuterα nelle operazioni...quindi scrivete
bash# pico ov3r.sh
e se l'editor pico Φ presente vi troverete nel file ov3r.sh che Φ vuoto perchΦ lo
...
bash# ./ov3r.sh 123.4.5.6      [123.4.5.6 naturalmente Φ il vostro ip]
cos∞ avrete cancellato il vostro ip da tutti i files presenti in /var/log/ , ma
chiaramente questo script Φ solo un assaggio, perchΦ se vi rompete il culo un po'
a studiacchiarvi il bash script language come ho fatto io, potrete farne qualcuno
che cancella i log molto migliore di questo, a voi sta la volontα di provare a farlo.
Se non ci riuscite potete contattarmi tramite mail all'indirizzo che metter≥ al fondo.
...
che lo fa cancellare Φ l'ultima, quindi se volete evitare che si autocancelli alla
fine del lavoro, dovete salvare lo script senza l'ultima riga.
Oltre al metodo che cancella direttamente il nostro ip dai file, ce n'Φ un altro che
consiste nel manomettere i files di log sostituendo il vostro ip ad un altro ip
scelto da voi, perchΦ ci sono alcuni programmini che
certi admin usano, che controllano costantemente la grandezza di questi file, cos∞
...
stessa quantitα di numeri per non alterare in alcun modo la grandezza del file, ovvero
se il nostro ip Φ il 123.4.5.6 noi lo potremo sostituire con un qualsiasi altro ip
formato allo stesso modo ***.*.*.* . Il problema che penso si ponga utilizzando
questo secondo metodo, Φ che se ci sono dei log strani l'admin si accorge che
qualcuno Φ entrato nel suo sistema, cosa che magari non capiterebbe se cancellassimo
completamente le stringhe con il nostro ip dentro; quindi lascio a voi l'iniziativa.
Questo secondo metodo Φ realizzabile tramite un corto bash script creato
appositamente che non vi scrivo sempre per il solito motivo che Φ bene non abituarsi
alla pappa pronta sempre (ma solo talvolta :P) e che bisogna impegnarsi per ottenere
dei risultati, quindi impegnatevi, e se non ce la fate c'Φ un indirizzo e-mail di
una persona ben disposta ad aiutarvi...cioΦ il mio!
NOTA BENE: Ricordatevi che per rendervi invisibili al netstat ed ad altre cose come
il w, avrete bisogno di modificare i suddetti programmi o di cambiarli con altri giα
...
S∞, una cosa l'ho dimenticata...dimenticavo di dirvi che per mettere qualche file
sul vostro sistema bucato potete utilizzare a grandi linee 3 metodi, o utilizzando
il "wget" che per≥ su molti sistemi non Φ presente
...
poi vi verrα chiesto se volete scaricare il file e voi dovrete digitare D e dopo
dovrete salvare il file, comunque Φ abbastanza semplice usarlo, tutt'alpi∙ fate
...
cazzata, perchΦ capita di fare anche quello :P, e
...
IRC: contattatemi tramite e-mail e poi si vedrα
...
domanderete endo sta? embe Φ l'ultima della fila della icon bar) e ci connet-
...
Purtroppo non tutti gli ftp supportano la porta del PASV mode e quindi non Φ
...
ma viceversa (upload - download del server) Φ RARO visto che dalla versione 4.0
di microsoft ftp server il passive mode Φ disattivato di default, sarebbe una
...
di destinazione ed il gioco Φ fatto. Se questo metodo Φ supportato vedrete i fil
...
Ovviamente per supportare velocitα cos∞ elevate entrambi i server devono essere
veloci, non Φ che prendo i files da un super ftp e gli sbatto su un FTP privato
...
Saluti: NoFlyZone-Crew , Ness1 , Do^Sh1n , Tommy_ , 
        ...
        FeltonSpray , ReSiNaRo (resinaro lo ho messo per ultimo 
                                cos∞ evito prese per il culo da
                                un certo qualcuno =)
...
In questo tutorial spiegher≥ come farsi un firewall personale in 5 minuti!
strumenti necessari
...
si fanno tenere in ascolto le porte 110 , 25 e 80 ai rispettivi Winsock cos∞
facendo si potrα intercettare qualunque attacco a queste tre porte
...
Invece facendo cos∞ si dice al winsock1 che in caso di connessione alla porta 
...
e cos∞ via per le altre porte , ma mettiamo caso che noi vogliamo anche che ci 
...
Ebbene...Φ giunto il momento da molti atteso(mah...), Φ finito il tempo dei
giochi e delle cazzatele, ora si fa sul serio!;-)
Prima di iniziare voglio precisare che quello che da qui in poi illustrer≥
non Φ cosa spicciola e facile: qui siamo di fronte agli attacchi pi∙ seri e
di una certa complessitα fattibile su una rete, sia questa una LAN o Internet.
Non fate cagate, per un buon Admin sgamarvi Φ cosa abbastanza facile, quindi
aspettate di sapere tutto, prima di provare! Incito invece, a chi ne ha la
possibilitα, di provare su una LAN propria.
Quello che espongo qui oggi non Φ nulla di nuovo: Phrack, Morris e compagnia
ne hanno giα parlato e molto!Ok?
Iniziamo!
...
Far≥ una spiegazione abbastanza breve perchΦ ne ho giα abbondantemente parlato
negli altri miei tut!

IP: Φ il protocollo che fornisce un servizio di distribuzione di datagrammi
inaffidabile.
Inaffidabile perchΦ non ci Φ data la garanzia che ci≥ che spediamo arrivi e in
che ordine(se arriva).Molto importante nel campo dello spoofing Φ l'header del
pacchetto, dove sono contenute le nostre"vitali" informazioni. Non sto qui a
farvi i vari schemini del caso perchΦ ho giα messo tutto nei primi due tutz.

TCP:il nome in sΦ dovrebbe giα dirvi a cosa serve...Transmission Control
Protocol. Bravi!Avete capito...tu?Lα in fondo?Non hai capito...ok...spiego per
tutti:-) TCP Φ ci≥ che ci permette di trasferirei nostri file senza perdite di
...
...Φ anche possibile forzare le trasmissioni...ma non ci riguarda da vicino!

Qui vi immetto lo schemino perchΦ ci tornerα utile in seguito:
...
Ed ora arriviamo a ci≥ che pi∙ ci interessa:
il SEQ i l'ACK (ricordiamo anche la flag SYN che fa parte del datagramma IP...
ne parlo come se non fosse importante...mah...Φ importantissima cmq!!!)
...
ci sarebbero anche PSH(push) e URG(urgent)...ma tanto giα sapete tutto no;-)
...
il server sarα l'host  B(bersaglio?:-))
Aprite il vostro bel telnet e connettetevi...vediamo ci≥ che succede nella
...
studiare!Cmq tcpdump dovrebbe esserci in ogni distribuzione...ed Φ sicuramente
...
A:190.10.10.1
B:190.10.10.2 (li ho usati simili per comoditα...non implica che debbano essere
                nella stessa subnet)
...
sia affascinante e utile ai nostri loschi scopi(dimenticavo...Φ un tutorial...
quindi di cose da bimbi cattivi non bisogna parlare, hihihihi).
Massimo anonimato, possibilitα di scavalcare firewall(e tanto altro che lascio
alla vostra immaginazione) ...cosa volete di pi∙ dalla vita? Purtroppo tutto
questo ben di Dio non Φ proprio facile facile da ottenere.
...
Il vedente Φ un po' il fratellino minore...quello che che ancora non conosce
...
ci servono per instaurare una connessione cos∞ facendo siamo in grado di vedere
appunto ci≥ che accade e i dati che riceviamo(da qui "vedente").
...
vedere i dati che ci interessano. Ma cos'Φ uno sniffer?
Uno sniffer Φ un programma che risiede in una LAN in attesa che qualche connessione
...
Ma come funzionano in pratica? Il fattore principale per uno sniffer Φ avere accesso
root sulla macchina. Questo perchΦ quando facciamo partire il nostro gingillo questo
ha bisogno di mettere l'interfaccia di rete in modalitα PROMISCUA e creare un socket
...
Creiamo quindi una chiamata cos∞:
...
Ora mettiamo l'interfaccia di rete in modalitα promiscua. Ma perchΦ metterla in questa
modalitα vi starete chiedendo(male se non lo state facendo!:-))? Beh...
se non lo facessimo vedremmo i dati rivolti solo alla nostra macchina, invece cos∞
...
L' Hijacking Φ una versione molto interessante dello spoofing vedente. Consiste
nel dirottamento della connessione in corso e nell'inserimento di dati nostri
nella connessione che vogliamo attaccare.
Che figata vero?:-)) C'Φ da precisare(purtroppo,altrimenti sarebbe il paradiso)
che questo tipo di spoofing lo si pi≥ fare quasi unicamente quando A e C(vedi
sotto) sono nella stessa sottorete, quindi questo accade solitamente in una LAN.

Facciamo uno schemino che ormai ci≥ preso gusto:-))
...
A questo punto l'attacco funge cos∞: A inserisce nella connessione in corso tra
...
aggiornerα il suo ACK e cercherα di rispondere con un nuovo ISN. Noi
rispondiamo con un ACK all'ISN del server. Cosa succede quindi? Il client(C)
resta in stato ESTABLISHED ma sarα scoordinato rispetto al nuovo ISN del server
(A).

2)(Metodo pi∙ grezzo se vogliamo perchΦ meno occulto) Inviamo al client dei
messaggi ICMP del tipo HOST(port)_UNREACH(vedi nuker su IRC) e noi allegramente
ci impadroniamo della connessione.

3)(Quello che useremo) A connessione giα avvenuta da Client e Server ci
connettiamo al server (chiaramente con l'IP del client...se no a che serve?:-)))
incrementando cos∞ l'ACK del server.Cos∞ facendo otteniamo la desincronizzazione
...
Questo Φ il dilemma!(era l'amleto vero?Che poi mi tiro la zappa sui piedi
facendo il figo:-)))
...
esempio in FIN_WAIT1. Nella parte che seguirα questo tutz, The Implementation,
vi proporr≥ un mio telnet hijacker...se intanto volete provare qualcosa cercatevi
...
Mi trovate su IRC in #noflyzone o scrivete a Vicio che mi passerα la mail!
Buono studio!

<<<<<<<<<<<<<&lt;Hack The Planet>>>>>>>>>>>>>>>>>>>>>

P.S. i ringraziamenti: a me, perchΦ mi sto sbattendo un casino!!Bravo Ryo, continua
	cos∞!:-DDD a Crashes, Vicio, Zukky83, [V]lad, anetrip,Xp, Deli, Pit, BIGA,
...
# Ne io [Evil] ne gli autori degli exploit da mΦ offerti si assumono
# la responsabilitα delle vostre azioni, quindi badate bene a quello
# che state facendo...

questa guida Φ copyright di [Evil]
www.evil87.cjb.net

Consumo: 1/2 Litro di thΦ
...
Una shell genericamente Φ un interprete di comandi, proprio come il
dos, quelle di cui parler≥ hanno solo un utilitα in pi∙ che capirete
continuando a leggere il tutorial...

2.Account

(Qui parlo di telnet ma un client per windows buonissimo, direi ottimo
Φ Putty che permette connesioni criptate SSH e via protocollo SSL, per
...
account, questo Φ possibile via telnet ( start -> esegui "telnet )
...
scenderanno gi∙ un p≥ di righe di testo, voi continuate a dare invio
...
tasti per dare comandi) e infine per ^C premete ctrl+c (questo perchΦ
...
scrivete 'vt100' (senza gli apici) che Φ il terminale standard per
connessioni telnet.
...
che volete utilizzare, le possibilitα sono sette:
...
quella che vi consiglio Φ la bash shell, facile semplice e completa
...
per i principianti sconsiglio vivamente VI, mentre consiglio pico il perchΦ
lo vedete anche dalla descrizione.
Scrivete pico e date invio, ora vi chiede il login scrivete quello che volete
e vi verrα chiesta la password immettetene una e se viene accettata vi verrα
chiesto di riscriverla, fate ci≥ che chiede e date invio, ora vi verranno
...
se tutto va bene verrα fuori questo
...
questa Φ la bash-shell scelta prima che aspetta i vostri comandi provate questo

telnet cyberspace.org

questo Φ un minuto esempio di bouncing =)

3.Fottiamo qualche password

Prima di tutto l'anonimitα , in questo caso sfruttiamo i wingate
...
peccato che siano criptate, beh tutto Φ facile con john the ripper, copiate
quello che Φ scritto sul file etc/passwd e fatelo decriptare da questo favoloso
programma.

S≥ che questa ultima parte l'ho curata veramente poco ma solo per il fatto che
...
Penso che cos∞ possa bastare per qualsiasi problema mailatemi a
...
Alla prossima e non scordate di visitare www.evil87.cjb.net (un p≥ di spam
ci vuole ogni tanto =)
...
Il mIRC scripting , in poche parole Φ "l'arte" di creare uno script , in questo
caso basandosi sul client mIRC 5.91, per i newbies:
uno script irc Φ un client che si connette ad irc , e compie azioni che di solito
un client normale non possiede...allora perchΦ non farsene uno?
...
Io sconsiglio di fare un war script, anche se dar≥ qualche dritta per farne uno
(o ci arriverete da soli), i colori bisogna impostarli dal men∙ Tools/Colors... e
da qui impostarli , Φ logico che se lo sfondo Φ nero le scritte vanno chiare!
per la grafica ci sarα da lavorare sui remote, ma in questa prima parte non ne
parler≥, quindi aspettate la seconda parte del tutorial.
per il nome non cΦ problema, io consiglierei di dare il vostro nick seguito dalla
parola script
...
questo file Φ presente nella directory del mirc sotto nome di Aliases.ini
...
il comando per oppare un utente Φ /mode #canale +o nickname , ma negli aliases
vediamo:
...
non pi∙ di tre! perchΦ nell'alias abbiamo visto +ooo ma noi mettendo +ooooo
potremo opparne fino a cinque! +ooooooo fino a sette e cosi via...
ma mettendo il caso che lo script che vogliamo fare Φ personale, potremo aggiungere
...
amici, tizio e caio..
e se noi vogliamo che con un singolo tasto oppiamo i nostri due amici basterα usare
una delle funzioni "nascoste" degli alias ovvero /f1 /cf2 /sf3

;questo Φ un commento
...
penso che questa parte la avete capita tutti , per≥ l'alias Φ ancora povero quindi
potremo aggiungere qualcosa per renderlo pi∙ simile ad un vero script

n15=/script /say []Nome[S]cript[]

in questo modo con il comando /script, il nostro script scriverα sul canale attivo
...
n17=/date /say 0,1Oggi Φ il 9$date
...
$server   'indica il server in cui si Φ connessi
...
ora premete F2 e il gioco Φ fatto! cΦ da spiegare solo una cosa ovvero, il
carattere :
|
vale come divisorio tra una azione ed un altra.
una cosa essenziale per costruire dei buoni alias Φ quella di sapere tutti i comandi
...
/echo {text} Scrive una frase che pu≥ essere vista solo da voi
...
un p≥ fino all'arrivo della prossima parte del tutorial
...
Uno dei maggiori pericoli della rete Φ lo sniffing,una delle tecniche 
...
e password trasmessi in chiaro.Se la vostra rete nn Φ sicura o volete 
collegarvi ad un server Unix che Φ su una rete diversa dalla vostra,x 
evitare il pericolo dello sniffing,conviene effettuare connessioni 
criptate....Secure Shell (ovvero Ssh) Φ il modo migliore x criptare le 
vostre connessioni verso altri sistemi e poter navigare tranquilli..
con ssh Φ possibile effettuare il tunnelling di connessioni x11 o di a
...
La Ssh1 Φ la + vecchia la Ssh2 Φ una completa riscrittura della vecchia 
versione ed Φ pi∙ sicura.Noi cmq ci occuperemo della priva versione...

                                
               \---->       InIzIam0      <----/


Ogni host su cui Φ installato ssh possiede una coppia di chiavi RSA 
(un algoritmo di crittografia a chiave asimmetrica) lunghe 1024 bit, una pubblica 
ed una privata. In pi∙, ogni utente che utilizza ssh pu≥ opzionalmente generare 
una propria coppia di chiavi RSA.

All'atto della connessione, il server comunica al client due chiavi pubbliche:

* una fissa di 1024 bit che Φ la vera e propria chiave dell'host  ////
...
Prevede che se il sistema da cui l'utente tenta il collegamento Φ elencato in uno
dei file /etc/hosts.equiv , /etc/ssh/shosts.equiv , $HOME/.rhosts , $HOME/.shosts ,
l'accesso Φ consentito senza password.PoichΦ questo metodo comporta una scarsa protezione 
verso i tentativi di spoofing,esso Φ disabilitato x default.
...
Questo metodo Φ la combinazione tra la precedente ed una autentificazione basata
su sistema RSA.In pratica l'accesso Φ consentito dai file /etc/hosts.equiv ,
/etc/ssh/shosts.equiv , $HOME/.rhosts , $HOME/.shosts e inoltre Φ presente 
nel file  /etc/ssh_known_hosts oppure $HOME/.ssh/known_hosts la chiave ke identifica
il client ke sta tentando la connessione,allora l'accesso Φ consentito.
...
con la chiave privata dell'utente,e ne da comuicazione al server,dimostrando cos∞ di avere 
la chiave privata,cos∞ l'utente pu≥ accederesenza passw .
...
Direi che con la teoria abbiamo finito , il prossimo appuntamento riguarderα
...
-----[4.2.2] Irc sock     -------> Anonimitα in irc
...
la pazienza di leggere :) Io scrivo perkΦ mi piace! Ora Φ notte, sabato e io sono qui tutto solo
davanti a questo mukkio di scritte su un monitor:stupendo:P ! Allora inizio a dire ke questo doc
parlerα di come usare sock e proxy per connettersi a irc, per navigare anonimi, e per molti
altri utilizzi pratici :) Vi rikordo ke a me l'anonimitα piace molto, e ho scritto anke un bel
tutorial immenso sui wingate. Come ke kos'Φ un wingate -__- . Dai leggetevi il mio tutorial
...
beh cmq molti dicono ke Φ necessario, quindi ce lo metto.
...
-a terzi.Io, l'autore, non mi assumo nessuna responsabilitα.
...
Diciamo ke per me l'anonimitα in rete Φ molto importante, come tutti voi,spero, anke io voglio
...
Gia` per quato riguarda l'anonimitα ho scritto un documento sui wingate, ke potete trovare sul
...
kaso, l'identitα nella grande net. Per esempio io voglio andare a guardarmi un sitazzo porno,
...
i cgi o altri bug, e ovviamente devo naskondermi dietro a proxy. Beh credo ke sia kiara l'utilitα
...
ke sito Φ :))))) ) ma vogliamo restare con un minimo di anonimitα. Perci≥ noi ( 151.24.52.12 )
passeremo prima per il server proxy, ke si collegherα al sito da noi rikiesto, e cos∞ se il sito
logga gli accessi, troverα nei log l'ip del proxy. Molti proxy NON loggano, un buona 20% per≥
,si. Quindi attenti a quello ke fate!! Per pi∙ sicurezza Φ meglio concatenare pi∙ proxy uno
dopo l'altro.... ma ne parler≥ in seguito. La stessa cosa succede con le sock per irc:
...
Anke in questo caso, il nostro hostname in irc comparirα come 10.5.54.154, e quindi sarα quasi
...
http://atomintersoft.com/products/alive-proxy/proxy-list/?ap=6  che Φ secondo
...
sovrautilizzati, ovviamente l'adnim sarα costretto a kiuderlo. Cmq non dovete preokkuparvi ke
...
importante riguardante i proxy... Ogni ip Φ classificanto in due categorie: Anonymus e transparent.
Diciamo ke i pi∙ abbondanti e sicuri sono quellu anonymus, perkΦ permettono una copertura totale
...
delle vulnerabilitα di tutti i S.O.  :) figooo. Skusate se non centrava niente :P Diciamo ke ora
avete imparato la teoria ( o NO ??? ) ora passiamo alla pratika. Intanto ringrazzio HopeHack
dei NewBie ( irc.azzurra.org #newbie )ke mi ha fatto un gran favore... non credo ke si pu≥ dire
...
Se usate internet explorer, dovrete andare nelle opzioni del browsers. Ci si pu≥ arrivare in due
modi: Tasto destro del mouse, proprietα, sull icona di intenet explorer, o una volta aperto il
...
si aprirα un altro dialog con varie informazioni. Noterete ke li ci sono le impostazioni del
proxy.Barrate la casella "Usa un server proxy" e inserite l'ip e la porta del vostro proxy
preferito nei from sottostanti. Ora fate "applica" et voilα, ora ogni sito web ke visiterete
...
bottone :P ) . Li potete settare varie opzioni ke non Φ opportuno diskutere ora perkΦ sono le 2
di notte:). Questo metodo Φ molto veloce e automatizzato, e vi offre una buona protezione!
Ricordatevi ke i proxy ( porte 80, 8080, 3128 ) naskonod l'ip solo sull'80 ( http ). Quindi per
esempio se voi visitate un sito col proxy siete coperti, ma se sul sito c'Φ una javachat, e voi
ci entrate, non siete pi∙ coperti, perkΦ la porta in questione cambia! Un altro metodo pi∙
efficase per usare i proxy, Φ andare nelle proprietα della vostra connesione, e inserire l∞ il vostro
proxy. In questo modo lo userα automatikamente con il vostro dial-up
...
Avete avuto un intuizione? si???? ki kazzo siete ??!! Se la risposta Φ no siete pi∙ normali:)
Beh diciamo ke sapendo ke pi∙ o meno il 20% dei proxy logga, per avere una certezza di stare
anonimi, Φ molto utili concatenare diversi proxy,  ( kazzo su 10 proxy ce ne sarα uno ke non logga!).
...
c'Φ un programma molto utile ke ci aiuta a svolgere i nostri lavorozzi:) MAggiore sarα il
numero dei proxy, e maggiore sarα la lentezza... Scegliete voi tra la galera o la bassa velocitα!
...
127.0.0.1, e questo in backround usa i proxy ke voi gli immettete nella lista. Ora non sconfiner≥
troppo nelle varie funzionalitα del programma: una volta aperto, andate su Option... e successivamente
...
configurato il programma, dovrete andare nella proprietα della connessione, e inserire come
server proxy "127.0.0.1" sulla "8088". FAte ok e riavviate il browsers:) e il gioko Φ fatto.
...
riporta sulla pagina Φ uguale al vostro... Se sono diversi allora i proxy funzionanO. Buon
divertimento!!


Uhm mi sa ke ora vado a dormire ke Φ tardi... Domani continuo con i proxy e le sock in irc:) !


[4.2.2] Irc sock         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

EKkomi sveglio. =) in questo paragrafo descriver≥ come usare le sock con Mirc, perkΦ Φ il client
...
di sock: La sock4 , ke Φ lo STANDARD e la pi∙ utilizzata , e la sock5 , ke Φ utilizzata veramente
pokissimo, anzi io NON l'ho mai usata, quindi non vi so dire... SO solo ke Φ sempre sulla 1080.
Per configurarvi la sock sul mirc dovrete aprire le opzioni dal men∙ file--> option, oppure
con " alt + o ", oppure con la seconda icona in alto a destra del mirc :) Sul dialog ke vi si Φ
...
li rikieda, ma Φ molto improbabile. Nel campo della porta inserite 1080, ke per le sock Φ sempre
uguale! Bene ora clikkate su "ok", e ora, come penserete voi, abbiamo finito. ASSOLUTAMENTE NO!
Devo cominciare col dire, ke molte sock sono k-lineate da molti server irc, Φ quindi necessario
cambiare sock. Molti server invece hanno un controllo di sock on connect, ovvero quando noi ci
connettiamo a irc, controllano la nostra 1080 ( e anke altre... ) e se Φ aperta, ci killano dal
server. KazzO! Poi la maggior parte sei server accetta connessioni solo da domini uguali al suo
e da quelli internazionali. Exaple: il server irc.asdijdassa.jp , accetterα connessioni solamente
dagli host .jp, .com, .org, .net, gov ..ecc, ma non gli altri domini nazionalI ( it, uk, ar... ).
Quindi Φ ovvio ke se avete una sock .jp, non potete pretendere di connetterevi a un server italiano,
...
ci sono server come irc.stalth.net sulla 5552, ke accettano tutti gli host ;))) Non Φ un caso
se tutti gli eggdrop sono su quel server :). Invece connettersi ad aZZurranet con sock4, Φ
totalmente impossibile, perkΦ Φ uno di quei server ke ti controlla la 1080..... c'Φ cmq un modo,
e ne parler≥ dopo. Altre net di irc come webchat ( irc.webmaster.com ) accettano host provenienti
da qualsiasi parte del mondo:) . Si pu≥ stare quindi trankuilli. Ma ora tokka a voi e alla vostra
...
firewall il Mirc vi darα un messaggio del tipo
***Unable to connect to firewall
In questo kaso non c'Φ niente da fare, apparte kambiare sock o server :) Credo di essere stato
...
SUHUHSUHSA no skerzo. Beh allora a tutto questo c'Φ un alternativa...Potete maskerare il vostro
ip in irc, usando i proxy, quelli normalissimi per http di cui ho parlato prima. La convenienza
Φ ke ce ne sono veramente tantissimi, e ke usano porte diverse dalla 1080,( 8080, 80, 8088, 3128 )
e talvolta Φ possibile  ingannare le protezioni dei server:).
Per usare i proxy, il procedimento Φ uguale a quello per le sock... Nelle opzioni del firewall
...
e con dominio .net .com o .org . A volte funzionano!! Non credo ke ci sia altro da dire perkΦ
l'uso Φ uguale alle sock:)

[4.2.5] Sock WinCrt      ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Conoscete wincRT? Come nO??? Beh allora ve lo diko io cos'Φ.... ! E` un client molto simile al
telnet, ma con molte funzioni in pi∙, ke di certo non analizer≥ ora, ma ce n'Φ una in particolare
...
vi si presenterα davanti, sarα molto simile a quella del mIRC quindi non credo ke devo spiegarvi
...
Ragazzi Φ inutile ke ridete.... anke se sapete giα tutto non me ne frega un kazzo... ci siete
...
I socket sono programmati in c... e ora io non so bene a cosa servono perkΦ di programmazione non
me ne intendo, ma vi assikuro ke non centrano un kazzo con le sock:) EKko mi sono sfogato!!!


[4.4] Proxy list        ▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒

Non so perkΦ ma devo smentire tutto quello ke ho detto prima...:) sikkome sono buono buonino
...
Ciao a tutti ragα.Non avete altro da fare allora avete deciso di leggere questo txt ke avete
...
invece di seguire le lezioni di merda, mi Φ venuto in mente di scrivere questo articolo sul
...
KAZZO!!! beh cmq molti dicono ke Φ necessario, quindi ce lo metto pure io...
...
PING? PONG!
-
Ma ke kazzo Φ? si kiedeva la mia mentolina lamah. Ho un pc particolarmente sportivo ? Adora gli
sport da tavolo come il ping - pong, e ogni tanto fα 2 tiri con il server? no :)
...
mio lag".... :) Bene in quei momenti ero molto confuso perkΦ non capivo ke kazzom dicessero,
ma nemmeno quelli ke scrivevano stavano effettivamente comprendendo le kose ke dicevano:)
Ora vi spiego io ke kos'Φ il Ping. Il ping Φ un programma, ke sta su qualunkue sistema operativo,
su qualunkue makkina, 386, solaris, unix e windows. Questo programma Φ molto semplice da usare,
e spesso funziona in backrounnd, ovvero il nostro pc usa Ping senza ke noi lo sappiamo.
Ping serve per capire se una makkina in rete  Φ connessa e funzionante, e funziona in un modo
...
Beh questo Φ molto semplice direi:) E` come se i sue pc comunicassero tra di loro e si kiedessero:
...
effettivamente connessi alla rete. In caso di mancata risposta, il server kiuderα lα nostra
...
Nel caso di irc lo skerma Φ pi∙ o meno questO:
...
un client si diskonnette bruskamente dal server, magari perkΦ gli salta la corrente o perkΦ
...
Bene ora vi sembrerα strano, ma il server non vi dirα assolutamente nulla. niente di pi∙ normale!
Sta solo aspettando ke gli forniate vostri dati. Digitate quindi

user 0 0 0 0
nick ADvanced

E dopo di ke vi si visualizzerα il MOTD nella finestra. Non mi sembra difficile quello ke abbiamo
...
risposto al PING!, perkΦ non Φ impostato per farlo automatikamente!!!! quindi il server ha
creduto ke noi fossimo caduti, e ci ha diskonnesso.... Il mirc invece, come potete capire,
non Φ cos∞ diverso da Telnet, ma una differenza Φ ke mirc, appena riceve una rikiesta di ping,
risponde subio in automatico con un bel PONG! :) bene dai anke kuesta Φ fatta spero abbiate kapito.
Bene ora credo ke voi abbiate capito kosa sia il ping :) Beh ora passiamo a capire kome funziona
in realtα, ovvero ke kosa c'Φ dietro:).

[4.3]╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗* C0m3 FuNz10n4 * ╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗╗

Saoete ke kos'Φ un pakketto? Non quelli ke si inviano per posta... quelli ke viaggiano sui
protocolli di rete.... Bene avete presente? Ci sono vari tipi di pakketti ( tcp/ip, udp, icmp..)
, e il nostro ping non Φ altro ke un tipo molto semplice di pakketto icmp. Beh in questo doc
non staremo qui molto ad approfondire sui pakketti icmp, ma tra poko scrivero un txt solo su
questo argomento cos∞ lo potrete capire meglio, e sikuramente lo troverete sul sito della crew
( http://mojodo.cjb.net ) . Allora il Ping!  non Φ altro ke un pakketto ICMP ECHO REQUEST,
contenente la stringa PING!. Appena una macchina riceve questo tipo di pakketto, se tutto
funziona correttamente, e "ping" Φ correttamente installato, risponde inviando un pakketto
...
il remote host cade.... si diskonnette, perkΦ noi gli abbiamo occupato tutta la banda coni nostri
ping, e quello badabooOOm. Se voi avete adsl e volete far cadere un vostro amiko con 56 k, non
sarα una cosa facile, ma cmq gli rallenterete di molto la connesione se lo pingate continuamente.
Se invece uno col 56 k prova a pingarvi, e voi avete adsl, il lamah cadera indegnosamente :))).
Quindi avete capito come si pu≥ sfruttare questo ping per rallentare o disconnettere utenti....
Beh per≥ non sempre, anzi il + delle volte, noi non abbiamo adsl, o t1, o una shell con t3....
Quindi bisognerα rikorre ad una tecnika DoS ( denial of service ) per disconnettere macchine
remote... Questa teknika Φ kiamata smurfing ( lhaMA!! ) e ne parleremo brevemente in seguito.
...
noi per≥ ( perlomeno a me ) non ce ne frega un kazzo del 99% di quelle:). Beh per pingare un
host ( provate ora con il localhost ), basterα scrivere ping indirizzo_ip
...
Bene diciamo ke Φ tutto apposto. Tutto quadra! Di defoult il ping di windows manda 4 pakketti,
...
ovvero il suo lag. Se questo valore Φ molto alto, signifika ke ha una connesione molto lenta, e
nel caso di un server questo Φ grave, perkΦ potrebbe avere troppa banda okkupata!
...
Mi sembra kiaro ke il ping serve anke per vedere se un host in rete Φ connesso o no! In questo
caso: NO. Ma pu≥ darsi anke ke l'hos di destinazione non sia "irraggiungibile" come in questo
caso: ovvero pu≥ essere ke il nostro programma i pakketti li invia correttamente, e ke l'host
di destinazione li riceve anke, ma non risponde con un pakketto ICMP_ECHO_REPLY, quindi vuol dire
ke l'host Φ connesso, ma ha gravi problemi col networking!
Un altra possibilitα Φ ke vi dia "Rikiesta scaduta", ke non so bene cosa sia, ma cmq non Φ bello:)
Proviamo il ping ora con varie opzioni.

Beh c'Φ l'opzione -t ke Φ molto utile. Ovvero pinga l'host infinite volte, finkΦ uno dei due si
...
ping -n 6000000 127.0.0.1 manderα 6000000 ping al vostro host:)
...
Beh diciamo ke se non avete ping Φ grave ma lo potete trovare sikuramente nell'ultima release
...
Ci sono per≥ alkune sostanziali differenze da linux: una Φ ke di defoult fa infiniti ping all
host di destinazione, e per stopparlo bisogna premere ^z ( control + Z ). Un altra Φ ke la size
dei pakketti icmp Φ 56 invece ke 32. Il resto pi∙ o meno funziona alla stessa maniera...
...
Bene non credo ke debba dire altro sul funzionamento.... anzi invece s∞ avete fatto caso all'opzione
[-s packetsize] ? io si adesso:) bene, da Windows non si pu≥ sciegliere la size in bite del pakketto
icmp, mentre qui si per esempio:

root@advanced:~# ping -s 100 127.0.0.1

Manderα i pakketti da 100 byte invece ke il defoult:) . Beh diciamo ke questo ke abbiamo visto fino
adesso Φ solo uno dei modi di pare il Ping...Con l'opzione -c numeropakketti invece stabiliremo
...
diffuso Φ quello ke abbiamo descritto fino adesso, ma ce ne sono molti altri utili. Questi non si
...
linux", ora sono ankora a metα, cmq vi diko ke Φ troPPo bellO compratevelo! io l'ho preso alla
smaU 2oo1.

[4.4.1] CtCp Ping  ▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒

Beh questo non Φ un vero e proprio tipo di Ping, ma ho ritenuto giusto introdurlo ugualmente.
Diciamo ke serve in irc, per vedere il ritardo ke h una persona ( lag ), o per vedere se per kaso
Φ caduta dato ke non ci risponde + in query. Oppure ( quello ke io uso di pi∙ ) quando smurfiamo
qualkuno e vogliamo veder se ha avuto effetto! sahbhbhba ke lamah:) Cmq il Funzionamento base Φ
lo stesso
...
una risposta e ce lo comunica. Quello Φ il lag.Se Φ superiore a 200 il server ci diskonnette.
...
Semplice no? Ora sappiamo ke 	`aLe` ha 4 secondi di lag... non Φ ottimo ma va bene.
Ma magari a voi potrebbe non andarvi bene il fatto ke la gente vi pinghi,perkΦ vi da fastidio,
potete benissimo da vostro mirc cambiare la ctcp reply! Beh per fare sta cosa Φ rikiesta una
minima conoscenza di mirc scripting...Anzi no va beh Φ lostesso ve lo faccio io il code... :)
...
Capito il gioketto? Kos∞ potete fregare i LamroZZoli ke non si fanno i kazzi loro... Proviamo ora
...
non leggere perkΦ la parte successiva Φ dedicada escclusivamente al pinGuInoZZo



[4.4.2] Icmp Ping  ▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒

Sapete cos'Φ l'icmp ping? come noooooooooooooo???????????????? Φ dall'inizio di questo documento
ke ne sto parlando ;) L'icmp ping Φ quello base, il pi∙ semplice, quello ke abbiamo fatto finora.
...
Un altro tipo di ping, sebbene il termine non sia completamente accurato, pu≥ essere ottenuto
...
pu≥ supporre ke il sistema sia attivo e funzionante, e valutanto la velocit≥ di risposta, anke
...
Fping Φ un'utility di ping semplice. Invece di inviare un pakketto ICMP e rimanere in attesa di
una risposta, il comando Fping invia molti pakketti in parallelo ed elabora le risposte kuando
le riceve.La generazione di comandi massicci di ping, pertanto, Φ molto pi∙ veloce rispetto all'
esecuzione in sequenza di rikieste distinte. E` ossibile elencare esplicitamente i sistemi o gli
 indirizzi ip di cui si desidera effettuare il ping nella riga di comnado, o si pu≥ fornire l'
elenco al comando con lo standard input.Se si dispone,per esempio, di un elenco di sistemi a cui
inviare il ping nel file "machinelist", il comando potrα essere il seguente:

advancedbox$ fping -a < machinelist

Se si desidera effettuare la scansione di reti complete(per esempio la rete 192.168.10.*), si
dovrα fornire l'elenco completo di di indirizzi ip. Usando perl dalla riga di comando, la
scansione pu≥ essere eseguita facilmente nel modo ke segue:
...
Nmap, dispone di una funzionalitα integrativa di ping sweep.E` sufficente fornire un elenco di
...
La definizione di ping di Nmap,quando si usa l'opzione -sP Φ,in effetti,un po' pi∙ ampia rispetto
al comando icmp.Invierα sia un normale pakketto di echo icmp sia un pakketto tcp ACK alla porta
80(http) del sistema.Anke se ICMP Φ blokkato, il protocollo TCP generalmente potrα passare. Se
Nmap riceve un pakketto RST (reset) dall host in risposta al comando ACK, si potrα cos∞
determinare ke il sistema Φ in funzione. Nell esempio precedente abbiamo usato la notazione
92.168.10.0/24 per definire gli host di cui effettuare la scansione.Questo signifika scansionare
tutti i sistemi della rete con una subnet mask a 24 bit( ovvero la rete di classe c).Nmap pu≥
...
Uhm raga in questo periodo sono veramente inkasinato, e questa parte dei DoS Φ molto lunga da
...
Non lo so perkΦ ho fatto questa cosa. Sfogo? ke kos'Φ?? Non lo so nemmeno io.. Φ una parte di
...
ahbssausahuhsa Φ matto:) :) Saluto anke netdigger, ke adesso si starα divertendo  al meet a
...
Tranquilli non Φ dannoso, l'ho fatto milioni di volte:) Φ divertente per≥ :)  AH... ormai mi sa
...
quando accender≥ il pc ( uppare il sito, leggere il doc, provare sto programma, fare un txt,...).
...
Per me il pc Φ,oltre una pozzo senza fondo di figate ke voglio scoprire TUTTE, Φ un modo per
conoscere nuova gente, nuove persone, da ogni parte di italia o del mondo, con idee spesso diverse
dalle mie!Non Φ possibile parlare con 30 persone da ogni parte di italia fuori dal pc, Φ per questo
ke io spesso mi confronto con altra gente da ogni zona!Non vi so spiegare il concetto ma tutto
ci≥ a mio parere Φ veramente stupendo... In chat sei libero di esprimerti, urlare, sfogarti.
Nessuno pu≥ farti niente al di fuori di bananrti. In chat liberi il tuo vero ego, non esiste
...
liberoliberoliberoliberoliberoliberoliberoliberoliberoliberoliberoliberoliberoliberoliberoli
liberoliberoliberoliberoliberoliberoliberoliberoliberoliberoliberoliberoliberoliberoliberoli
Sei libero di pensare , di parlare, di dire , di esprimerti e questo Φ stupendo. Io so ke quando
...
interagire con la mia vita, non farmela passare davanti. L'importante Φ ke non passi TUTTA davanti
al monitor. Ma perkΦ ora ke fuori Φ bello e sono le 5 quasi, stop scrivendo ste cose ke voi non
leggerete?? Boh! non ne ho idea:) volevo solo far sapere ci≥ ke penso. A dimentikavo! questa
...
solo 4 volte senza mai spegnerlo! Facciamo progressi zio Billy era ora!!! Ha anke una gestione
 utenti grafika favolosa!!! finalmente mia madre non leggerα le mie cose, non runnerα i miei
programmi, non mi disinstallerα roba a kaso come faceva prima :)Ora posso tenere tutte le
immagini porno ke mi pare senza ke lei possa accedere alla cartella!uhshuauhasuhsa.
Gli ho creato un account "mamma" ke ha meno privilegi di un guest su ftp :) Pu≥ solo usare office!
...
-" Hem papa te non puoi immaginare nemmeno cosa faccio col pc, se sto in chat Φ per parlare e
   discutere con gente ke mi pu≥ aiutare....Quando io mi connetto la notte ci sono sempre minimo
...
YASGYSAGYSASA ;) Φ rimasto zitto poi :) Boh spero ke condividiate! Alla prossima ragazzi! aLe ti aM.....mazzO!
...
trattase (anke se in linea generali) un p≥ di programmazione ...
L'argomento del tutorial sarα il PERL


                                       INIZIAMO
--------------------------------------------------------------------------------

Il perl Φ un linguaggio di programmazione orientato x il web .... in pratica sarete
...
Iniziamo un p≥ di programmazione ...... (piattaforma win32)
...
print "NoFlyZone Crew  \n"; ----> il comando print ordina di stampare ci≥ che Φ
...
Nn resta ke provare il programma ... se fate doppio click si aprirα un finestra
dos ...ma nn farete in tempo e leggere il prog .. xkΦ dato che nn avrete commesso
errori , il prog si arresterα e chiudendosi automaticamente :)

Quindi potrete benissimo andare in c:\perl\bin e dare il seguente comando
perl c:\fatevoi:)\nnsaprei:).pl ..... questo sarα il risultato !
...
Scommetto ke avete giα capito tutto ma x i meno furbi vi spiego tutto x bene :)
...
$denaro = '0.00';
print "Apprendere da questo tutorial ti Φ costato solo $denaro \$ \n";
...
Avete visto che il perl nn Φ confusionario e difficili come altri linguaggi ??? :))
...
inizier≥ la seconda con le definizioni,usi e particolaritα  dei : numeri,strighe
...
creare i vostri pacchetti da soli e spedirli nella rete!Non Φ una figata?:-)
Per cui partiamo subito! Vi raccomando di studiare bene questa parte se volete
poi capire qualcosa dei vari programmi che vi mostrer≥ in seguito sullo spoofing.
Dar≥ cmq per scontato alcune parti inerenti alla teoria del TCP che trovate
nei primi due tutorial scritti da me e per l'ICMP quello scritto da [V]lad.
Premetto che non mostrer≥ quasi nulla di nuovo, quindi non rompetemi le palle
dicendomi che ho rippato o cagate varie,ok?:-)
Ultima cosa...visto lo scarso tempo a mia disposizione questo capitolo si dividerα
in due parti questa (theory) e una seconda (Implementation) in cui mostrer≥ dei
miei programmi e in cui spieger≥ il codice in C. Quindi, se giα non lo sapete,
nel frattempo studiatevelo per bene!
Iniziamo!

Prima che mi dimentichi...l'O.S. a cui far≥ riferimento Φ linux e simili...mi
...
a cui dovremo poi fare seguire la chiamata all'opzione IP_HDRINCL che ci servirα per
...
definiscono gli header dei protocolli che intendiamo usare. User≥ quindi una cosa simile:
...
Possiamo cos∞ completare il tutto con i nostri bei puntatori, cos∞:
...
Sarα cmq tutto pi∙ chiaro con gli esempi pratici del prox articolo!

----[IP Header Struct]----

Ora andiamo sotto usr/include/netinet e vediamo cosa c'Φ di bello:
...
Cosa c'Φ qui di interessante? Beh...per il problema del Byte Order vi rimando
a un altro mio tut sui socket dove Φ giα tutto bello spiegato!
Gli altri campi dovrebbero giα esservi chiari, ma ve li ripasso molto velocemente!

Version: beh...per ancora qualche tempo sarα 4:-))

IHL: sta per Internet Header Lenght...direi chiaro(preciso che Φ in 32 bit)
...
Id: Φ usato per identificare il frammento. Mi sembra ovvio che questo campo Φ
    usato solo quando i pacchetti sono frammentati.

Fragment Offset: qui il primo datagramma ha offset 0 e l'ultimo avrα valore
		 uguale al tot_len.

Ttl: quanti hop il pacchetto pu≥ fare prima che venga scartato e mandi un ICMP
     time exceed. Viene decrementato ad ogni hop di 1.

Check: header checksum utilizzato per verificare l'integritα del datagramma.
...
Il nostro compito qual Φ? Semplice...scrivere un nostro pacchetto e mandarlo in
...
(ricordo che quello che ho scritto qui sopra,come per gli altri protocolli, Φ
...
A differenza del TCP Φ molto pi∙ semplice avendo solo 4 campi.
E' una cosa simile:
...
Anche qui il checksum Φ calcolato come mostrato sopra.
...
Se per caso volete giα provare a pacioccare voi da soli vi ricordo che
...
Fate attenzione al campo udp perchΦ varia a seconda di alcune distro.
...
Vi butto solo l∞ una piccola cosa tanto da stimolarvi un po'...avete notato
che quando vi costruite i vari pacchetti inserite l'indirizzo IP di partenza e
la porta, vero?
Eheheheh...beh,il trucco Φ poi tutto l∞:-)
Dovrei ancora parlarvi della frammentazione dei pacchetti,ma sinceramente so
ancora troppe poche cose e ve ne parler≥ quando analizzeremo teardrop.c :-)

Con questo vi lascio e vi d≥ appuntamento al prox tut in cui finalmente
arriveremo a fare qualcosa di pratico! Dopo 4 tut tutti teorici forse Φ ora di
premere un po' i tasti della tastiera(voi...io li ho giα premuti abbastanza!)
e di usare un po' di gcc :-)
Il prox tut sarα sullo spoofing vedente, spero di riuscire a scrivervelo
per dopo Natale!
Colgo quindi l'occasione per fare a tutti gli Auguri di un buon Natale e
di iniziare l'anno nuovo con una bella sbronza e, perchΦ no, tra le tette di una
...
-------------------------------------------------------------- About PHP: --
Il sito di principale su cui trovare tutte le informazioni su PHP Φ:
	http://www.php.net

PHP (Personal Home Page tools, cos∞ chiamato perchΦ l'autore, Rasmus
Lerdorf, lo aveva dapprima sviluppato per uso privato per la sua homepage)
Φ un linguaggio di scripting dal lato server (server-side) integrabile
direttamente nel codice HTML.
L'ultima versione ad oggi del php parser Φ la 4.0.6 quindi far≥ riferimento
ad essa, che Φ anche la pi∙ diffusa.

-------------------------------------------------------------- Vantaggi: --
I principali vantaggi di PHP sono:
- l'interprete PHP (o processore di hypertesto PHP o parser) pu≥ essere
  integrato facilmente nel web server Apache come modulo ed Φ
  piuttosto leggero, in confronto ad altri tool.
- Φ disponibile gratuitamente (i dettagli della licenza li trovate sul
...
- funziona sui sistemi UNIX (Linux compreso) ed anche sui sistemi
  Windows, permettendovi cos∞ di portare facilmente gli script da
  un ambiente all'altro.
- Φ giα fornito di tutte le librerie per accesso ai database pi∙
  ...
  del formato gif sono a pagamento, perchΦ gif Φ un formato
...
- facilita il debugging: fa uscire gli errori direttamente sulle
  ...
  l'errore e il numero di riga su cui si Φ fermato il parser.
- pu≥ essere utilizzato anche come linguaggio di scripting al
...
scripting. PHP Φ la tecnologia alternativa ad ASP (Active Server Pages) di
...
L'installazione e la configurazione di PHP, di Apache, di mSQL o MySQL Φ un
...
per vedere se Φ giα stata data una risposta al vostro problema. C'Φ anche
...
cos∞ fortunati), allora potete passare subito al paragrafo successivo.
...
Io consiglio di usare la prima perchΦ Φ sicuro che funziona su tutti i
server, o quasi...
...
scrivere Φ Hello World. Il mio primo programma in assoluto era un "hello
world" in html, non era un vero programma, per≥ era il primo. :)
Questo banale programma vi permetterα di verificare la correttezza
...
andate all'URL corrispondente, che sarα ovviamente del tipo:

	http://hostname/percorso/prova.php

Nel mio caso, la Document Root di Apache Φ impostata a:
	/usr/local/apache/htdocs/

ed ho messo prova.php in quella directory. Quindi da web si vede come:
        http://localhost/prova.php

Se il vostro computer non Φ connesso a nessuna rete, oppure non avete
...
leggere il file prova.php. Ricordo che il comando per cambiare i permessi Φ
chmod(1), quello per cambiare possessore e gruppo di un file Φ chown(1).
...
In alto a destra c'Φ il titolo della pagina, mentre la prima riga consiste
...
linguaggi classici come il C/C++ o il Perl Φ che non bisogna utilizzare una
...
In questo senso si dice che PHP Φ "embedded in HTML", proprio come
JavaScript, con la differenza per≥ che JavaScript Φ un linguaggio lato
client, mentre PHP viene elaborato dal lato server. Un'altra differenza Φ
che, essendo embedded e dovendo essere interpretato dal client, JavaScript
Φ visibile all'utente (basta guardare i sorgenti della pagina html), mentre
...
aprirα una nuova finestra, con lynx il tasto \ fa passare al sorgente e
viceversa.

Come vedete l'utente non ha modo di conoscere il vostro codice PHP, poichΦ
...
servite tutte pagine PHP; generalmente conviene far s∞ che solo le pagine
...
processore PHP Φ innocuo: esso non trova alcuna sezione PHP da interpretare
...
come estensione per le pagine .php, non c'Φ ragione di nascondere l'uso di
php, visto che Φ gratuito, no? ;)

E' opportuno indentare opportunamente il codice html per migliorarne la
leggibilitα e questo vale ancor di pi∙ quando ci sono degli script dentro.
Nel caso di Hello Word, Φ meglio aggiungere un accapo:
...
Quest'ultima forma Φ per me preferibile poichΦ non introduce nel codice HTML
...
normale editor di testo, quale vi(m) o emacs, non Φ necessario acquistare
...
sistema visuale pu≥ generare tutto il codice per voi. Se cos∞ fosse tutti i
programmatori perderebbero il lavoro (o si metterebbero a fare questi tool
miracolosi anzichΦ pagine web). Gli ambienti integrati possono essere
utili per riunire in un unico posto tutti gli strumenti che il
programmatore deve usare; ma sono solo un'interfaccia: non c'Φ niente di
...
pu≥ anche essere racchiuso tra i tag pi∙ brevi:
	<? ?>

oppure tramite il tag &lt;script> standard di html:
	&lt;script language="php"> &lt;/script>

PoichΦ per≥ i tag <? ?> sono usati da XML, non potete usarli se volete
usare XML in combinazione con PHP pena ambiguitα ed errore e dovete quindi
...
Il punto e virgola finale, poichΦ c'Φ una sola istruzione, non Φ
necessaria, ma io preferisco usarlo sempre. Pi∙ precisamente il ; finale Φ
opzionale dopo l'ultima istruzione, ossia prima di ?> e simili. Non Φ
obbligatorio lo spazio dopo <?, <%, <%= o prima di %>, ?>, ma io preferisco
usarlo sempre, perchΦ penso migliori la leggibilitα.
...
di php, dovete riavviare apache per renderla effettiva, poichΦ per ragioni
di efficienza  il file di configurazione di php viene letto solo una volta
quando apache si avvia e i suoi settings vengono mantenuti in memoria, anzichΦ
...
La funzione echo Φ l'istruzione di output pi∙ semplice e pi∙ usata.
Attenzione per≥ che in realtα non Φ una vera e propria funzione come quelle
che pu≥ definire l'utente, ma piuttosto un costrutto cablato nel
linguaggio, e per ci≥ non si deve racchiudere la lista dei suoi argomenti
tra ( ), nel caso fossero pi∙ d'uno:

<% echo("Hello", " World!"); %>
Φ un errore, scrivete invece:
<% echo "Hello", " World!"; %>
...
separandole tramite ";", ma solo la prima espressione valutata sarα
...
Φ una forma sintattica abbreviata equivalente a:
...
fornite parametri al comando echo. Anche qui Φ un errore racchiudere i
parametri tra ( ).
...
Di queste tre invece solo la prima Φ vista correttamente dal parser:
...
Se si va accapo prima di %> anche le ultime due sono corrette. Non c'Φ
...
considera parte del commento). Probabilmente sarα corretta nelle prossime
versioni, anzi sicuramente sarα corretta, (informazioni all'url:
http://bugs.php.net/?id=8203).
Questo per dire che se trovate bachi in PHP non dovete attendere anni
finchΘ vengano corretti. Vantaggi evidenti dell'open source, che Φ in grado
...
del C (che Φ prevalentemente compilato), in PHP non si dichiarano le
variabili (per lo meno quelle "normali") e per rendere la vita facile al
parser tutti i nomi di variabile vanno prefissati con un simbolo, che Φ $.
PoichΦ non esiste una dichiarazione di variabile, come si fa allora a
...
essa sarα intera, se gli assegnate una stringa sarα una stringa, ecc...:
...
variabile cambierα e diventerα una stringa e viceversa, e cos∞ anche per
tutti gli altri tipi. Questa flessibilitα Φ tipica dei linguaggi
interpretativi.

PHP Φ case sensitive nei nomi di variabile, come il C e quindi la variabile
$FOO Φ diversa da $Foo o da $FoO. Tuttavia PHP non fa differenza di case
...
carattere di underscore e pu≥ comprendere altre lettere e cifre, il
...
Un intero ha sempre il segno; la dimensione Φ dipendente dal sistema
utilizzato; di solito il range per gli interi Φ quello che si pu≥
rappresentare in complemento a 2 con registri di macchina a 32 bit:
	[-2^31=-2147483648, 2^31-1=2147483647]

Le variabili in virgola mobile hanno un tipo detto "double" (cos∞ come
...
la keyword "integer" pu≥ essere abbreviata in "int":
...
ad $a. La seconda gli assegna ancora il valore 10, ma il tipo Φ double.
...
che converte la variabile di nome var nel tipo type che pu≥ essere una
...
$a = 10.6;
settype($a, integer);	# $a adesso Φ un intero e vale 10
...
di array, poichΦ array Φ anche il nome di un altro costrutto che vedremo).
...
tipo Φ nessuno, infatti la funzione gettype ritorna la stringa "NULL".
...
Per sapere se una variabile Φ presente nella tabella dei simboli
...
aver fatto l'unset di una variabile, finchΦ non la si inizializza o non se
ne setta il tipo, isset su quella variabile varrα sempre false. La funzione
...
// stampa 1: ogni intero !=0 usato come boolean Φ considerato vero
echo (boolean)-1;
%>
...
questo Φ conveniente poichΦ ogni valore (numerico e anche non numerico) pu≥
essere usato direttamente come valore di veritα: Φ considerato vero se Φ
...
che ai double. Se uno degli operandi Φ un double, il risultato sarα double;
se entrambi sono integer il risultato sarα intero.

Il manuale dice che la divisione / Φ una divisione intera e ritorna un
intero se i due operandi sono interi (o stringhe che vengono convertite ad
interi), mentre se uno degli operandi Φ un numero a virgola mobile, viene
effettuata la divisione tra double.

In realtα che le cose non stanno proprio cos∞ e che viene effettuata la
divisione decimale anche quando ci sono due operandi interi non divisibili.
Se volete sempre la divisione intera, Φ consigliabile fare un cast:
...
relazionali <, >, <=, >=, ==, !=, la cui sintassi Φ identica a quella del
C. Dal C, PHP prende a prestito anche l'operatore ternario ?:, la cui
sintassi consigliata Φ:

 (expr1) ? (expr2) : (expr3);

questo operatore permette di effettuare una selezione di due espressioni
(expr2 ed expr3), in base al valore di veritα (rispettivamente true e
false) della prima. L'operatore valuta e ritorna il risultato
dell'espressione selezionata, che pu≥ a sua volta far parte di una
...
Come in C, un assegnamento Φ una espressione che ritorna il valore
assegnato e l'operatore = (da non confondersi con ==), Φ associativo da
...
decrementare dell'unitα delle variabili all'interno della valutazione di
una espressione.
...
Associativitα e precedenza degli operatori Φ espressa da questa tabella,
...
nella prima colonna (ammesso che si possono associare, altrimenti Φ
...
addii non lo far≥... anche perchΦ non parto! ;)
A presto arriverα anche la seconda parte, trateerα di:
-  funzioni dei socket,
-  funzione date(),
...
- gli insistenti, ricordate: domandare e' legittimo, rispondere e' cortesia. E
  ...
  e/o sordi, quindi evitare dei "domande-flood" (?) in chan e/o in query Φ la prima
...
proprio
   ...
   Il linguaggio C Φ il fondamento dei sistemi UNIX. Un minimo di conoscenza di
   questo linguaggio Φ importante per sapersi districare tra i programmi
distribuiti
   in forma sorgente.
...
--------------------------------------------------------------------------------
   ...
   Per cominciare ad imparare il C Φ necessario che nella vostra linux box sia
   ...
   Dove sorgente Φ il vostro codice (es.: asdsa.c) , l'eseguibile e il nome che
...
grandi
   linee se date un sorgente "atry.c" verrα compilato in C se date "atry.C" il
...
--------------------------------------------------------------------------------
   ...
   comode Φ creare un MAKEFILE in grado di automatizzare questo compito.
...
--------------------------------------------------------------------------------
    ...
    Ecco i segni principali e i loro significati
     ...
     Uso  : A x B dove x Φ uno dei segni sotto elencati
...
--------------------------------------------------------------------------------
    ...
    sue variabili.
    Una variabile Φ un'area di memoria dedicata all'allocamento di dati e verra'
...
--------------------------------------------------------------------------------
     ...
     ____ if/else:
       ...
       Decide se un'operazione Φ vera o no :
     ...
     ____ do/while:
       ...
       Inizia un ciclo e ne esce fintantoche' la condizione nn Φ vera
       ...
       Scrive  CIAO fintantoche' i Φ minore di 5 e ogni volta lo incrementa

     ____ for():


       Inizia un ciclo e ne esce solo se una condizione Φ vera:
       ...
       se i < 5 se Φ minore incrementa i scrive ciao e continua altrimenti
       esce
...
-----------------------------------[C #2]--------------------------------------
 ...
 [3] Le Funzioni() 
  ...
  notare che la funzione ritorna un INT quindi Φ dichiarata con un INT, da notare
...
int main(void)
{
 ...
 for(;;)
 {
  ...
  /*
     ...
     *punt Φ un puntatore alla struttura tm definita nel file /usr/include/time.h 
  ...
  */     
  ...
  user_time=time('\0');
  punt=localtime(&user_time); // localtime Φ la funzione inclusa in time.h
  dec++;
  printf("\n%d",punt->tm_hour);
...
gruppi[...]. Un puntatore Φ un gruppo di celle che pu≥ contenere un indirizzo.
Quindi, se C Φ un char e p un puntatore che punta a C, possiamo rappresentare
...
L'operatore unario & fornisce l'indirizzo di un oggetto, perci≥ l'istruzione
...
p = q;	/* p Φ il puntatore definito all'inizio del tut */
...
Se p Φ un puntatore a una struttura, possiamo applicargli l'operatore ->
...
Ok,prometto che il prox articolo sarα sulle implementazioni dello spoofing
vedente,giuro!:-D
...
Come avete notato nn c'Φ stata nessuna interpretazione del contenuto
del comando print.....
...
il risultato sarα : lordvicio ripetuto 3 volte

$a = "lordvicio " x 3;
print '$a';

Anke con i singoli apici il risultato sarα lo stesso ...

.........potrete provare innumerevoli combinazioni..........


$a = "1" x (3*4); ----------> il risultato sarα 1 ripetuto 12 volte

$b = (10-5) x 3;  ----------> il risultato sarα 5 ripetuto 3  volte

Esercitatevi provando + combinazioni,Φ tutto molto semplice :)
...
L'uso dei numeri in perl Φ molto semplice in quanto l'interprete Perl
...
il 4 verrα elevato alla seconda,il risultato sarα 16
...
chomp ($eta);
chomp ($stupido);
print "Il tuo nome Φ $nome e hai $eta anni";
...
per≥ vi ho fornito tutto l'occorrente x potere smanettare da soli :)
...
Il prossimo appuntatamento con questa guida al perl spero nn sarα troppo
...
-----------------------------[Puntatori in C++]--------------------------------
  ...
  Un puntatore Φ una variabile che contiene un indirizzo di memoria e questo inidirizzo
  molto spesso Φ la locazione di un'altra variabile.
  ...
  es2]
      ...
      int *p
      float f;
      p= (int *) &f;   // <- Il codice ora Φ corretto perche' abbiamo castato l'indirizzo
                       //    con (int *)
 ...
 ---------------------------------------------------------------------------------------
  ...
  Da questo codice notiamo che Φ possibile assegnare ad un puntatore un intero array,
  ...
  ---------------------------------------------------------------------------------------
   ...
   dove Φ possibile creare uno struct "NODO" che contiene un valore e un puntatore
   che serve a collegare la nostra lista :)
   Questo programma Φ stato scritto e compilato con il BORLAND C++ 5.0
  ...
  In pratica Φ possibile riempire la lista inserendo in coda cioe' un elemento
  dopo l'altro oppure in testa ovvero in cima la lista.
  Poi e possibile cancellare un dato valore ed Φ possibile inserire prima di
...
Questo linguaggio Φ orientato x il web,in una prima anticipazione si 
pu≥ definire come quel linguaggio ke recupera le informazione ke 
ci vengono mostrate da un database....
Ci soffermiamo un attimo a spiegare cosa vuol dire "database"
Database comunemente tradotto in italiano "base di dati" nn Φ altro
ke un insieme di dati ke viene organizzata e gestita da un sotfware 
specifico DBMS (DataBase Management System)...
Non mi soffermo molto sul DBMS x ora vi basta sapere ke Φ un software
...
controllare l'accesso ai dati e quindi assicurarne a loro integritα..
...
"viciodb" Φ il mome del database e "ciccio" Φ il nome dell'user

se la stringa Φ esatta 

psql mostra un prompt simile al seguente:

mydb=>

A questo punto si pu≥ interragine con i comandi SQL (terminandoli 
con il comando \g x farli eseguire)..
Ma si pu≥ interragire anke in maniera diversa,ovvero invocando
invocando psql col seguente comando:

psql -f istruzioni.sql mydb vicio

In questo caso il programma esegue le istruzioni
presenti nel file .sql,cos∞ si posso ottimizzare le 
...
X ora mi fermo qua ...
Il prossimo articolo tratterα la creazione di un database :P
...
Nei casi dove "i386" Φ menzionato, se stai lavorando su una differente piattaforma,
...
directory potrebbero generare piccole somiglianze a quello che Φ presentato di seguito.

Potresti incontrare delle difficoltα con il punto 7 se hai fatto cambiamenti ai
...
Bene bene bene..... e cos∞ vorresti sapere come configurare l'adsl su linux?????
Come come????
Hai l'Alcatel USB e non sai come configuraro???? No problem guy! Questo tutorial
Φ fatto apposta per TE!!!

Premessa:
Il metodo analizzato Φ stato testato solo con mandrake 8.0 e 8.1... se funzia
...
fare Φ renderlo eseguibile: chmod +700 /root/startadsl
...
----------------------------------------------
|L'articolo Φ orientato sulla condivisione   |
...
START_SMB a 'yes'
cos∞ al prossimo reebot partirα automatica-
...
Al file ke avrete di default basterα
...
basterα aggiungere le seguenti righe
...
accedere alla condivisione,occerrerα
...
il documento Φ rivolto ad utenti alla prime armi;
...
Capita cos∞ di dover andare ad applicare delle apposite correzioni (patches) ai nostri 
...
come "diffs" (man diff!), cioΦ semplici files di testo che esprimo le differenze al codice 
...
/pub/OpenBSD/patches/. Potete cos∞ prelevare una patch alla volta, oppure direttamente 
...
Ad esempio troviamo "002: SECURITY FIX: November 12, 2001", cioΦ la patch numero 2 pubblicata 
...
esempio con il cvs.
Per l'utilizzo del cvs avete giα capito cosa dovete fare! ;) 
...
# patch -p0 < 002_ssh.patch
   alla fine del processo se tutto Φ andato bene dovremmo trovarci:
...
pu≥ essere utilizzato anke per ki ha + OS sul proprio PC o su un unico HDD, kiaro
Φ ke la documentazione sul LILO in rete se ne trova in quantitα industriale e anke
sui vari CD di Linux ed Φ in alcuni casi anke ben fatta e molto esauriente, quindi
...
sembra anke logico.Troverete installato Linux nella parte /tc/lilo.conf questa Φ la
parte essenziale del Loader perchΦ qui trovere informazioni su quello ke LILO fa al
...
comando cos∞ possiamo ovviare premendo ALT+SHIFT prima ke appare la scritta LILO sul
monitor.
-----------------------------------------------------------------------------------
...
sono come dire impiccati, e hanno riskiato in molti casi, di sputtanarsi il pc, dunque
 a mio parere, il sistema Φ molto semplice i passaggi nn sono molti basta solo
...
caso le cose diventano molto + semplici, xkΦ basta avviare quella specie di Fdisk
tipo quello di winzozz e rimuovere le partizioni di Linux, bhΦ sinceramente spero
ke Linux rimanga sempre nel Vs pc xkΦ Φ un bel sistemimo, eheheh, vabbΦ nn facciamo
...
A mio modesto parere il Borland C++ Builder Φ un ottimo ambiente di sviluppo in quanto assembla
due qualitα veramente fantastiche: la velocitα di esecuzione dei programmi (nonchΦ 
ottimizzazione del codice) e la semplicitα di sviluppo....esso infatti Φ un ambiente RAD....come
il VisualBasic per capirci.... che come metodo di programmazione Φ criticato da molti.... ma a 
me piace! Non darα le soddisfazioni della programmazione a basso livello ma sicuramente neanche
le nottate passate a studiare le documentazioni delle API. E' inutile comunque dire che per un
buon coder la programmazione a basso livello Φ indispensabile(non che non si possa fare
...
Parte Prima ---- Introduzione(si...ma Φ l'ultima giuro) sul Borland C++ Builder

Personalmente utilizzo la versione 5.0 Entrprise e devo dire che Φ ricchissima di componenti...
...
familiaritα con gli ambienti RAD....se non Φ cos∞ imparerete sicuramente in un attimo....(modo
...
un Form in modo dinamico..... questa Φ una cosa molto utile.... e semplice:
...
Giα.... bazzeccole direte voi..... ma sapete forse disabilitare la combinazione CTRL-ALT-CANC??
Si?????? BhΦ io la metto lo stesso che non si sa mai....
<----------------------------------------------------------------------------------
//Per fare questo per≥ occorre far credere a winpuzz che stiamo eseguendo uno screen-saver....
MOUSEKEYS mk;
...
void __fastcall TForm1::FormCloseQuery(TObject *Sender, bool &CanClose) 
    ...
    ShowMessage("E cos∞ vorresti uscire eh???? AHAHAHAHA"); // e questo sfotte
...
: TForm(Owner)
{
	...
	SetWindowRgn(Handle, MyRgn, true);// E cos∞ la mostriamo
}
<--------------------------------------------------------------------------------------

Questa Φ proprio bastarda!! Nascondere il pulsante Star/Avvio Muhahahahaha
<-------------------------------------------------------------------------------------------
...
Reperire il nome dela diredtory di esecuzione del programma pu≥ sempre fare comodo...
<--------------------------------------------------------------------------------------------
...
Anche suonare un waw non Φ inutile
<--------------------------------------------------------------------------------------------
#include &lt;win32\mmsystem.hpp> //ci serve per≥ questo bell'include
...
Terminiamo il tutorial con un tocco di classe(bhΦ insomma...): Impedire all'utente di chiudere
...
BhΦ che dire?....io mi sono divertito a fare sto tut....e magari ne far≥ altri sull'argomento!!
...
L'idea di questo tutorial mi Φ venuta leggendo l'articolo di []Lord[V]icio[]
sulla difesa (nfz n'2).

Oggi Φ il 05/01/2001 Φ da poco che ho windows XP , e i miei commenti sono :

1 - Un p≥ pi∙ stabile dei suoi precedenti
2 - Sfrutta troppo e inutilmente la cpu
3 - Il terzo commento Φ pi∙ un'osservazione :
...
Inoltre Windows Φ a pagamento (e chi mai a pagato windows? :) al contrario di
linux , come o potuto vedere oggi in edicola a 9.900 lire cΦ la redhat 7.2 bella
che completa! (non prenderla Φ da veri coglioni) ma non sono qua per fare critiche
ma bens∞ per farvi conoscere pi∙ a fondo windows

~~>Registro di sistema

Regedit : questo comando fatto partire da dos , o da start/esegui visualizza il
registro di sistema
Il registro di sistema Φ la chiave di tutto , da li puoi fare di tutto e di pi∙ ,
...
ancora molto altro , ma per fare capire anche ai principianti far≥ esempi pratici.
(Se non funziona provate "regedt32.exe"

~>Hkey_Classes_Root

Questa chiave appena si espande mostra svariate cartelle che con un p≥ di intuito
...
chiamata "shell" e un'altra sotto "shell" chiamata "Open with Evil" e un'altra sotto
 ...
 questo Φ un p≥ inutile per≥ potrebbe servire a programmatori che hanno fatto
 programmi per la gestione di immagini , musica ecc...
insomma questa Φ la gestione dei file , delle estensioni e delle ole drop di windows,
sembrava pi∙ difficile vero?
una cosa che ancora non ho capito e penso che ben pochi lo hanno capito , Φ il
...
Φ la voce che riguarda ogni file indipendentemente dall'estensione e dal nome ,
spesso programmi come winzip , zipzilla , winamp , winrar ecc.. influiscono su
questa chiave e a volte pu≥ risultare utile ripulirla.
...
nome 645FF040-5081-101B-9F08-00AA002F954E , questo Φ il cestino , sulla destra
trovate una stringa "predefinito" cliccateci due volte sopra e cambiate il valore da
cestino in quello che volete voi

~>Hkey_Local_Machine

Questa chiave di registro Φ quella che contiene le informazioni su hardware/software ,
...
floppy , le lettere delle unitα ecc..) , questa chiave ha il pregio di mantenere le
...
Default_Monitor vi indicherα il monitor in uso e le sue informazioni (driver creatore
 ecc...).

3.Hardware

Questa chiave contiene informazioni e configurazioni su hardware con comunicazioni
via seriale , settaggi delle porte , nonchΦ del processore ed i suoi componenti
(recurabili in parte su hardware/description/system/centralprocessor) oltre ad id,
dominio ecc...

4.Software

Software da come si pu≥ capire contiene informazioni sui software installati sul
vostro PC , questa chiave Φ parallela a Hkey_current_user/software , ma contiene
informazioni meno dettagliate sui programmi , una cosa utile pu≥ essere ripulire
...
se cambio il valore in 19 la porta sarα la 25 se cambio il valore in 15 la porta
sarα la 21 ecc...
...
esempio : Quando cΦ posta  , quando si verifica un errore , quando si spegne il pc
ecc...

2.Control panel

Control panel ovvero Pannello di controllo !va che nessuno lo sapeva :)! , contiene
informazioni sullo schermo e le proprietα assegnate , sul desktop e i relativi
colori...
...
diventerebbe troppo lungo e il secondo Φ che le altre chiavi dipendono pi∙ o meno una all'altra , comunque ne cito almeno una piccola  descrizione per una.
...
che vengono aggiornate a velocitα quasi supersonica anzi "sonica" :) quando ne
vengono installate o rimosse dal sistema

~

E NO , NONO ragazzi non Φ ancora finita , ora vi beccate ancora il backup del
registry (anche perchΦ se combinate casini questa Φ l'unica soluzione "o quasi")
 , vi anticipo giα che dopo la pappardella del backup , ci saranno i trucchi di
...
Che mai faremo? mhhh quel 5 li Φ un po poco ma che cosa Φ?
Il 5 indica il numero di istanze del backup da conservare quindi aumentiamolo! ma
senza esagerare , facciamo 10 - 15 - 20 anche 30 e tutto verrα salvato su
C:\Windows\Sysbackup
Ma se questo non partisse all'avvio in automatico allora il comando manuale Φ questo:
...
Siamo alla fine del tutorial :( , come avrete notato Φ molto pi∙ lungo dei
precedenti , del resto dopo una pausa cos∞ lunga non potevo deludere i lettori!
...
2. Winsock, oltre alle funzioni giα definite con il socket Berkeley,
   richiede sempre la chiamata a due funzioni specifiche: WSAStartup e
   WSACleanup. WSAStartup() Φ una funzione chiamata dal programma durante una
   ...
   specificando le proprie funzionalitα. In pratica questa funzione serve al
   programma per specificare quale versione dell'API winsock necessita.
   WSAStartup pu≥ essere chiamata diverse volte durante l'attivitα del
   ...
   WSACleanup() Φ una funzione che viene chiamata dal programma per
...
3. In un errore nel socket, l'API Berkeley utilizza la funzione errno() con
   ...
   Socket_Error, il cui valore Φ -1; il programma chiamerα quindi la funzione
   WSAGetLastError, che verificherα la causa che ha provocato l'errore nel
   winsock.
4. A differenza di quanto avviene con i programmi unix, che necessitano di
   ...
   pertanto, non sarα necessario inserirlo. In  fase di  compilazione si
   scegliera'

Introduzione vbs

Uno dei fastidiosi problemi creato per facilitare ed abbellire le pagine web Φ
il linguaggio script.
I maggiori esponenti di questo linguaggio sono il vbs ed il JS alias visual basic
script e java Script. Solitamente lo si pu≥ introdurre nelle pagine web, o
...
vbs pu≥ essere giudicato al massimo pi∙ semplice in quanto riprende determinati
algoritmi del vb visual basic quindi alcune parti 30% possono sembrare simili o
del tutto uguali, ecco perchΘ Φ stato chiamato visual basic script, dato che Φ il
...
In seguito verrα immessa la parte principale, ossia lo script comandi ed infine
verrα chiuso con listruzione..
...
Commenti
I commanti in vbs possono essere inseriti nel medesimo codice seguiti da (), cos∞
...
Bene ma quali sono i comandi del linguaggio vbs?, dato che questo Φ un testo di
iniziazione al linguaggio script citer≥ solo i pi∙ banali se non essenziali.
...
Esaminiamo ora questi comandi singolarmente e le loro propietα.

Msgbox Φ il comune pop up ossia il messaggio con lopzione del click min 1, max 3.
...
vorremo un messaggio che ci dica semplicemente buongiorno lo script sarα il seguente

&lt;script language=vbs>
msgbox Buongiorno
&lt;/script>

in questo caso dato che non abbiamo specificato la proprietα del messaggio di errore,
di domanda, di informazione etc etc, il nostro messaggio sarα un semplice messaggio di notifica.

Se invece decidevamo di dare un messaggio di errore dovevamo inserire la proprietα
del messaggio. Le proprietα fondamentali delle msgbox in vbs sono le seguenti
...
E cos∞ via per tutte le altre modalitα di messaggio.
...
di ModalitαVisuslizzazione la modalitα, quindi il parametro di visualizzazione, se
si desidera farla aprire normalmente a tutto schermo si pu≥ anche non inserire la
modalitα di visualizzazione.
Quindi per far aprire la cartella c:\programmi, la funzione sarα
...
aperta in maniera minimizzata dobbiamo introdurre la modalitα di visualizzazione,
questa proprietα Φ

VbMinimizedNoFocus

Quindi il codice per far si che venga aperta in maniera minimizzata sarα
...
fine. Questo comando Φ simpaticamente utilizzato da quelle persone che io reputo lo
...
Un esempio di Do loop pu≥ essere implicato nelluso delle msgbox, infatti basta
...
Il codice quindi sarα il seguente
...
In questo caso avremo la ripetizione infinita del messaggio buongiorno dato che Φ
incamerato nella funzione Do Loop
Questa funzione do loop pu≥ essere implicata in qualsiasi comando vbs ad esempio
pu≥ essere inserita anche nellapertura di una directory o magari come giα spiegato
di una pagina web.
Quindi per esempio per aprire allinfinito la cartella c:\ il codice sarα il seguente
...
Il vbs Φ da molti giudicato anche abbastanza pericoloso a causa di determinate
...
lasciando lutente ignaro di ci≥ che stia accadendo teoria che riprende la funzione
da me giα pubblicata
...
Ormai la maggior parte della gente Φ venuta a conoscenza di un sistema di
intercettazione chiamato Echelon.
Nelle seguenti righe pi∙ che spiegare cos'Φ Echelon, spiegher≥ come proteggersi
da questi sistemi.

Il sistema Echelon Φ nato verso la fine degli anni '80; progettato e amministrato
dalla NSA (giα conosciuta per le Crypto AG), Φ utilizzato per intercettare normali
e-mail, fax, telex e telefonate che viaggiano nella rete di telecomunicazioni
mondiale.
Probabilmente questo lo sanno in molti, ma quello che pochi sanno Φ l'esistenza di
un sistema chiamato Ermes.

Ermes Φ stato progettato qui in Italia, precisamente a Torino; esso Φ un sistema
...
Tutto ci≥ Φ possibile grazie ad un algoritmo, che Φ un procedimento di calcolo,
...
[ Be, ad essere sincero, l'ultima frase non Φ farina del mio sacco :) ]
Ma chi ci dice che questo Ermes non intercetti a sua volta le nostre comunicazioni?
Se Φ vero che Ermes pu≥ bloccare le intercettazioni di Echelon, perchΦ non lo usano?
...
la societα a sostituire l'intero sistema operativo.
Una volta installato, Carnivore si assicura un alto livello di accessibilitα ai
canali di comunicazione rendendo innocua ogni protezione, interna al server,
relativa ai dati. Questa vulnerabilitα pu≥ pero rivelarsi pericolosa poichΦ porge
il fianco agli attacchi informatici degli hacker che una volta impadronitisi del
sistema potrebbero disporre di uno strumento dalle alte potenzialitα distruttive.
PoichΦ come dimostrato dagli studi dell'istituto indipendente IIRTI (International
Intellectual Property Training Institute), chiunque in possesso delle password pu≥
utilizzare il programma, Carnivore pu≥ rappresentare una pericolosa arma a doppio
taglio.
...
Se noi cifriamo un documento Φ ovvio che sarα intercettato con prioritα massima.
...
steganografia.
Ma cosΦ la steganografia? Essa pu≥ essere definita come una tecnica integrativa
...
della nostra canzone preferita pu≥ contenere anche un testo, un immagine, una
registrazione vocale in formato diverso dall'mp3 o altro ancora. Inoltre, senza
lo stesso software steganografico che Φ servito per introdurre il file secondario
all'interno del file principale, risulterα quasi impossibile accorgersi dell'intruso.
L'unico accorgimento necessario a non insospettire i pi∙ esperti Φ quello di
...
700x400 pixel sarα ragionevole attendersi che le relative dimensioni possano essere
...
Un programma abbastanza diffuso Φ s-tools e lo potete trovare all'indirizzo
http://www.ecn.org/crypto/soft/s-tools4.zip
Un'altro programma Φ il Camouflage, scaricabile da http://www.camouflage.freeserve
.co.uk/Camou121.zip
...
Ricordate che dovunque esista un canale di comunicazione, Φ impossibile controllare
...
l'introduzione del DIODO LASER. Bella comoditα' eh :D .

Motivi per cui conoscere e studiare la tecnologia dei cd:

Veniamo alla parte che c'interessa di pi∙' cioΦ'
...
ed il raggio laser Φ deviato sullo strato dorato.
...
IL sottoscritto non si assume alcuna responsabilitα per l'uso di ci≥ che viene
...
sono registrati a velocitα costante (Constant Linear Velocity - CLV),questo
...
la precisione
di riproduzione dipende dal quarzo.In definitiva, l'audio in uscita Φ determinato
 unicamente dalla frequenza di campionamento di 44.100 campioni al secondo; per
questo motivo, il bit rate medio dal disco Φ di 4,321 Megabit al secondo.
...
Φ di circa 0,5 micron; il raggio laser focalizzato Φ largo meno di 2 micron all'
...
Ad una velocitα costante di circa 1,2 metri al secondo, la precisione di tracking
richiesta Φ impressionante (un tracking corretto di un CD equivale a percorrere
...
con una tolleranza di +/- 0,5 micron). Quindi, Φ come pilotare un aereo su un largo
...
riuscire abbastanza bene, ma il disco sarα maggiormente soggetto ai danni in
...
prestazioni nel tracking e nella correzione degli errori, un qualunque CD pu≥
...
Disclaimer:Questo tutorial e' stato scritto scopo educativo e di ricerca.
IL sottoscritto non si assume nessuna responsabilitα per l'uso di ci≥ che viene
...
(se vi interessasse l'apertura numerica Φ definita come il seno dell'angolo che
...
Come sempre quando ti viene un'idea in mente a volte non Φ il caso di seguire 
...
un' po di tempo anche perchΦ come dattilografo faccio realmente ...are Buona 
...
la serie non Φ finita qui proximi tut su algoritmo in Vb per mime e posta, Mime 
e html e per finire exploit di posta :)cos∞ il quadro sarα abbastanza chiaro. 

Un ringrazimento particolare a Meph che sα sempre come farmi passare notti 
...
comprendere il testo Φ neccessario sapere come funge SMTP, cos'Φ un header 
etc... Chi volesse pu≥ far riferimento a tutte le guide SMTP in circolazione su 
internet, agli rfc relativi o meglio ancora al tut che ho redato un mesetto fα. 
Per quanto riguarda gli RFC e gli STD vi rimando a un link niente male, pu≥ 
...
su Internet. Il suo successo Φ stato talmente elevato cosicchΘ il formato RFC 
822 Φ stato adottato, totalmente o parzialmente, oltre i confini di Internet e 
...
comunitα di utenti. RFC 822 Φ stato progettato per specificare un formato per 
...
Anche nel caso del solo testo, tuttavia, RFC 822 Φ inadeguato per le necessitα 
...
821/822 Φ il fatto che limitano il contenuto dei messaggi di posta elettronica 
...
maggior parte di questi problemi senza introdurre alcuna incompatibilitα grave 
con il mondo esistente di posta RFC 822. In particolare, descriver≥:

* Un campo di intestazione [MIME-Version], nel quale viene specificato il numero 
di versione per dichiarare che un messaggio Φ conforme con questa specifica e 
...
* Un'intestazione [Content-Type], specificato da RFC1049, il quale pu≥ essere 
...
messaggio e specificare completamente la rappresentazione nativa di tali dati.
	...
	** Valore "application" che nel Content-Type viene utilizzato per 
	trasmettere i dati di applicazioni o i dati binari, e perci≥, utilizzato 
	...
	** Valore "image" che nel Content-Type viene utilizzato per trasmettere 
	non dovr≥ mica dirvi anche questo ... :). 	
...
* Un campo di intestazione Content-Transfer-Encoding, che pu≥ essere utilizzato 
...
E giα qua direte che @@, avete voluto la bici ora pedalate :)

MIME Φ stato progettato con attenzione come un meccanismo estendibile, Φ 
presumibile che l'insieme di coppie di content-type/sottotipo e dei loro 
parametri associati crescerα significativamente con il tempo. E' probabile che 
...
RFC 822 Φ stato pubblicato nel 1982, daquella volta c'Φ stato realmente solo uno 
...
proprio alla getione dei messaggi MIME-Version come giα detto in precedenza Φ il 
...
cui si potesse specificare la versione. Nella notazione BNF tutto ci≥ si traduce 
in

            MIME-Version: = text

Dove "= text" rappresenta un valore che nel precedente caso Φ "1.0"

E' importante ricordarsi sempre che la specificazione di questo header Φ 
obbligatoria ma univoca, non ci capiterα mai di trovare pi∙ volte riportato 
...
Lo scopo del campo Content-Type Φ di descrivere i dati contenuti nel corpo al fine 
...
Il campo di intestazione Content-Type Φ utilizzato per specificare la natura dei 
dati nel corpo di un'entitα, dando identificativi di tipo e di sottotipo e 
fornendo informazioni ausiliarie. Dopo i nomi di tipo e di sottotipo, il resto 
del campo di intestazione Φ semplicemente un insieme di parametri, specificati 
in una notazione di attributo/valore.

L'insieme di parametri significativi Φ diverso per i vari tipi. Tra i parametri 
definiti un parametro ricorrente Φ sicuramente "charset" (insieme di caratteri, 
non lo tradurrei hihihihiih Φ come tradurre directory = direttorio) che serve a 
dichiarare la serie di caratteri utilizzata nel corpo.

Di solito, il Content-Type superiore Φ utilizzato per dichiarare il tipo 
generale di dati, mentre il sottotipo specifica un formato specifico per quel 
tipo di dati. Cos∞, un Content-Type "image/xyz" Φ sufficiente per dire che i 
dati sono un'immagine, anche se il client dell'utente non dα alcuna specifica 
...
tipi cos∞ composti dovrebbero essere rappresentati come "application" o 
"multipart", ma lo vedremo pi∙ avanti.

I parametri non modificano il  content-subtype e non influiscono sui requisiti 
del sistema host.

BenchΘ la maggior parte dei parametri abbia senso solo con alcuni content-types, 
...
qualsiasi motivo, gli deve essere dato un nome che inizia con "X-". Ci≥ serve a 
...
Oltre questa sintassi, l'unico vincolo sulla definizione di nomi del sottotipo Φ 
chiedere che l'utilizzo non deve creare conflitti. Questo Φ vincolato per fare 
in modo che due comunitα diverse utilizzino lo stesso sottotipo per scopi 
diversi
...
* Valori privati ( iniziando con " X-" ) pu≥ essere definito bilateralmente tra 
...
I sette Content-Types predefiniti standard sono: 
	...
	text -- informazioni testuali. Il sottotipo principale Φ "plain", indica 
	testo chiaro (non formattato). Nessun software speciale Φ richiesto per 
	...
	testo arricchito o formattato da moduli dove un software applicativo pu≥ 
	...
	sottotipi includono cos∞ qualsiasi formato di elaboratore testi 
	leggibile. Uno molto semplice e diffuso Φ il sottotipo richtext.
	...
	"parallel"  e "digest"  per le entitα multipart in cui ogni parte Φ di 
	tipo" message" principale.

	message -- un messaggio incapsulato. Ha i seguenti sotto tipi: il 
	sottotipo principale Φ "rfc822" secondo proprio la notazione RFC822, 
	"partial" Φ definito per messaggi parziali per frammentare la spedizione 
	di messaggi troppo grandi, 	"External-body", Φ definito per la 
	...
	video -- video.Il sottotipo iniziale Φ "mpeg".
...
chiaro con la serie di caratteri US-ASCII, che pu≥ essere specificata 
esplicitamente come: 

	Content-type: text/plain; charset = us-ascii

Nell'assenza di qualsiasi campo di intestazione Content-Type o campo di 
intestazione MIME-Version, Φ impossibile essere certo che un messaggio Φ in 
effetti testo nella serie di caratteri US-ASCII. Ad esempio sorge chiaro il 
problema in questo contesto se dovessimo spedire il carattere Ç che non esiste 
...
Il campo Content-Transfer-Encoding Φ utilizzato per indicare il tipo di 
trasformazione che Φ stata utilizzata per rappresentare il corpo in un modo 
accettabile per il trasporto.

Avere una proliferazione di Content-Transfer-Encoding Φ indesiderabile e non 
necessario. Tuttavia, stabilendo anche un solo meccanismo Content-Transfer-
Encoding non Φ sufficente. C'Φ un tradeoff tra il desiderio di una codifica 
compatta ed efficiente di dati largely-binary e il desiderio di una codifica 
leggibile di dati che Φ prevalentemente a 7 bit. Per questo motivo esistono due 
meccanismi di codifica:

	una codifica "leggibile" e una codifica "densa".

Il campo Content-Transfer-Encoding Φ progettato per specificare una mappatura 
invertible tra la rappresentazione "nativa" di un tipo di dati e una 
rappresentazione che pu≥ essere scambiata prontamente utilizzando protocolli di 
trasporto di posta di 7 bit. Questo campo non Φ stato definito da nessuno 
standard precedente. Il valore del campo Φ un singolo token che specifica il 
...
Questi valori non sono capsensitive. CioΦ, Base64, BASE64 e bAsE64 sono del 
tutto equivalente. 

Un tipo di codifica 7BIT richiede che il corpo sia giα in una rappresentazione a 
sette bit mail-ready. In questo caso Φ il valore di default Φ: 
...
token "binariy" Φ che "binariy" non richiede conformitα a qualsiasi limite su 
lunghezza di linea o alle semantiche SMTP CRLF, mentre i token bit-width 
richiedono tale conformitα.
...
sul Content-Type diverso dall'algoritmo per cui Φ stato codificato o dai 
...
Transfer-Encoding Φ esplicitamente e fortemente scoraggiata, come sembra 
probabilmente destinata a impedire l'interoperabilitα con piccolo vantaggio 
potenziale. Il loro utilizzo Φ permesso solo come il risultato di un accordo fra 
...
ASCII. Cos∞, ad esempio, supponete che un'entitα abbia campi di intestazione 
come: 

	Content-Type: text/plain; charset=ISO-8859-1 
	Content-transfer-encoding: base64

questo verrα interpretato come corpo in base64 sapendo che i dati in origine 
...
Content-Types. In particolare, Φ impedito appositamente di utilizzare qualsiasi 
...
conversione Φ la gestione di linea. Quando si converte da quoted-printable a 
...
La codifica Quoted-Printable Φ progettata per rappresentare dati che in gran 
parte sono formati da ottetti che corrispondono a caratteri stampabili nella 
serie di caratteri ASCII. Codifica i dati in un modo tale in cui Φ improbabile 
...
esseri umani. Un corpo che Φ completamente ASCII pu≥ essere codificato anche in 
Quoted-Printable per assicurare l'integritα dei dati se il messaggio passasse un 
...
In questa codifica, gli ottetti devono essere conformi alle seguenti regole:
	...
	* Regola #1: (rappresentazione generale a 8 bit) qualsiasi ottetto, 
	tranne quelli che indicano un'interruzione di linea, pu≥ essere 
	rappresentato da uno "=" seguito da una rappresentazione esadecimale di 
	due cifre del valore dell'ottetto. Cos∞, ad esempio, il valore 12 
	(avanzamento pagina ASCII) pu≥ essere rappresentato da "=0C" e il valore 
	61 (ASCII EQUAL SIGN) pu≥ essere rappresentato da "= 3D". 
	...
	* Regola #3: (spazio bianco): Ottetti con valori di 9 e di 32 sono  
	...
	non devono essere rappresentati cos∞ alla fine di una linea codificata. 
	...
	regola Φ necessaria a causa di qualche MTAs (messaggio Transport Agents, 
	...
	* Regola #4 (Line Breaks): Un'interruzione di linea in una parte di 
	...
	(RFC 822), che Φ una sequenza CRLF, nel Quoted-Encoding stampabile. Se a 
	sequenze isolate CRs e LFs o LF CR e CR LF Φ permesso di essere 
	...
	* Regola #5 (Soft Line Breaks): La codifica Quoted-Printable richiede 
	...
	su una linea codificata indica che la linea Φ stata interrotta nel testo 
	codificato. Cos∞ ad esempio se dobbiamo codificare la seguente stringa:

		Ora Φ il tempo perchΘ tutte le genti vengano in aiuto del loro 
		paese.

	questo verrα rappresentato, nella codifica Quoted-Printable, come: 

		Ora Φ il tempo = tutte le genti vengano = in aiuto del loro 
		paese.

	Sarα poi il client a ripristinare la stringa

	Nel limite di 76 carattere non vengono contati i CRLF e gli "=".

La codifica quoted-printable rappresenta un compromesso fra leggibilitα e 
affidabilitα in trasporto. 



***   Base64 Content-Transfer-Encoding   ***

Il Base64 Content-Transfer-Encoding Φ progettato per rappresentare sequenze 
arbitrarie di ottetti in un modulo che non Φ umanamente leggibile. Gli algoritmi 
...
"=", Φ utilizzato per indicare una funzione di elaborazione speciale ).

Il processo di codifica Φ rappresentato da gruppi di 24 bit di immissione che 
come output diventano stringhe di 4 caratteri codificati. Continuando da 
sinistra a destra, un gruppo di immissioni a 24 bit Φ formato concatenando 3 
gruppi di immissioni a 8 bit. 

Questi 24 bit sono quindi trattati prendendo 6 bit alla volta per 4 volte e 
concatenandoli, ognuni gruppo da 6 bit Φ tradotto in una singola cifra. Nel 
flusso di input Il primo bit sarα il bit high-order nel primo byte e l'ottavo 
bit sarα il bit low-order nel primo byte, e cos∞ via.

Ogni gruppo a 6 bit viene utilizzato come indice in un array di 64 caratteri 
stampabili. Il carattere definito dall'indice Φ messo nella stringa di output. 
...
divisibile per 24? Molto facile visto che tutto Φ rappresentato da un numero ben 
preciso di ottetti avremo solo i seguenti casi

	*   l'ultimo gruppo di bit Φ un multiplo integrale di 24 bit; in questo 
	caso l'unitα finale di output sarα pari alla stringa di 4 caratteri come 
	il resto della codifica

	**  L'ultimo gruppo di bit da codificare Φ esattamente di 8 bit; in 
	questo caso l'unitα finale di output codificato sarα pari a 2 caratteri 
	derivati dalla tabella pi∙ due caratteri "=" che fungono come caratteri 
	di riempimento

	*** L'ultimo gruppo di bit da codificare Φ esattamente di 16 bit; in 
	questo caso l'unitα finale di output codificato sarα composto da tre 
...
prima della codifica base64. La cosa importante da notare Φ che questo pu≥ 
...
Nella costruzione di un client di posta, pu≥ essere desiderabile permettere a un 
corpo di fare riferimento a un altro, di conseguenza, i corpi possono essere 
etichettati utilizzando il campo di intestazione "Content-ID", il quale Φ 
...
La capacitα di associare qualche informazione descrittiva a un corpo dato Φ 
spesso desiderabile. Ad esempio, pu≥ essere utile contrassegnare un corpo 
"image" come " un'immagine dello Space Shuttle". Tale testo pu≥ essere messo nel 
...
Text Content-Type Φ destinato all'invio materiale principalmente testuale. ╚ il 
Content di default. Un parametro "charset" pu≥ essere utilizzato per indicare la 
serie di caratteri del testo di corpo. Il sottotipo principale di testo Φ 
"plain". Questo indica testo chiaro (non formattato). Il Content-Type di default 
per posta Internet Φ "text/plain"; "charset = us-ascii".
...
di tali rappresentazioni Φ che esse sono in parte leggibili anche senza il 
software che li interpreta. ╚ utile, quindi, distinguerle, a livello pi∙ alto, 
da dati cos∞ non leggibili come immagini, audio o testo rappresentati in un 
modulo non leggibile. Nell'assenza di software di interpretazione appropriato, Φ 
ragionevole visualizzare sottotipi di testo all'utente, mentre non Φ ragionevole 
...
Per promuovere l'interoperabilitα pi∙ larga di testo formattato semplice, Φ 
stato definito un sottotipo estremamente semplice di "testo", il sottotipo 
"richtext". Questo sottotipo Φ stato progettato per soddisfare i seguenti 
criteri:
...
La sintassi di "richtext" Φ molto semplice. Tutti i caratteri si rappresentano 
da soli, con l'eccezione del carattere "<" (ASCII 60), quale Φ utilizzato per 
...
e 62). Ogni comando di formattazione pu≥ essere non pi∙ di 40 caratteri in 
lunghezza, tutto in US-ASCII, limitato all'alfanumerico e al trattino (" - "). E 
non mi permetto di descrivere tutti i cmd di richtext, ma comunque Φ importante 
...
Content-Type "multipart" deve venire inserito nell'intestazione dell'entitα. Il 
...
Una parte di corpo che inizia con una riga vuota Φ permessa. In un tale caso, 
l'assenza di un campo di intestazione Content-Type implica che l'incapsulamento 
Φ testo chiaro US-ASCII. Gli unici campi di intestazione che hanno un 
...
La distinzione fra un messaggio RFC 822 (SMTP testuale) e una parte di corpo Φ 
...
corpo Φ un'immagine. Per rappresentare l'ultima, la parte di corpo deve avere 
...
Come stabilito precedentemente, ogni parte di corpo Φ preceduta da un limite di 
incapsulamento. Il limite di incapsulamento non viene visualizzato dentro una 
qualunque delle parti incapsulate. Cos∞, Φ cruciale che il client che compone 
sia in grado di scegliere e specificare il limite univoco che separerα le parti.
...
grado di riconoscere e separare le parti di qualsiasi entitα multipart, anche di 
un sottotipo non riconosciuto.

Come stabilito nella definizione del campo Content-Transfer-Encoding, una 
codifica diversa da "7bit", "8bit" o "binario" non Φ permessa per le entitα del 
...
incorporate nei corpi di messaggi del tipo "multipart".
	...
	* Multipart: La sintassi comune
	...
	Il campo Content-Type per le entitα multipart richiede un parametro, 
	"boundary ", utilizzato per specificare il limite di incapsulamento. 


	Cos∞, un campo di intestazione multipart Content-Type tipico potrebbe 
	assomigliare a questo:

                 Content-Type: multipart/mixed;
                      boundary=gc0p4Jq0M2Yt08jU534c0p

	Questo indica che l'entitα stessa Φ formata da numerose parti, ognuna 
	con una struttura che Φ sintatticamente identica a un messaggio RFC 822, 
	...
	Il limite di incapsulamento che segue l'ultima parte di corpo Φ un 
	delimitatore distinto che indica che nessuna ulteriore parte di corpo 
	seguirα. Un tale delimitatore Φ identico ai delimitatori precedenti, con 
	...
	da 2 testi in asciii
                 ...
                 Questo Φ testo in chiaro immesso implicitamente.
                 Non finisce con un line break.
                 --simple boundary
                 Content-type: text/plain; charset=us-ascii

                 Questo Φ testo chiaro immesso esplicitamente.
	...
	all'interno di un'altra entitα multipart Φ permesso esplicitamente. In 
	tali casi, per cause evidenti, l'attenzione deve essere presa per 
	assicurare che ogni entitα annidata multipart deve utilizzare un 
	delimitatore di limite diverso. 

	L'unico parametro obbligatorio per il multipart Content-Type Φ il 
	parametro di boundary. (Se per caso sembra che un limite termini con 
	spazio bianco, lo spazio bianco deve essere essere cancellato perchΘ 
	...
	Complessivamente, il corpo di un'entitα multipart pu≥ essere specificata 
	come segue, seguendo la notazione BNF:
        ...
        * Sottotipo (principale) The Multipart/mixed
        ...
        Il sottotipo principale per multipart, "mixed", Φ utilizzato quando le 
        parti di corpo sono indipendenti. 

        * Sottotipo Multipart/alternative

        Il multipart/alternative Φ sintatticamente identico a multipart/mixed, 
        ma le semantiche sono diverse. In particolare, ognuna delle parti Φ una 
        versione "alternativa" delle stesse informazioni. I client dovrebbero 
        riconoscere che il contenuto delle diverse parti Φ intercambiabile. Il 
        client dovrebbe scegliere il "meglio" che il tipo ha basato 
        sull'ambiente e sulle preferenze o sull'offerta dell'utente. 

        Questo pu≥ essere utilizzato, ad esempio, per inviare posta in un 
        formato di testo personalizzato in modo tale in cui pu≥ essere 
	...
	* Sottotipo Multipart/digest
        ...
        Questo tipo Φ sintatticamente identico a multipart/mixed, ma le 
        ...
        "testo/plain". Questo Φ fatto per permettere un formato di compendio pi∙ 
        ...
        comprensione:
            ...
            Subject: come Φ stato scrivere la guida?
...
messaggio di posta. Per questa operazione comune, Φ definito uno speciale 
Content-Type, "message". 

Comporre messaggi di rifiuto e di invio in questo modo preserverα le 
informazioni di tipo sul messaggio originale e permetterα di presentare 
correttamente esso al destinatario ed Φ perci≥ fortemente consigliato.

Come stabilito nella definizione del campo Content-Transfer-Encoding, non si pu≥ 
...
caso e i dati all'interno del corpo possono essere ancora codificati.            
	...
	* Sottotipo (principale) Message/rfc822
        ...
        * Sottotipo Message/Partial
        ...
        Un sottotipo di messaggio, "partial", Φ definito per permettere a grandi 
        oggetti di essere consegnati fragmentati per poi essere riassemblati dal 
        client. (il concetto Φ simile a frammentazione IP/reassembly 
        nell'Internet Protocols di base). Questo meccanismo pu≥ essere 
        ...
        "Message/Partial" indica cos∞ che il corpo contiene un frammento di un 
        messaggio pi∙ grande.

	Tre parametri devono essere specificati nel campo Content-Type di tipo 
	Message/Partial: 

	Il primo, "id", Φ identificativo univoco da utilizzare per far 
	corrispondere asseieme le parti. (di solito, l'identificativo Φ 
	essenzialmente un message-id); se messo in virgolette doppie, pu≥ essere 
	qualsiasi message-id. Il secondo, "number", un intero, Φ il numero 
	parte, che indica dove questa parte va inserita nella sequenza di 
	frammenti. Il terzo, "total", un altro intero, Φ il numero totale di 
	parti. Questo terzo sottocampo Φ richiesto sulla parte finale ed Φ 
	facoltativo sulle parti precedenti. Questi parametri possono essere dati 
	in qualsiasi ordine.

        Cos∞, la parte 2 di un messaggio di 3 parti pu≥ avere l'uno o l'altro 
	...
	insieme, il risultato Φ un messaggio di formato RFC 822 completo, che 
	pu≥ avere il proprio campo di intestazione Content-Type e pu≥ contenere 
	cos∞ qualsiasi altro tipo di dati.
	...
	intestazioni delle entitα inclusive. 
	...
	* Sottotipo Message/External-Body
	...
	Non mi dilungo oltre su questo sottotipo tenete presente che pu≥ 
	...
	Il meccanismo external-body non Φ progettato per essere limitato al 
	reperimento di file, come mostrato nell'esempio. Oltre questo, uno pu≥ 
...
elettronica computazionale. (l'ultimo, in particolare, pu≥ porre problemi di 
...
Di solito, il sottotipo di applicazione spesso sarα il nome dell'applicazione a 
cui i dati sono destinati.

Questo non significa, tuttavia, che qualsiasi nome del programma applicativo pu≥ 
...
devono essere registrati con IANA.
	...
	* Sottotipo (principale) The Application/Octet-Stream
        ...
        Il sottotipo principale di applicazione, "octet-stream", pu≥ essere 
        utilizzato per indicare che un corpo contiene dati binari.

        NAME -- un nome suggerito per i dati binari se memorizzati come 
        un file.

        TYPE -- il tipo generale o la categoria di dati binari. Questo Φ 
        ...
        PADDING -- il numero di bit di riempitivo che Φ stato aggiunto al 
        bitstream comprendendo il contenuto effettivo per produrre i dati 
        acclusi byte-oriented. Questo Φ utile per l'inserimento di un bitstream 
        in un corpo quando il numero totale di bit non Φ un multiplo della 
        dimensione del byte, ricordate base64 il principio Φ lo stesso

	Tuttavia, l'utilizzo di tali conversioni Φ esplicitamente scoraggiato a 
	causa di una mancanza di portabilitα e standardizzazione. 
        ...
        * Sottotipo Application/PostScript
        ...
        rischi gravi di sicurezza. Mentre di solito Φ sicuro inviare postscript 
        a una stampante, dove il potenziale danno Φ molto vincolato e limitato.
	...
	jolly, pu≥ sembrare a prima vista essere innocuo, tuttavia, questo 
	...
	operatori di file potenzialmente pericolosi, poichΘ Φ abbastanza 
	...
	* Sottotipo The Application/ODA
	...
	Il sottotipo "ODA" di applicazione Φ utilizzato per indicare che un 
...
Il sottotipo iniziale di base Φ specificato per soddisfare questo requisito 
fornendo un comune formato audio.

Il contenuto del sottotipo "audio/basic" Φ audio codificato utilizzando ISDN 
u-law [PCM] a 8 bit. 
...
riferisce a video codificato secondo lo standard MPEG, etc à



***   Ricapitolando   ***

Non Φ sicuramente atteso che tutte le implementazioni future supporteranno tutti 
i Content-Types descritti, nΘ che essi condivideranno le stesse estensioni. Per 
promuovere l'interoperabilitα, tuttavia, Φ utile definire il concetto di "MIME-
conformance" (conforme - MIME) che permette l'interworking utile di messaggi.

Un agente di utente di posta che Φ MIME-conformant dovrα:
...
specificata come un parametro se non Φ US-ASCII.
...
estensioni:
	...
	ASCII, cioΦ tutti i caratteri rappresentati dai valori dell'ottetto 0-
	127.
	...
	"mixed". Application: -- Offrite la capacitα per rimuovere l'uno o 
...
Si dice che Φ un client che incontra le precedenti condizioni Φ MIME-conformant.            
...
Per correttezza, visto che ogni promessa Φ un debito vi sendo la guida allo 
standard MIME, fatene buon uso, nonstante quello che Φ successo Φ giusto cos∞, 
...
Prima di definire 'compilatore' Φ meglio dare qualche altro termine.

Codice Sorgente: Φ la lista di istruzione che noi vogliamo far eseguire al nostro programma 
ed Φ scritto in linguaggio C.

Codice macchina: Φ il contenuto dei file eseguibili Φ scritto in un modo quasi incomprensibile 
ed Φ il linguaggio preferito dei computer perchΦ cos∞ 'ragionano'.

Adesso che avete imparato ben due vocaboli posso spiegarvi a grandi linee che cosa Φ un 
compilatore e cioΦ un interprete tra il Codice Sorgente e il Codice macchina, in poche parole 
non fα altro che prendere il codice che scriviamo, controllarlo da eventuali errori e 
tradurlo nel codice macchina che potrα poi essere eseguito sul computer.

[ Input e Output standard ]

Per facilitare scriver≥ adesso un piccolo programma che stamperα a video una scritta da 
noi preimpostata:
...
Il programma sopra indicato stamperα sullo standard Output (di solito il monitor) la scritta:

Sono il re del mondo
e sono anche un'po megalomane

Ma adesso analizziamo il codice e vediamo di capire che cosa Φ successo.
...
La seconda riga main() { richiama la funzione main che sarα come una scrivania per noi, da 
...
Adesso printf("Sono il re del mondo\ne sono anche un'po megalomane"); questa Φ la funzione 
printf che serve appunto a stampare su standard output (monitor) quello che vogliamo. A 
differenza di main qui abbiamo passato dei valori alla funzione e cioΦ una sequenza di 
...
la funzione pu≥ essere abbandonata.
...
Senza stare a ripetere tutto da capo illustrer≥ solo le nuove righe:

Char scritta[100] crea una matrice di caratteri (cento per l'esattezza), Φ come avere cento 
celle tutte in grado di contenere un carattere. ( il concetto sarα illustrato debitamente
 nel capitolo sulle matrici dimensionali )

La funzione scanf("%c", &scritta); Φ la funzione che si occuperα di prelevare da standard 
input (di solito tastiera) ogni carattere premuto e passarlo in sequenza alla matrice 
'scritta', finchΦ non viene premuto invio o arriva un carattere EOF (End Of File).
La sintassi Φ la seguente scanf("tipo di variabile da prelevare", &variabile finale); infatti 
%c stα a significare proprio Character (carattere).
Il simbolo & Φ l'operatore unario che vedremo in seguito; qui Φ responsabile del passaggio 
...
invio, la matrice scritta apparirα cos∞:

_____________________________________________________
|L|a| |b|e|l|l|a| |l|a| |v|a| |a|l| |f|o|s|s|o| | | | ECCETERA fino alla centesima
»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»
Bene spero che abbiate capito tutto forse Φ meglio che facciate qualche esperimento e 
...
in alcuni computer pu≥ cambiare, questo dipende dall'architettura hardware e non ci potete 
...
Come potete notare il suffisso 'unsigned' non fα altro che dire al programma che quel numero 
non pu≥ avere segno negativo e questo fα in modo che il valore massimo raddoppi e il minimo 
...
Adesso parliamo delle costanti, cioΦ quei valori che dopo un assegnamento non possono essere 
pi∙ modoficati.
La sintassi di dichiarazione Φ la seguente:
...
/ (divisione).
Altro operatore Φ il modulo che ha il simbolo % che ritorna il resto della divisione 
effettuata e quindi ritorna 0 in caso il primo numero sia un multiplo del secondo.

Al fine di rendere pi∙ scorrevole il codice si pu≥ usare questa sintassi:
...
Le strutture di controllo servono per fare test di validitα su determinate variabili.
...
successivamente viene raccolto il valore 'numero' che verrα immesso dall'utente e infine verrα 
eseguito il test che verificherα se il numero Φ maggiore di 47, in caso di positivitα verrα 
...
In questo modo il codice risulterα molto pi∙ leggibile.
...
Adesso passiamo ad un'altra struttura di controllo, e cioΦ lo switch.
La sintassi Φ la seguente:
...
Anche qui il meccanismo Φ semplice se il valore presente nella variabile da testare Φ 
uguale a 0 verranno eseguite le rispettive istruzioni se uguale a 1 le altre etc etc. Da 
notare l'istrzione break che fα terminare in qualsiasi punto venga richiamata la funzione 
switch.
...
Adesso mi occuper≥ dei pi∙ usati cicli, come il while, il for, il do while.

La sitassi del while Φ la seguente:
...
condizione per il quale il ciclo deve essere eseguito Φ che la variabile contatore sia minore 
o uguale a 9 ( e cioΦ deve essere eseguita dieci volte ), la condizione risualta vera per i 
primi dieci cicli grazie all'operazione ++contatore ( che Φ come dire contatore = contatore 
...
Fare attenzione alla posizione dell'operazione ++contatore che Φ stata messa di proposito in 
...
Il ciclo for Φ pi∙ complesso ma risulta pi∙ compatto, la sua sintassi Φ la seguente:
...
ognuno di questi campi pu≥ essere omesso semplicemente lasciando lo spazio vuoto ( ma 
...
La differenza a prima vista non si vede ma pu≥ causare un errore infatti se prefisso il ++ 
...
Infine il do while, la sintassi Φ la seguente:

do {
istruzioni }
while(condizione);

Il funzionamento Φ identico al while, l'unica differenza stα nel fatto che prima vengono 
eseguite le operazioni e poi viene verificata la condizione.

[ Le matrici come variabili multidimensionali ]

Abbiamo giα usato le matrici ma non ne abbiamo ancora parlato.
...
perchΦ anche il compilatore non li segnala, creando un array di 90 int avremmo una 
...
indice pu≥ essere un numero oppure una variabile che lo rappresenta.
...
ciascuna per un totale di 5 * 5 * 5 elementi e cioΦ 125 int che possono essere assegnati 
nel solito modo  nomearray[indicex][indicey][indicez] = valore;.
Un array pu≥ contenere quante dimensioni si voglia e pu≥ essere di qualsiasi tipo ( int, 
char, double etc ).

--------------------------------------------------------------------------
Spero di essere stato chiaro nello spiegare le basi del liguaggio C e se ne avr≥ la 
possibilitα continuer≥ con altri tutorial che lo descriveranno pi∙ nel dettaglio e ne 
...
comprati, i primi panettoni mangiati, cosa fare? PerchΦ non proseguiamo col
...
Oggi facciamo un portscanner pi∙ sicuro, meno loggabile. Questo perchΦ?
PerchΦ usando i raw faremo in modo di mandare ad ogni porta un pacchetto con flag
SYN e, se questa risponde, allora sarα aperta e il nostro kernel manderα un bel
RST. Noi non saremo loggati. E' molto raro infatti trovare server che logghino
delle semplici richieste SYN...abbiamo cos∞ ottenuto quello che volevamo:
SEGRETEZZA. In questo mondo o si Φ paranoici o si viene beccati, quindi occhio!!
Rispondo subito alla domanda che vi frullerα in testa: perchΦ mandiamo un RST?
Semplice, non Φ il kernel che comanda quella connessione, ma noi in tutto e per
...
il vostro IP cioΦ, il server da scannare(sia IP che URL) e la porta d'inizio e
...
sniffatevi mentre lo fate, Φ una prima sorta di spoofing).
...
Buona parte del codice Φ presa da Spoofit.h di coder@reptile...in fondo il
concetto Φ quello,perchΦ riscrivere tutto da capo?;-) Lo ringrazio giα da adesso!
...
/*I soliti include...nulla di nuovo sotto il sole...se volete sapere di pi∙ perchΦ
...
int DEV_PREFIX;
/*Ci servirα per determinare se usera ppp o eth...qui Φ ppp*/
...
Bravi:-) Il primo sarα quello di ricezione, il secondo quello d'invio.*/
...
/*Con questa funzione controlliamo che il nostro device(che Φ ppp in questo caso)
funzioni e che riesca a fare la chiamata al socket. Restituisce il valore 0 se Φ
...
/*Beh...ma sapete giα tutto!!La solita routine di checksum...giα vista anche questa!*/
...
/*EHEHEHEH...qui iniziano le cose pi∙ carine! Come notate ho messo che Φ stato
preso da coder@reptile.Vediamo di parlarne un po'!
E' una funzione che si aspetta:
fd Φ il socket di ricezione(noi gli passiamo,come visto sopra, fd_recive)
*infoz Φ il puntatore alla struttura wait_packet che va a riempire i
...
in C e il codice non Φ molto difficile!Leggetevelo per bene!*/
...
/*Come sopra con la differenza che questa Φ la funzione che spedisce i nostri
pacchetti!
Spiego alcuni campi:
il primo Φ il socket che manda i pacchetti.
sp_data: IP e TCP option da trasmettere(NULL per non trasmettere nulla)
sp_ipoptlen: lunghezza dell'IP option(per noi sarα 0)
...
sp_ack: chissα cos'Φ?:-)))
sp_flags: quella che per noi sarα sampre SYN
...
/*Anche questa Φ giα tutta spiegata perchΦ stava alla base del mio piccolo sniffer!
Riguardatevi TCP/IP tut parte 2...Φ tutto l∞!*/
...
Se la porta a cui ho spedito il mio pacchetto RSTta la connessione Φ perchΦ
la porta Φ chiusa. Se invece risponde con un pacchetto con flag SYN|ACK
e un seq diverso da 0(non so quant'Φ...dovrei prevederlo...ma qui non ci interessa,
basta sia =! da 0) allora la porta Φ aperta e,come detto sopra, ci pensa il nostro
...
il servizio delle porte la accetto volentieri!Il costrutto case Φ poco
elegante:-)))*/
...
Hola a tutti!Eccoci qui...mi davate x disperso eh?:)Tranquilli...Φ che sotto
esami non ho molto tempo!X questo vi propongo qui oggi un piccolo tool,nulla
di speciale,ma che cmq vi sarα molto utile,se non altro x lo studio del codice!
Faccio subito un piccolo Disclaimer:NON Φ nulla di NUOVO! Giα FuSyS l'ha fatto
e giα lui si Φ ispirato a Coder@reptile! Non rompetemi le palle dicendo che ho
rippato,ok?Il meccanismo E' questo...non possono essere tanto diversi ok?
Bene...non star≥ qui a spiegarvi tanto cosa fa...anzi,x nulla!=)
...
MARSIO,Xanthic',Click...e tutti quelli vedr≥ gioved∞ all'InfoSecurity(spero...)
...
int DEV_PREFIX; /*serve x l'interfaccia che useremo,in questo caso eth!*/
		/*Φ la lunghezza in byte dell'header chiamato con SOCKET_PACKET*/
		/*dovrα concordare con il valore dell'interfaccia che usiamo(vedere main)*/
sig_atomic_t WAIT_PACKET_WAIT_TIME = 0;
...
struct IPhdr
{
        ...
        unsigned short checksum;	/*mah...cosa sarα mai?:)))*/
...
int if_init(char *device)	/*questa parte Φ il controllo degli errori*/
				...
				/*#include&lt;linux/if.h>...vedetevelo,non fa mai male!*/
{
	...
	if ((if_fd = socket(AF_INET, SOCK_PACKET, htons(0x3)))==-1) {
        	perror("Can't Open PACKET SOCKET.");
		exit(1);	/*ricordate nel meeting?Questa Φ la chiamata a sock_packet*/
	...
	if(ioctl(if_fd,SIOCSIFFLAGS,&netif)<0) {
		...
		exit(1);/*interfaccia in modalitα promiscua...vi serve x leggere i pacchetti non*/
...
void ethclose()  /*funzione che mette l'eth in modalitα promiscua(chiaramente usa l'ioctl*/
...
/*questa sotto Φ la funzione che sniffa i pacchetti che arrivano alla vostra macchina*/
/*dovreste ormai ritrovarvi con codice...riguardatevi il mio piccolo sniffer e la*/
/*teoria sui puntatori...non Φ difficile*/
int wait_pk(int fd,struct wait_packet *infoz,                         /* fd Φ un socket di ricezione*/
...
/*ok...tutto quello che c'era l∞ sopra Φ contenuta nel bellissimo spoofit.h di coder@reptile!*/
/*avrei anche fatto prima a mettere l'include...ma cos∞ pochi se lo sarebbero letto e */
...
int main(int argc, char *argv[])
{
        ...
        /*la riga sopra Φ quella fondamentale per i nostri loschi scopi!Provate a fare qualcosa di simile a "echo + + >>/.rhosts"*/
...
i due(vedere meeting). Ora il client resta in Fin_wait finchΦ il kernel non
cestina la connessione, e noi impersoniamo lui visto che possiamo tranquillamente
sniffare i corretti SEQ e ACK! Cos∞,dopo aver pulito la shell, risniffiamo i dati
e,se tutto Φ andato bene, inseriamo i nostri evil_data:)sotto mentite
...
Io vi lascio solo questo perchΦ ora ho altre cose in mente da sviluppare...se
...
Come non detto...Φ passato un giorno e mi sono rimesso a pacioccarlo:)
Vi allego un secondo main in cui potete inviare i cmd interattivamente...metterli
cioΦ durante l'esecuzione del programma e non nel codice come nel precedente.
Non ho una LAN attualmente a disposizione su cui provare le modifiche...in teoria
dovrebbe andare,se qualcuno lo provasse e vedesse dei bug Φ pregato di correggerli
...
Una piccola postilla...NON l'ho chiamato SauroN solo perchΦ Φ uscito il film
...
Ben 278 pagine...beh,come numero finale "NoFlyZone_zine_vecchio_stile" non c'Φ male.
Voi cosa dite? Ci siamo sbattutti parecchio x quest'ultimo numero e credo che il
lavoro complessivo non sia affatto male...ce n'Φ proprio x tutti i gusti!
Una notevole parte sulla programmazione ha distinto questo numero da quelli vecchi!
E poi, diciamolo...siamo nati da 6 mesi e abbiamo giα fatto 3 numeri belli corposi!
La voglia direi che c'Φ...e le conoscenze aumentano di numero in numero:)
Ma la grande novitα sarα nel prossimo numero: abbiamo deciso di cambiare un po'
...
siamo posti credo che x qualche mese non uscirα nessun nuovo numero, magari qualche
tut nel sito (dateci il tempo di studiare:)).
Cmq non sarebbe male avere anche qualche vostro parere...se pensate che vada bene cos∞
ditecelo, se volete cose pi∙ tecniche fatecelo sapere: la mail sapete cos'Φ e come
...
"ricontrollata"...
al prossimo numero manca ancora molto... anche perchΦ ci saranno _molte_ novitα, ma
non Φ ancora tempo di previews...
attendete e continuate a leggere i nostri txt sul sito di nfz:

 http://www.noflyzone-crew.cjb.net/

ed ora vi delizier≥ con:
