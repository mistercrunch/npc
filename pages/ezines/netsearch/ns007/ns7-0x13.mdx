...
mucha informaci≤n, tambien puedes especificar determinadas cosas que el
...
informaci≤n que puedes consultar. Empecemos a ver que significa cada uno de
...
ficheros de datos que te puedan dar informaci≤n del sistema, por ejemplo, si
...
como todos sabemos ext2 es el FS principal, los demas son a±adidos, como el
...
nada de el ni modificar sino es via kernel (compilando y a±adiendo nuevas
cosas), pondre dos ejemplos de lo que podemos conseguir de este directorio,
por ejemplo, imaginate que no sabes si la eth0 esta a±adida como
dispositivo, pues haciendo un "cat /proc/net/dev" podrias mirar si esta el
dispositivo a±adido en el sistema y aparte ver si ha transmitido algo, la
...
en Θl, por ahora cambiaremos la cantidad de ficheros que el sistema puede
...
la maquina no rebootea, desactiva ese tipo de funci≤n. para poner 1 al
...
KERN_ALERT    "<1>"  /* debe tomarse una acci≤n inmediata
KERN_CRIT     "<2>"  /* condiciones crφticas
KERN_ERR      "<3>"  /* condiciones de error
KERN_WARNING  "<4>"  /* condiciones de advertencia
KERN_NOTICE   "<5>"  /* condici≤n normal pero significativa
KERN_INFO     "<6>"  /* informaci≤n
KERN_DEBUG    "<7>"  /* mensajes del nivel de depuraci≤n

No me metere en temas de programaci≤n porque no lo veo necesario para este
...
modificar o simplemente sacar informaci≤n de como tenemos el sistema:
...
intentare hacerlo lo mßs resumido posible. Con este directorio podemos
controlar todo lo relacionado a este protocolo, ya sea icmp, tcp, udp ....,
ahora explicaremos los ficheros que nos podremos encontrar en Θl:
...
masquerade, para maquinas que esten realizando esa funci≤n.
...
peque±os trozos) se resensamblan (defragmentan) anteas de empezar el
...
mini-estudio sobre Θl me ha dado la razon, es importante en el sistema y se
