...
Hace ya un a±o desde el numero anterior, como pasa el tiempo.. Aunque parez-
ca lo contrario, durante este a±o no hemos estado parados, ni mucho menos.  
...
En cuanto a la extra±a mania que alguna gente le tiene a las traducciones,
simplemente no la entiendo. Yo personalmente, si tengo que elegir entre leer
un texto en ingles o en un espa±ol bien traducido, prefiero el espa±ol. Me
...
ficheros distintos. Aun asi, se ha a±adido una nueva caracteristica al
...
que acompa±a al ezine y ejecutar "./nextract -s ns004.txt", con lo que se
...
gran parte de la eficacia del algoritmo (asi como tambien en el tama±o
...
* El maximo comun divisor de p-1 y q-1 debe de ser peque±o
...
es porque ya sabes lo que es.
 ...
 Representare el sumatorio con   \      por poner un ejemplo.
                                ...
                                /____
                                 i = 0
 (
(     <- esto intenta ser un parentisis "tama±o grande" :P
(
 (
...
Para simplificar los calculos, se van a tomar dos primos peque±os: p=5
...
- LUCENA L╙PEZ, Manuel Jose, Criptografia y seguridad en Computadores.
...
automatica cuando entremos en nivel 3 a±adiremos la siguiente linea
...
fichero /etc/vfstab ya a±adiremos la siguiente linea
...
[] por (). Despues a las comillas se les puede a±adir un numero, viendolo en
...
Bueno, y despues de esta peque±a introduccion veamos un codigo de una shell-
...
veniente a±adir un exit(0). Eso es lo que vamos a hacer a continuacion.
...
remoto, comprobando el tama±o de la ip, pudiendo cambiar una ip por otra sin
...
todo de esto de NetSearch :).. Era alla por el a±o 1997 sino recuerdo mal..
...
seguimos al pie del ca±on ;). Como iba diciendo, en dicho canal empezo a
...
Al a±o aproximadamente de la apertura del canal surgio la idea de crear un
...
partir del tercer numero la cosa se enfrio.. y paso mas de un a±o sin
...
como maximo, y no llevar caracteres especiales excepto la e±e. Si quereis
...
Alla por marzo del 96 la compa±ia 3com se invento una aparatito de lo mas 
curioso que cumplia perfectamente las labores de la agenda electronica 
aparecida a±os antes (vease casio) pero que ademas iba mas alla con 
...
escucha. Siempre podremos enriquecer mas nuestro pppd a±adiendole mas 
parametros.
...
Y activamos la opcion de forwarding. Dos maneras:
        ...
        a)  A±adir linea: gateway_enable="YES" al fichero /etc/rc.conf 
...
que solo afectan a las aplicaciones de dicha empresa.
   ...
   El virus Melissa, tan famoso el a±o pasado, utilizaba el lenguaje de macros
...
sistema para enga±ar al pobre usuario que sin tener conocimientos basicos sobre
como protegerse de agresiones externas (de lo que no les culpo, ya que la culpa
es de los medios, que cada vez mas se empe±an en asociar la seguridad
...
mercado). Bueno, me dejo de preambulos y comienzo este peque±o vistazo a
...
peque±a funcion que lo oculte, aunque si el que lo edita es un poco avispado no
...
pone Internet y mientras su US Robotic 56K emite unos extra±os pero graciosos
...
enviar un mensaje al movil que se acaba de comprar su compa±ero de clase.
...
apaga el ordenador.
   ...
   (Mientras, en las entra±as del Pentium 500 con 128 de RAM)
...
peque±o gusano se dejo caer a traves de Outlook en el directorio inicio de
...
peque±o apa±o en el registro tocando los punteros del escritorio para que el
...
las 4 de la ma±ana y tengo que levantarme a las 7:30 para ir a clase. Que
...
Nada mas, saludos a toda la pe±a, y a leer, que son dos dias.
...
michelines, acompa±ado de un pantalon color beis y unos calcetines que no
...
retorica. Tambien suele usar ruidos extra±os para evitar las pausas que
...
"tembleque de pierna", y consiste en un peque±o temblorcillo que le empieza
...
causarle unos da±os cerebrales importantisimos e irreparables, tomeselo con
...
cuando llega a clase o solo lo parece. Esta prueba suele acompa±arse de
...
todas las anteriores tienes ante ti a un ser sumamente extra±o.
...
intentara hacer el dibujo lo mejor posible, incluso a veces a±adiendole
...
a±o me esta tocando vivir uno de estos "especimenes" en mis propias carnes,
...
Cuando estemos a±adiendo los "record" nos dara la opcion de a±adir un PTR,
que sale se±alada por defecto. No se trata de otra cosa que de una DNS
inversa. (tendriamos que crear nosotros la zona)
Ejemplo: IP: 192.168.1.2
DNS Inversa: 1.168.192.In-addr.arpa


Recuerda a±adir en la configuracion del TCP/IP, en las DNS, tu ip de la
maquina.
...
2d. Fecha de Constitucion........: 19790320 /* a±o mes dia (todo junto)*/
...
3c. Fecha de Concesion...........:    /* a±o mes dia (todo junto) */
...
* Apuntes de clase de NT de 2║ de Administracion de Sistemas Informaticos.
...
&lt;aViNash> bueno... tengo 19 a±os, soy del sur y ahora mismo estoy en el
...
&lt;aViNash> mmm, bueno.... recuerdo k a eso de los 14 a±os me compre mi 
          ...
          pIII a 550, pero como mi ni±a no hay nada ;).
...
&lt;aViNash> bueno... maravillosos a±os. la idea de 2500Hz surgio de las  
...
&lt;aViNash> bueno... el tiempo en si no lo recuerdo, creo k estuvimos unos 2
          a±os. lo formabamos codex, neuronal, crash, damnation, kay,
...
a±ada nada nuevo, es algo que me ha salido espontaneamente. Todos los
...
(pobrecillos no tienen la culpa de que se les olvide hacer una peque±a
rutina que mida el tama±o de las variables para que cuando hay una variable
grande y otra variable peque±a no se meta la grande en la peque±a) se les
olvida chequear el tama±o de las variables(puede ser tanto texto como
...
variable de menor tama±o, esto sucede muy a menudo y en la actualidad se dan
...
que hay que hacer es averiguar el tama±o de la variable local que soporta a
esta cadena que introducimos nosotros de tal forma que escribiendo ese
tama±o y a su vez 8 bytes mas que ocupan ebp y ret podamos tener nuestro
buffer formateado y listo para desbordar el buffer.

Para averiguar el tama±o de la variable local sigamos probando con el gdb,
...
en la pila lo que queramos. El codigo de este exploit lo a±ado al final(lo
...
Esto es porque la pila tiene una estructura un tanto extra±a y hay que
...
He de comentar que la forma aqui expuesta no es nada extra±a, en C
normalmente tambien se formatea el buffer para que tenga la misma
estructura, solo que hay que a±adir un par de lineas mas y un par de bucles.
Lo unico que realmente creo que sea molesto es a±adir la shellcode y
calcular su tama±o, en cuanto a lo demas esta tirado, es coser y cantar ya
que el tama±o de los nop's los vamos a poner nosotros con perl y el tama±o
...
peque±ito programa en C como el que sigue:
...
main()
 ...
 // aqui podeis poner vuestra propia shellcode
  ...
  "\x03\xcd\x80\x31\xdb\x89\xd8\x40\xcd\x80\xe8\xd9\xff"
  "\xff\xff/bin/sh";
 // acordaros de a±adir el punto y coma al final de la variable shellcode
...
Bien, sabemos que el tama±o de nuestra cadena de formato va a estar mas o
...
Es extra±o, pero hemos sobreescrito 3 bytes del ebp y no se ha producido un
segmentation fault, ha sido cuando hemos ocupado su cuarto byte cuando se ha
producido. Bien esto es buena se±al quiere decir que hemos encontrado el
...
temiendo de que el tama±o de $NOPS+$SHELLC+$RET no es el correcto. Vamos a
medir su tama±o porque esto no puede ser:
...
caracter que se haya imprimido de la shellcode, esto lo hacemos asi:
	...
	Nota: el vi(editor de texto que estoy utilizando) me hace cosas
              extra±as cuando pego el contenido de SHELLC aqui, por lo que
...
contando y buscando el caracter que nos esta haciendo la pu±eta:
...
a±adirlos individualmente para que no se nos corte nuestra cadena
formateada, porque si se corta nuestra cadena no tendra el tama±o deseado y
...
\xab\xb0\x08\x04\x03\xcd\x80\x31\xdb\x89\xd8\x40\xcd\x80\xe8\xd9"'`      
/bin/shp≈  ┐p≈  ┐"
...
<-->
	...
	Un saludo a toda la pe±a que me conoce y espero que este documento
...
tema despues de a±os de estudio. En Internet se puede encontrar mucha
informacion sobre Echelon, pero al ser un asunto polemico, es dificil saber
que es cierto y que es exageracion. Por mi parte, intentare ce±irme solo a
...
mara±a de pactos y alianzas entre gobiernos, se ha tratado de explicar cada
...
inteligencia enemigos y descifrar esta informacion. A esto hay que a±adir que
...
Durante 40 a±os se han desarrollado sistemas cada vez mas potentes,
...
presidente Ronald Reagan a±adio a estas tareas la seguridad de los sistemas
...
EEUU; los mensajes descifrados pasan al equipo de lingⁿistas, que los
...
pocas se±ales escapan a este minucioso rastreo.
...
oceano Pacifico y el este del oceano indico.
  . El GSCB de Nueva Zelanda recoge se±ales en el sur del oceano Pacifico,
...
precision en la transmision que impedia que las se±ales del hemisferio sur
...
[10] "Secret Power", p. 28. Ver nota [3].
	...
	Actualmente, la estacion de Morwenstow intercepta las se±ales de los
...
latinoamericanos, entre ellos el satelite Morelos de la compa±ia telefonica
mejicana [13].
...
militares. El papel de estas estaciones fue muy importante en los a±os
...
Dentro de territorios nacionales se utilizan tambien se±ales en VHF (Very
...
de direcciones en alta frecuencia) intercepta se±ales con el unico proposito
...
un peque±o centro en la Base de las Fuerzas Aereas de Lackland, San Antonio,
...
Ferret en los a±os 60; los Canyon, Rhyolite y Aquacade en los 70; los Chalet,
...
http://www.the-times.co.uk/news/pages/sti/98/05/31/stifocnws01003.html?112407
	...
	Al a±adir a Menwith Hill la primera base de intercepcion de satelites
...
se±al de los satelites Vortex de 2¬ generacion, geosincronos, y trafico
...
a±os 80, este sistema se dedica a espiar satelites de otros paises, asi como
...
presume que podrian rastrear trafico militar y diplomatico transeuropeo.
  ...
  . GT-6: Instalado a finales de 1.996, se cree que intercepta las se±ales de
los satelites geosincronos de 3¬ generacion, Advanced Orion (tambien
...
satelites en orbita, recibiendo asimismo se±ales de otros satelites que
monitorizan las comunicaciones terrestres, interceptando se±ales de radio y
...
espia, toda se±al de comunicacion.


3.3. Los diccionarios Echelon

	Las posibilidades de intercepcion de Echelon es ya de por si una
realidad escalofriante. Pero a esto hay que a±adir su capacidad para
...
llegar las se±ales a las estaciones, son procesadas por sistemas como el
...
la compa±ia britanica Memex [25], se mueve por las bases de datos de mensajes
...
del a±o), buscando secuencias de palabras, numeros de telefono y fax, y
...
"gestor de diccionarios" de cada agencia se ocupa de a±adir, borrar o cambiar
...
con una peque±a modificacion; una llamada a fork() para que el BitchX no se
quede bloqueado hasta que se cierre el xterm remoto.

El xploit tiene unos peque±os inconvenientes, debido a que la direccion de
...
barre±o con agua mientras se seca el pelo con el secador, y vera como ve las 
cosas de otro color)
...
llevo entenderlo 4 meses de una asignatura un pelin pe±azo ;) pero podemos
...
mas que en eso. Por ejemplo, si tomamos una porcion muy peque±a de la memoria
...
actuales) habia demostrado a±os atras que con una buena organizacion del 
...
nombre "datos" y de tama±o 20 bytes. Esta es la forma tradicional y
...
da al programa. Usualmente esto se hace con:
	...
	char *datos;
        datos=(char *) malloc(TAMA╤O_DE_MEMORIA);
...
que significan esas pu±eteras siglas ;) es simplemente la zona de memoria
