...
```
            WinHelp virus programmers reference        ▄█████▄ ▄█████▄ ▄█████▄
                        Compiled by                    ███ ███ ███ ███ ███ ███
                        Darkman/29A                     ▄▄▄██▀ ▀██████ ███████
                                                       ███▄▄▄▄ ▄▄▄▄███ ███ ███
                                                       ███████ ██████▀ ███ ███
...
Searches for A-keywords.
  ...
  Usage       ALink(`keyword[;ákeyword'] [,átype[,á`context-string'
              [,á`window-name']]])
  Alias       AL
  ...
  Parameters  keyword
              ...
              æ@Æ character and the name of the Help file.
              window-name
              ...
              this parameter should be "main".áIf this parameter is not
...
Opens a control panel applet with a specific tab on top.
  ...
  Usage       ControlPanel(`cpl-name'[,á`panel-name',átabnum])
  Alias       None
...
Executes a program.
  ...
  Usage       ExecFile(`program'[,á`arguments'[,ádisplay-state[,
              `context-string']]])
  Alias       EF
...
Searches for K-Keywords
  ...
  Usage       KLink(`keyword[;ákeyword'] [,átype[,á`context-string'[,
              `window-name']]])
  Alias        KL
...
Registers a function in a Dynamic Link Library (DLL) as a custom help macro.
  ...
  Comments    If WinHelp cannot find the DLL, an error message is issued and
              ...
              when running WindowsáNT) must the _pascal calling convention.
...
Opens or prints the specified file.
  ...
  Usage       ShellExecute(`filename',á[options[,áshow-flag[,áoperation[,
              path[, `context-string']]]]])
  Alias       SE
...
Runs the specified program if it is not already running.
  ...
  Usage       ShortCut(`window-class',á`program'[,áwParam[,álParam[,á
              `context-string']]])
  Alias       SH
...
Tests whether an ALink macro has an effective link to at least one topic.
  ...
  Usage       TestALink(`keyword[;ákeyword]')
  Alias       None
...
Tests whether a KLink macro has an effective link to at least one topic.
  ...
  Usage       TestKLink(`keyword[;ákeyword]')
  Alias       None
...
Winhlp32.exe command-line switches
  ...
  winhlp32.exeá[[-H]á[-G[n]]á[-Wáwindow]á[-Kákeyword]á[-Pápop-up]
               [-NácontextNum]á[-IátopicID]áhelpFile]
  ...
  -Pá      pop-up specifies that the window will be shown in a pop-up window.
           This command cannot be used with -W.
  -Ká      keyword specifies the topic to open using a keyword. This command
           cannot be used with -I or -N.
  -Ná      contextNum specifies the topic to open using a topic number, which
           must be defined in the [MAP] section of the HPJ file. This command
           cannot be used with -I or -K.

  -Iá      topicID specifies the topic to open using a topic ID string (#
