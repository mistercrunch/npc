...
L'utente Φ pregato di non mettere in atto quanto scritto. Esso Φ stato pubblicato solo
...
INFORMAZIONE, e l'informazione per legge Φ LIBERA. Leggendo queste righe tu ACCETTI I
...
Attualmente non sto curando nΦ accaet nΦ brokerall (www.brokerall.da.ru) come vi citai. 
Chiamatemi anche sfaticato ma io ho un solo computer, e questo (connesso ad una lan locale) 
non Φ usato solo da me purtroppo.
In cambio vi prometto di far aggiornare un po' pi∙ costantemente il sito, con testi forse 
un po' di meno.
Cosa posso dirvi, che sto cercando di ampliare la rivista anche per 
quanto riguarda un po' di programmazione, soprattutto C. 
Non lamentatevi per favore, anche perchΦ al momento accaet Φ composta da poche persone 
e quelle poche non sono tanto "attive", senza fare nomi naturalmente... :)
Colgo l'argomento per farvi un annuncio importante: accaet Φ in cerca di nuovi membri 
...
Inutile dirvelo, ma ve lo dico lo stesso, che non faremo nΦ sorteggi nΦ ampolle come nel lotto, 
e non siamo intenzionati a collaborare con il primo lameruccio che mi fa vedere un testo... :)
Quindi fatevi avanti, male che vα non vi insulteremo di certo :)
Mentre sto scrivendo, quello che pi∙ mi mormora alla testa Φ il fatto che oggi sono poche 
...
a me questo non piace, anche perchΦ se faccio una promessa con i lettori mi piace 
sempre mantenerla, non tanto per fare il moralista del caxxo ma perchΦ alle 
soddisfazioni dei lettori si aggiungono di conseguenza le mie.
Anche se nella pagina dei credits non Φ stato inserito il webmaster di accaet Φ Ghent :)
Quindi per consigli, miglioramenti o errori scrivete lui e non pi∙ me.
...
Mi dispiace di non aver lasciato in sospeso il fakeDos, ma proprio non c'Φ il
tempo di sistemarlo per bene... vi lascio il sorgente com'Φ al momento, piu'
...
> filmato giα compilato (l'swf, insomma). Per studiare
> tutti gli effetti ed imparare qualcosina
> dovrei avere il .fla, ma come ben sai non Φ possibile!
> O si? Esistono programmi che 'craccano' il filmato
> dando il .fla?

La questione Φ un po' diversa... la macromedia ha rilasciato le specifiche
...
Esistono pero' vari programmi, che ti consiglio di cercare, che possono permetterti
 ...
 in Flash (come serie di keyframes, pero'... tutti sullo stesso layer), poi c'Φ l'ottimo
...
Per questa volta Φ tutto... scriveteci :)
...
/===========================::=============================::=================/
	...
	`distruggere lo stack` [programmazione C]. In molte implementazioni C Φ
	possibile corrompere lo stack d'esecuzione scrivendo dopo la fine di un
	array dichiarato auto in una routine. Si dice che il codice che fa ci≥
	distrugga lo stack, e pu≥ causare l'uscita dalla routine saltando a un
	indirizzo casuale. Questo pu≥ causare i bugs data-dependent pi∙ 
...
Negli ultimi mesi c'Φ stato un notevole incremento di vulnerabilitα
...
concetti della memoria virtuale e esperienza con gdb pu≥ essere utile ma
non necessaria. Premettiamo inoltre che stiamo lavorando con un CPU Intel
x86, e che il sistema operativo Φ Linux.

Alcune definizioni di base prima di iniziare:
Un buffer Φ semplicemente un blocco di memoria del computer che gestisce
pi∙ esempi dello stesso tipo di dati. I programmatori di C associano
normalmente con la parola buffer, array. Pi∙ comunemente, array di
...
momento d'avvio dello stack. Creare Φ un overflow vuol dire riempire
...
Per capire cosa sono i buffer dello stack dobbiamo prima capire come Φ
...
La regione testo Φ fissata dal programma e include il codice
(istruzioni) e dati in sola lettura. Questa regione corrisponde alla
sezione del testo del file eseguibile. Questa regione Φ normalmente
contrassegnata in sola lettura e ogni tentativo di scriverci darα luogo a
...
sezioni data-bss di un file eseguibile. La sua dimensione pu≥ essere
cambiata con la chiamata brk(2). Se l'espansione dei dati bss o lo stack
dell'utente esaurisce la memoria disponibile, il processo Φ bloccato ed
verrα eseguito di nuovo con uno spazio di memoria maggiore. La nuova
memoria Φ aggiunta tra i dati e i segmenti dello stack.
   ...
   Uno stack Φ un tipo di dati astatto usato frequentemente in 
informatica. Uno stack di oggetti ha la proprietα che l'ultimo oggetto 
inserito sarα il primo oggetto eliminato. Questa proprietα Φ chiamata 
Last-In-First-Out (LIFO).

   Sono definite molte operazioni negli stacks. Le pi∙ importanti sono PUSH e 
...
dimensione dello stack eliminando l'ultimo elemento inserito nello stack.
                           ...
                           PerchΦ usiamo uno Stack?
                           ~~~~~~~~~~~~~~~~~~~~~~~~

  I computer moderni sono progettati tenendo conto i linguaggi di 
programmazione ad alto livello. La tecnica pi∙ importante per strutturare 
dei programmi introdotta dai linguaggi ad alto livello Φ la procedura o 
...
caratteristica Φ implementata con l'aiuto dello stack.

  Lo stack Φ anche usato per allocare dinamicamente le variabili locali 
...
restituire valori dalla funzione.
   ...
   Uno stack Φ un blocco contiguo di memoria contenente dati. Un registro 
chiamato stack pointer (SP) punta alla cima dello stack. La base dello 
stack Φ a un indirizzo fissato. La sua dimensione Φ adattata dinamicamente 
dal kernel al momento dell'avvio. La CPU implementa le istruzioni di PUSH 
e di POP.

   Lo stack Φ composto da frammenti logici di stack che sono "pushed" 
...
funzione.
   ...
   A seconda dell'implementazione lo stack diminuirα (verso gli indirizzi 
di memoria pi∙ bassi), o crescerα. Nei nostri esempi useremo uno stack che 
...
Φ comunque un'implementazione dipendente. Pu≥ puntare all'ultimo indirizzo 
...
(indirizzi numericamente pi∙ bassi, Φ spesso conveniente avere un frame 
...
stack, questi offset cambiano. In alcuni casi il compilatore pu≥ tener 
conto del numero di parole nello stack e cos∞ correggere gli offset, in 
alcuni casi non pu≥, e in tutti i casi Φ necessaria un'amministrazione 
...
dallo SP richiede pi∙ istruzioni.

   Di conseguenza, molti compilatori usano un secondo registro, l'FP, per 
riferirsi sia alle variabili locali sia ai parametri perchΦ la loro 
distanza dall'FTP non cambia con i PUSH e i POP. Su processori Intel, il 
BP (EBP) Φ usato per questo scopo. Dato che il nostro stack cambia, i 
parametri attuali hanno offset positivi e le variabili locali hanno offset 
negativi dall'FP.

   La prima cosa che una procedura deve fare quando Φ chiamata Φ salvare 
l'FP precedente. Poi copia l'SP nell'FP per creare il nuovo FP, e avanza 
l'SP per riservare spazio per le variabili locali. Questo codice Φ 
chiamato "procedure prolog". Quando la procedura termina, lo stack deve 
essere pulito di nuovo; questa procedura Φ chiamata "procedure epilog". Le 
...
L'istruzione 'call' inserirα l'instruction pointer (IP) nello stack. 
Chiamaremo l'IP salvato return address (RET). La prima cosa nella funzione 
Φ nel procedure prolog:
...
locali sottraendo la sua dimensione da SP.
   ...
   Dobbiamo ricordare che la memoria pu≥ essere solo indirizzata in 
multipli della dimensione di parola. Una parola nel nostro caso Φ 4 bytes, 
o 32 bits. Cos∞ il nostro buffer di 5 bytes andrα ad occupare 8 bytes (2 
parole) di memoria, e il nostro buffer di 10 bytes andrα ad occupare 12 
bytes (3 parole) di memoria. Questo perchΦ SP Φ stato sottratto da 20. 
Tenendo presente ci≥, il nostro stack Φ qualcosa di simile, quando 
function() Φ chiamata (ogni spazio rappresenta un byte):
...
stack                                                                     stack
   ...
   Un buffer overflow Φ il risultato dell'inserimento di dati in uno 
buffer in quantitα maggiore che il buffer pu≥ gestire. Come facciamo ad 
...
------------------------------------------------------------------------------
   ...
   Questo Φ un programma che ha un tipico errore. La funzione copia una 
determinata stringa usando strcpy() e non strncpy(). Se esegui questo 
programma ci sarα una violazione di segmentazione. Vediamo come appare lo 
...
stack                                                                     stack
   ...
   Che sta succedendo? PerchΦ abbiamo una violazione di segmentazione? 
Semplice. strcpy() sta copiando il contenuto di *str in buffer[] finchΦ 
non viene trovato un carattere nullo nella stringa. Come possiamo vedere 
buffer[] Φ molto pi∙ piccolo di *str. buffer[] Φ lungo 16 bytes, e stiamo 
...
valore esadecimale Φ 0x41. Questo significa che l'indirizzo di ritorno Φ 
ora 0x41414141. E' al di fuori dello spazio del processo. Questo perchΦ 
...
quell'indirizzo, si ha una violazione di segmentazione. 
   ...
   Cos∞ un buffer overflow ci permette di cambiare l'indirizzo di ritorno 
...
codice arbitrario. Appena prima di buffer1[] sullo stack c'Φ SFP, e prima 
di esso, l'indirizzo di ritorno. Questo Φ 4 bytes dopo la fine di 
buffer1[]. Ma ricorda che buffer1[] Φ 2 parole e quindi 8 bytes. Quindi 
l'indirizzo di ritorno Φ 12 bytes dopo l'inizio di buffer1[]. 
Modificheremo il valore di ritorno in modo tale che l'istruzione di 
assegnamento 'x =1;' verrα saltata dopo la chiamata di funzione. Per fare 
ci≥ aggiungiamo 8 bytes all'indirizzo di ritorno. Il nostro codice sarα:
...
Φ dove Φ memorizzato l'indirizzo di ritorno. Vogliamo evitare 
...
------------------------------------------------------------------------------
   ...
   Possiamo vedere che quando chiamamo function() il RET Φ 0x80004a8, e 
l'assegnamento Φ all'indirizzo 0x80004ab. La prossima istruzione che 
vogliamo eseguire e all'indirizzo 0x80004b2. Un piccolo calcolo ci dice 
che la distanza Φ 8 bytes.
...
facciamo se nel programma non c'Φ il codice che vogiamo exploitare? Come 
possiamo inserire istruzioni arbitrarie nel suo spazio d'indirizzo? La 
risposta Φ mettere codice arbitrario nel buffer che stiamo exploitando, e 
...
codice che vogliamo eseguire; lo stack sarα una cosa simile:
...
Il codice per avere una shell in C Φ qualcosa del genere:
...
0x8000133 &lt;main+3>:     subl   $0x8,%esp
	...
	Questo Φ il preludio della procedura. Dapprima salva il vecchio 
	...
	cos∞ questo lascia spazio per due parole (8 bytes).

0x8000136 &lt;main+6>:     movl   $0x80027b8,0xfffffff8(%ebp)

	Copiamo il valore 0x800027b8 (l'indirizzo della striga "/bin/sh") 
	nel primo puntatore di name[]. Questo Φ equivalente a:
...
0x800013d &lt;main+13>:    movl   $0x0,0xfffffffc(%ebp)
	...
	Questo Φ equivalente a:

	name[1] = NULL;

	La chiamata a execve() comincia qu∞.
...
0x80002c0 <__execve+4>: movl   $0xb,%eax
	...
	Copia 0xb (11 decimali) nello stack. Questo Φ l'indice nela tavola 
	syscall. 11 Φ execve.
...
Come possiamo vedere c'Φ molto per la chiamata execve(). Tutto quello che 
dobbiamo fare Φ:
...
motivo? Il programma continuerα ad andare a prendere istruzioni dallo 
stack, che pu≥ contenere dati casuali! Il programma terminerα sicuramente 
con un core dump. Voggliamo che il programma termini senza problemi se la 
chiamata execve fallisce. Per fare ci≥ dobbiamo aggiungere una chiamata 
d'uscita dopo la chiamata execve. Com'Φ una chiamata d'uscita?
...
------------------------------------------------------------------------------
   ...
   La chiamata exit metterα 0x1 in EAX, metterα il codice d'uscita in EBX, 
ed eseguirα "int 0x80". Ecco tutto. La maggior parte delle applicazioni 
ritornano 0 all'uscita per indicare che non ci sono stati errori. Mettermo 
0 in EBX. La nostra lista di passi Φ ora:
...
------------------------------------------------------------------------------
	...
	movb   $0x0,null_byte_addr
        ...
        movl   $0x0, %ebx
	int    $0x80
        la stringa /bin/sh va qu∞.
------------------------------------------------------------------------------

   Il problema Φ che non sappiamo dove sarα messo nello spazio di memoria 
del programma stiamo cercando di exploitare il codice (e la stringa che lo 
segue). Un modo per saperlo Φ quello di usare JMP e CALL. Le istruzioni 
JMP e CALL possono usare indirizzamenti IP relativi, e ci≥ significa che 
...
l'indirizzo di ritorno quando viene eseguita CALL. Tutto ci≥ di cui 
abbiamo bisogno Φ copiare l'indirizzo di ritorno nel registro. 
L'istruzione CALL pu≥ semplicemente chiamare l'inizio del nostro codice. 
Tenendo conto che J sta per l'istruzione JMP, C per CALL e s per la 
stringa, il flusso d'esecuzione sarα ora:
...
bytes prende ogni istruzione, il nostro codice sarα:
...
compilarlo ed eseguirlo. Ma c'Φ un problema. Il nostro codice si modifica 
...
trasferire il controllo ad esso. Per fare ci≥ metteremo il nostro codice 
...
------------------------------------------------------------------------------
   ...
   Funziona! Ma c'Φ un ostacolo. Nella maggior parte dei casi dobbiamo 
riempire un buffer di caratteri. Dato che ogni byte nullo nel nostro shell 
code indicherα la fine della stringa, la copia verrα terminata. Non ci 
devono essere bytes nulli nello shell code per far in modo che l'exploit 
funzioni. Cerchiamo di eliminare i bytes (e allo stesso tempo renderlo pi∙ 
piccolo).
...
nel buffer. Questo esempio dimostrerα questi punti.
...
------------------------------------------------------------------------------
   ...
   Quello che abbiamo fatto Φ stat oriempire l'array large_string[] con 
l'indirizzo di buffer[], che Φ dove si trova il nostro codice. Poi copiamo 
il nostro shellcode all'inizio della stringa large_string. strcpy() 
copierα poi large_string nel buffer senza fare alcun controllo, e riempirα 
l'indirizzo di ritorno, sovrascrivendolo con l'indirizzo dove il nostro 
codice Φ ora allocato. Una volta che Una volta che terminiamo il main e 
cerchiamo di tornare, verrα eseguita una shell.

   Il problema che dobbiamo affrontare quando cerchiamo di riempire il 
buffer di un altro programma Φ cercare di capire a quale indirizzo starα 
il buffer (e quindi il nostro codice). La risposta Φ che per ogni 
programma lo stack comincerα allo stesso indirizzo. La maggior parte dei 
programmi non inseriscono pi∙ di poche centinaia o migliaia di bytes nello 
stack ogni volta. Conoscendo dove comincia lo stack possiamo cercare di 
indovinare dove sarα il buffer che stiamo cercando di riempire. Ecco un 
piccolo programma che stamperα lo stack pointer:
...
------------------------------------------------------------------------------
   ...
   Assumiamo che questo Φ il programma che vogliamo exploitare:
...
------------------------------------------------------------------------------
   ...
   Come possiamo vedere questo non Φ un processo efficiente. Cercare di 
indovinare l'offset anche se conosciamo l'inizio dello stack Φ pressochΦ 
impossibile. Avremo bisogno di centinaia di tentativi, e nella peggiore 
delle ipotesi anke migliaia. Il problema Φ che abbiamo bisogno di sapere 
esattamente dove l'indirizzo del nostro codice comincia. Un modo per far 
aumentare le nostre possibilitα Φ di imbottire il nostro buffer overflow 
...
metα del nostro overflow con essa. Metteremo il nostro shellcode al 
...
verranno eseguiti finchΦ ci sarα il nostro codice. Nell'architettura Intel 
l'istruzione NOP Φ lunga un byte e traduce a 0x90 in codice macchina. 
Assumendo che lo stack comincia all'indirizzo 0xFF, che S sta per lo 
shellcode, e che N sta per l'istruzione NOP, il nuovo stack sarα qualcosa 
del genere:
...
------------------------------------------------------------------------------
   ...
   Una buona selezione del nostro buffer size Φ circa 100 byte in pi∙ 
rispetto alla dimensione del buffer che stiamo cercando di riempire. 
Questo prenderα il posto del nostro codice alla fine del buffer che stiamo 
...
indovinato. Il buffer che stiamo cercando di sfruttare Φ lungo 512 bytes, 
cos∞ useremo 612. Cerchiamo di exploitare il nostro programma di test con 
il nuovo exploit:
...
possibilitα di gran lunga. Proviamolo in un vero caso di buffer overflow. 
...
Warning: Color name "δ^1ñFF
                           ░
                            ≤V

ñ1ñ╪@ñΦ▄   /bin/shñ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ
 ...
 ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ñ
...
ñ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ ┐ññ  ┐ññ ┐ññ ┐ññ ┐ññ
^C
[aleph1]$ exit
...
ñ1ñ╪@ñΦ▄   /bin/shñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ
 ...
 ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐H
...
ñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐
...
Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ 
...
┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ ┐Hñ 
...
ñ1ñ╪@ñΦ▄   /bin/sh√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√ ┐T√
...
magici. Se xterm Φ installato comesuid root avreo una shell root.


                           Piccolo Buffer Overflows
                           ~~~~~~~~~~~~~~~~~~~~~~~~

   Ci saranno volte che quando il buffer che stai cercando di riempire Φ 
talmente tanto piccolo che lo shellcode non entrerα in esso, e 
sovrascriverα l'indirizzo di ritorno con le istruzioni al posto 
dell'indirizzo del nostro codice, o il numero dei NOP che puoi inserire 
davanti alla stringa Φ talmente piccolo che le possibilitα di indovinare 
...
hai accesso alle variabili d'ambiente del programma.
   ...
   Quello che faremo Φ mettere il nostro shellcode in una variabile 
...
programma viene eseguito, e ogni mofifica fatta da setenv() Φ allocata 
altrove. Lo stack all'inizio Φ una cosa del genere:


      &lt;stringhe>&lt;argomenti>NULL&lt;envp pointers>NULL&lt;argc>&lt;argv>&lt;envp>

   Il nostro nuovo programma avrα una variabile in pi∙, la dimensione 
...
------------------------------------------------------------------------------
   ...
   Al primo tentativo! Ha certamente aumentato le nostre possibilitα.


                              Trovare Buffer Overflows
                              ~~~~~~~~~~~~~~~~~~~~~~~~

   I buffer overflows sono il risultato dell'inserimento di pi∙ 
informazioni dello gestibile in un buffer. PoichΦ il C non ha alcun 
...
quando vanno a riempire le stringhe. gets() Φ una funzione che legge una 
linea da stdin in un buffer finchΦ non trova una nuova linea o un EOF. Non 
esegue controlli per i buffer overflows. La famiglia di funzioni scanf() 
pu≥ creare problemi se stai inserendo una sequenza di caratteri non nulli 
(%s), o inserendo una sequenza non vuota di caratteri da un set specifico 
(%[]), e l'array preso in considerazione non Φ abbastanza lungo per 
accettare l'intera sequenza di caratteri, e non hai stabilito la lunghezza 
massima della stringa. Se l'obiettivo di ognuna di queste funzioni Φ un 
buffer di lunghezza statica, e l'altro argomento Φ qualcosa di derivato 
dall'input dell'utente c'Φ buona possibilitα che Φ presente un buffer 
overflow.

   Un altro costrutto di programmazione solito che troviamo Φ l'uso di un 
...
while, tali programmi sono semplicemente exploitabili.
   ...
   Infine, grep(1) Φ tuo amico. I codici per sistemi operativi gratuiti e 
...
Questo programma Φ stato creato x solo scopo informativo, se farete danni
sul vostro sistema o verso terzi, io non mi assumo nessuna responsabilitα!

D4n1a Φ una semplicissima backdoor scritta in perl, che a differenza di
...
Appena si aprirα il demone esadecimalmente,scirviamo per 2 volte di
seguito il comando:
...
Bene,adesso la nostra backdoor Φ pronta.
...
Cos∞ il programma lo metterα al posto vostro.
...
In primo luogo si deve spiegare quello che Φ un socket. Semplicemente un socket Φ un modo
per parlare con altro computer. Pi∙ precisamente, Φ una modalitα di parlare con altro computer
usando i descrittori di file standard di Unix. In Unix, tutte le azioni di I/O  sono fatte
attraverso la scrittura a la lettura di un descrittore di file. Un descrittore di file Φ solo
...
Le differenze fra i due tipi sono le seguenti:
      ...
      Datagram Sockets - queste socket utilizzano il protocollo UDP ("User Datagram Protocol"); sono
			 senza connessione perchΘ non necessitano di avere una connesione aperta come nelle Stream 
			 Sockets. Nel pacchetto c'Φ l'informazione della destinazione

Potrebbe essere detto molto di pi∙ riguardo a questi due tipi di socket, ma Φ sufficiente per avere i concetti
di base delle socket. Comprendere cos'Φ una socket e questi due tipi socket internet Φ un buon punto di partenza,
ma Φ necessario sapere come lavorare con loro. Si imparerα questo nelle prossime sezioni.

3. STRUTTURE
=======================================

lo scopo di questa sezione non Φ di insegnare le strutture ma dire come sono usate nella programmazione delle socket
in C. Se non sa cos∞Φ una struttura, il consiglio Φ leggere un manuale di C e impararlo. Per il momento, si lasci dire
che una struttura Φ un tipo di dato contenente altri tipi di dato, i quali sono raggruppati insieme in un singolo tipo
definito dall'utente.

Le strutture sono utilizzate nella programmazione delle socket per mantenere le informazioni dell'indirizzo.

La prima struttura Φ struct sockaddr che tiene le informazioni riguardo al socket
...
La struttura seguente non Φ molto usata ma Φ definita come una unione (union).
...
Ad ogni modo, la definizione della struttura Φ la seguente:
...
Ora, Φ possibile parlare meglio della struttura hostent. Nell'esempio del Client Stream, si nota
l'uso di questa struttura. Questa struttura Φ usata per ottenere informazioni riguardo l'host remoto.

La struttura Φ la seguente:
...
Questa struttura Φ definita nel file header netdb.h

All'inizio queste strutture confonderanno molto, ma dopo aver cominciato a scrivere qualche
linea, e dopo aver visto gli esempi, sarα pi∙ facile capirli. Vedere come usarli controllando
...
Questo primo Φ chiamato "Network Byte Order" ed alcune macchine immagazzinano internamente i loro
...
C'Φ una funzione chiamata htonl() che lo convertirebbe. Le seguenti funzioni sono utilizzate per convertire:
...
Si deve pensare perchΘ si necessita di questo. Allora, quando si finirα la lettura di questo documento,
tutto sembrerα pi∙ facile. Tutto quello ci cui si ha bisogno Φ di leggere e fare molta pratica.

Una cosa importante, Φ che sin_addr e sin_port (dalla struttura sockaddr_in) deve essere Network
Byte Order (si vedranno negli esempi le funzioni qui descritte per convertire e si comincerα
a capire).

5. INDIRIZZI IP
=======================================

In C, ci sono alcune funzioni che aiuteranno a manipolare gli indirizzi IP. Si Parlerα di 
funzioni inet_addr() e inet_ntoa().
	...
	/* Si ricordi che questo Φ possibile se si ha una struct dest del tipo sockaddr_in */
...
Si ricordi che inet_addr() ritorna l'indirizzo in Network Byte Order, per cui non Φ necessaria la chiamata htonl().

6. FUNZIONI IMPORTANTI
=======================================

In questa sezione si metterα la sintassi della funzione, il file header che si deve includere per chiamarla, e piccoli
commenti. Oltre alle funzioni menzionate in questo documento, ci sono pi∙, ma si Φ deciso di mettere solo queste. Forse
...
controllare i codici sorgenti del client stream e del server stream (Sezioni 8 e 9).
  ...
  =============
	...
	la funzione socket() ritorna un intero che Φ il descrittore del socket stesso, che si pu≥ utilizzare nelle
	chiamate di sistema successive. Ritorna -1 in caso di errore (questo Φ utilizzato nel controllare gli errori
	nelle routine).
  ...
  ===========
	...
	Gli argomenti sono i seguenti:
		...
		fd      -> Φ il descrittore di file del socket, restituito dalla chiamata socket()
		my_addr -> Φ un puntatore a struct sockaddr
		addrlen -> Φ il valore derivato da sizeof(struct sockaddr)

	bind() Φ usato per assegnare un indirizzo IP al socket chiamata (viene utilizzato con la chiamata listen());
	serve per poter associare un socket con una porta (sulla macchina dove si Φ aperto il socket). In caso di 
	errore ritorna -1.

	Si pu≥ mettere l'IP automaticamente:

		server.sin_port = 0;  	              /* bind() sceglierα una porta a caso */
		server.sin_addr.s_addr = INADDR_ANY;  /* mette l'IP del server automaticamente */

	Un importante aspetto riguardo le porte e la bind() Φ che tutte le porte prima della 1024 sono riservate.
  ...
  ==============
	...
	Gli argomenti sono i seguenti:
		...
		fd        -> Φ il descrittore di file del socket, restituito dalla chiamata socket()
		serv_addr -> Φ un puntatore alla struct sockaddr che contiene l'indirizzo IP di destinazione e la porta
		addrlen   -> Φ il valore derivato da sizeof(struct sockaddr)

	connect() Φ utilizzato per connette su una porta definita di un indirizzo IP. Ritorna -1 in caso di errore.
  ...
  =============
	...
	Gli argomenti sono i seguenti:
		...
		fd      -> Φ il descrittore di file del socket, restituito dalla chiamata socket()
		backlog -> Φ il numero di connessioni che si vogliono mantenere

	listen() Φ utilizzato quando si attendono un delle connessioni in arrivo; questo se si vuole
	che ci si possa connettere all'host su cui risiede la socket in ascolto. Prima di chiamare listen(),
	si deve chiamare bind() o il socket si metterα in ascolto su un numero di porta casuale. Dopo la chiamata
	...
	di sistema Φ:
		...
		2. bind()
		3. listen()
		4. accept() /* Nella prossima sezione si spiegherα come utilizzare accept() */
  ...
  =============
	...
	Gli argomenti sono i seguenti:
		...
		fd      -> Φ il descrittore di file del socket, restituito dalla chiamata listen()
		addr    -> Φ un puntatore a struct sockaddr_in dove risiedono le informazioni sull'host 
			   chiamante e sulla porta
		addrlen -> Φ il valore derivato da sizeof(struct sockaddr_in) prima il suo valore Φ passato alla
		           chiamata accept()

	Quando si cerca di connettersi all'host del socket in ascolto, si deve utilizzare accept() per avere la
	connessione. E' molto semplice da capire: si ottiene la connessione se si Φ accettati.

	Successivamente, si avrα un piccolo esempio sull'uso di accept()  perchΘ c'Φ una piccola differenza 
	...
	D'ora in poi, fd2 sarα usato per aggiungere le chiamate send() e recv().
  ...
  ===========
	...
	Gli argomenti sono i seguenti:
		...
		fd     -> Φ il descrittore al socket verso il quale si vogliono spedire dati
		msg    -> Φ un puntatore al messaggio che si vuole spedire
		len    -> Φ la grandezza (in bytes) del messaggio da spedire
		flags  -> specifica delle opzioni di spedizione (settarlo a 0)


	Questa funzione Φ usata per spedire dati con socket stream o socket datagram CONNESSA.
	Se si vuole spedire dati con un socket datagram NON CONNESSA si deve usare sendto().

	send() ritorna il numero di bytes spediti e ritornerα -1 in caso di errore.
  ...
  ===========
	...
	Gli argomenti sono i seguenti:
		...
		fd      -> Φ il descrittore del socket da cui si vuole ricevere
		buf	-> Φ un puntatore al messaggio in arrivo
		len 	-> Φ la grandezza massima del messaggio
		flags 	-> specifica delle opzini di ricezione (settarlo a 0)

	Come si Φ detto riguardo a send(), questa funzione Φ usata per ricevere dati con socket stream
	o socket datagram CONNESSA. Se si vuole ricevere dati con una socket datagram NON CONNESSA si
	deve usare recvfrom().

	recv() ritorna il numero di bytes letti e ritornerα -1 in caso di errore.
  ...
  =============
	...
	Gli argomenti sono i seguenti:
		...
		to	-> Φ un puntatore a una struct sockaddr
		tolen	-> Φ grandezza dell'indirizzo del socket remoto (set it to sizeof(struct sockaddr))

	Come si nota, sendto() Φ come send(). Ha solamente due ulteriori argomenti: "to" 
	e "tolen".

	sendto() Φ usata con un socket datagram NON CONNESSA e restituisce il numero di byte
  ...
  ===============
	...
	Gli argomenti sono i seguenti:
		...
		from	-> Φ un puntatore a una struct sockaddr
		fromlen	-> Φ un puntatore a un intero che specifica la grandezza dell'indirizzo del
  ...
  =============
	...
	close() Φ utilizzato per chiudere la connessione sul descrittore del socket. Se si invoca close(),
	nessuno sarα pi∙ abilitano a scrive o legge sul socket, e se qualcuno cerca di leggere/scrive riceverα un errore
  ...
  ================
	...
	Gli argomenti sono i seguenti:
		...
		fd	-> Φ il descrittore del socket di cui si vuole sospendere l'attivitα 
		how	-> per questo argomento ci sono 3 possibilitα:
				
					0 -> ricezione disabilitata
					1 -> spedizione disabilitata
					2 -> spedizione e ricezione disabilitate

	Quando how Φ settato sul valore 2, equivale ad utilizzare la chiamata close().
  ...
  ===================
	...
	Gli argomenti sono i seguenti:
		...
		hostname  -> Φ un puntatore ad un array che contiene il nome dell'host
		size 	  -> Φ la grandezza dell'array del nome dell'host (in bytes)
	
	gethostname() Φ utilizzato per ottenere il nome del'host locale

7. ALCUNE PAROLE RIGUARDO AL DNS
=======================================

Questa sezione Φ stata scritta per sapere cos'Φ un DNS. DNS sta per "Domain Name Service" e fondamentalmente,
Φ usato per ottenere gli indirizzi IP. Per esempio, bisogna conoscere l'indirizzo IP di queima.ptlink.net
e attraverso il DNS si otterrα 212.13.37.13. Questo Φ importante, perchΘ le funzioni che abbiamo visto prima
...
#include &lt;stdio.h>
#include &lt;netdb.h>   /* Questo Φ il file header necessario per la funzione gethostbyname() */
#include &lt;sys/types.h>
#include &lt;sys/socket.h>
...
In questa sezione, si mostrerα un esempio di server stream. Il codice sorgente Φ tutto commentato
...
#define PORT 3550   /* porta che verrα aperta */ 
...
#include &lt;sys/socket.h>
#include &lt;netinet/in.h>
#include &lt;netdb.h>        /* netbd.h Φ necessario per struct hostent */
...
int main(int argc, char *argv[])
{
  ...
  char buf[MAXDATASIZE];  /* array in cui sarα immagazzinato il testo ricevuto */
  
  struct hostent *he;         /* struttura da cui si otterranno informazioni riguardo all'host remoto */
  struct sockaddr_in server;  /* informazioni sull'indirizzo del server */

  if(argc != 2) /* questo Φ utilizzato perchΘ il programma necessita di due argomenti (IP) */
  {	      
...
Essendo un semplice essere umano, Φ quasi certo che ci sono alcuni errori in questo documento. Quando dico errori
intendo errori di Inglese (perchΘ la mia lingua non Φ l'Inglese) ma anche errori tecnici. Vi prego di scrivermi se
troverete qualche errore.

Ma bisogna capire che questa Φ la prima versione del documento, cos∞, Φ naturale che non sia completo (come in pratica
di fatto io penso sia) ed Φ anche molto naturale fare degli errori stupidi. Comunque, il codice sorgente presente nel
documento e giusto.
...
/===========================::=============================::=================/
 ...
 L'argomento che andr≥ a trattare Φ cmq molto leggero e semplice da       
 ...
 'sto cazzo di linguaggio ? Cmq se non lo sapete imparatelo, Φ            
 facilissimo e dα risultati interessanti..                                
 /=\=/=\=/=\=/=\=/=\=/=\=/=\=/=\=/=\=/=\=/=\=/=\=/=\=/=\=/=\=/=\=         
 Come dicevo.. Ormai tutti sanno programmare, qual'Φ adesso come adesso   
 lo scoglio pi∙ grande per chi programma ? Passare a programmi che        
 sfruttato le potenzialitα della rete. Infatti raramente negli help       
 troverete voci tipo "Internet" o altro di facilmente collegabile alla    
 programmazione di "rete". E al massimo quello che si trova Φ difficile   
 da capire e ci sono degli esempi che servono a poco o a niente.          
 Invece Φ tutto molto semplice (per lo meno le basi) ed io sono riuscito  
 a capire le basi in meno di un'ora grazie alle spiegazioni che           
 ho trovato.                                                              
 Allora.. In Visual Basic per fare programmi che sfruttano le             
 potenzialitα della rete (chat, finger, demoni, ecc..) abbiamo bisogno    
 ...
 Adesso nella barra degli strumenti sarα apparita un icona con due        
 computer. Andategli sopra con il mouse e apparirα la magica scritta..    
 "Winsock". E adesso ?                                                    
 ...
 due principali protocolli della rete, cioΦ il TCP e l' UDP.              
 BhΦ, non sto qui a spiegarvi l'esatta differenza millesimale tra i due,  
 dato che questo non Φ luogo e capirne la differenza non Φ essenziale.    
 ...
 Questo Φ il disegnino della chat, ora per≥ devo dirmi i nomi altrimenti  
 il codice che vi d≥ non vi vα. Vi d≥ solo i nomi importanti, alcune      
 ...
 porta su cui Φ in ascolto e clicca Connetti                              
 Per≥ sarα meglio che ve le spiego, vero ? Allora..                       
 ====================================================================     
 Private Sub cmdascolta_Click()                                           
     lblstato.Caption = "IN ASCOLTO"                                      
     TCP1.LocalPort = Val(txtporta)                                       
 ...
 winsock.. SendData                                                       
 Per chi sa l'inglese la comprensione Φ semplicissima !! SendData =       
 spedisci dati.                                                           
 ...
 di txtchat.                                                              
 vbCrLf per chi non lo sapesse Φ uguale a Invio.                          
 ====================================================================     
 Private Sub cmdisconnetti_Click()                                        
     lblstato.Caption = "INATTIVO"                                        
     TCP1.Close                                                           
 End Sub                                                                  
 C'Φ da poco spiegare.. Chiude la connessione tramite il comando close.   
 ...
 Private Sub TCP1_Connect()                                               
     ...
     txtip = TCP1.RemoteHost                                              
 End Sub                                                                  
 Mh.. Quando la connessione Φ avvenuta (TCP1_Connect) nella casella       
 ...
 Private Sub TCP1_ConnectionRequest(ByVal requestID As Long)              
     ...
     txtip = TCP1.RemoteHostIP                                            
 End Sub                                                                  
 Giα qui Φ la cosa Φ pi∙ interessante.. Allora..                          
 Quando c'Φ una richiesta di connessione (TCP1_ConnectionRequest) il      
 ...
 per vedere meglio se la connessione Φ attiva o meno) e dice che la       
 ...
 Private Sub TCP1_DataArrival(ByVal bytesTotal As Long)                   
     ...
     txtchat = "Ricevuto:" & Data & vbCrLf & txtchat                      
 End Sub                                                                  
 Questa Φ la procedura che il programma segue in caso di arrivo di        
 dati (TCP1_DataArrival).                                                 
 Dim Data as String definisce i dati che arrivano (che per comoditα ho    
 ...
 da Ricevuto: e seguiti dal testo giα presente nella chat.. E con questo  
 ho finito.                                                               
 ====================================================================     
 ...
 se lo stato della connessione Φ sconnesso allora fai x.                  
...
hanno capito che Blade Runner Φ molto piu' che una cagatella nel quale un poliziotto 
...
"vivere". Qualcosa che cresce/si evolve, si riproduce, muore, Φ vivo?
Di solito si usa definire cosi' qualcosa di vivo, qualcosa che fa le cose che ho appena 
scritto. Ma poi, ci pensate a quanto Φ vaga, 'sta definizione. Ecco, prendiamo il noto 
...
questa Φ online, cerca e scarica dei suoi plug-in, li ingloba e li usa. 
Aggiungete che a quanto so Φ stato reso noto come scrivere un plug-in per tale virus, 
quindi chiunque ne sia capace puo' dotarlo di nuove funzionalita'... confesso, 
questa Φ una cosa che me lo rende simpatico, anche a me che di solito schifo 
il virus writing in quanto spesso distruttivo.
Passiamo al punto due... si riproduce? Eh, si', questa Φ scontata, per un virus: 
...
legata alla vita artificiale. Lo ammetto, questa Φ una cosa che mi affascina. 
...
ma in confidenza: Φ solo perche' io ho un Celeron500 e lui un Athlon 900 
con una GeForce2, vuoi mettere la fluidita'?). C'Φ tanta gente che fa 
...
i segnali arrivano al cervello, che similmente a quanto Φ accaduto col robot 
...
Eh, speriamo bene... c'Φ chi dice che la razza umana, come ogni altra, 
Φ destinata a soccombere lasciando il nostro pianeta azzurro a nuovi esseri viventi. 
...
"Secondo te cosa c'Φ dopo la morte?" 
"Io mi accontenterei di capire cosa c'Φ prima".
...
quello che c'Φ alla base. Se voi siete convinti che sia tutta opera di un Dio, beati voi: 
per me Φ una risposta troppo banale, non c'Φ sfizio a chiudere la questione cosi'.
"Dio Φ onnipotente ed Φ il creatore". Cioe', si', ci vuole assai a risolvere 
...
Mi stavo chiedendo cos'Φ la realta'. Gia', perche' sto riprendendo 'sto file dopo 
...
Il problema Φ che anche nell'ipotesi che ci si riuscisse a spiegare da cosa e come Φ 
generata la realta' che percepiamo, resterebbe sempre da spiegarsi cos'Φ l'ambiente 
nel quale ci si spiega cos'Φ la realta' (che frase contorta). 
...
E' quella la realta'? E come si fa ad esserne sicuri? E se Φ quella, cos'Φ?
...
Ma provate a lanciarlo ora. "Che cagata 'sto gioco", direte. Gia', perche' Φ del 
...
allargarsi dal punto nel quale Φ arrivato il proiettile. 
...
E quanto tempo Φ passato dall'ormai squallido Wolf3D? Saranno si' e no 10 anni. 
...
Ancora una volta, Φ solo una questione di complessita', di scala... come la 
robotica e/o la genetica potrebbero portare ad un replicante, androide, 
cyborg o come vogliate chiamarlo anche migliore dell'uomo, che altro non Φ che 
...
interpretare come un hack della Matrice, che Φ possibile compiere solo una volta 
...
La Terra Φ un pianeta abbastanza piccolo appartenente al sistema solare.
Insieme ad altri pianeti orbita attorno alla stella Sole, ed il sole non Φ che una 
delle circa 500000 stelle che formano la nostra galassia, la Via Lattea.
La via lattea non Φ che una delle tante galassie che formano l'universo conosciuto.
Vi siete mai chiesti cosa c'Φ oltre entrambi gli estremi?
...
noi (ed infatti giudichiamo "veloce" una mosca), perche' Φ piccola rispetto a noi... noi 
che ad esempio giudichiamo lento lo spostarsi del sole... ma tutto cio' Φ ancora 
comprensibile, si Φ lontati dagli estremi....  e se una galassia come noi la conosciamo 
...
certezze che vi si trovano... perche' se si Φ certi di qualcosa, la si da' per scontata e 
non ci si pensa... 
E se non si pensa, si perde... la sconfitta peggiore, perche' il nostro pensiero Φ l'ultimo 
gradino in fatto di complessita', impossibile da emulare con efficacia al giorno d'oggi... 
...Φ quello che ci permette di distinguerci dagli animali e dalle macchine e che 
ironicamente, allo stesso tempo, potrebbe permettere di creare qualcosa di piu' 
complesso di noi stessi... per giungere li', dove nessun uomo (?) Φ mai giunto prima.

...ah, se l'uomo crea una forma di vita (?) anche migliore di se', ed Φ capace di 
preparargli un ambiente adeguato, reale (?) o virtuale (c'Φ differenza? Quale?) che sia, 
...
"I've... seen things, you people wouldn't believe. 
 ...
 I watched C-beams glitter in the dark near the TannhΣuser Gate.
...
Dunque, mi presento, mi chiamo oVERDRIVE, e ho scritto qusto articoletto, che poi Φ una traduzione che ho modificato, per far si che gli utenti di OpenBSD possano trovare delle informazioni per configurare il proprio sistema operativo e usufruire del servizio ADSL. 
...
L'indirizzo 10.0.0.138, Φ l'indirizzo di default che indica fisicamente il vostro modem 
Alcatel, quindi una volta configurata l'interfaccia alla quale Φ connesso il modem 
(vi spiego dopo come fare), potete fare un semplice ping -c1 10.0.0.138, ed  
esso vi risponderα. L'unico utente che deve leggere questo file Φ l'utente 
...
Ho inserito gli ip dei server DNS del mio provider (non dovrei fare publicitα), voi inserite i vostri. 
Come vi dicevo prima, ora dovete configurare la vostra interfaccia di rete alla quale 
Φ connessoil modem Alcatel. Assegnate alla medesima  
...
la soluzione Φ la ricompilazione del kernel !!! 
Non temete non Φ una cosa difficile. 
...
Rebootate... non Φ una parlaccia... dovete riavviare la macchina, e durante il prompt del loader, digitiamo "bsd.2" 
...
In questo modo il boot caricherΦ direttamente il kernel da voi compilato. 
...
Θ stato scritto da Peter Chiocchetti.  
...
In Linux si usano due programmi, essenzialmente:
     ...
     Come si pu≥ intuire tratter≥ il caso di accesso con autenticazione PAP.
     ...
     per voi Φ internet, dovete specificarlo lo stesso...). Per fare questo  
     ...
     Fatto tutto ci≥ il nostro sistema Φ pronto, e possiamo quindi passare alla
     realizzazione dei veri e propri script di connessione, cioΦ quelli che
     ...
     pi∙ semplice anche se la meno sicura Φ quella di dare i permessi di 
     ...
     Tutto ci≥ che ho scritto Φ stato il frutto di un "collage" tra consigli di
...
un mio amico non aveva niente da fare cos∞ si Φ messo a distribuirli, voi provate,
...
/===========================::=============================::=================/
 ...
 Consumo: ora vedo che c'Φ da mangiare... NULLA!                               
 ...
 Innanzitutto spieghiamo cos'Φ il POP3 (Post Office Protocol): sarebbe il      
 protocollo che consente agli utenti di accedere alle caselle di posta e di    
 recuperare le e-mails.                                                        
                                                                               
 Di solito la porta utilizzata dal server POP3 Φ la 110. Quando ci             
 ...
 Una volta collegati, dopo che il server si Φ presentato, si entra nella       
 ...
 sezione si pu≥ interagire e fare le proprie scelte. DopodicchΦ, quando ci     
 disconnettiamo dal server, quest'ultimo entra nella sezione detta 'UPDATE',   
 e cioΦ aggiorna i nostri cambiamenti e infine ci saluta.                      
 ...
 classico e quello tipo 'APOP'. Vi spiego solo il primo (pi∙ semplice).        
 ...
 pass menevanto                                                                
 +OK 88 messages                                                               
(l'esempio non Φ autobiografico!)                                                                                                                            |
 in questo caso tutto Φ andato bene, ci ha accettati e ci ha detto quanti      
 ...
 Col comando STAT ci facciamo dire quanti messaggi di posta ci sono e quant'Φ  
 ...
 molto utile, in questo modo leggiamo le prime righe dei messaggi, cos∞ da     
 capire se magari possiamo cancellarle o meno.                                 
                                                                               
 Esiste anche il comando UIDL, ma Φ anche difficile, quindi lasciamo perdere   
 ...
 Oki, per ora Φ tutto, forse ci saranno altri miei arts, ci si bekka!          
 mR_bIs0n
...
http://members.xoom.it/vetranks2000/superhacker.AzzurraNet.log il re dei lamer Φ svelato!
...
Gente vi saluto.
Aspettando il prossimo numero di dn (se ci sarα un domani...) vi posso consigliare
di scaricare qualche foto di Aria Giovanni. Ah per tutti quelli che non sanno
chi Φ (ovvio), Φ una pornostar che appare essere na patatona di primo genere :)
Apparte gli scherzi, adesso il team penso si prenderα una pausa; intanto si 
organizzerα meglio, acquisirα nuovi membri (lo spero!) e si preparerα all'uscita
...
pagnotta                    Φ il cognome della mamma di un mio amico :)
