...
```
         ...
         -::-Vive le sex! Vive le Pot! Vive la RΘvolution!!!
...
'.________  __ __ ___________ ____ ______________ ____ ___ ______ __ __
      ...
      Aero Box                                         Endrix
      Les Exploits                                     [G]r4FX
      UtilitΘs des # de credit card                    Neuro
      Espionage TΘlΘphonique                           Endrix
...
Bon pour le #3, on a essayer de l'amiliorer de facon considΘrable, et cela
a cause des mauvaises critiques pour le #2. De ma part, je suis pas mal
satisfait du rΘsultat, a vous d'en juger. On a inclus un petit programme
de PWLcracker avec le zine, et cela apres avoir eu plusieurs demandes pour
ce type de programme.

NOTICE:
Moi, Neuro, j'ai pas eu beaucoup le temps d'Θcrire pour ce zine, je vais me
reprendre dans le prochain numΘro.


L'avenir ce cette zine :
»»»»»»»»»»»»»»»»»»»»»»»»

 Bon, comme tout les groupes de hacking/phreaking QuΘbΘcois, notre but est
d'amΘliorer la scΦne du QuΘbec dans ce genre de chose. Donc, maintenant, notre
...
- Ondes et frΘquences
- Boxes
- Fontionnement du systΦme tΘlΘphonique de BELL
- et autres...

 Donc, si vous Ωtes bon dans ce domaine et que aimeriez Ωtre dans un groupe comme
le notre, hey bien envoyez-nous un email α pyrofreakmag@hotmail.com avec le contenue
des choses que vous savez faire et nous allons vous recontacter α ce sujet.


Joke du mois:
De la part de BuBBle:

Deux spermatozo∩des sont en pleine discution.
Le premier dit α son compatriote : "Dis, c'est encore loin les ovaires ?"
Le second lui rΘponds : "Tu parles, on n'est encore qu'aux amydales." 
...
But de cette box :
»»»»»»»»»»»»»»»»»»
Bon, cette maniΦre de phreaking peut Ωtre utiliser pour faire des appels
gratuits intΘrieurs ainsi qu'extΘrieures(mΩme en Australie). Ca peut Ωtre
dur α faire marcher sur quelques tΘlΘphones publiques mais dans l'ensemble
ca doit marcher. Avant de partir, je vous suggΦre de prendre une petite marche
et d'aller voir pour trouver un telephone publique pour savoir quelle sera
le meilleur maniΦre de le faire fonctionner sur ce telephone.

Outils dont vous aurez desoin :
»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»
- Un couteau(petit pour auter de la protection PVC. Ce qui est le tube de mΘtal
             qui protΦge contre le monde de toucher aux fils).

               OU
  Un tourne-VIS(si la ligne tΘlΘphonique va la le planchΘ et qui passe en dessus
                d'une plaque de mΘtal qui peut Ωtre dΘvissΘ trΦs facilement)
...
- Un changeur de fil(ou Phone socket) :
                    ...
                    || <-Ligne de tΘlΘphone
               _____||_____
              |    /  \ <-Ligne qui est connectΘ et donc vous pourrez voir
              | +/      \+ |  l'intΘrieur du fil
              |  (       ) |  
              | + (     ) +|
              |  ( (   ) ) |
              |    (    )  |          ) = fils enveloppΘs de plastique de diffΘrence couleurs
              |____||||||__|
                     |||
                     |||
                     ||| <-Ligne tΘlΘphonique qui est connectΘ au tΘlΘphone
                     |||      (Couleur : CrΦme, Noir ou blanc)
                   TΘlΘphone



- Des fils dΘjα arrangΘ(vous aurez auter les bouts de platique α chaque extrΘmitΘ des fils)
...
1 - Arrangez vous pour trouver les fils. Soit en dessous de la plaque de mΘtal ou dans le tube
    qui protΦge les fils.
...
1) SΘparez les 2 fils noirs avec les siceaux
2) Utiliser les pinces pour auter quelques centimΦtres de plastiques sur les 2 fils noirs.
3) Couper les autres fils
4) DΘvisser les +
5) Connecter les fils noirs aux + (du phone socket) et α l'autre bout du socket, votre tΘlΘphone
              ...
              |____||||||__|              
                     |||
                     |||
                     ||| <-Fils qui se connecte au tΘlΘphone
                     |||      (Couleur : CrΦme, Noir ou blanc)
                   TΘlΘphone


 C'est finnit !!!! Maintenant il va Ωtre possible pour vous, de faire des appels
gratuit avec ce systΦme. Mais j'ai une meilleur idΘe pour vous... Si vous connectez
votre modem α la place de votre tΘlΘphone ?? hein ?? HeY oui, il va Ωtre possible de faire 
...
utiliser.  J'ai dΘcidΘ d'Θcrire ce texte pour ceux qui ont la tΩte 
d√r et qui ne savent pas encore c'est quoi.  Je suis sur que ce texte 
va Ωtres utile α beaucoup de monde...


-==Qu'est-ce que'un "Expl0it"==-

Bon pour faire une dΘfinitions pas trop fuckΘ d'un "Expl0it", je vais 
commencer comme ca !  C'est un programme qui exploite un ou des "bugs" 
dans un "sotfware" spΘcifique.  Chaque "Expl0its" est diffΘrents, exploite 
des bugs diffΘrents, et fait des choses diffΘrentes.  C'est pourquoi chaque
"Expl0its" est spΘcifique α un programme en particulier.  Les "Expl0its" ont 
ΘtΘ codΘs pour avoir accΦs au compte "root" ou (en franτais correct) le 
compte Administrateur du systΦme.  Ils accΦdent α ces comptes en exploitant un 
"bugs" dans un certain programme, si celui-ci est utilisΘ par l'utilisateur
"root".  
Sur les OS (Operating system) de type UNIX, certains programmes ou scripts
doivent 
Ωtres roulΘs par l'utilisateur "root" (ou pour vous facilitΘ la taches:  l'UID
0) 
dans le but d'effectuer une tΓche qui ne peut pas Γtres effectuΘe par les autres 
utilisateurs.  Donc en termes facile, un "Expl0it" fait crasher le programme qui 
roule en "root" pour vous donner l'invite (prompt) du root.  TR╚S souvent c'est
celui-ci #.

-==Comment on utilise un "Expl0it"==-

Les "EXpl0its" sont codΘs a 99% du temps en langage C.  Pour utiliser votre
"Expl0it" 
il vous faut :

1- un "shell" sur une boite (box) du systΦme que vous voulez utilise le
"Expl0it".

-=OU=-

2- Vous devez utiliser le mΩme OS que la boite (box), que vous voulez hacker,
utilise. 
Pour utiliser un "Expl0it" vous devez compiler le code source, ou le binaire
dans votre rΘpertoire de travail du shell.  Bien entendu vous voulez utiliser un
"shell 
account" hackΘ ou un shell qui ne vous appartiens pas...  Pour copier le code
source ou 
le binaire du "Expl0it" dans votre shell, utiliser un programme FTP (CuteFTP ou
autres) 
et "uploader" l'Expl0it, ou bien utiliser "rz" si vous possΘdez un "dialup
shell".
J'espΦre que vous savez tous comment utiliser un programme FTP.

Une fois l'exploit uploader sur le systΦme il reste seulement α le compiler ce
qui ce fait 
comme suit :
...
codeurs aiment 
rire des gens qui ne connaisse pas le C, ils vont insΘrer un bugs dans l'exploit
pour qu'il ne 
se compile pas ou mΩme encore pour qu'il ouvre un port (backdoor) dans votre
systΦme pour ensuite 
...
Bon... vous avez compilΘ votre exploit.  Maintenant si tous a bien ΘtΘ vous
devriez Ωtres en 
mesure de l'utiliser... le fun commence.  Vous allez savoir que l'exploit a
fonctionnΘ, 
si vous voyez l'invite (prompt) de utilisateur "root" (#) α la place de celui
des utilisateurs 
normaux ($).  Mais, ce ne sont pas tous les exploit qui fonctionne pareil, il
peut arriver que 
certains exploit demande des lignes de commandes diffΘrentes.  Donc, il est trΦs
important de 
...
endrix                  === >           pour le site :   http://www.insecure.org/sploits_linux.html
 ...
 ____ __ _ _ _______________ ________ ________ __PyroFreak Magazine #3              
:                   
|                    UtilitΘs des # de credit card 
'.________  __ __ ___________ ____ ______________ ____ Par Neuro __ __


Vous est t'il dΘja arriver d'obtenir des numeros de carte de crΘdit ainsi que
...
qui n'a pas encore ΘtΘ aborder dans d'autres zines auparavant selon ma connaisance.
Bonne lecture!!!
...
vΘrifier la validiter de la carte, parce que cette technique ce fais tres 
rapidement (quelques minutes). Vous allez sur un site XXX conventionnel qui
accepte les #cc(numeros de carte de crΘdit), et vous aller dans la section
...
est souvent vΘrifier en meme temps que la carte.
...
l'exeption pres, dΘpendant de la sΘcuriter du site), mais dans 90% des cas, 
cette information n'est pas vΘrifiΘ.

Numero de la carte: Votre numero valide que vous avez trouver
Date d'expiration: Ainsi que la date d'expiration.

Ensuite vous avez juste a cliquer sur le petit bouton CGI qui vΘrifie votre
demande immΘdiatement en moins de 2 mins, si vous etes accepter, votre carte
...
Celle-ci est beaucoup plus intΘressante, mais aussi plus risquer. Elle consiste
a s'acheter des billets de spectacles. Pour le Quebec, je vais prendre 
l'exemple du rΘseau admission. Vous avez juste a appeler au rΘseau admission
...
d'identiter correspondant aux informations que vous avez donner au tΘlΘphone.
Pour plus d'informations sur les fausses cartes d'identitΘs, aller lire
HAQCH #1, un article sur ce sujet a ΘtΘ Θcrit dans ce numero. 

Il reste juste a aller ramamser vos billets, et le tour est jouΘ.
...
Technique #3:
Vous abonner a une connection internet par tΘlΘphone (Pour 1 ans c'est prΘfΘrable)
...
abandonner, mais cette technique demande extremement de prΘparation, et elle
est aussi une des plus risquΘe.

Technique #6:
Vous faire livrer de la bouffe (Pizza,poulet,etc..). La encore, cela demande
de la prΘparation, et ne fonctionne pas a toutes les places, quelques 
restaurants demande de voir la carte de crΘdit avant de vous donner la
livraison.


Dans un prochain numΘro, d'autres techniques vont etres abordΘes et plus en
profondeur. Si vous en avez vous aussi qui n'on pas ΘtΘ m'entionnΘes ici, 
envoyer les moi par e-mail a: neuro@frphreak.com 
 ...
 ____ __ _ _ _______________ ________ ________ __PyroFreak Magazine #3              
:                   
|                      Espionage tΘlΘphonique
'.________  __ __ ___________ ____ ______________ ____ Par Endrix __ __

          ________________________________________________
        <((Espionage tΘlΘphonique avec un tΘlΘphone sans-fil))>
         »»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»


 Bon, tout a commencΘ par un bel aprΦs midi....

&lt;endrix> Oui allo ?!
&lt;personne> HEY FRED !!! QuessΘ tu fous ???
...
avec mon tΘlΘphone sans-fil  et commence α me promener(sur le mΩme bord de la
...
α traverser la rue, encore avec mon tΘlΘphone sans-fil et rendu l'autre bord, la 
...
que si c'Θtait moi qui parlait au tΘlΘphone. Je peux entendre mais pas parler. La, je commence
α me poser des questions.. Tout d'un coup, j'entend le nom de la personne, c'Θtait ma voisine
de 2 maisons(trΦs cute en passant.. ;)) ). 

 Donc la je suis allΘ la voire et je lui ai demandΘ quel tΘlΘphone elle avait. C'Θtait pas le 
mΩme mais sont tΘlΘphone utilisait la mΩme frΘquence que le mien. C'est alors que je me suis 
pauser la question suivante : Est-ce possible de faire ca avec tout les tΘlΘphones qui on la 
mΩme frΘquence que le mien ??? HEY OUI !!!! Mais comment ??? Ben je connais un d00d qui 
travaille au Radio Shack et je lui ai demandΘ si c'Θtait possible de scanner les frΘquences 
dans la ville pour trouver la mΩme que la mienne. Finallement, oui !! Il a un petit appareil 
qui fait ca. Alors je l'emprunte(pour le ramener plus tard) et le je me suis amusΘ toute la 
journΘe α faire ca. 

 DΦs que je capte la mΩme frΘquence que mon tΘlΘphone, je l'allume et j'Θcoute. Je vous avertis,
ce genre de chose ne marche pas α tout les coups. Je me suis fait souvent couper la ligne par je
sais pas trop quoi mais environs 1 fois sur 2 ou 3 ca doit marcher.

 Alors maintenant, vous saurez comment espionner vos amis(es) qui ont les mΩme tΘlΘhones que vous 
et les faire chier le lendemain avec ca !!! Je suis que sur un tΘlΘphone sans-fil, il y a une puce
comme les avios tΘlΘguider, bien tout ce qui marche α commande qui dΘfinnit la frΘquence exacte pour
le faire fonctionner. Ces puces ce vendent alors je vais tester quelques petites choses et probablement
que je vais Ωtre capable d'espionner plusieurs frΘquences de tΘlΘphone pour entendre le plus de conversation.
Si c'est possible, je vais aller demander α du monde de voir leurs tΘlΘphones et... sans rien faire savoir, 
je vais me procurer la frΘquence du tΘlΘphone et aller acheter la puce qui fonctionne avec ca!!!

Voilα !!! ;) Happy phreaking !
...
But de cette BOX : 
»»»»»»»»»»»»»»»»»»
Une Black BOX est un montagne qui arrange votre tΘlΘphone de maniΦre que 
quant une personne de l'extΘrieur va appeller chez vous, l'appel va Ωtre 
gratuit pour cette personne. Donc, si votre tante appel tout le temps(comme
dans mon cas α moi) l'appel va Ωtre gratuit pour elle. Ce systΦme peut Ωtre
bon pour des appels jusqu'α 30 minutes amis aprΦs ca, Bell va commencer α
ce poser des questions et dans ce cas, hehe c'est pas bon !! ;)


Comment ca marche :
»»»»»»»»»»»»»»»»»»»
Ce que cette BOX peut faire, est d'empΩcher que le voltage baisse jusqu'α
10v quand tu rΘponds au tΘlΘphone. Donc ce petit bΘbΘ garde le voltage de 
la ligne α 36v et ca va faire pensΘ α Bell que le tΘlΘphone sonne encore
pendant que tu parles. Toutes les piΦces peuvent Ωtre achetΘ chez Radio Shack.
Ca va vous coutez moins de 2$ pour faire ca.

Contruction :
»»»»»»»»»»»»»
SchΘma #1 pour la plupart des tΘlΘphones :

LED : Petite lumiΦre qui conduit le courant
        **         LED ON: BOX ON       **

   Ligne     >--------------------VERT-->     AU
TΘlΘphonique >--+   1.8k  LED  +-ROUGE-->  TΘlΘphone
                |----///---|>--|
                |              |
                +----->/<------+
                  Interrupteur
...
╔quipement :
»»»»»»»»»»»»
- 1 RΘsistance de 1.8k de 0.5Watts
- 1 LED de 1.5v 
- 1 Interruteur

Bon Hey bien pour le faire, veuillez suivre le schΘma expliquer ci-dessus.
Faites tout bien α la lettre et vous verez que ca marche trΦs bien.


Contruction :
»»»»»»»»»»»»»
SchΘma #2(la plus utilisΘ) :


      **        LED ON: BOX OFF      **

   De la    >---------------VERT-->     Au
   Ligne    >-----+-     +-Rouge-->  TΘlΘphone
                  |  LED |
               +->/<--|>-+
               |         |
               +--///---
...
Vous aurez besoin du mΩme Θquipement. Voici un schΘme plus expliquΘ :
...
les fils de la ligne tΘlΘphonique s'arrangent bien avec les fils du tΘlΘphone.
Il ne faut pas avoir de l'expΘrience en Θlectronique pour faire ce genre de 
chose. 

 Peut'Ωtre que vous aurez α faire quelque modification pour le faire fonctionner
mais c'est rien de difficile. Essayez demandez α une personne qu'elle appelle, juste
pour voir si ca marche sinon, essayer de nouveau... Ce n'est pas d√r mais parfois
...
Quand l'interrupteur est α On, vous allez juste pouvoir recevoir des appels et quand 
vous allez recevoir un appel, le LED va flashΘ et quand vous allez prendre le tΘlΘphone
le LED va rester allumΘ pendant votre conversation. PrΦs avoir parlΘ avec la personne,
vous deverez mettre l'interrupteur α Off.
...
'.________  __ __ ___________ ____ ______________ ____Par Endrix__ __
 ...
 Bon euhh... Par quoi commencer.... Un Coktail Molotof est du feu qui br√le trΦs
longtemps et ca br√le en tabarouette alors faites attention avec ce genre de chose
le faite pas sur vous mΩme(duh!) il faut choisir... Votre gardienne, votre prof de
Francais.... ;)
...
- Une bouteille de verre d'un 1 litre(de prΘfΘrence)
- Le bouchon de la bouteille
- 500 ml de combustible(Vodka, Gaz, PΘtrole.....)
- Quelque chose pour allumer(Linge imbibΘ dans le combustible, mΦche..)
- Un couteau qui coupe bien
- Quelque chose pour allumer


Fabrication(avec une mΦche) :
»»»»»»»»»»»»»»»»»»»»»»»»»»»»»

1- Faite un petit trou dans le bouchon pour pouvoir entrer la mΦche

2- Verser le combustible dans la bouteille(vous pouvez toujours faire des
   mΘllanges de combustible... Moi je l'avais fait et une fois j'ai obtenue
   du feux de diffΘrente couleur...)
...
1- TrempΘ le morceau de linge dans le gaz

2- Verser le combustible dans la bouteille(vous pouvez toujours faire des
   mΘllanges de combustible... Moi je l'avais fait et une fois j'ai obtenue
   du feux de diffΘrente couleur...)

3- Rentrer le linge α moitiΘ dans la bouteille
...
5- euuuuu just don't stay there ok !!!! ;)
 ...
 Bon hey bien j'espΦre que vous allez avoir du fun avec ca et ne vous faites
pas mal svp !!!!!
...
Si vous avez des persΘes suicidaire,  LISEZ PAS L'ARTICLE
...
assez dure de parler d'explosif sans etre traiter de tΘroriste. C'est aussi
pourquoi cet article n'a pas ΘtΘ signer par personne, pour pas avoir trop
de probleme. Donc , voici le dΘbut de l'article, qui est tres court pour ce
numero, car, je vais essayer d'adapter les prochains cours, pour qu'ils
soient plus axΘs sur la chimie que sur les explosifs en tant que tel.

Le plastique explosif

IngrΘdients
1 partie de Vaseline (gΘler de pΘtrole)
...
MΘlanger ces produits ensembles, dans un endroit humide. Laisser toujours les
produits mΘlangΘs dans un endroit humide, pour enlever les risques d'explosion
involontaire.

Quand je parle d'endroit humide, je veux dire tres tres humide. Comme dans un
chaudron d'eau a 2 Θtage (bain mari). Ou dans une chambre de bain.
...
ATTENTION: Pour allumer ce produit, il faut ABSOLUMENT se servir d'un dΘtonateur
...
tellement simple α faire que c'est "crap". Je trouve que c'est la box la plus 
simple α faire. Elle sert α une et seule chose : enregistrer une conversation
tΘlΘphonique. Mais de la maniΦre que je le fait, la qualitΘ de la transmition
...
- Un tΘlΘphone (duh!)
- Des fils(prΘfΘrence de couleur diffΘrente)
- Pince pour auter le plastique sur les fils
- Tourne-Vis
- Enregistreuse
- Fer α souder
- Un adapteur d'Θcouteur(que tu branche dans ton systΦme de son ou dans ton CD
  player)
...
1 - Avec le tourne-vis, ouvrir la parti du tΘlΘphone que vous parlez/Θcoutez dedans
    et l'ouvrir dΘlicatement sans rien brisΘ.

2 - Vous devriez voir une petite plaque ou les 2 fils de l'Θcouteur et les 2 fils du 
    micro de rejoignent pour pouvoir brancher le fil qui est au tΘlΘphone.

3 - Si vous le voyez, dΘgueiner 4 fils de couleur diffΘrente(de prΘfΘrence) et de
    mΩme longueur.

4 - Souder les 4 fils sur les 4 places de la plaque ou les fils ce rejoignent. C'est un
    travail dΘlicat α faire parce qu'il ne faut pas qu'aucun fil ne se touche.

5 - Rassembler un fil de l'Θcouteur avec un du micro et le faire pour les 2 autres. Comme ca
    sur un fil vous aurez une p⌠le de l'Θcouteur et une du micro et de mΩme pour les 2 autres.
...
6 - Maintenant avec l'adapteur d'Θcouteur, il est supposΘ d'avoir 2 fils dessus(ca peut dΘpendre)
    Souder 2 fils α un de l'adapteur et 2 autre α l'autre fils

    Note : S'il y a 4 fils α l'adapteur, il faut les mettre 2 par 2.
...
Auteur(s)                : B|aster
URL de tΘlΘchargement    : Http://pyrofreak.cjb.net
OS                       : Windows 95, 98
Note sur 10              : 10/10
Note(s) du programme(s)  : Ce programme ne contien aucun Bug
Option(s)                : HeY voila mon ami B|aster a fait un superbe
                           IP Scanner qui fontionne trΦs bien, mΩme trop
                           bien !!! Inclus dans le programme, il y a dΘjα
                           de inclus dedans de certains serveurs de l'outaouais
                           comme : VidΘotron, Cactuscom, AtrΘide .... C'est
                           le IP scanner le plus vite que j'ai essayΘ α date.
...
Commantaire(s) Additionnel(s) : Il est vraiment bon aller le tΘlΘcharger au
                                vite pour l'essayer.
»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»
...
URL de tΘlΘchargement    : Http://www.aggressor.net
OS                       : Windows 95, 98
Note sur 10              : 9/10
Note(s) du programme(s)  : Le programme souvent a de la misΦre α auto-
                           configurer le modem mais c'est trΦs facile de 
                           le faire soit mΩme. Aussi, avant d'arrivΘ α 
                           l'interface de travail du programme, il y a 
                           plusieurs fenΩtres pour multiple raisons... Mais
                           il est vraiment excellent et mΘrite d'Ωtre 
                           tΘlΘchargΘ au plus vite !!!
Option(s)                : - Suffer2 (SYN Attack, Spoofed IP)
                           - Boink (BasΘ sur Teardrop)
                           - Land
                           - OOB Nuker (Spoofed)
                           - PortScanner
Fonction(s) du programme : Faire des attaques sur diffΘrentes machine
Commantaire(s) Additionnel(s) : L'interface Simple et AvancΘ est trΦs bien!
»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»
...
OS                       : SunOS, Irix, Linux
Note sur 10              : 8/10
Note(s) du programme(s)  : Ce petit bΘbΘ fait plantΘ une machine ;)
...
Commantaire(s) Additionnel(s) : Aucun problΦme de compilation
»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»
...
OS                       : Linux
Note sur 10              : 8/10
Note(s) du programme(s)  : Fait gellΘ l'ordinateur de la cible
...
Commantaire(s) Additionnel(s) : ProblΦme de compilation ;))
»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»
...
╔tΘ 92 - Premiere ICMP attacks sur les serveur de efnet 
...
Bon, c'Θtait la petite histoire IRC, maitenent voici comment vous cree un serveur sous linux et Unix
...
faites ./configure    apres make config   la ces un peux plus compliquΘ, mais avec plusieurs essaient, ca
devrais marcher.
La moitiΘ de la configuration est faite.
...
repertoire que vous avez specifiΘ dans make config), pour vous aider aller a www.routing-com.undernet.org
...
Si vous dΘsirer faire publier votre article dans le #4, envoyez le a
