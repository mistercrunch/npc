...
|Questo documento ha  solo una ragione  di esistere, e  cioΦ quella di  insegnare   |
|qualcosa a qualcuno.                                                               |
...
|s≥ che ci sono anche modem che provvedono automaticamente alla condivisione  dei   |
|pacchetti per i  prodotti microsoft sul  mercato. Ma il  vantaggio di farlo  via   |
|software Φ che puoi  montare un 386 in  modo che funzioni da  mascheratore degli   |
...
|                                                                                   |
|                                                                                   |
|  BenchΦ io  non abbia  ancora spiegato  cos'Φ il  Mascheramento (siate  pazienti) |
|  comincer≥ col  dirvi cosa  vi serve  in ordine  per usare  il Mascheramento.  Vi |
|  servono:                                                                         |
|                                                                                   |
|  -2 o + computer                                                                  |
...
|  cominciare a spiegarti cos'Φ il Mascheramento:                                   |
|                                                                                   |
+---(Che cos'Φ?)--------------------------------------------------------------------+
|                                                                                   |
| Io s≥ che il Mascheramento Φ un processo complicato cosi prima di cominciare con  |
| la parte tecnica  comincer≥ con un  esempio che vi  farα capire +  facilmente il  |
| Mascheramento. Una  persona A  ama una  persona B  ma la  persona A  non Φ nella  |
| stessa scuola di B, per≥ ha il suo numero di telefono. C'Φ anche un amico comune  |
| (C) che conosce A ed Φ nella stessa scuola  di B. Cosi A parla con C, e C  parla  |
| con B senza  menzionare chi Φ  A. Dicendo che  le informazioni sono  solo per se  |
...
|                                                                                   |
| Per  capire  meglio  far≥  una breve  introduzione  al  TCP/IP.  TCP/IP stα  per  |
...
| Copy Protocol). Il TCP/IP Φ un protocollo che controlla la tua comunicazione,  e  |
...
| indirizzi ip  cosi strutturati  Φ detto  ipv4, ora  si stα  diffondendo il nuovo  |
| protocollo ipv6, che diventerα standard entro 3 anni, dove gli indirizzi ip sono  |
| composti da 8 numeri esadecimali  (0-9 a b c d  e f) separati da due  punti (:),  |
| che per motivi che non vi st≥ ora a spiegare Φ stato creato, cmq prevalentemente  |
...
| ->Io sono 1.2.3.3 e st≥ mandando la prima parte a 1.2.3.4                         |
| =>Io sono 1.2.3.4 e l'ho ricevuta                                                 |
| ->Io sono 1.2.3.3 e st≥ mandando la seconda parte a 1.2.3.4                       |
| =>Io sono 1.2.3.4 e io non ho ricevuto niente                                     |
| ->Io sono 1.2.3.3 e te la st≥ mandando di nuovo                                   |
| =>Io sono 1.2.3.4 e io l'ho ricevuta                                              |
| ->Io sono 1.2.3.3 e st≥ aspettando                                                |
| =>Io sono 1.2.3.4 e sono pronto, ciao                                             |
| ->Io sono 1.2.3.3 ciao                                                            |
|                                                                                   |
| Io s≥ che questa pu≥ sembrare una cosa da bambini, ma una comunicazioni di  dati  |
...
| ip locale 10.0.0.1 che Φ connesso a internet tramite una linea telefonica, e con  |
| un  ip  dato dal'ISP  (Internet  Service Provider)  che  Φ 11.1.1.4  .  Che cosa  |
| significa?  Se qualcuno  su internet  prova a  contattare 11.1.1.4  riceverα un   |
| responso, ma  se invece  provasse a  contattare 10.0.0.1  non riceverebbe  alcun  |
| responso anche se Φ lo stesso computer, per il semplice fatto che l'ip  10.0.0.1  |
| non c'Φ su internet (o magari c'Φ, ma si tratta comunque di un'altro pc). Allora  |
| noi abbiamo 10.0.0.2 finchΦ 10.0.0.X si connette a 10.0.0.1 . In questo caso noi  |
| consideriamo  10.0.0.1 come  gateway (il  gateway Φ  una sorta  di uscita  verso  |
| un'altro network, un  gateway pu≥ essere  linkato tra 10.0.1.X  e 10.0.0.X ,  ma  |
...
| consideriamo come gateway ma c'Φ  1 dettaglio, per 10.0.0.1 diventa  un gateway.  |
...
| messaggio non Φ davvero mandato in   broadcast perchΦ non ci sono  route  logici  |
| tra i  2 computer,  pertanto il  messaggio viene  solamente loggato) Ma   allora  |
| che  cosa fα  il  Mascheramento?  BΦ, esso  dα  il  proprio ip (11.1.1.4  Φ l'ip  |
...
| ->Io sono 10.0.0.1 e io processer≥ la tua richiesta                               |
| ->Io sono 14.1.1.4 e voglio contattare 1.2.3.3                                    |
| _>Io sono 1.2.3.3 e st≥ aspettando tuoi comandi                                   |
|                                                                                   |
...
| Φ solamente valido se avete un kernel 2.2.X o superiore. Se non avete un  kernel  |
| 2.2.X suggerisco di  farvi l'upgrade del  kernel (www.kernel.org per  i sorgenti  |
| del kernel  da compilare).  Questo perchΦ  i vecchi  kernel contengono bugs, non  |
| supportano l'hardware nuovo e rendono  il vostro sistema molto vulnerabile  agli  |
| attacchi.  Ora  vi  esporr≥  brevemente  le  basi  del  kernel:                   |
|                                                                                   |
...
|                                                                                   |
| Ora potete vedere  tutte le opzioni  che potete usare,  questo Φ differente  per  |
...
| sullo  schermo,   non  ti   preoccupare,  Φ    normale.                           |
...
| permetterα al  tuo boot  manager  di  riconoscere il  tuo nuovo kernel all'avvio  |
...
| audio e  irc. Questa  Φ  l'unica cosa da   fare inoltre Φ riavviare  Φ quella di  |
...
| kernel,  guarda  se  tutto nel   boot Φ  andato  correttamente,  se qualcosa   Φ  |
| andato sbagliato rifai  tutto daccapo, e  non ti preoccupare,  la ricompilazione  |
| del kernel Φ per tutti difficile la prima volta.                                  |
|                                                                                   |
+---(Avviare il Mascheramento)------------------------------------------------------+
|                                                                                   |
| Infatti non Φ molto  difficile quando tu hai  il tuo kernel. Ora,  per dialogare  |
...
| aggiustare Φ il parametro -s parametro. 10.0.0.0 Φ il tuo indirizzo di network e  |
| il 4 Φ il pi∙ alto indirizzo ip che viene mascherato. Il tuo server ora dovrebbe  |
| andare. Eh si, Φ davvero cosi, tu  hai molta teoria e molta preparazione con  il  |
| tuo kernel, e ti sono bastate 2 linee di comando per far andare il tuo server.    |
|                                                                                   |
+---(Configurare i tuoi client)-----------------------------------------------------+
|                                                                                   |
| Tu hai un server, ma cos'Φ un server senza client? Cos'Φ un supermercato senza i  |
| prodotti? C'Φ solo una  piccola cosa che dovresti  fare se non l'hai  giα fatta:  |
