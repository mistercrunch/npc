...
+---(Segnali)----------------------------------------------------------------------+
| Un segnale Φ  una notificazione ad  un  processi che  si Φ verificato  o che  Φ  |
...
|   Non Φ detto pero' che un un segnale termini il processo.                       |
|   La chiamata kill fa molte cose diverse a seconda dei valori dei suoi           |
|   argomenti.                                                                     |
|   Se pid Φ 0 il segnale viene inviato a tutti i processi nel gruppo              |
|   relativo al trasmettitore.                                                     |
|   Se pid Φ -1 e il trasmettitore non Φ il super user , il segnale viene          |
|   inviato a tutti  i processi il cui ID dell utente reale uguaglia l'ID          |
|   utente effettivo del processo trasmittente.                                    |
|   Se l'argomente pid Φ -1 ed il trasmettitore Φ i super user il segnale          |
|   viene inviato a tutti i processi che non siano processi di sistema             |
|   (proc. con pid 0 e 1 ).                                                        |
|   Se l'argomento pid Φ negativo ma diverso da -1 il segnale viene                |
|   inviato a tutti i processi il cui ID di gruppo di processi uguaglia            |
|   il valore assoluto di pid .                                                    |
|   Se l'argomento sig Φ zero viene svolta la verifica degli errori ma             |
|   non viene inviato alcun segnale. Si puo' usare questa condizione per           |
|   esempio per esaminare la validitα del pid.                                     |
|   Roxxa un casino sta robba =). continuiamo.                                     |
|   Il comando kill cmq Φ usato generalmente per l'invio di segnali                |
|   prendendo i suoi argomenti dalla riga di comando ed Φ applicato con            |
...
| c'Φ nΦ un altro degno di nota ctrl-\  che termina l'esecuzione di un processo  e |
...
| notificate dal kernel causano la generazione di segnali ad es. SIGURG Φ generato |
...
| presenti  questo. Questa  funzione Φ  chiamata signal  handler.Un processo  puo' |
...
|                                                                                  |
| Cio' che dice questa dichiarazione Φ  che signal Φ una funzione che  restituisce |
| un  puntatore  ad  una  funzione che  restituisce  un  intero.  L'argomento funz |
| specifica l'indirizzo di funzione (*) che Φ void ( non restituisce niente ) . Ci |
...
| possibilitα a una singola funzione di gestire piu' segnali.                      |
|                                                                                  |
...
| SIGKILL  : Unico modo garantito per terminare un processo poichΦ il              |
|            processo ricevente non lo puo' ignorare                               |
| SIGPIPE  : Quando un processo esegue una scrittura su una pipe o una             |
|            FIFO e non c'Φ alcun processo che legga dalla                         |
...
| SIGSEV   : AMATO ? o odiato ?Questo Φ generato da un difetto                     |
...
|            di memoria a cui non Φ consentito l'accesso                           |
| SIGSTOP  : Questo arresta un processo.che puo' essere ripreso con SIGCONT        |
| SIGSYS   : Indica che Φ stata fatta una chiamata di sistema non valida           |
...
| SIGSTP   : Questo segnale Φ inviato ad un processo quando viene battuto          |
...
| Ciau belli Lupsyn Una  cosa sola : non  Φ solo xkΦ avete  il drago , che  dovete |
| sentirvi grandi....!                                                             |
|                                                                                  |
+----------------------------------------------------------------------------------+
```
