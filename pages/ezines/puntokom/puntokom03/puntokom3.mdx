...
dON't Kill Us for thE drawing... Is our first AsCII!!
...
cracking, phreaking, mp3, Arte digital, Virus y mas... La pagina se 
actualiza
cada 15 dias/1 mes... La direccion es:
...
ATENCION! Necesitamos gente interesada en publicar sus notas para la 
revista,
...
* Que no contenga acentos ni enies, asi nos aseguramos una mejor 
legibilidad.
...
Otro mes mas... Parece que cada dia las horas corren mas rapido.. No sienten
...
% 006 %  ----------------> CURSO DE PROGRAMACION p. I-T.PASCAL 
(NightCrawler)
...
un boton para el dedo pulgar cuya funcion puede ser programada por el 
usuario.
Su precio, es de no creer, solo $29,73.
Link ====--------===> www.trust.com
...
independiente de cada parlante. Su costo es de aproximadamente $412.
Link ====--------===> www.teac.com


Visioneer Strobe Pro Nt:
------------------------
Este escaner de pagina viene con cables USB, paralelos y seriales, los 
cuales
incluyen una entrada de energia. Se activa automaticamente cuando detecta el
papel. Captura imagenes color a 600 x 300 dpi. Aproximadamente su costo es 
de
$286 .
...
manejar 832 canales. Este sistema es el sucesor del AMPS que solo soportaba 
666
canales. El total de canales es dividido en dos bandas A y B, cada una a 
cargo
de un provider (Movicom y Miniphone en Bs. As.) y cada banda recibe el 
nombre
de Non-Wireline y Wireline respectivamente ya que la segunda estaria 
reservada
para la compa~ia de telefonos local (Telefonica y/o Telecom). Cada banda 
consta
de 416 canales (832/2) y cada uno tiene un uso especifico. Por ejemplo, de 
cada
...
Lo importante es que todo el sistema de celulares esta conectado 
(OBVIAMENTE)
al sistema telefonico convencional por medio de COs (Central Offices u 
Oficinas
Centrales) que hacen las veces de Switches comunes y corrientes, pero que 
estan
conectadas a MTSOs (Movile Telephone Switching Offices) que es desde donde 
se
...
los provea de equipo. En el caso de Movi, ni siquiera, ya que no es 
propiedad
...
empresa de Telefonica. Estas tres ultimas empresas son "socias" para no 
decir
...
informacion sobre quien es la competencia de Unifon, pero despues de todo no 
es
...
Entonces se reunieron los expertos de USA y convirtieron la escucha de la 
banda
de 800mhz en ilegal, y solucionaron el problema. hahaha
O como cuando querian prohibir el uso de beepers en Estados Unidos, porque 
los
usaban los narcos. haha, un tipo que trafica merca va a dejar de usar un 
beeper
...
venidos de un total de 832 canales. Estos estan numerados desde el 1 hasta 
el
1023 con un agujero en el medio desde el canal 800 al 990 que estan sin uso,
para poder codificar cada canal con una palabra de 10 bits. Sin embargo 
parece
...
11 bits con el MSB en 0. Cada canal tiene una frecuencia asociada, por lo 
que
hablar de canal es lo mismo que hablar de frecuencia. A continuacion la 
tabla
de asignacion de frecuencias por canal:

Chan   Mobile   Cell Site		RECs: Reverse Channels
  nº    RECs     FOCs			      (de Movi A Torre)
...
diferencia de 45Mhz. Interesante. Fijense tambien que el desplazamiento 
entre
...
canales, y despues con el advenimiento de los nuevos canales del NAMPS (y 
como
...
Celular deriva de la forma en que estan geograficamente dispuestas las 
antenas
de un sistema. Como todos sabran existe una antena por zona, dedicada a 
manejar
...
El total de canales se dividiria en 4 grupos, uno para cada tipo de celda: 
A,
B, C y D. Este modelo se lo conoce como "Patron de Reutilizacion de 4 
Celdas"
...
En realidad debido a la naturaleza de las radio se~ales, son circulares, 
pero
...
con una antena o con la otra (siempre tiende a quedarse con la de mas 
potencia)
justamente por los bordes de los hexagonos. Si nosotros estuviesemos 
hablando
en un cierto canal, de una cierta antena (celda) y nos pasaramos a otra 
celda
...
Primero, la base de la celda esta constantemente chequeando la potencia con 
que
...
mismo tipo de informacion de las otras antenas, y tiene los datos 
suficientes
...
medio del canal de VOZ ACTUAL (porque seria ridiculo mantener un canal 
asociado
de control por cada canal de voz que se esta usando). Si se pudiera 
interceptar
...
es fundamental saber donde esta cada antena, al menos las de su zona de 
accion
...
bobo como encender el aparato y discar, ya se estan poniendo en 
funcionamiento
...
Se puede decir entonces que cada movil, tiene una identificacion unica 
(hehehe)
Esta identificacion consiste en some data, pero en realidad solo dos datos 
son
...
indica las caracteristicas del movil, pero que para el caso no son 
relevantes.
...
llamadas FUERA del sistema del que uno es abonado. Una onda de 
compatibilidad
mundial donde uno podria irse a Japon con su propio movil y efectuar 
llamadas
...
mediante una red de computadoras llamada Red Nacional Celular. Una vez que 
se
...
ahora el que hace la llamada necesita saber a donde fue a parar. Previo 
aviso
...
abonados de TODOS los sistemas con acuerdo mutuo de roaming, primero 
permitiria
...
cada carrier pudiera chequear los datos del movil ANTES de habilitar la 
llamada
...
------------------------------------------------------------------------------
% 003 % Passwords en Hotmail y Yahoo
------------------------------------------------------------------------------

             COMO OBTENER LOS PASSWORDS DE YAHOO Y HOTMAIL

Es muy facil tal vez ya lo sepan pero nunca esta demas recordarlo no 
creen???
...
vos tenes dos cuentas: pedro@yahoo.com y juan@yahoo.com , si perdes la clave
...
desde tu cuenta de yahoo manda un email a help_passw@yahoo.com , en el
...
Al rato vuelves a chequear tu email por donde mandastes la peticion y tal 
vez
...
forgotten_pwds@hotmail.com (con tu cuenta de hotmail) , en el subject
va la cuenta de la cual queres averiguar el pass y en el mensaje del email
tenes que poner dos cosas: tuemail@hotmial.com:password y al rato te
...
help_passw@yahoo.com tal vez al tiempo lo cambien a otra direccion como ser
help_password@yahoo.com. Es simple suposicion pero en caso que suceda
...
Curso Programacion   - BASIC  II Parte -
----------------------------------------

En la primera parte estuvimos viendo los comandos PRINT e INPUT. Ahora
...
OPERADORES LOGICOS:
   AND .-  Une dos condiciones. Si ambas son verdaderas entonces realizan la
           consecuencia. (Si yo Estudio pero no me Preparo no saco 
excelente)
           Ej: IF A=0 AND B=0 THEN PRINT "A y B valen 0"
   OR .-   (uno O dos) Si se cumple una o la otra se realiza la 
consecuencia.
...
IF NUMERO <> 1 AND NUMERO <> 0 THEN PRINT "El numero no es ni uno ni 
cero!!!"
...
Matrices, y abrir-cerrar archivos (OPEN).
                                                     ...
                                                     FLASH
                                              -. darthblair@yahoo.com .-


------------------------------------------------------------------------------
% 005 % Locura espontaneA
------------------------------------------------------------------------------
...
dejamos un poco de joder, porque realmente un tipo con tanta trayectoria 
como
...
sea un poco jodido, pero es mejor, asi la gente se pone las pilas e 
investiga,
y conoce todo mas a fondo, no como con el Winpersa que toca dos clicks y 
listo
...
------------------------------------------------------------------------------
% 006 %  Programacion en Turbo Pascal [Parte 1]
------------------------------------------------------------------------------

En esta seccion comenzaremos con un curso practico acerca de como programar
...
Este compilador soporta un uso de memoria no superior a los 64 mb, en caso 
de
superarlo sera necesario implementar modificaciones a nuestro programa de 
ma-
...
/////////////////////////////////////////////////////////////////////////////
ACLARACION: entre los corchetes se coloca la cantidad de digitos de la 
cadena
...
/////////////////////////////////////////////////////////////////////////////
CONSIDERACIONES: -todas las intrucciones se escriben finalizadas por un 
punto
                  y coma.
...
{el mayor y el menor pueden unirse al igual, en tal caso se transformarian 
en
>= y en <= }
...
-Visualizacion por pantalla o impresora: esto me permite visualizar los 
datos
           ya sea de manera impresa por pantalla o en su defecto por 
impreso-
           ra.
...
Bueno espero que esten felices de tener todas las consideraciones y 
conceptos
...
[ Nota de AdIdAs[]: Che Night... Mucha formalidad ahi!!! Si llega a leer 
este
...
El codigo esta escrito en un lenguaje generico, o sea que puede ser 
facilmente
...
Antes que nada tratare de dar una peque~a explicacion de lo que es un 
vector.
...
El metodo de ordenacion Burbuja-Switch elimina los defectos del metodo 
burbuja
...
Este metodo es bastante complicado de explicar!!! Bueno, voy a hacer lo 
mejor
que pueda....
...
igual a la cantidad de elementos sobre 2 (Si la cantidad de elementos es 
impar
...
se va a recalcular, siendo su valor la mitad del salto anterior. O sea que 
el
salto ahora va a ser 2, por lo que se compararan los elementos 1 con el 3, 
el
...
Listo, aca termina la nota de vectores... Espero que les haya servido de 
algo,
...
de este juego en tercera persona, que va a dar que hablar o nos va 
...
mejor que el Tiberium Sun. El Renegade va a ser un juego al estilo Tomb 
Raider
...
poco que se nombra es que ahora habra dragones (Eso va a estar bueno de 
ver).
...
violento que no debia estar al alcance de los chicos? Bueno, no me imagino 
que
...
decir que si nosotros le disparamos con una escopeta en el brazo somos 
capaces
...
por que mato soldados en un juego no voy a salir a la calle a matar al 
primero
que vea.... (o si?).... maybe...
...
La compania Maxis vuelve a lanzar otro simulador de "algo". Como a 
principios
de los '90 saco el Simcity Classic, Simcity 2000, Simant, Simearth, Simfarm
,etc., ahora vuelve con el The Sims el simulador de Familia. Si leyeron 
bien,
...
familia de locos reventados, podemos hacerlo, tambien podemos hacer una 
pareja
...
Todos los mails que quieran mandar a  ---> puntokomzine@hotmail.com <--- 
seran
...
Y? Que les parecio este numero? Nos vemos de nuevo en Julio, tratando de
mantenerlos informados... No sean pelotudos y escriban!! Queremos saber que
les parecen las secciones de la revista, que les gusta y que no, 
sugerencias,
...
ImPossiBle 2 SoundtrAck (Que buena que esta la cancion de LimP BizKIt!), 
KoRn,
