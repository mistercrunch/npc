---
title: "FIH-07-Install_Config_Serv"
ezine: "fih"
---

# FIH-07-Install_Config_Serv
...
Y para instalar el Active Perl, tambiΘn lleva un setup y no hay que hacer nada de especial. 
Simplemente decirle la ruta del perl en el httpd.conf

#!c:/program files/perl/perl.exe

Esto es la instalaci≤n bßsica.
...
# Dynamic Shared Object (DSO) Support --> Aquφ se cargan los m≤dulos de apache
...
TambiΘn se le puede decir un directory index:

DirectoryIndex filename.html

Si queremos que un cgi se ejecute automßticamente:

DirectoryIndex /cgi-local/index.pl

TambiΘn podemos utilizar la autentificaci≤n con HTTP con el Apache:
...
Mßs cosas. Imßginemos que tenemos archivos .bak que no queremos que la gente los pueda ver, 
...
TambiΘn se le puede decir al Apache que cualquier cgi que vea, este dentro del ScriptAlias o no, 
lo ejecute, esto no es recomendable, pero por si acaso basta con decirle esto:

AddHandler cgi-script .cgi

TambiΘn se le puede decir al apache que una pßgina shtml la interprete como SSI 
...
Los directorios server-estatus y server-info estßn comentados por defectos y mejor dejarlos asφ 
...
TambiΘn se le puede decir al apache que cuando vaya a http://localhost/~juan/ 
...
El apache tambiΘn puede hacer de proxy, esta opci≤n por defecto estß desactivada, 
...
max_execution_time = 30 --> mßximo tiempo de ejecuci≤n de un script php. 
