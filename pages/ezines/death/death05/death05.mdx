...
Este documento tem por funτπo descrever como utilizar o Linux para
conectar-se a um Provedor Internet atravΘs de uma linha discada. Por
enquanto ainda nπo sπo abordadas a utilizaτπo de softwares de mail, news,
etc.

Assim que sobrar um tempinho, estarei lanτando um "kit conexπo", que poderß
ser utilizado em qualquer mßquina com Perl instalado.

1.1 Acentuaτπo

Este documento foi escrito na Lφngua Portuguesa, portanto contΘm acentuτπo.
Se vocΩ nπo consegue visualizar os caracteres acentuados, vocΩ tem as
seguintes opτ⌡es, em ordem:

a) Ler o Portuguese-HOWTO (sim, este ╔ em portuguΩs - *quase* igual ao
nosso!) 
...
VocΩ deve ter instalado os softwares de rede (pacote N, no Slackware - nem
todos sπo necessßrios), alΘm de ter compilado o kernel com suporte a PPP.

Identifique em qual saφda serial o seu modem estß conectado. Lembre-se:
...
Logo ap≤s, utilize o "su" para tornar-se root (vocΩ nπo estß lendo este
...
Esta aτπo produzirß o seguinte:

telematica:/home/farias# ls -l /dev/modem
lrwxrwxrwx   1 root     root            4 Jan 18  1997 /dev/modem -> /dev/cua1

A configuraτπo padrπo normalmente permite acesso direto α /dev/modem apenas
para o usußrio root.

VocΩ pode utilizar o software "minicom" para testar sua configuraτπo e para
observar as sequΩncias de conexπo do seu Provedor. VocΩ irß precisar delas
se desejar automatizar o processo de conexπo.

2.1 Arquivos de Configuraτπo.
...
Para obter maiores informaτ⌡es sobre estes arquivos, consulte "man 5
hosts_access". Estou considerando que vocΩ nao esteja conectado α uma LAN
e/ou que nπo tenha acesso α um IP estßtico (se vocΩ nπo sabe o que Θ, com
certeza nao tem).

Neste ponto, vocΩ precisa saber qual Θ o name server (DNS server) de seu
...
VocΩ pode utilizar atΘ 3 name servers, que serπo consultados em ordem, no
...
localhost 127.0.0.1
0.0.0.0   telematica
#* "telematica" Θ o nome de minha maquina, troque pelo nome da sua.
#End of /etc/hosts

Uma mφnima configuraτπo Θ necessßria em "/etc/networks".
...
VocΩ precisa agora configurar as opτ⌡es a serem utilizadas pelo pppd,
durante a conexπo. Para isto, crie o arquivo "/etc/ppp/options":
...
Aconselho ainda a criaτπo do arquivo /etc/ppp/scripts/ppp-off, contendo o
seguinte:
...
Com isto, jß Θ possφvel testar uma conexπo manual ao provedor. Se tudo
...
VocΩ pode utilizar o "minicom", que tem a possibilidade de sair sem ressetar
o modem (Ctrl + A + Q ou Alt + Q). Agora vocΩ deve logar-se como "root"
("su" nπo irß funcionar). Disque para seu Provedor, logue-se (informe seu
username e senha quando solicitado), e estabeleτa PPP no servidor (digitando
"ppp" ou selecinando a opτπo equivalente em algum menu - depende do
provedor). VocΩ poderß observar o aparecimento de "lixo" em sua tela, tipo:
...
A opτπo -d habilita o debug, o que poderß ajudß-lo na resoluτπo de
problemas.

Alguns segundos depois, vocΩ poderß verificar a interface PPP, digitando:
...
* inet addr: indica o seu n·mero IP para esta conexπo.
* P-t-P: indica o n·mero IP do servidor.

Se tudo ocorreu bem, parabΘns, pois vocΩ estß conectado. Contudo, existe um
porΘm. Se, por acaso, vocΩ nπo conseguir acessar outros hosts que nπo seja o
roteador do provedor (neste caso, 200.248.171.4), vocΩ deve verificar a
...
Se nπo aparecer uma rota default, ou entπo demorar muito para aparecer toda
a tabela de roteamento, Θ provavel que o pppd esteja tendo problemas para
configurar a rota default. Isto poderß acontecer no caso de jß existir uma
rota default *antes* de vocΩ rodar o pppd. Neste caso, digite (como root):

telematica:/home/farias# /sbin/route del default

╔ provavel que esta rota esteja sendo definida nos scripts rc de
inicializaτπo do Linux (/etc/rc.d/rc.inet1, no SlackWare), portanto comente
as linhas necessßrias.

Teste agora a resoluτπo de nomes, "pingando" um site que vocΩ tenha certeza
que esteja on-line. VocΩ irß notar que haverß uma certa demora atΘ que o
nome seja resolvido, mas se este atraso nπo for demais, estß tudo OK.
...
Agora jß Θ possφvel automatizar a conexπo.

3.2 Automatizando o processo de conexπo.

Eu utilizo scripts para o "chat" na automatizaτπo da conexπo, os quais nunca
me deram problemas, portanto nunca me preocupei em aprender a utilizar o
"dip".

Com estes scripts, vocΩ necessitarß digitar apenas um comando para
estabelecer conexπo com seu provedor de acesso. IMPORTANTE: Somente utilize
estes scripts se vocΩ testou a conexπo manual e a mesma funcionou 100%!!

Para executar tais scripts, vocΩ precisarß ser root, ou membro do grupo do
pppd (lembrei deste fato agora, e nπo pretendo reescrever este documento
...
#
# Este script nπo Θ seguro, jß que Θ possivel visualizar os argumentos
# atravΘs do 'ps', mas Θ bastante fßcil de entender. Se desejar algo
...
LOCAL_IP=0.0.0.0	# IP local. DinΓmico = 0.0.0.0
REMOTE_IP=0.0.0.0	# IP remoto. Geralmente 0.0.0.0
# NETMASK=255.255.255.0	# Netmask, se necessßrio.
#
# Exportar varißveis para que estejam disponφveis para o script ppp-on-dialer
export TELEPHONE ACCOUNT PASSWORD
# 
# Localizaτπo do script 'ppp-on-dialer'. TEM que ser o caminho absoluto,
# nπo me pergunte o porquΩ!
#
DIALER_SCRIPT=/etc/ppp/scripts/ppp-on-dialer
#
# Inicia a conexπo
# 
# A maioria das opτ⌡es estπo informadas no arquivo /etc/ppp/options.
#
...
#
# Segunda parte dos scripts de conexπo...
# 
exec chat						\
	TIMEOUT		30				\
	ABORT		'\nBUSY\r'			\
...
Para maiores informaτ⌡es sobre scripts de conexπo, leia a manpage do chat, o
PPP-HOWTO.

3.2.1 Testando os scripts de conexπo.
...
Em outro VC ou xterm, tambΘm como root, digite:

telematica:~# /etc/ppp/scripts/ppp-on &
[1] 678

Se vocΩ nπo rodar o script em background (utilizando o '&'), vocΩ nπo terß o
prompt liberado atΘ que o pppd termine (quando o link Θ terminado).

Agora, retorne ao VC (ou xterm) onde o log estß sendo mostrado. VocΩ deverß
observar algo semelhante ao que segue (desde que tenha especificado a opτπo
'-v'para o chat):
...
Observe as ·ltimas 3 linhas de log. Se vocΩ receber estas, existem boas
chances de vocΩ estar conectado! :-) Agora, teste a conexπo, primeiramente
'pingando' o "remote IP". Logo ap≤s, faτa o mesmo com o IP de um site que
sabidamente esteja on-line. Depois destes dois testes, utilize nomes ao
invΘs de IP's para testar o seu "/etc/resolv.conf".
...
farias@spock.ucpel.tche.br com o log da conexπo (ei! nπo mande todo o
/var/log/messages!), que assim que eu lembrar minha senha nesse servidor eu
leio e respondo! :-)

Se tudo ocorreu bem, vocΩ pode navegar α vontade, pois vocΩ jß faz parte da
grande rede!!

3.2.2 Desconectando...

Para desconectar-se, utilize o script /etc/ppp/scripts/ppp-off. Novamente,
vocΩ deve ser root para executar esta tarefa (ou membro do grupo do pppd,
mas isto Θ outra hist≤ria...). VocΩ poderß observar o seguinte no arquivo de
...
A idΘia bßsica deste documento era apenas ajudar os usußrios a estabelecerem
uma conexπo PPP com um provedor de acessos. Quanto α utilizaτπo de softwares
clientes, como browsers, leitores de e-mail, programas de ftp, etc,
aconselho a leitura do ISP-Hookup-HOWTO ou entπo entre em contato com seu
Linux Guru local. Ainda nπo consegui organizar meu tempo para criar um
documento mais abrangente.

Espero que este documento possa ajudar qualquer usußrio com o mφnimo de
conhecimento em Unices a conectar-se a Internet. Se meu objetivo nπo foi
alcanτado, solicito que entrem em contato comigo para que se possa melhorar
este breve e simples tutorial. A intenτπo Θ tornar o Linux um SO o mais
amigßvel possφvel, e embora seja uma tarefa difφcil, Θ algo que *pode* ser
