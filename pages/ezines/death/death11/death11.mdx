...
-   Polvora, explosivo liquido ou gßs de carbureto (a quantidade pode va-
    riar de acordo com a sua necessidade, quanto + maior sera a explosao)
    o gas de carbureto, e o mesmo gas usado em maτaricos, vc encontra  em
    qquer casa de material de contruçao.
...
4o) Encaixe a bateria de alimentaçao do receptor na sua devida entrada.
...
terminais da lampada, ou se vc quiser, pode fazer um outro esquema q faça
...
***OBS.:  Ah, E POR FAVOR NAO ESQUEÇA DE CARREGAR O CAPACITOR!!!!!!!!!!
;)
...
OBS: Essa matéria foi escrita apenas como alerta aos perigos de se usar um
Navegador que tenha erros (Bugs) que possibilitem açΣes t╞o prejudiciais ao
sistema como as descritas abaixo.


O Navegador Infernet Exploder 5.0 permite que uma página, executando scripts
(seja JavaScript ou VBScript) faça a exportaç╞o dos Favoritos do IE5, sem
pedir confirmaç╞o ao usuário, gravando-os em um arquivo.

Essa página ou arquivo ficará com os endereços dos Favoritos em forma de
links, bastando ao usuário apenas clicar uma vez nela para ir para a página
"Alvo".

Até aí, n╞o há muito problema...o máximo que uma página pode fazer é encher
o HD do cliente com páginas tendo os Favoritos como conteúdo (o que demoraria
uma década).

Um dos problemas de se fazer isso é que quando os Favoritos s╞o exporatado,
aparece uma mensagem: "A exportaç╞o dos Favoritos foi concluída com êxito".
Se isso aparece várias e várias vezes, o usuário teclaria Ctrl+Alt+Del e
cancelaria o processo.

Porém, isso n╞o é tudo que é permitido fazer com a exportaç╞o dos Favoritos.
Ao executar o comando para a exportaç╞o dos Favoritos, é necessário fornecer
o caminho e o nome onde ele ficará gravado. Ex: C:\Windows\Favoritos.html.
Eu testei várias vezes, até ter a certeza de que o navegador realmente n╞o
pediria permiss╞o para gravar os favoritos em determinado arquivo.

Foi aí que tive a idéia de especificar um arquivo já existente.
Fiz uma cópia de segurança do C:\Windows\Win.ini e executei um comando,
utilizando JavaScript para poder gravar nesse arquivo que já existe, para ver
qual seria a reaç╞o do Navegador, descobrindo que o arquivo especificado
para ser o recipiente dos Favoritos já existe no HD do cliente...a mensagem
que apareceu foi essa: "A exportaç╞o dos Favoritos foi concluída com êxito".

Achei estranho e resolvi abrir o Win.ini para ver o resultado e, qual n╞o
foi minha surpresa?

Todo o conteúdo do Win.ini havia sido trocado pelos Favoritos do IE5.

N╞o acreditei e resolvi especificar o C:\Command.com para verificiar se isso
aconteceria com um arquivo binário...BUMBA...substituído...adeus Command.com
(É óbvio que havia feito Backup antes).

Ent╞o, creiei uma página e a publiquei para ver o resultado.
Essa página tem comandos em JavaScript para substituir arquivos como:
...
Pedi a um amigo meu que entrasse no site (avisei-o com antecedência sobre
o que aconteceria e pedi para que fizesse backup de seus arquivos) e
descobri que a página é letal ao sistema operacional (nada que um disco de
boot e backups corretos n╞o consertem).

Porém, avançando minha pesquisa, tentei tornar a página um tanto mais letal.
Tentei fazer com que ela sobrescrevesse o Explorer.exe (Já sabia que arquivos
em execuç╞o n╞o podem ser alterados, porém, n╞o costumava nada tentar).
Tentei também fazer com que os seguintes arquivos fossem sobrescritos:
...
Porém, descobri que n╞o dá para substituir arquivos que estejam marcados como
Somente Leitura.

Bom...chega de explicaç╞o e aí vai o código:
...
OBS: Para cada arquivo que for substituído, aparecerá a mensagem:
"A exportaç╞o dos Favoritos foi concluída com êxito".
No caso acima, aparecer╞o 7 mensagens.

Ess código dentro de uma página fará com que o usuário que entrar nela,
perca seus arquivos de sistema que est╞o especificados acima (o que, para
usuários inexperientes, seria a morte, pois n╞o saberiam utilizar um
Sys a: c:

O código pode ser utilizado para substituir qualquer arquivo dentro do
computador do pobre coitado que estiver utilizando o IE5, desde que:

1º - Você conheça sua localizaç╞o;
2º - N╞o esteja marcado como Somente Leitura ou Escondido.


Se quiser ver como funciona esse código, entre em:
...
E chamada  de SP. as partes a serem analisadas  serπo o 'local array2' 
e o 'return  address', so isso e necessßrio para termos root! yes =). 
