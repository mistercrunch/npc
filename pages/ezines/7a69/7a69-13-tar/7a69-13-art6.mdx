...
```
 -=( 7A69#13 )=--=( art6 )=--=( Principios para la escritura )=--=( ProdiX )=-
	...
	Los LKM, (Loadable Kernel Modules), son peque±as (o no tan peque±as)
funcionalidades que se a±aden al kernel de forma dinamica, es decir, sin tener
...
insecure mode) en nuestro /etc/rc.securelevel.
 	Para dar soporte a LKM en el kernel solo tenemos que a±adir una linea
...
--------------------
	...
	a.1) Modulos "system call": Sirven para a±adir syscalls al sistema o
	     modificar una syscall a±adiendo la nuestra propia y haciendo que
	     esta llame a la antigua.
	a.2) Modulos "virtual file system": Sirven para hacer VFS's...
	a.3) Modulos "device driver": Para a±adir nuevos pseudo-dispositivos.
	...
	a.5) Modulos "miscellaneous": Son todos los modulos que no se
	     ...
	     a±adir dos syscalls. ┐Desventajas? Si, por ejemplo a±ades una
...
cada tipo de modulo tiene sus propios a±adodos caracteriscitos, pero eso ya se
...
#include &lt;sys/lkm.h>  
	...
	Por supuesto, iremos a±adiendo mas cabezeras en funcion de lo que
...
del tipo de modulo que sea, se a±adira antes de la definicion de la funcion
entry:
 ...
 * elemento de esta estructura. El segudo elemento es el tama±o de la
...
-->	ramblo$ gcc -O2 -Wallá-D_KERNEL -I/sys -c mimkdir.c
...
-->	ramblo# modstat
-->	Type    Id Off Loadaddr Size Info     RevÇModule     Name
