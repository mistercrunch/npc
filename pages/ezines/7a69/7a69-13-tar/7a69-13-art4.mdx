...
- 2. La fuente de alimentacion arranca el generador de reloj, y envia la se±al
  #POWERGOOD al bus.

- 3. La CPU recibe el se±al #RESET, y se coloca en modo REAL Con los registros a
...
- 4. La ejecucion comienza en 0xFFFFFFF0, 16 bytes por debajo del limite de
  ...
  tama±o de la tabla de interrupciones) adecuado para modo real.
...
Bueno, ahora es cuando setup.S debe averiguar el tama±o de la RAM.
...
Creamos la global description table, el tama±o de esta es dependiente de la
...
continuaci≤n obtenemos la velocidad del procesador. Aqui vemos como Linux
...
Curioso, el reloj en la CMOS estß en BCD (Binary Coded Decimal).
...
Inicializamos los distintos terminales seg·n hemos compilado el kernel.
...
No hace gran cosa, solo pilla el tama±o de la tabla de ksyms. Volvemos a
init/main.c.
...
cache si todavia no se ha a±adido a la lista de caches.
...
Creamos el procfs y vamos creando directorios seg·n los defines. 
...
Pero a·n tenemos el hilo padre.
