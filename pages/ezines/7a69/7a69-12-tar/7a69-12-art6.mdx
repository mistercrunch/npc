...
#####################################################################
#   Peque±a guia sobre el funcionamiento de los archivos de log     #
#   de un sistema linux.                                            #
#   by Umma Kynes                                                   #
#####################################################################

"El mΘtodo mßs seguro de guardar un secreto es convencer a la gente
de que ya lo sabe."
                                  Antigua Sabiduria Fremen

Este articulo esta dedicado a mi mentor, Legba, el me mostro' todo
este mundillo. Gran parte de este documento es una traducci≤n de un
articulo suyo, con algunas ampliaciones de mi cosecha. :-))

Introduccion
┐Por que he hecho esto?

Todo es fruto del empe±o y de la curiosidad, cuando estudio algo me gusta
hacer peque±os articulos o resumenes de lo que veo para entenderlo bien y para
...
Un peque±a aclaracion: en todo el documento, uso la palabra _log_ refiriendome
...
Peque±a aclaracion: Cualquier usuario puede crear su propio programa que crea
...
es hacerse algun tipo de herramienta o un peque±o script con cualquiera de las
...
maquina, pudiendose introducir tambien lineas falsas. Basta hacer un peque±o
...
sucede siempre. El hecho de usar esto ya complica mucho la manipulaci≤n de
los ficheros de log.
...
actuar como un ni±o que le quita veinte duro a su madre del monedero y que
...
letoduke login: paul
Password:
Last login: Wed May  22 12:41:17 on tty2  <---- íMira esto!
Have a lot of fun...
paul@letoduke:~ >
...
demasiado peque±o, por ejemplo si tiene un tama±o de 4184 bytes, cuando por
ejemplo mi sendmail es de un tama±o de 26488 bytes, esto da mucho que pensar.
En el caso de que el tama±o sea razonable habria que intuir la version (si,
...
ejemplo del contenido del fichero utmp, para ello he realizado un peque±o
...
Tipo de Login: 2                  Esta entrada se le a±ade al fichero
...
-------------------------
Tipo de Login: 1                  Esta otra tambien se a±ade al arrancar,
...
Direccion IP: 0 0 0 0
-------------------------
Tipo de Login: 8                  Esta entrada se a±ade poco despues del
PID: 146                          arranque, unos segundos despues,
Terminal:                         dependiendo de la velocidad de la maquina
ID: l2                            y se a±ade cuando se carga el software
...
normal es que tenga un tama±o considerable, asi que mejor vuelca la salida a un
...
a±aden tres entradas, estas tres son las que vimos antes en el ejemplo de
utmp, que son de tipo 2 (reboot), 1 (runlevel), 8 (matar proceso) y ademas
se a±aden tantas entradas como terminales virtuales haya, todas con tipo de
...
terminales virtuales, se a±adiran una entrada con tipo de login (6) todas
...
como resultado de un comando logout o exit, por tanto se a±aden al fichero
...
un login de tipo 5 (INIT_PROCESS), para despues a±adir un login de tipo 6
...
e) Apagado de la maquina: Cuando se apaga la maquina, se a±ade en primer
...
Como ves, el procedimiento de a±adir entradas en el wtmp es similar al utmp
...
deteccion de algun proceso extra±o llamado "keylogger" o "cracker" o
...
esta sucediendo algo extra±o en su maquina.
...
de log crece rapidamente, lo que hara' que tenga un tama±o considerable, mas
...
problema es modificar la informacion de forma coherente para enga±ar al
administrador, ademas tenemos la dificultad a±adida de que acct hace logs de
