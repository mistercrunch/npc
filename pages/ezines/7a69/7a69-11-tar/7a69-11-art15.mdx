...
noche a la mañana, sino que sera gradual (se calcula que durara unos 10 años),
...
Al tener un tamaño de 16 bits, fija el tamaño maximo de un datagrama a 65535
bytes. Aunque normalmente se trabajara con tamaños mas pequeños: 1500 bytes,
576 bytes, etc.

* Identification. (16 bits)
Un número que permite, junto con las direcciones origen y destino y el campo
...
tamaño maximo que puede tener un datagrama para circular por esa red. Este
...
tamaño. Como respuesta a esta pregunta nacio la fragmentacion de IP.
...
se diese el caso de llegar a una red con MTU mas pequeña que el tamaño del
...
* Fragment Offset.(FO)
Con este campo se controla la situación del fragmento dentro del datagrama.

* Total Length. (TL)
Una vez restado el tamaño de la cabecera indicara el numero de bytes del
...
que el tamaño del mismo, el modulo IP del gateway comprueba el flag 'DF' de la
...
recalculado. Se le añaden los datos restantes del datagrama original, tendra
...
completo (no fragmentado aun) de tamaño 2048 bytes llega a una red de MTU =
...
libro lo decia de coña o no :)), que dice que el proceso para recomponer un
...
otro. Lo que se hace en realidad es mantener un buffer del tamaño maximo que
...
poner aqui una pequeña introduccion para los que no tengan muy claro aun el
...
Si señor, amigo mio. El archiconocido y ya superado 'ping de la muerte', esta
...
texto. Sabemos que el tamaño maximo de un datagrama IP es de 65535 bytes, de
...
tamaño maximo permitido para un datagrama, que es 65535, para proceder al
reensamblado. Pero si el tamaño del datagrama que enviamos es mayor... pues
...
el tamaño del paquete, con lo que hacia todo el trabajo el solito.

Sabiendo que la cabecera IP es de 20 bytes (sin opciones) y que la cabecera
de un ping (ICMP) es de 8 bytes, solo quedaba añadir los datos suficientes
...
De lo que se trata es de meter fragmentos con offset 0 y tamaño 0 en la lista
...
Lo que se pretende aqui es mandar fragmentos lo suficientemente pequeños como
para que ciertas partes de la cabecera TCP (no se usa, en principio, con
paquetes UDP) queden fuera del primer fragmento.

El tamaño minimo de datos que puede llevar un fragmento es de 8 bytes
...
fragmento2:
           IHL = 5
           MF = 0 (o 1, depende)
           FO = 1 (empezamos en el 8º byte)
           TL = depende (se pueden seguir mandando fragmentos pequeños, o
                         mandar el resto del paquete en este).

Las formas de evitar este tipo de ataques son: o bien fijar un tamaño minimo
 ...
 * 04/05/1999
 *
 * Tipico SYN-scanner al que le he añadido la posibilidad de enviar el SYN en
...
mas pequeñas se soluciona de diferente manera (aunque sigue siendo con
...
atravesar (por tener una MTU menor que su tamaño), enviara un ICMP al host
origen indicandole que tiene que fragmentarse y tambien 'el tamaño que deben
...
tamaño igual a su MTU (excepto quizas el ultimo).
...
Ripe, Anarion, Jafar, mnm y resto de peña del canal #root ;].
...
[3] - "Computer Networks", 3ª edicion, Andrew S. Tanenbaum, 1996.

[4] - "Internetworking with TCP/IP", volumen I, 3ª edicion, Douglas E. Comer.
