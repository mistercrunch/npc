...
Come ho giα esposto agli amici SPP, non essendo io un profondo conoscitore di linguaggi,
...
Allora mi Φ venuta un'idea, quella di scrivere una serie di articoli sulla Meteorologia,
campo in cui lavoro e in cui sicuramente tutti si sono imbattuti: ..sarα bello domenica
o pioverα?, ..cadrα neve in montagna?, ..c'Φ nebbia in autostrada: corro o non corro? (in
...
Φ di gettare i semi per far germogliare un qualche minimo interesse.
...
L'atmosfera Φ costituita da un miscuglio di gas di vario genere (anche di quelli emessi
dopo una sostanziosa pasta e fagioli ;-) ), alcuni in quantitα costanti, altri in quantitα
...
Questo perchΘ l'aria diventa sempre pi∙ rarefatta a mano a mano che aumenta la distanza
dalla superficie terrestre, ovvero perchΘ la sua densitα diminuisce con l'aumentare della
...
Di queste zone, quella che a noi interessa maggiormente Φ la Troposfera, perchΘ Φ la zona
...
casi di inversione termica, cioΦ quando la temperatura aumenta con la quota.
I movimenti verticali sono responsabili della formazione delle nubi.
La Troposfera Φ caratterizzata da un andamento medio in cui la temperatura diminuisce con
l'aumentare della quota, e dalla concentrazione di vapore acqueo.


				VAPORE ACQUEO E UMIDITA'
Sappiamo che la materia pu≥ essere in tre stati diversi: solido, liquido o gassoso.
Per passare da uno stato ad un altro (ad una data pressione) c'Φ bisogno di una certa
quantitα di energia per poter rompere le forze che tengono unite tra loro le molecole.
Per quanto riguarda l'acqua, la quantitα di energia per passare da uno stato all'altro Φ:
...
Maggiore Φ la temperatura dell'aria, maggiore sarα la quantitα di vapore che potrα contenere;
maggiore la quantitα di vapore, maggiore sarα la tensione di vapore (cioΦ la pressione dovuta
al vapore acqueo).
La tensione di saturazione Φ invece la quantitα massima di vapore che pu≥ essere contenuta,
ad una certa temperatura, in una certa massa d'aria.
Quindi, se ad una massa d'aria viene aggiunta una quantitα di vapore che eccede quella
relativa alla massa stessa, la quantitα di vapore eccedente condensa.
PoichΘ ad ogni temperatura corrisponde un valore di massa d'acqua contenuta in una massa
d'aria, vediamo che il vapore Φ funzione della temperatura:
...
L'umiditα relativa Φ il rapporto tra una massa di vapore acqueo presente rispetto alla
massa di vapore acqueo in saturazione.

Se l'umiditα dell'aria supera il punto di saturazione relativo ad una certa temperatura
...
bens∞ di piccolissime goccioline o aghi di ghiaccio (qualche centesimo di millimetro).
...
Altostrati: come i cirrostrati, ma pi∙ spessi.
Altocumuli: sono pi∙ estesi dei cirrocumuli, e sono formati da cristalli di ghiaccio
 e da goccioline d'acqua.
Nembostrati: pi∙ spesse e scure degli strati, sono cariche di pioggia e/o neve.
...
Cumuli: a forma di ammassi tondeggianti, meno imponenti dei cumulonembi, generalmente
 non danno luogo a precipitazioni (si pu≥ dire che sono sintomo di bel tempo); si
...
Cumulonembi: a forma di torre, possono produrre violenti temporali e a volte anche
 trombe d'aria; nella parte pi∙ alta si espandono orizzontalmente, assumendo una
...
Nubi stratiformi: caratterizzate da dimensioni orizzontali prevalenti su quelle verticali.
 ...
 sono nulli, cioΦ quando l'aria non tende a mescolarsi), e hanno una struttura piuttosto
...
Nubi cumuliformi: hanno dimensioni verticali notevoli, e sono caratteristiche di aria
 instabile ( quando si Φ in presenza di forti correnti verticali), animate da correnti
...
Nubi orografiche: sono causate dal movimento ondulato che assume l'aria quando scavalca una
 catena montuosa.
 Nella fase di salita l'aria si espande raffreddandosi e, se vi Φ sufficente umiditα, questa
 condenserα formando le gocciline delle nubi.
 Nella fase di discesa l'aria si comprime riscaldandosi, provocando cos∞ l'evaporazione
 delle goccioline d'acqua.
 Il risultato Φ la formazione di nubi di forma lenticolare: queste nubi appaiono stazionarie,
...
Lo stesso Φ avvenuto per l'articolo "Guida newbie all'IP spoofing" che altro 
non era che la seconda parte del progetto "0N0S3NDAi" elucubrata dal pur 
ottimo FuSyS. L∞ per i veri newbie c'era ben poco da spolpare. 
Al momento non avevo sottomano la prima parte del progetto e cos∞ 
...
Ma ecco quello che ne Φ uscito.
...
di suddivisione od organizzazione della rete pi∙ noti sono il modello ISO-OSI
...
modalitα di trasmissione. 
I dati vengono trasmessi in segmenti di byte, chiamati pacchetti. 
Nella manualistica sulle comunicazioni di rete Φ frequente trovare ancora il 
termine ottetto al posto di byte. L'ottetto, che ha il significato di 
insieme di otto bit, sostituiva la parola byte perchΘ al momento della 
progettazione del protocollo TCP esistevano sistemi con byte formati da nove
bit. Oggi, pur con la quasi totalitα dei sistemi che utilizzano byte di otto 
bit, questo termine viene ancora  utilizzato anche se Φ anacronistico.
...
byte-stream (la spiegazione di byte stream Φ pi∙ avanti). Nello stesso strato
opera il protocollo UDP, che trasmette i dati mediante l'invio di datagrammi.
Ad un livello pi∙ basso, nello strato della rete operano tre protocolli 
...
Come abbiamo giα detto, nello strato dell'applicazione operano i programmi di
rete. Verranno descritte le modalitα in cui questi programmi inviano e ricevono
...
full-duplex, cioΦ un trasferimento di dati bidirezionale, in cui l'host 
...
passiva della porta, la quale si dice che Φ in stato d'ascolto. A questo 
punto si Φ aperta una sessione di comunicazione punto-punto tra i due host. 
...
protocolli UDP e IP, che segmentano i dati in datagrammi, unitα di 
informazioni indipendenti trasmesse non sequenzialmente. 

Il flusso dei dati TCP pu≥ essere di due tipi: flusso di massa (ftp, news) e
flusso interattivo (telnet, rlogin).
Il meccanismo di trasmissione byte-stream adottato da TCP appare a prima vista
pi∙ farraginoso ed Φ pi∙ lento di quello usato da UDP, ma ha il vantaggio di
essere estremamente affidabile perchΘ, a differenza di quest'ultimo, TCP si 
...
destinazione. In questo caso TCP induce l'host mittente a ridurre la velocitα
di trasmissione.

Ma come fa il protocollo a garantire la ricezione dei pacchetti di dati TCP? 
Per capirlo dobbiamo esaminare com'Φ fatto un segmento TCP.
Un segmento di dati TCP pu≥ essere diviso in 18 campi.
...
6) Reserved: Φ un campo a 6 bit riservato, che verrα utilizzato in impieghi
...
8) Flag ACK (Acknowledgment Control Bit o bit del controllo delle conferme): 
   ...
   numero di acknowledgement Φ valido e deve essere letto.
...
12) Flag FIN (Finish Control Bit o bit del controllo di fine invio): campo di
    ...
    pi∙ dati da trasmettere.
13) Dimensione Finestra: campo a 16 bit che indica il numero di ottetti di 
    dati che l'host mittente accetterα. Questo numero definisce la grandezza 
...
14) Checksum (somma di controllo): campo a 16 bit che permette al modulo TCP 
    ...
    controllo Φ calcolata tramite una tecnica definita "complemento ad uno".
15) Puntatore urgente: campo a 16 bit che punta all'ultimo byte di dati 
    urgenti nell'area dati TCP.
16) Opzioni: campo di lunghezza di bit variabile, che pu≥ contenere tre 
...
L'host ricevente, se accetta la connessione, invia due segmenti. Il primo Φ 
...
scelto casualmente. L'host trasmittente trasmetterα a sua volta un messaggio di
acknowledgement, con il flag ACK impostato a 1 e il numero di acknowledgement
uguale al SEQ del server aumentato di 1. Ci≥ crea la comunicazione anche
nell'altro senso. 
...
l'host mittente invia contemporaneamente  tanti segmenti quanti ne pu≥ 
...
per qualche problema della rete, riceverα solo i pacchetti 1, 2  e 4. 
La risposta dell'host ricevente sarα che ha ricevuto solo i segmenti 1 e 2 e 
trasmetterα i segmenti ACK1 e ACK2. L'host mittente, ricevuta l'informazione 
ritrasmetterα i segmenti SYN3 e SYN4, ma contemporaneamente trasmetterα i 
segmenti SYN5 e SYN6, riempiendo cos∞ la finestra di trasmissione. 
E cos∞ via. Questo tipo di trasmissione viene chiamato metodo delle finestre 
scorrevoli o sliding windows. In questo modo, l'host ricevente pu≥ inviare 
una sola risposta di conferma per una ricezione di pi∙ segmenti 
contemporaneamente.
...
ricevente pu≥ comunicare solo l'avvenuta ricezione dei segmenti e non anche 
il contrario.
...
connessione TCP.  Mettiamo che il client invii FIN. Il server risponderα con un
flag ACK, che starα ad indicare di avere ricevuto la richiesta di termine della
connessione e, generalmente, invierα a sua volta un flag FIN, effettuando la
...
raramente perchΘ pochissime applicazioni TCP hanno bisogno di utilizzare questo
tipo di chiusura.

Una sessione TCP, pertanto, si pu≥ trovare in diverse condizioni, a seconda 
...
- SYN-RECEIVED: Φ lo stato dell'host ricevente che ha ricevuto ed inviato a 
  sua volta una richiesta di conferma per la connessione e sta aspettando la 
  conferma;
- ESTABLISHED: significa che la connessione tra i due host Φ aperta;
- FIN-WAIT1: Φ lo stato di attesa di una richesta di termine della sessione o
  di una conferma di una richiesta di termine della sessione precedente;
- FIN-WAIT2: Φ lo stato di attesa per una richiesta di termine della sessione
  dall'host remoto;
...
- CLOSING; Φ lo stato dell'attesa della conferma della richiesta  di termine 
  della connessione da parte dell'host remoto;
- LAST-ACK: Φ lo stato dell'attesa della conferma della richesta di termine 
  della connessione che l'host ha inviato all'host remoto;
- TIME-WAIT: l'host Φ in attesa di chiudere la connessione, dopo l'invio 
  della conferma della sua richiesta di termine della connessione;
- CLOSED: la connessione tra i due host Φ terminata.
...
RFC 768
E' un protocollo che  permette una consegna non garantita, ma pi∙ veloce dei
dati, trasmessi mediante l'invio di datagrammi. Una seconda caratteristica di
tale protocollo Φ che l'host mittente, durante la trasmissione dei dati, non
...
connessione del protocollo TCP. In questo caso il client effettuerα un'apertura
attiva della porta, mentre il server effettuerα un'apertura passiva, ricevendo i
dati senza comunicare l'avvenuta ricezione. La mancanza di questo meccanismo di
controllo rende meno affidabile, ma sicuramente pi∙ veloce, la trasmissione dei
dati. 
...
evitando eventuali congestioni del flusso. Inoltre essa dovrα provvedere alla
frammentazione e riassemblggio dei dati voluminosi. 
Con UDP Φ comunque possibile una verifica dell'integritα dei dati trasmessi e
viene permesso l'invio dei dati in broadcasting e in multicasting, cosa non
permessa al protocollo TCP. 

Ma come Φ fatto il datagramma UDP? Esso Φ composto da un header di 4 byte,
contenente quattro campi, e un'area dati. I primi due campi indicano la porta
sorgente e la porta destinazione del protocollo. Il terzo campo Φ il message
...
dati per la verifica dell'integritα delle informazioni. L'area dati Φ di lunghezza
...
datagramma IP. L'host ricevente, se avrα una porta in listening, accetterα il
pacchetto eliminando l'incapsulazione IP. L'analisi del datagramma UDP
permetterα all'host ricevente di verificare le informazioni del datagramma.
PoichΘ il volume dei dati inviati tramite il protocollo UDP pu≥ essere elevata,
i datagrammi UDP tendono ad essere frammentati in pi∙ pacchetti. Un controllo
della velocitα pu≥ essere effettuato con la produzione da parte dell'host server
...
pi∙ finchΦ il server non risponde con un messaggio soure quench.
...
RFC  791
L'Internet Protocol (IP) Φ il principale protocollo utilizzato per la
...
Significa unitα di dati autocontenente e, solitamente, specifica il tipo di
consegna dei dati, per distinguerlo dalla trasmissione dei dati byte-stream.
Pertanto, si pu≥ comprendere che  il datagramma IP, pur utilizzando lo stesso
metodo di trasmissione, Φ diverso dal datagramma UDP.
...
in parole di 32 bit.
- Type of service (TOS): campo di 8 bit che specifica la prioritα di consegna
dei dati e le decisioni progettuali. Tuttavia, il TOS non viene supportato da
parecchi host e router. Questo campo Φ a sua volta suddiviso in 5 sottocampi.
...
scarsamente utilizzato. Il secondo sottocampo (delay), cos∞ come gli altri tre,
definisce una prioritα non generale come il primo, ma dipendente dalle
...
l'affidabilitα. L'ultimo sottocampo (cost) indica che si vuole massimizzare il
costo.
...
poichΘ in caso di una sua attivazione che sia in contrasto con il MTU il
...
frammento, che avrα valore uguale a 0.
- Fragment offset: campo di 13 bit che indica la distanza o punto di rottura
dall'inizio del datagramma in caso di frammentazione. Questo valore verrα
utilizzato da IP per la ricostruzione dei frammenti.    Dato che la lunghezza
massima di un datagramma Φ di 65535, questo campo deve poter puntare in
qualsiasi punto del pacchetto e deve poter contenere valori che vanno da 1 a
65535.
- Time-to-Live (TTL): campo di 8 bit che indica quanto tempo potrα vivere il
pacchetto durante il suo viaggio nella rete. Questo campo viene decrementato di
un'unitα ad ogni passaggio in un router. Il router che riceva un pacchetto
scaduto prima del suo arrivo a destinazione, lo distruggerα ed invierα un
...
integritα viene gestito dal protocollo che ha creato il pacchetto.
- Indirizzi IP di partenza e di arrivo: source IP address Φ un campo di 32 bit
che contiene l'indirizzo IP dell'host mittente. Destination IP address Φ un
...
applicazioni di rete. Questo campo Φ suddiviso in tre sottocampi: copia, classe
opzione e numero opzione. Non tutti i router ed host supportano l'IP options.

In rete si indica per MTU il valore di unitα massima di trasferimento del
pacchetto che si vuol trasferire. Quando un programma di rete cerca di inviare
un pacchetto pi∙ grande della MTU di quella rete, esso viene frammentato in pi∙
parti. La frammentazione pu≥ avvenire anche durante il tragitto, se il router
ha una MTU di rete pi∙ piccola di quella della rete da dove Φ partito il
pacchetto.
Come si Φ giα detto, il riassemblaggio dei pacchetti avviene per mezzo dei tre
...
compiti: il primo Φ  di trasportare i messaggi d'errore della rete; il
secondo Φ di trasportare i messaggi di richiesta, i quali contengono sia
...
non contemplava la possibilitα di fornire notifiche di errori nel trasporto
dei dati. I progettatori decisero di semplificare l'attivitα di IP e di
affidare questo compito al nuovo protocollo ICMP, con alcuni limiti. Infatti
ICMP non Φ in grado di risolvere i problemi scaturiti durante il trasporto dei
dati, e non pu≥ comunicare il verificarsi di un errore ai router intermedi, ma
solo all'host che ha generato il pacchetto. Una particolaritα che Φ utile
menzionare Φ che un messaggio di errore non pu≥  generare altri messaggi di
errore. Ci≥ evita la produzione di messaggi a catena, che provocherebbero le
cosiddette "tempeste broadcast).

Ma ecco la descrizione del pacchetto ICMP.
L'header ICMP Φ formato da tre campi, di cui due a 8 bit e il terzo a 16 bit,
...
richiesta. Ci≥ avviene perchΘ il pacchetto del protocollo ICMP, per il
trasporto nella rete, viene incapsulato in un datagramma IP.

Il primo campo contiene il valore Type ed Φ di 8 bit. Questo valore indica i 15
tipi diversi di messaggio ICMP.
Il secondo campo contiene il valore Code ed Φ anch'esso di un byte
(1 byte= 8 bit). Questo valore contiene i codici o le categorie esistenti
per ogni messaggio. Ad esempio, l'ICMP di tipo 3 Φ un messaggio di errore che
...
Come si pu≥ vedere, ICMP comprende 5 messaggi di errore e 10 messaggi di 
...
Lo scopo principale del protocollo ICMP Φ proprio il messaggio di errore del
tipo 3, cioΦ destinazione irraggiungibile. Questa peculiaritα di ICMP serve per
...
all'host sorgente ogni volta che non gli Φ possibile consegnare il pacchetto
dei dati. L'impossibilitα di consegna dei dati pu≥ dipendere da un problema nel
trasferimento dei dati o da un  problema di routing. Nel primo caso il codice
di errore Φ di host irraggiungibile, mentre nel secondo caso viene prodotto un
...
codice 8: host di origine isolato (non pi∙ usato)
...
pi∙ breve. Esso ha una propria tabella di routing in cui vengono memorizzate le
...
utilizzando un percorso non ottimale, il router che Φ situato nel primo nodo
dove inizia il percorso non ottimale informa l'host di ci≥ con un messaggio di
...
pacchetto. Questo campo Φ chiamato "time to live" (TTL). Se il TTL raggiunge lo
...
passaggio. In genere ci≥ Φ dovuto ad un errore nelle tabelle di routing che
produce un ciclo di routing, cioΦ un ciclo di loop continui tra due router che
...
Questo tipo di errore pu≥ essere generato dall'host ogni qualvolta ci sia una
condizione di errore sconosciuta o quando un datagramma non contiene tutte le
informazioni richieste dal protocollo. Nel primo caso l'ICMP di tipo 12 avrα il
codice 0 ( cattivo header IP), mentre nel secondo caso avrα il codice 1
...
protocollo non orientato alla connessione, come IP, Φ privo di questo controllo
del flusso. PoichΘ i router utilizzano datagrammi IP, senza un adeguato
...
informando l'host mittente che Φ necessario rallentare la velocitα del flusso.
...
Innanzi tutto Φ bene ricordare che lo smistamento dei pacchetti si chiama
routing. Gli host preposti allo smistamento di questi pacchetti si chiamano
router (o anche gateway). Il routing pu≥ essere diretto od indiretto: nel primo
caso il routing avviene tra due host connessi direttamente alla stessa rete o
sottorete; nel secondo caso, i dati vengono trasmessi ad almeno un router, cioΦ
un host che si occupa del successivo smistamento ad un altro router o all'host
ricevente. Ogni router ha una sua tabella di routing, che non Φ altro che una
tabella, contenente tra le altre cose, una serie di indirizzi di destinazioni
successive, con a fianco un valore numerico (metrica) che indica la qualitα
della destinazione. La tabella di routing pu≥ essere statica e descritta in un
...
particolaritα del messaggio di annuncio Φ che il router pu≥ inserire nel
pacchetto ICMP pi∙ di un indirizzo di router. Ogni indirizzo di router Φ
...
l'indirizzo rimane valido, che di norma Φ di 1800 secondi.
...
internet grouper) per verificare se l'host di destinazione Φ raggiungibile e
...
risponde con un messaggio di tipo 8. L'utilizzo di questo tipo di ICMP Φ utile
per la verifica della connettivitα.
...
dell'indirizzo di riciclo o loopback, che Φ 127.0.0.1. Se il pacchetto torna
...
Φ utile bloccare gli ICMP di tipo 0 e 8.
...
Quando i pacchetti inviati da un host mittente devono raggiungere pi∙ host
...
loro. Il problema Φ che diversi router sono configurati per non inoltrare i
pacchetti con la modalitα broadcasting.
...
224.0.0.1, significa che il gruppo broadcast Φ formato da tutti gli host e i
router che partecipano a un multicasting IP su una sezione della rete.
Il pacchetto IGMP Φ formato da cinque campi.

- Version: campo che indica la versione del protocollo utilizzato. Con IGMP Φ
settato sul valore di 1
- Type: pu≥ avere due valori; il valore 1 significa che il messaggio IGMP Φ il
...
controllo, per la verifica dell'integritα del dato.
...
interattive, che permettono di fare arrivare le informazioni a pi∙ host.
...
Recentemente Φ stata rilasciata dalla Microsoft una patch che risolve il
problema.

3.2.4  Il protocollo ARP

RFC 826
L'Address Resolution Protocol Φ un protocollo che viene utilizzato per
risolvere un indirizzo IP di un host client nel relativo indirizzo MAC. No,
nulla a che vedere con la gloriosa casa  Apple. Il MAC (media access control) Φ
l'indirizzo fisico dell'host, detto anche indirizzo hardware della scheda di
rete.

Il processo in cui opera tale protocollo Φ il seguente: innanzi tutto il client
verificherα se il server fa parte della stessa rete o di una rete remota;
l'indirizzo IP del server sarα confrontato con la maschera di sottorete del
...
Se il server si trova nella stessa rete avverrα qwuanto segue: l'host client,
...
del server. Il pacchetto Φ inviato all'indirizzo broadcast e, quindi, a tutti
gli host del segmento i quali esamineranno il pacchetto. Gli host a cui non Φ
diretto il pacchetto, lo ignoreranno dopo averlo esaminato. L'host a cui Φ
destinato il pacchetto, inserirα le informazioni nella propria cache ARP ed
invierα il proprio indirizzo MAC all''host mittente. Dopo questo passaggio, pu≥
avere inizio la comunicazione tra il client ed il server.

Quando il server si trova in una rete remota, il client controllerα nella
propria cache ARP se ha l'indirizzo MAC del router predefinito. In caso
negativo, esso invierα un pacchetto ARP contenente il proprio indirizzo IP e
indirizzo MAC, con il quale richiederα l'indirizzo MAC del router. Quindi
invierα il pacchetto al router. Il processo Φ identico, da router a router,
fino al server.
...
- Protocol Address Lenght: campo che indica la lunghezza dell'indirizzo di
  protocollo
- Op code: campo che indica se il pacchetto Φ una richiesta od una risposta ARP
...
- Target's Hardware Address: campo che indica l'indirizzo hardware dell'host
  destinatario
- Target's Protocol Addres: campo che indica l'indirizzo hardware dell∞host
  destinatario
...
condividere i dati tra due host remoti. Questa nota Φ improntata alla 
descrizione delle applicazioni socket, e quindi non parler≥ del NetBios.
...
Tecnicamente su usa pure dire che un socket Φ una rappresentazione astratta 
...
L'API socket quindi si pu≥ definire come un'interfaccia tra le applicazioni
...
dell'ARPA, gli uomini dei laboratori dell'universitα di Berkeley,  
...
sotto windows. Si dice che il winsock Φ progettato come un componente WOSA 
(Windows Open Service Architecture). 

Nel 1991 nacque il Winsock Group, una societα formata da produttori e 
...
con nuove funzionalitα. Oggi siamo alle 2.2. 
Diversamente dalla versione 1.1, che era focalizzata interamente progettato
sul TCP/IP, la nuova API Φ indipendente dal protocollo di trasporto. 
...
Il funzionamento del socket non Φ particolarmente complesso, ma per capirlo 
ci sono alcuni termini che dovrete memorizzare. 
Ogni socket  host Φ costituito da cinque elementi informativi: il protocollo
...
connessione si servono di protocolli la cui trasmissione non Φ garantita, 
come IP e UDP. 
...
Il terzo tipo,  pi∙ rudimentale, viene chiamato "raw socket", da raw=grezzo. 
...
Nel modello dello stack dei protocollo TCP-IP, Φ come dire che il raw socket
...
Un esempio di tale tipo di comunicazione Φ quando un'applicazione crea un 
...
Per chi si vuole dilettare nella creazione dei pacchetti, Φ opportuno 
...
Trumpet  Winsock, versione 3.0d che per≥, a detta di alcuni esperti, rende il
...
secondo non Φ implementato, e vengono qui citati solo per completezza di
esposizione.


4.3  Funzionamento del socket

Ora passiamo ad una breve descrizione tecnica di ci≥ che avviene quando un 
programma comunica con un altro in rete. Credo che ci≥ sia utile anche per 
chi non si intende di programmazione, se non altro perchΦ potrα fornirgli
l'input a sviluppare i concetti qui espressi. 
Per costoro, Φ utile almeno riassumere quanto segue.
...
ad una chiamata di funzione. Ci≥ permetterα l'esecuzione della sezione di 
codice della funzione chiamata. Per fare un esempio, un nome di una funzione 
che vedremo pi∙ avanti Φ "socket". I valori (cioΦ argomenti o chiamati anche 
...
Variabile: Φ la rappresentazione che il linguaggio di programmazione usa per
definire dei dati, che possono essere numeri o stringhe di caratteri. Quando 
ad una variabile non Φ stato assegnato un valore, si dice che la variabile 
non Φ stata inizializzata. Ad una stessa variabile possono essere assegnati 
valori diversi.

Header file o file d'intestazione: Φ un file che contiene delle definizioni 
d'interfaccia di classe e/o dichiarazioni di funzione. Ha estensione .h. Un 
esempio Φ il file iostream.h che, tra le altre cose, dice al compilatore come 
utilizzare i comandi di imput ed output (<< >>). Quando nel sorgente l'header
Φ compreso in due parentesi acute (&lt;iostream.h>) significa che il file si 
trova nella directory predefinita dal compilatore. Se invece l'header Φ 
...
deve prima configurarlo nella giusta maniera. Se ricordate ci≥ che abbiamo 
detto pi∙ sopra, possiamo raggruppare i socket in  due tipi principali: 
...
Se il programma client utilizza il protocollo TCP si dice che Φ orientato 
...
Protocol_family: l'interfaccia socket puo effettuare comunicazioni su pi∙ 
tipi di reti e, quindi, pu≥ utilizzare diversi protocolli come TCP/IP, XNS 
ed i protocolli interni di unix. XNS Φ uno stack di protocolli simile a 
...
Quella utilizzata per le comunicazioni internet Φ la costante simbolica 
...
Socket_type: come abbiamo giα visto esistono cinque tipi di socket, dei quali
solo tre sono comunemente implementati: SOCK_STREAM, che indica che verrα 
utilizzato la comunicazione byte stream; SOCK_DGRAM che indica che verrα 
utilizzata la comunicazione con invio di datagrammi; SOCK_RAW, che indica che
verrα utilizzato un protocollo di basso livello, come ICMP e IGMP.
...
e address_lenght.
Come si pu≥ notare, anche intuitivamente, dai nomi dei parametri, Φ' la 
...
locali, Φ chiamata funzione "bind()"; il socket del server viene informato
su quale porta deve stare in ascolto da questa funzione. 
Una piccola digressione sulla funzione bind(). La chiamata a bind pu≥ essere 
utilizzata in 3 modi diversi:
1. I server registrano il loro indirizzo, ben noto, nel sistema. 
In pratica dicono al sistema: "Questo Φ il mio indirizzo e qualsiasi 
...
2. Un client pu≥ registrare un indirizzo specifico per se stesso, in modo che 
...
certo indirizzo unico, affinchΦ l'altra estremitα abbia un indirizzo di 
...
sistema socket. 
2. mioindr: Φ un puntatore all'indirizzo specifico della struttura del 
protocollo. 
3. lunghdir: dimensione della struttura del protocollo. 

Come? Ah, c'Φ qualcuno che non conosce le definizioni descrittore e 
puntatore? 

Puntatore: Φ un indirizzo di memoria associato ad un tipo di dato, dove 
...
manipolare il contenuto di un file consiste nell'aprire uno stream, cioΦ un
...
altre funzioni, per compiere operazioni sul file stesso. Tale puntatore Φ 
...
Questa chiamata di sistema Φ usata da un server orientato alla connessione 
per indicare che Φ disposto a ricevere le connessioni, cioΦ si trova in stato 
di attesa. 
Solitamente Φ eseguita dopo le chiamate di sistema socket e bind ed 
immediatamente prima della chiamata accept. Quando arrivano pi∙ richieste di 
...
essere accodate dal sistema mentre Φ in attesa che il server esegua la 
chiamata accept. 
...
vie, a sua volta chiamerα  la funzione "write()" per trasmettere i dati e la 
funzione "read()" per ricevere i dati di risposta a una porta specifica che 
verranno inviati dal server. 
A questo punto, il server chiamerα anch'esso le funzioni read() e write(), 
...
Se il programma client non Φ orientato alla connessione,  per configurare le 
informazioni locali esso chiama la funzione "bind" e la funzione "sendto"
per la configurazione delle informazioni remote. Dato che il socket Φ senza
connessione, non viene effettuata la chiamata alla funzione connect.
A sua volta, il programma server senza connessione ascolterα passivamente
i dati che giungeranno dalla porta di protocollo e chiamerα la funzione 
...
trasmetterα dati in risposta con la stessa funzione sendto ed il client
riceverα tali dati utilizzando la chiamata alla funzione recvfrom (invece di
recv o read), esattamente come avviene nel server. 
In questo tipo di comunicazione, il protocollo utilizzato non permette una
connessione diretta tra il programma client ed il programma server, cioΦ una
connessione detta point-to-point tra due punti di arrivo.
Come abbiamo giα visto, i dati vengono trasmessi mediante l'uso dei 
datagrammi.

4.7    Riepilogo

Ricapitolando, terminata la fase della configurazione, durante la 
trasmissione dei dati l'API del socket chiamerα cinque diverse funzioni: 
...
Select(): Φ solitamente usata dai programmi server o da programmi client 
particolarmente sofisticati e permette ad un singolo progesso di verificare 
lo stato di pi∙ socket. Richiede cinque parametri e sono: 
...
Ora Φ ovvio che chi non conosce l'arte della programmazione, pi∙ in lα non 
pu≥ andare. Memorizzare le varie funzioni chiamate da un programma durante lo
stabilimento di una connessione non serve a molto se non si smanetta in C o 
VB.  Lo sforzo di imparare un linguaggio verrα ricompensato dalla capacitα di
poter capire e commentare, ad esempio, un programma per creare pacchetti TCP,
leggendo il sorgente o magari di poterlo modificare per una diversa utilitα.
...
mio PC, quando ad un certo momento (non so perchΦ!!!) mi viene in mente un
...
.... ma dove lo avr≥ messo? ke casino sto picc∞! vabbΦ ora lo cerco.
Dopo un po' di ricerke lo trovo, lo lancio e .... sorpresa immensa
per i miei occhi .... tutte le unitα dalla A alla Z erano delle
...
Quale dei milioni di programmini del kaººo ke mi sono installato mi
avrα fatto tutto sto casino?
E va bene, ora mi Φ definitivamente passato il sonno; non posso
...
cancello tutte queste condivisioni (non so se c'Φ un metodo per
...
su TUTTE le lettere di unitα del tuo PC (non solo su C:, come facevano
altri programmini simili).
Le conseguenze le capisci da solo! ;-P
PoikΦ l'aggiunta di condivisioni crea delle chiavi nel registro ke
normalmente non esistono, per eliminarle bisogna andare a cancellarle
con il regedit. Non Φ sufficiente un semplice file .reg! Quelle chiavi
...
dove x Φ una lettera dalla A alla Z.
Se l∞ avete altre kiavi vi consiglio di lasciarle stare, a meno ke
non siate sicuri ke non volete nessuna condivisione su quel PC!
Potrebbero servire alla condivisione ke usa il vostro papα quando va in
ufficio! ;-))
Dopo aver fatto ci≥, riavviate Windows e tutto tornerα come era prima.
...
con le prome lettere di unitα, quelle dei floppy. :-O ... poco male!
...
imposti (se impostati) del tipo "Channel 1: %chan1 (dove %chan1 sarα sostituito
...
Se si trava solamente  significa che il colore Φ quello di default.
Il comando "$null" indica l' assenza di una chiave assegnata ad una variabile
(in questo caso un "%chanX").
A questo punto dobbiamo settare il menu che ci permetterα di far funzionare il
...
entrando nel server, per far si che questo script funzioni sarα in fine
...
non sempre Φ necessario settare manualmente le variabili nel file remote.ini,
...
Per questo script creeremo per semplicitα un nuovo file .mrc (chiamiamolo
...
"¿¿░║⌐o.,,.o⌐║░¿¿░║⌐[- Canale Restituito da $me -]⌐║░¿¿░║⌐o.,,.o⌐║░¿¿" | /mode
# -kipnmtslo
.-
.&Set Modes
...
che servirα per il funzionamento del "takeover"
...
Con il TakeOver si ha la possibilitα di deoppare automaticamente tutti quelli
presenti nel canale tranne noi stessi e di bloccare il canale impostando il
"/mode # +smpti" ed in pi∙ cambiano automaticamente il topic in "<--- 4,1
TaKeOvEr 1,0--->"
...
E qu∞ finisce la seconda parte, mi raccomando non divertitevi troppo con il
takeover :-) hihihihi.
