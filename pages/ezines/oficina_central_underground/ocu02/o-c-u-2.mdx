...
```
 ...
 modificar una DLL del ventanukos a la que añade multitud de nuevos comandos accesibles
...
- MM Capture avi -
	...
	Pues eso...captura en ofrmato video lo que hace la victima.
        ...
        otro los segundo y el tama±o de la imagen.
...
- TCP file receive/send -
 ...
 04  **                SISTEMA DE TELEFONIA MOVIL GSM (I CAPITULO)                **
   ...
   La tarjeta SIM es un chip a travΘs del cual nos identificamos ante nuestro operador GSM
 y a travΘs de este con otros operadores. Representa la lφnea GSM que tenemos contratada,
 ya que la pongamos en uno u otro terminal mantenemos el numero de abonado. La SIM, consta
 de un c≤digo de seguridad de acceso llamado PIN, el cual estß formado por cuatro dφgitos.
 Dicho c≤digo podemos cambiarlo a voluntad e incluso anular su petici≤n.Estß formada por un
 microprocesador, una memoria ROM y otra RAM de 8K, en la que se encuentran: ╖	Una agenda
 de 90 o 100 posiciones para almacenar nombres y n·meros de telΘfono. ╖	Una agenda de 14 o
 16 posiciones donde quedarßn almacenados los SMS recibidos. ╖	Una agenda de 10 posiciones
 donde almacenar 10 nombres y n·meros para crear una restricci≤n de llamadas a dichos
 n·meros. S≤lo con PIN 2. La SIM consta de un c≤digo de acceso, el PIN 1, de 4 dφgitos, de
 manera que si lo introducimos 3 veces err≤neamente se bloquea. En este caso nos pide el
 PUK 1 que consta de 8 dφgitos, si lo introducimos correctamente nos desbloquea el PIN 1.
 Si el PUK se introduce err≤neamente 10 veces, la SIM se bloquea indefinidamente. Dispone-
 mos tambiΘn de un PIN 2 para activar la la marcaci≤n fija (agenda de 10 posiciones) o
 control del coste, restricciones,etc. Si lo introducimos 3 veces mal, nos pide el PUK 2
 que consta de 8 dφgitos, el cual, una vez introducido correctamente, nos desbloquea el
 PIN2. Si el PUK 2 se introduce err≤neamente 10 veces, el PIN 2 se bloquea indefinidamente.
 La podemos encontrar en dos tama±os: ISO, grande o Plug-in peque±a
...
Hace  tiempo  tenia las  ganas de hacer un articulo en el cual, se ense±ara a
...
-> write
   ...
   Donde      :
                ...
                # buffer : es la direccion de un espacio reservado ke tine datos
                          ke se kieren enviar.
                # long_buffer: es el tama±o del buffer.
...
un telΘfono, un m≤dem y una computadora para muchos fines diferentes: Divertirnos,
informarnos, estudiar, cocinar, planificar viajes y otras actividades que amplφan
nuestro quehacer diario. Para Kevin Mitnick el quehacer diario en sus ·ltimos diez
a±os fue el explorar y ôexplotarö computadoras ajenas y sistemas telef≤nicos. ┐Su 
profesi≤n? ôHackerö de nacimiento.

Este ôterrorista electr≤nicoö, como lo cataloga el Departamento de Justicia de los
Estados Unidos, conocido en los medios como el ôC≤ndorö, fue capaz de crear n·meros
telef≤nicos imposibles de facturar (para la compa±φa telef≤nica Θl era James Bond,
con un n·mero que terminaba en 007), pudo apropiarse de 20.000 n·meros de tarjetas
de crΘdito de habitantes de California y burlarse del FBI por mßs de dos a±os con 
s≤lo un telΘfono celular alterado y un computador portßtil.

Es la peor pesadilla de las empresas de telefonφa. Es sospechoso de robar el software
de mßs de media docena de fabricantes de telΘfonos celulares y tenφa el control de 
tres oficinas centrales de telΘfonos en Manhattan y de todos los centros de conmutaci≤n
de California, dßndole la habilidad de escuchar cualquier conversaci≤n telef≤nica o, 
si no eras una persona de su agrado, modificar el telΘfono de tu casa de tal manera 
que, cada vez que levantaras el auricular, una grabadora pedφa que depositaras 25 
centavos. 

┐C≤mo se forma un Hacker?

Como Hacker, la carrera de Mitnick tiene sus inicios en 1980 cuando apenas contaba 
16 a±os y, obsesionado por las redes de computadoras, rompi≤ la seguridad del 
sistema administrativo de su colegio, pero no para alterar sus notas, lo hizo 
ôsolo para mirarö. Su bautizo como infractor de la ley fue en 1981. Junto a dos 
amigos entr≤ fφsicamente a las oficinas de COSMOS de Pacific Bell. COSMOS 
(Computer System for Mainframe Operations) era una base de datos utilizada por la 
mayor parte de las compa±φas telef≤nicas norteamericanas para controlar el registro
de llamadas. Una vez dentro de las oficinas obtuvieron la lista de claves de seguridad,
la combinaci≤n de las puertas de acceso de varias sucursales y manuales del sistema
COSMOS. La informaci≤n robada tenφa un valor equivalente a los 200 mil d≤lares. Fueron
delatados por la novia de uno de los amigos y debido a su minorφa de edad una Corte
Juvenil lo sentenci≤ a tres meses de cßrcel y a un a±o bajo libertad condicional.

Luego de cumplido el perφodo de tres meses el oficial custodio encargado de su caso
encontr≤ que su telΘfono fue desconectado y que en la compa±φa telef≤nica no habφa 
ning·n registro de Θl.

Sus objetivos iban creciendo a cada paso y en 1982 entr≤ ilegalmente, via m≤dem, a
la computadora del North American Air Defense Command en Colorado. Antes de entrar
alter≤ el programa encargado de rastrear la procedencia de las llamadas y desvi≤ el
rastro de su llamada a otro lugar. El FBI, creyendo que habφa hallado a Mitnick, 
allan≤ la casa de unos inmigrantes que estaban viendo televisi≤n.

Un a±o mßs tarde fue arrestado de nuevo cuando era estudiante de la Universidad del 
Sur de California. En esta ocasi≤n entr≤ ilegalmente a ARPAnet (la predecesora de 
Internet) y trat≤ de accesar a la computadora del Pentßgono. Lo sentenciaron a seis
meses de cßrcel en una prisi≤n juvenil en California.

En 1987, luego de tratar de poner su vida en orden, cay≤ ante la tentaci≤n y fue 
acusado, en Santa Cruz California, de invadir el sistema de la compa±φa Microcorp 
Systems. Lo sentenciaron a tres a±os de libertad condicional y luego de la sentencia
su expediente desapareci≤ de la computadora de la policia local.

Luego busc≤ trabajo en lo que mejor sabφa hacer y solicit≤ empleo en el Security 
Pacific Bank como encargado de la seguridad de la red del banco. El banco lo rechaz≤
por sus antecedentes penales y Mitnick falsific≤ un balance general del banco donde 
se mostraban pΘrdidas por 400 millones de d≤lares y trat≤ de enviarlo por la red. 
Afortunadamente el administrador de la red detuvo el balance antes de que viera la luz.

Ese mismo a±o inici≤ el escandalo que lo lanz≤ a la fama. Durante meses observ≤ 
secretamente el correo electr≤nico de los miembros del departamento de seguridad
de MCI Communications y Digital Equipment Corporation para conocer c≤mo estaban 
protegidos las computadoras y el sistema telef≤nico de ambas compa±φas.

Luego de recoger suficiente informaci≤n se apoder≤ de 16 c≤digos de seguridad de 
...
querφan obtener una copia del prototipo del nuevo sistema operativo de seguridad
...
Mitnick fue un mal c≤mplice y, a pesar de que habφan trabajado juntos, trat≤ de 
echarle toda la culpa a DiCicco haciendo llamadas an≤nimas al jefe de Θste que 
trabajaba en una compa±φa de software como tΘcnico de soporte. Lleno de rabia y 
frustraci≤n DiCicco le confes≤ todo a su jefe que los denunci≤ a Digital y al FBI.

Mitnick fue arrestado en 1988 por invadir el sistema de Digital Equipment. La 
empresa acus≤ a Mitnick y a DiCicco ante un juez federal de causarles da±os por
4 millones de d≤lares en el robo de su sistema operativo. Fue declarado culpable
de un cargo de fraude en computadoras y de uno por posesi≤n ilegal de c≤digos de
acceso de larga distancia.

Adicional a la sentencia el fiscal obtuvo una orden de la corte que prohibφa a 
Mitnick el uso del telΘfono en la prisi≤n alegando que el prisionero podrφa obtener
acceso a las computadoras a travΘs de cualquier telΘfono. A petici≤n de Mitnick el 
juez lo autoriz≤ a llamar ·nicamente a su abogado, a su esposa, a su madre y a su 
abuela y s≤lo bajo supervisi≤n de un oficial de la prisi≤n.

Este caso produjo revuelo en los Estados Unidos, no s≤lo por el hecho delictivo 
sino por la tßctica que utiliz≤ la defensa. Su abogado convenci≤ al juez que 
Mitnick sufrφa  de una adicci≤n por las computadoras equivalente a la de un 
drogadicto, un alcoh≤lico o un apostador. Gracias a esta maniobra de la defensa
Mitnick fue sentenciado a s≤lo un  a±o de prisi≤n y al salir de allφ debφa 
seguir un programa de seis meses para tratar su ôadicci≤n a las computadorasö.
Durante su tratamiento le fue prohibido tocar una computadora o un m≤dem y 
lleg≤ a perder mßs de 45 kilos.

Para 1991 ya era el Hacker que habφa ocupado la primera plana del New York Times 
...
Internet podφa ver su correo electr≤nico.

En 1992, y luego de concluir su programa, Mitnick comenz≤ a trabajar en una agencia 
de detectives. Pronto se descubri≤ un manejo ilegal en el uso de la base de datos y
fue objeto de una investigaci≤n por parte del FBI quien determin≤ que habφa violado
los tΘrminos de su libertad condicional. Allanaron su casa pero habφa desaparecido 
sin dejar rastro alguno. Ahora Mitnick se habφa convertido en un Hacker pr≤fugo.

El fiscal no estaba tan equivocado cuando pidi≤ la restricci≤n del uso del telΘfono. 
TambiΘn en 1992, el Departamento de Vehφculos de California ofreci≤ una recompensa de
1 mill≤n de d≤lares a quien arrestara a Mitnick por haber tratado de obtener una 
licencia de conducir de manera fraudulenta, utilizando un c≤digo de acceso y enviando
sus datos vφa fax.

El Fφn

Luego de convertirse en pr≤fugo de la justicia cambi≤ de tßctica y concluy≤ que la 
mejor manera de no ser rastreado era utilizando telΘfonos celulares. De esta manera
podrφa cometer sus fechorφas y no estar atado a ning·n lugar fijo. Para ello 
necesitaba obtener programas que le permitieran moverse con la misma facilidad con 
que lo hacφa en la red telef≤nica.

Luego de varios intentos infructuosos, en cuanto a calidad de informaci≤n, se 
encontr≤ con la computadora de Tsutomu Shimomura la cual invadi≤ en la Navidad
de 1994. Shimomura, fφsico computista y experto en sistemas de seguridad del 
San Diego Supercomputer Center, era ademßs un muy buen Hacker, pero era de los
ôchicos buenosö, ya que cuando hallaba una falla de seguridad en alg·n sistema
lo reportaba a las autoridades, no a otros Hackers.

Shimomura not≤ que alguien habφa invadido su computadora en su ausencia, utilizando
un mΘtodo de intrusi≤n muy sofisticado y que Θl nunca antes habφa visto. El intruso
le habφa robado su correo electr≤nico, software para el control de telΘfonos celulares
y varias herramientas de seguridad en Internet. Allφ comenz≤ la cuenta regresiva para
Mitnick. Shimomura se propuso como orgullo personal atrapar al Hacker que habφa 
invadido su privacidad.

Hacia finales de enero de 1995, el software de Shimomura fue hallado en una 
cuenta en The Well, un proveedor de Internet en California. Mitnick habφa 
creado una cuenta fantasma en ese proveedor y desde allφ utilizaba las 
...
Shimomura se reuni≤ con el gerente de The Well y con un tΘcnico de Sprint 
(proveedor de servicios telef≤nicos celulares) y descubrieron que Mitnick 
habφa creado un n·mero celular fantasma para accesar el sistema. Luego de
dos semanas de rastreos determinaron que las llamadas provenφan de Raleigh,
 California. 

Al llegar Shimomura a Raleigh recibi≤ una llamada del experto en seguridad 
de InterNex, otro proveedor de Internet en California. Mitnick habφa invadido 
otra vez el sistema de InterNex, habφa creado una cuenta de nombre Nancy, 
borrado una con el nombre Bob y habφa cambiado varias claves de seguridad
incluyendo la  del experto y la del gerente del sistema que posee los 
privilegios mßs altos. De igual manera Shimomura tenφa informaci≤n sobre
la invasi≤n de Mitcnick a Netcom, una red de base de datos de noticias.

Shimomura se comunic≤ con el FBI y Θstos enviaron a un grupo de rastreo
por radio. El equipo de rastreo poseφa un simulador de celda, un equipo
normalmente utilizado para probar telΘfonos celulares pero modificado para
rastrear el telefono de Mitnick mientras este estß encendido y aunque no 
estΘ en uso. Con este aparato el celular se convertirφa en un transmisor
sin que el usuario lo supiera.

A medianoche terminaron de colocar los equipos en una Van y comenz≤ la 
b·squeda de la se±al, porque eso era lo que querφan localizar; no buscaban
a un hombre porque todas  as fotos que tenφan eran viejas y no estaban 
seguros de su aspecto actual, el objetivo de esa noche era determinar el 
lugar de procedencia de la se±al. Ya para la madrugada localizaron la se±al 
en un grupo de apartamentos pero no pudieron determinar en cußl debido a
interferencias en la se±al.

Mientras esto ocurrφa la gente de InterNex, The Well y Netcom estaban 
preocupados por los movimientos que casi simultßneamente Mitnick hacφa
en cada uno de estos sistemas. Cambiaba claves de acceso que Θl mismo
habφa creado y que tenφan menos de 12 horas de creadas, utilizando c≤digos
extra±os e ir≤nicos como no,panix,  fukhood y fuckjkt. Estaba creando 
nuevas cuentas con mayores niveles de seguridad como si sospechara que
lo estaban vigilando.

El FBI, Shimomura y el equipo de Sprint se habφan reunido para planificar la 
captura. Shimomura envi≤ un mensaje codificado al buscapersonas del encargado
en Netcom para advertirle que el arresto se iba a realizar al dφa siguiente, 
16 de Febrero. Shimomura envi≤ el mensaje varias veces por equivocaci≤n y el
encargado interpret≤ que Mitnick ya habφa sido arrestado adelantßndose a 
realizar una copia de respaldo de todo el material que Mitnick habφa almacenado
en Netcom como evidencia y borrando las versiones almacenadas por Mitnick. 
Habφa que realizar el arresto de inmediato, antes de que Mitnick se diera 
cuenta de que su informaci≤n habφa sido borrada.

Cuando faltaban minutos para dar la orden el simulador de celdas detect≤ una 
nueva se±al de transmisi≤n de datos vφa celular y simultßnea a la de Mitnick,
muy cerca de esa zona. Algo extra±o estaba haciendo Mitnick con las lφneas 
celulares, Shimomura trat≤ de advertirle al agente del FBI pero ya todo estaba
en manos de ellos, Shimomura de ahora en adelante no era mßs que un espectador 
privilegiado. El FBI no pensaban hacer una entrada violenta porque no creφan que
Mitnick estuviera armado, pero tenφan que actuar muy rßpido porque sabφan el da±o
que este hombre podφa causar en un solo minuto con una computadora. Se acercaron
lentamente hasta la entrada del apartamento de Mitnick y anunciaron su presencia,
si no les abrφan la puerta en cinco segundos la echarφan abajo. Mitnick abri≤ 
la puerta con toda calma y el FBI procedi≤ a arrestarlo y a decomisar todo el 
material pertinente discos, computador, telΘfonos celulares, manuales, etc.

De regreso a su hotel Shimomura decide chequear la contestadora telef≤nica 
de su residencia en San Diego. Se qued≤ en una pieza cuando escucho la voz 
de Mitnick quien le habφa dejado varios mensajes con acento oriental en tono
de burla. El ·ltimo de estos mensajes lo habφa recibido ocho horas despuΘs 
de que Mitnick habφa sido arrestado y antes de que la prensa se hubiera 
enterado de todo el asunto. C≤mo se realiz≤ esa llamada a·n es un misterio
al igual que el origen y objetivo de la segunda se±al de Mitnick.

Este persistente hacker actualmente estß siendo juzgado y enfrenta dos cargos
federales, uso ilegal de equipos de acceso telef≤nico y fraude por computadoras.
Puede ser condenado por hasta 35 a±os y a pagar una multa de hasta medio mill≤n
de d≤lares. Mitnick tambiΘn es sospechoso de robar el software que las compa±φas
telef≤nicas piensan usar para todo tipo de procesos, desde la facturaci≤n hasta 
el seguimiento del origen de una llamada pasando por la decodificaci≤n de las 
se±ales de los telΘfonos celulares para preservar su privacidad.

El vuelo final 
       

Todos los cargos bajo los cuales
ha sido acusado Kevin Mitnick podrφan 
suponerle mßs de doscientos a±os
de prisi≤n si es declarado culpable 
       
"No culpable" alcanz≤ a declararse Kevin Mitnick ante el Gran Jurado de 
California, que el pasado 30 de septiembre lo acus≤ de 25 cargos por los
cuales podrφa ser condenado a mßs de doscientos a±os de presidio. 
ôEsta acusaci≤n revela el amplio da±o que Mitnick caus≤ mientras era un 
fugitivo de la justicia. Los delitos por computadora permiten a sofisticados
criminales causar estragos alrededor del mundo usando s≤lo una computadora y
un modem como sus armas. Queremos con esta acusaci≤n dar un paso adelante en
los esfuerzos federales por perseguir y capturar a los hackersö dijo la fiscal
encargada del caso, Nora Manella. Los cargos por los que fue acusado Mitnick y
su ayudante Lewis Depayne, de 36 a±os, incluyen el robo de software, fraude 
electr≤nico, da±o a las computadoras de la Universidad del Sur de California, 
robo de archivos electr≤nicos e intercepci≤n de mensajes de correo electr≤nico.
Entre las compa±φas afectadas por las actividades del llamado ôCondorö se 
cuentan Motorola, Nokia, Fujitsu y Nec. Se supone que los da±os causados por 
Mitnick en los dos a±os y medio durante los cuales fue un fugitivo suman millones
de d≤lares, especialmente por el software robado y las inversiones que debieron
realizar las empresas para proteger sus sistemas. El asistente del fiscal David
Schindler dijo que Mitnick -quien actualmente tiene 33 a±os- serφa probablemente
sentenciado a ômuchos a±osö si es encontrado culpable, negßndose, sin embargo, a
ser mßs especφfico, bajo el argumento de que se trata de un ßrea legal muy nueva.
Todos los cargos bajo los cuales ha sido acusado Mitnick podrφan suponerle mßs de
doscientos a±os de prisi≤n si es declarado culpable de todos ellos. 
Ya en abril de este a±o el famoso hacker habφa sido declarado culpable por un 
jurado de Carolina del Norte por el uso del quince n·meros de telΘfonos celulares
robados para llamar a bases de datos electr≤nicas. Igualmente se le conden≤ por
haber violado el regimen de libertad condicional al que estaba sometido luego de
ser encontrado culpable de penetrar ilegalmente en sistemas de informaci≤n de 
corporaciones de informßtica. 
  
Mitnick: hacker, cracker y phone phreaker 
  
La definici≤n de un cracker es alguien que trata de violar el acceso a un 
sistema adquiriendo passwords. La mayorφa de los crackers son adolescentes
nada bondadosos y que buscan dar sus golpes destruyendo o alterando la data
de un sistema. Tienden a unirse en grupos muy peque±os, secretos y cerrados
al contrario de los inmensos, abierto y policulturales hackers. Se espera 
que un verdadero hacker haga algo de crackin juguet≤n y conozca muchas de las
tΘcnicas bßsicas, pero cualquiera que pase de la etapa de larva puede caer en
la tentaci≤n y, debido a su creciente deseo de realizar algo por razones 
inmediatas, benignas y prßcticas, no vea nada de malo en invadir cierta 
seguridad y privacidad para poder lograr una meta. Para el cracker el invadir
un sistema no requiere de misteriosos estados de iluminaci≤n mental, pero sφ
mucha persistencia y la testaruda repetici≤n de trucos bien conocidos en los 
puntos dΘbiles de un sistema, tratan de descubrir informaci≤n clasificada 
hurgando al azar y con ciega persistencia. Suele decirse que los crackers 
son s≤lo hackers mediocres y que su nivel de educaci≤n e inteligencia sobre
un sistema es menor. 
Los phone phreaker son los mßs famosos en los medios de comunicaci≤n por 
los desastres que han hecho a travΘs de los a±os. En los a±os 60 ya existφan
los Phone Phreaks y la gran victima era AT&T. Uno de los mßs famosos Phone 
Phreaks de esa epoca era John Draper, alias Captain Crunch 
(http://www.fc.net/phrack.html). El descubri≤ que modificando una caja de 
...
gratis. Hace alg·n tiempo el hacer phreaking fue una actividad semi-respetable
dentro de la comunidad hacker; habφa un acuerdo de caballeros donde el hacer
phreaking era bien visto como juego intelectual y como una forma de exploraci≤n,
pero el robo de servicios era tab·. La modernizaci≤n de las redes hizo necesario
que los phreakers utilizaran tΘcnicas menos Θticas, como robar n·meros de calling
cards: los obtenian colocßndose cerca de alg·n telΘfono p·blico y memorizando el
n·mero de tarjeta que marcaba un usuario descuidado, una vez obtenido el n·mero y
la clave la informaci≤n era esparcida de tal manera que en un caso se llegaron a
...
Como decφa en mi pßgina, este texto lo he conseguido de la pßgina de ByronHack,
la cual os recomiendo. En la mφa teneis su direcci≤n. Bueno, a lo que iba; a mφ
no me parece nada justo el trato que le han hecho al se±or Mitinick. ╔l es un 
genio como pocos los hay. Tuvo sus aventuras y desventuras, hizo cosas malas, ya
se sabe, pero nadie cuenta con las buenas. Nadie se da cuenta de que gracias a Θl
los sistemas de seguridad ahora son mßs complejos y currados. Nadie se ha dado 
cuenta del gran logro que consigui≤: sobrevivir durante dos a±os con un m≤vil y
un PC. Eso nadie lo valora, el hecho de que un s≤lo hombre en su gran cruzada logr≤
despistar, atormentar y acojonar a la seguridad informßtica de todo un pais. El FBI
lo persigui≤ como a un perro y el Gran Mitnick hizo con ellos lo que quiso.Pero ya 
...
Yo no soy de los que piden su libertad, ya que, seg·n dicen, rob≤ gran cantidad de 
...
suyo y eso jamßs se me ocurrirφa. Y lo segundo es la gran cruzada que ha montado y en 
