...
|-----|---------------------------------------------|----------------|
|  18 | Sitios de InterΘs                           | NovaHack Zine  |
|-----|---------------------------------------------|----------------|
...
Esta expresi≤n es utilizada para ejecutar una sentencia en el caso que una condici≤n establecida sea verdadera, o de lo ocntrario se podra ejecutar una sentencia distinta. Su sintaxis es:
...
Si la condicion se cumple se realizarßn las instrcciones despues del THEN, si la condici≤n no se cumple realizara las instrucciones despues del ELSE. Si despues del THEN o ELSE se escribe mßs de una instruccion se debera comenzar con un BEGIN y terminar con un END;. 
...
El ciclo FOR repite una sentencia un determinado n·mero de veces que se indica al momento de llamar el ciclo.
...
Donde identificador es la variable que se incrementarß, inicio es el primer valor que tendrß dicha variable y fin es el valor hasta el cual se incrementarß la misma.
...
Es posible hacer que un ciclo cuente hacia atrßs, es decir, que la variable en vez de incrementaese se decrementa. Para esto cambiamos la palabra TO por DOWNTO, y colocamos el valor mayor a la izquierda y el menor a la derecha. Ejemplo:
...
Los ciclos while ofrecen la ventaja de que la ejecucion se realiza mientras se cumpla una condici≤n, por lo tanto es posible controlar el n·mero de repeticiones una vez iniciado el ciclo. Su sintaxis es:

WHILE &lt;condicion> DO
	instrucci≤n;

Donde condici≤n es la condicion que se evaluara si es verdadera entra al ciclo, si no se sale o no entra. Si se utiliza mas de una instruccion se pondran las sentencias Begin y End como en el ciclo FOR:
...
Este tipo de ciclos es muy parecido a los ciclo While, la diferencia entre ambos es que en WHILE la condici≤n se eval·a al principio del ciclo, en cambio en REPEAT-UNTIL se evelua al final, lo que significa que en un ciclo REPEAT-UNTIL la sentencia se ejecutarß por lo menos una vez, cosa que no puede ocurrir en el ciclo WHILE. Ejemplo:
...
Esta forma de control se utiliza cuando se va a evaluar una expresi≤n que puede contener varios datos diferentes y en cada dato deberß realizar una acci≤n especial. Por ejemplo, si se crea un men· con diferentes opciones se realizarß un determinado proceso para cada acci≤n, aunque la selecci≤n por parte del usuario se haga desde un mismo lugar. Sintaxis:
...
Vamos a ver si os habiais estudiado el capi¡tulo 1. ┐De quΘ os suena eso de segmento? .................................
...
NOTA: Despues de PROC le podeis a±adir FAR o NEAR. Ya veremos la diferencia mas adelante. De momento si no se pone nada se toma como FAR. FAR y NEAR son directivas tambien.
...
Veremos mas sobre las etiquetas (que son etiquetas NEAR o FAR) mas adelante.
	...
	NOTA: En los ejemplos los modos de direccionamiento se apreciaran en el segundo operando, el primero sera siempre un registro.
		...
		- Direccionamiento inmediato: 
			...
			El operando es un registro. Por cierto, el operando fuente y el destino deben ser de igual tama±oo.
           ...
           ej. : MOV   ax,[57D1h]  --> mover a ax el valor almacenado en la dir. [57D1h] (offset de DS, en hexadecimal)
			...
			El operando se encuentra en una direccion se±alada por un registro de segmento*16	mas un registro base (BX/BP) o I¡ndice (SI/DI). (Nota: BP actua por defecto con SS).
		 ...
		 - Indirecto con Indice o indexado: 
			El operando se encuentra en una direcci├│n determinada por la suma de un 
registro de segmento*16, un registro de ├¡ndice, SI o DI y un desplazamiento de 8 ├│ 16 bits. 
  ...
  1.2 - Directivas
	...
	Directivas hay muchas pero las que vamos a ver ahora son las de definicion de variables. Asi¡ entendereis lo que va dentro del DATOS SEGMENT ese. Podeis poner dos cosas:
		...
		- Definicion de datos.
			Para definir variables. Hay 5 posibles segun el tama├▒o del tipo de datos que definamos:
...
Bueno para los sedientos de aprender mas os voy a pasar una direccion donde viene TODO sobre el ensamblador del 80x86 y ademas en espa±ol. Eso si¡, es bastante denso y requiere de algunas nociones en programacion y arquitectura. De todas formas aqui¡ lo teneis --> http://atc.ugr.es/docencia/udigital/
...
============================================================================================
[4]--------------------------INTRODUCCI╙N A MICROCONTROLADORES---------------------------[4]
============================================================================================
...
+-----------------------------+
 ...
 No podemos olvidar que la INTEL fue la compa±φa de que desarroll≤ el primer microprocesador en el mundo, el 4004 en el a±o 1971. Este microprocesador, con s≤lo 2300 transistores, fue el origen de una gran cantidad de dispositivos que se desarrollaron posteriormente, los cuales integran actualmente mßs de un mill≤n de transistores en un encapsulado. Tampoco podemos olvidar del hecho que INTEL ha desarrollado microprocesadores tan exitosos como el 8086, 80286, 80386 y 80486 y mßs recientemente la lφnea PENTIUM. 
 
Pero, quien dijo que se quedarφa durmiendo en los laureles en materia de microcontroladores (nuestro caso de estudio), pues no, INTEL  ha fabricado microcontroladores de 8 y 16 bits, cada uno de ellos tiene amplias posibilidades de uso y prestaciones que los hacen adecuados para gran cantidad de aplicaciones. Los microcontroladores INTEL han tenido gran aceptaci≤n, hasta el punto que otros fabricantes han optado por producir sus propias versiones de estos dispositivos, como el caso de SIEMENS y PHILIPS.

Introducci≤n

 En ente mundo del hacking y de la internet, se habla de microprocesadores y computadores a gran escala, pero tambiΘn existen otros dispositivos ( si estßs en la universidad estudiando alguna carrera ligada a la electr≤nica o eres curioso) habrßs oφdo acerca de MICROCONTROLADORES; si no es asφ, no importa ( jejeje..) acß comenzaremos alguillo respecto al tema. J

Para darle un toque mßs formal al tema, comenzaremos diciendo que hoy en dφa, el incremento competitivo en el mercado de la industria electr≤nica crea la necesidad de dise±ar sistemas con mejores caracterφsticas, de menor tama±o, bajos requerimientos de energφa, mejor realizaci≤n, teniendo un especial Θnfasis sobre todo en la facilidad de duplicidad del sistema dise±ado. La l≤gica definida por el usuario y la realizada por el fabricante permite individualizar a los sistemas dise±ados, asφ como tambiΘn apegarse mßs a los requerimientos especφficos del usuario.

Ya tanta palabrerφa nos cansa no?, jeje....

Bueno, entonces ┐QuΘ es un microcontrolador?

Un microcontrolador es todo un "sistema mφnimo" dentro de un solo dispositivo, lo cual ofrece un enorme panorama hacia el mundo de la compatibilidad. Este dispositivo contiene:
un CPU (basado principalmente en un microprocesador de 4, 8 ≤ 16 bits), puertos paralelos de entrada y salida, puerto serie, timers, contadores, memorias, y en algunos casos hasta convertidores anal≤gicos digitales, todo eso dentro de un solo chip.

Pero ┐Por quΘ un microcontrolador y no un microprocesador?

 Pues bien, digamos que el microcontrolador estß encaminado bßsicamente hacia aplicaciones concretas en donde el espacio, y n·mero de componentes es mφnimo, ademßs, los cambios o ampliaciones futuras del sistema son casi nulos. Por otro lado, un microprocesador se destina a sistemas donde su expansi≤n a corto o mediano plazo es factible. A pesar de que un microprocesador es mßs rßpido que un microcontrolador para la ejecuci≤n de sus instrucciones, en la mayorφa de los casos es necesario interconectarlo con dispositivos perifΘricos.

  Ademßs, un microcontrolador puede ser utilizado con un mφnimo n·mero de componentes en trabajos especφficos y en un amplio rango de aplicaciones, tales como: sistemas de control de alarmas, tableros de control en industria automotriz, en la instrumentaci≤n mΘdica, teclados de computadora, sistemas portßtiles de almacenamiento de datos, equipos de laboratorio, etc.

 Caracterφsticas de los microcontroladores 8X51
...
╖	32 lφneas de entrada/salida, direccionables bit a bit , (cuatro puertos), muy importante ;-)
╖	Comunicaci≤n asφncrona Full Duplex
╖	5 fuentes de interrupci≤n
╖	Oscilador interno.

Descripci≤n de las lφneas (Pines) del 8051.

Acß te presento el microcontrolador 89C51/52:

(ver la micro89C52.bmp que se encuentra dentro del zip)


Descrici≤n de las conexiones:

+----------------------------------------------------------------------------------------+
| Nem≤nico     | No. de Pin   |  Tipo    |    Funci≤n                                    |
|----------------------------------------------------------------------------------------|
...
|              |              |          | impedancia. El puerto 0 es tambiΘn multi-     |
...
|              |              |          | Cuando se escriben 1's, las lφneas pueden ser |
|              |              |          | utilizadas como entradas o salidas. Como en-  |
|              |              |          | tradas las lφneas que son externamente        |
...
|              |              |          | se±ales de control de dispositivos externos   |
|              |              |          | como los siguientes:                          |
|              |              |          |                                               |
...
Bueno escribo este texto por que es un tema que a mφ en lo particular fue una de las primeras cosas que me llamo la atenci≤n y trate de hacerlo basßndome en algunos textos que estaban un poco incompletos pero gracias a ellos y algunos otros mas me eh dado una idea para clonar un celular, todo lo que aquφ expongo son cosas que e aprendido con el tiempo y al igual que todo ser humano puedo tener errores, pero bueno ahora si pasemos a lo que nos interesa.

+-----------+
| CLONACI╙N |
+-----------+

En este primer articulo de Clonaci≤n de celulares voy a explicarles como se clona un Nokia 51XX/61XX, en posteriores artφculos manejaremos Motorola, Ericsson o alg·n otro pero bueno va la explicaci≤n de como hacerlo con los Nokia ya mencionados:
...
-Luego marcar *#639#
-Nod pide un numero asφ que hay que ponerlo [este es el MIN que esta en el ESN]
-Luego nos pide el Sys ID [mas abajo pongo los SIDs de MΘxico]

Aquφ estßn los SIDs de MΘxico:


24578                   Baja California Sur Baja California Norte
24594                   Sinaloa Sonora
24582                   Chihuahua Durango
24581                   Nuevo Le≤n Tamaulipas Coahuila
24586                   Jalisco Colima y Michoacßn
24588                   Guanajuato  Aguas calientes Zacatecas QuerΘtaro San Luis Potosφ
24590                   Tlaxcala Guerrero Oaxaca Veracruz Puebla
24592                   Chiapas Tabasco Campeche Yucatßn Quintana Roo 
24580                   DF  Edo de MΘxico  Morelos Hidalgo

Como verßn no es muy difφcil de programar un Nokia 51XX/61XX pero esto es lo mas fßcil, para 
poder clonar un celular aparte de esto se le necesita cambiar el ESN del celular ya que cuando encendemos el celular este manda la se±al a la centralita si los datos que tiene: ESN y MIN son correctos nos permitirß hacer una llamada o acceder al sistema de lo contrario cuando intentemos hacer algo va a sonar tuu... tuu... tuu... bueno y para poder cambiarle el ESN se necesitan dos cosas. 1. un cable para poder conectar el celular a la PC y 2. un programa para poder cambiarle el ESN.

Aquφ les voy a poner los esquemas para construir un cable M2BUS para un celular 51/XX y 61XX
...
Por ultimo hace falta el programa ahora aquφ les va la direcci≤n de donde esta el programa 
http://usuarios.lycos.es/forostmu/nokia.zip

Bueno el programa que les he puesto es el Easy IMEI v1.01 el cual es muy fßcil de usar solo se necesita seleccionar el puerto [COM 1, COM 2, COM 3 o COM4], luego el modelo del celular [51XX o 61XX] luego el tipo de cable [FBUS, M2BUS o Auto Detect] por ultimo poner el nuevo ESN o IMEI y darle en Write to phone y listo el celular ahora si esta listo.



Bueno ahora por ultimo una breve explicaci≤n de lo que se debe de hacer, primero se necesita
tener toda la info del celular que se quiera clonar para esto basta con pedirle el celular 
a la victima con el pretexto de que quieres jugar tantito y sacarle la informaci≤n necesaria, ahora ┐que informaci≤n es la necesaria y como sacarla? 

1. La informaci≤n necesaria es el MIN o numero de telΘfono [NOTA: no es el que tu marcas para que no se te ocurra poner el que tu marcas]

2. Bueno para conseguir esta info se hace algo muy sencillo *3001#12345# y entraras otro men· aquφ encontraras los NAMs y Serial No. que son los que nos interesan, vienen mas pero de nada nos sirven, ahora lo que hacemos es darle en el NAM 1 [que es el que normalmente se usa] y debe de mostrarte un numero de 10 dφgitos, ese es el que debes apuntar luego te sales de ese men· y te vas a Serial No. que es el numero de serie de celular [Este es el ESN] ahora lo anotas y listo devuelves el celular [Por cierto que no te pase lo que a mφ no dejes que nadie te vea lo que estas haciendo y si lo logras hacer no te pongas a presumir por ahφ o te pueden ocurrir dos cosas una que se cachen y dos que nunca nadie te vuelva a prestar su celular].

Ya con los datos solo necesitamos que hacer lo que antes se habφa explicado programar el celular y cambiarle el ESN por los que sacamos.
...
*#639# y tambiΘn se puede meter los datos directamente dando *3001#12345#, esto lo aclaro 
por si a uno se le hace mas fßcil o mejor programarlo por ahφ pues bien, da la misma.
...
NETSTAT posibles argumentos > [-a] [-e] [-n] [-s] [-p proto] [-r] [!intervalo]
  ...
  -e            Muestra estadφsticas de la conexion Ethernet. Se puede combinar
                con el argumento -s.

  -n            Muestra n·meros de puertos y direcciones en formato numΘrico
  ...
  -p !proto     Muestra conexiones del protocolo especificado por !proto; que
                puede ser tcp o udp. Si se usa con la opci≤n -s para mostrar
                estadφsticas por protocolo, proto puede ser TCP, UDP o IP.

  -r            Muestra el contenido de la tabla de rutas [muy interesante].

  -s            Muestra estadφsticas por protocolo. En forma predeterminada, se
                muestran para TCP, UDP e IP; se puede utilizar la opci≤n -p
                para especificar un subconjunto de lo predeterminado.

  !intervalo    Vuelve a mostrar las estadφsticas seleccionadas, haciendo
                pausas en el intervalo de segundos especificado entre cada
                muestra. Presione Ctrl+C [como en linux :D] para detener la
                actualizaci≤n de estadφsticas. Si se omite, netstat imprimirß
                la actual informaci≤n de configuraci≤n una vez.
...
Ok gente, me despido de todos y ya saben si tienen alguna duda o comentario escribanme a daru_rules@hotmail.com , tambien nos les haria da±o que visitaran mi pagina www.darux.cjb.net.
...
Puerto inicial   Puerto final     <------label1 y label2
 _______           _________
ª_______ª         ª_________ª     <--------text1 y text2

         _________
        ª Iniciar ª               <-------------Command1
         ---------+

   ___________________________
  ªEscaneando:                ª   <---------------Label3
   ---------------------------+
...
Esto es fßcil de hacer solo hay que tener algunos conocimientos de como soldar circuitos y esas cosas.
...
- Dos conectores DB25 con tapa (en caso de que la conexi≤n sea por el paralelo).
- Dos conectores DB9 con tapa (en caso de que la conexion sea por el serial).
- 2 mts o mßs de cable de 12 hilos (No blindado).
- Esta±o.
- Soldador.
...
Para el DB25 o cable paralelo, los cables deben de ir como se muestra a continuaci≤n, ademßs de esto le he dado a cada conexi≤n el color de un cable para que no te pierdas ;-)
...
Despues se inicia la conexion entre las dos mßquinas y woala, ya tienes una red entre dos maquinas ;-)
...
Tenemos al fancooler  solo necesitas un desarmador peke±o para desarmarlo un un poco de aceite
...
Parte I : Caracterφsticas de Los tipos de Seguridad
------------------------------------------------------
...
Debido a que es muy difφcil encontrar paginas en Internet con sistemas de seguridad ya que estas paginas no son abiertas a todos los usuarios, tendremos que enfocarnos en las paginas mas comunes que tienen sistemas de seguridad, las paginas para adultos, sin embargo lo que se aprenderß aquφ puede ser usado con cualquier tipo de pagina que posea un sistema de seguridad.

Los tipos de seguridad mßs comunes son los siguientes:
  *Basic http Auth
...
El HTML Form based password esta basado en formas html, utiliza el mΘtodo get o post para obtener dos cadenas de una forma html, una cadena es el username y la otra es el password, Este sistema de seguridad es el mas complejo de crackear.
Ejemplo :

Username : Marco
Password : ********

Este tipo de password es complejo por que no se conoce el destino o servidor donde se encuentra el gui≤n que procesa la forma, ademßs para crackear unos de estos passwords se necesita mucho tiempo ya que los ataques son muy lentos, sin embargo tambiΘn voy a explicar como se crackean estos sitios :) .
...
Volviendo a los AVS, estos poseen un servidor central lo cual los hace muy vulnerables por ejemplo el Adultcheck , su servidor central es http://www.adultcheck.com/ y todos los ID que estΘn en la base de datos de este servidor serßn aceptados en cualquiera de las miles de paginas que tienen a Adultcheck como sistema de seguridad, para adquirir un adult check es necesario pagar una cantidad ademßs de tener tarjeta de crΘdito para pagar, pero como a nosotros no nos gusta pagar :), vamos a crackear el sitio, esto de el servidor central nos hace todo mas fßcil ya que solo tenemos que crackear un solo sitio y tendremos acceso a miles, bueno ahora si viene lo bueno, ┐C≤mo le hago para crackear el sitio?, Que pasarφa si empiezo a mandarle una y otra solicitud de autenticidad al servidor?, si le empiezo a mandar solicitudes como : hkdhhshsdk, hkhdhds, lklklk4oiu, jkskjsd, no pasarφa nada, simplemente el servidor las descarta por no tener sentido (esto claro depende de el tipo de filtro que el
sitio tenga), el AC debe comenzar con 4 dφgitos seguidos de 6 a 12 letras, ejemplo : 4563superman, 2347batman, entonces si manda una petici≤n y luego otra y otra y todas tienen sentido, el servidor las analiza todas y compara cada ID con su base de datos, imagina esto, si compara 1 ID con toda su base de datos, que pasara si tiene que comparar 30000 IDs, simplemente no es posible para el servidor y tarde o temprano dejara pasar algunos IDs (Esto no es totalmente cierto sin enbargo es una idea para los newbies) , puedes conseguir
unos 50 ID por hora + o - en el AC normal , el servidor del AC gold tiene mas capacidades y es mucho mas difφcil crackearlo asφ es que obtendrßs unos 5 ID por hora , ya se lo que estas pensando ┐C≤mo le hago para mandar esas solicitudes?, bueno solo necesitas un programa que se conecte a http://www.adultcheck.com/ y le mande solicitudes y como le manda solicitudes? Mediante los mΘtodos http_accept , http_user y http_password , para hacer las conexiones debes dominar un lenguaje de programaci≤n y claro saber como programar para Internet, ademßs debes saber HTML (si no sabes nada de esto no te preocupes por fortuna para ti ya existen
programas que puedes conseguir en Internet :) ) ; bueno pero los AVS no se van a dejar que les ganes tan fßcil , asφ es que han desarrollado mΘtodos para protegerse de los ataques, el mas importante es que detectan tu IP y si mandas alrededor de 5 a 10 solicitudes y estas son rechazadas el Server banea tu IP y descarta todas las solicitudes que le mandes sin importar que tengan o no sentido , se pone mas difφcil , bueno entonces que es lo que tengo que hacer? , pues fßcil atinarle en las primeras 5 solicitudes , :) es broma , bueno si el AVS no se va a dejar ganar nosotros tampoco , si el AVS me banea cada 5 solicitudes entonces cada 5 solicitudes cambio de IP :) , para cambiar de IP necesito proxys , un proxy es una direcci≤n IP que act·a como intermediario en una conexi≤n , para evitar que me banee puede cambiar de proxy cada 2 o 3 solicitudes y de esta manera casi ninguno de mis proxys serß baneado , basta con 200 proxys para poder crackear algunos ID (100 o mas) , entonces ya sabemos que es lo que necesitamos : una lista con miles de IDs creados por ti , podrφa ser
1000marcos hasta 9999marcos y una lista con 200 proxys mφnimo , estos no puedes crearlos tienen que ser verdaderos ya que ellos serßn los que realicen las conexiones dejßndote an≤nimo ya que tu IP no intervendrß y no podrßn rastrearte, ahora tal vez digas ôaaaaaaaa tengo que teclear desde 1000marcos hasta 9999marcos de uno por uno?,que guevaö , bueno la verdad no , existen herramientas para generar listas :), en todo pienso, despuΘs veremos como utilizar estas herramientas , con respecto a los proxys puedes conseguirlos en
Internet o con tu proveedor mas cercano (creo que soy yo) , Este tipo de ID (4 dφgitos seguidos de 6 a 12 letras) solo sirve en los sistemas Adult Check Regular y Adult Check Gold, todos los AVS funcionan de manera similar lo ·nico que varia es el tipo de ID y por lo tanto tienes que crear diferentes listas al final anexare todos los tipos de IDs. Bueno aquφ se termina el AVS password :) .

Vamos a pasar a el Basic http password y dejemos el segundo tipo para el final .

Aquφ es lo mismo solo que el tipo de ID es diferente, se requieren mas proxys, se requiere una lista con username y passwords ejemplo : marcos:cracker, los ":" separan el user y el password , este tipo de sistema es mas difφcil que el AVS ya que no hay servidor central y es necesario crackear cada sitio , ademßs hay sistemas muy variados, algunos tienen filtros de palabras otros de velocidad otros ambos y todos banean IPs pero varia la cantidad de solicitudes que puedes hacer , algunos banean el IP si la primera solicitud es rechazada esto es un problema ya que consumirß tus proxys rßpidamente para este tipo de sistema necesitas una gran cantidad de proxys (mas de 1000) , pero son muy pocos la gran mayorφa puede ser penetrada fßcilmente, y otros pueden ser casi imposibles de crackear (sin embargo nada es imposible).

Este sistema de seguridad es una aplicaci≤n y puede procesar las peticiones desde la barra de direcciones de el explorador, un ejemplo de petici≤n es la siguiente : http://marcos:cracker@members.somesite.com/ , esta es una petici≤n para acceder a la pagina http://members.somesite.com/ con el ID user : marcos pass : cracker , asφ es que si deseas programar una aplicaci≤n deberß tomar el user y el pass de una lista y adherirlos a la direcci≤n que deseas crackear y luego mandar las solicitudes ,puedes conseguir los programas en Internet los mas usados son : el Access Diver , Goleen Eye , Ares , PassCraft y Brutus , la mayorφa de los sitios de calidad poseen este sistema de seguridad .

El HTML form based password debe ser crakeado al igual que el Basic Http password , para cada sitio , esto quiere decir que tampoco poseen un servidor central , en este tipo de sistema necesitas conocer la direcci≤n de el gui≤n que procesara la forma y la variable que identifica al username y al password , te preguntaras , ┐y como puedo saber eso? Pues algunas veces es fßcil y otras veces es muy difφcil , para conocer el gui≤n que procesara la forma y la variables debes examinar el c≤digo fuente de la pagina que deseas crackear , encontraras una etiqueta llamada #FORM>dentro de esta etiqueta deberß estar lo siguiente:
#FORM action="somesite.com/login.cgi">somesite.com/login.cgi es la direcci≤n de el gui≤n que procesa la forma de igual manera deberßs encontrar el nombre de lasvariables :

username : #INPUT type=text value=æÆ name=variableuser>
password : #INPUT type=password name=variablepassword>

Con estos tres datos puedes atacar un sitio protegido con HTML form based password , existen programas que analizan automßticamente las formas y obtienen estos 3 datos , sin embargo algunas paginas poseen trampas para ocultar el destino de la forma y el nombre de las variables , estas trampas suelen ser redirecciones , esto es que en el c≤digo fuente no se encuentra directamente la direcci≤n de el gui≤n que procesa la forma , en este caso deberßs analizar minuciosamente el c≤digo fuente y encontrar el destino de la forma , puedes
...
icarusscrack@hotmail.com para Preguntas o Comentarios. (si alguna parte de este tutorial es confusa o no entienden algunos de los tΘrminos por favor envφenme un e-mail y yo les responderΘ tratando de explicar mas claramente las dudas que se tengan ).
...
A partir de la versi≤n 5.6 los passwords estßn codificados con MD5. 
...
Una vez que se tiene ese pass en MD5 lo ·nico que hay que hacer es colocarlo en una cadena como la anterior, codificar la cadena en base64 y colocarla en una cookie en mi propia mßquia. 
...
fputs($fp, $cookie);
fclose($fp);
print "Message Not Found!"; /* Esto es s≤lo para que el admin no sospeche */
?>
(como ves debes crear tambiΘn un archivo llamado cookie.txt) 
...
?>
(despuΘs sustituiremos "cadena" por otra cosa.
...
2║ Se espera a que el admin lea el mensaje, y despues se mira el archivo cookie.txt que habφamos creado en nuestro server. 

3║ Copia la cadena que aparece en el cookie.txt entre "admin=" y ":" 

4║ Ahora hay que decodificar ese c≤digo, poruqe estß en base64, para ello puedes utilizar un programa adecuado o visitar http://www.isecurelabs.com/base64.php 

Una vez decodificado deberφas tener una cadena como Θsta: 

usuario:pass_en_MD5:lenguaje 

5║ Ahora hay que crear una cookie con lo conseguido, para ello se utiliza el test.php sustituyendo lo que tenemos por la palable "cadena", asφ: 

$admin = base64_encode("usuario:pass_en_MD5:lenguaje");
setcookie("admin","$admin",time()+2592000);
?> 

6║ Para que esa cookie se cree en nuestra mßquina hay que visitar la web objetivo (la que tiene el PHPNuke vulnerable) y autentificarse como un usuario cualquiera. 

7║ Una vez dentro hay que mandarse un mansaje privado a uno mismo con el siguiente contenido: Se abre el mensaje y la cookie, si todo ha ido bien aparecerß en nuestra mßquina, pero con la direcci≤n de nuestro server, de modo que debemos cambiarle la URL por la del objetivo. 

8║ S≤lo queda reiniciar el navegador (cerrarlo y abrirlo) y vilver al sitio objetivo, y ya estß, verßs que estßs autentificado como admin. 

Soluci≤n: 

Atualizar a una versi≤n mas nueva de PHPNuke. 
...
============================================================================================
 ...
 Como Hacker la carrera de Kevin Mitnick comenz≤ a los 16 a±os, cuando
 -obsesionado por las redes de computadoras- rompi≤ la seguridad del sistema
 administrativo de su colegio.
 
 Mr. Mitnick.
 
 Todo aquel que sea conceptualizado como un Internauta se caracteriza por  utilizar un telΘfono, un modem y una computadora para muchos fines  diferentes: Divertirnos, informarnos, estudiar, cocinar, planificar viajes  y otras actividades que amplian nuestro quehacer diario. Para Kevin Mitnick  el quehacer diario en sus ·ltimos diez a±os fue el explorar y "explotar"  computadoras ajenas y sistemas telef≤nicos. ┐Su profesi≤n?  "Hacker" de  nacimiento.
 
 Este "terrorista electr≤nico", como lo cataloga el Departamento de Justicia  de los Estados Unidos, conocido en los medios como el "C≤ndor", fue capaz de  crear n·meros telef≤nicos imposibles de facturar (para la compa±φa  telef≤nica el era James Bond, con un numero que terminaba en 007), pudo  Apropiarse de 20.000 n·meros de tarjetas de crΘdito de habitantes de  California y burlarse del FBI por mas de dos a±os con solo un telΘfono  celular alterado y un computador portßtil.
 
 Es la peor pesadilla de las empresas de telefonφa. Es sospechoso de robar el   software de mas de media docena de fabricantes de telΘfonos celulares y  tenia el control de tres oficinas centrales de telΘfonos en Manhattan y de  todos los centros de conmutaci≤n de California, dßndole la habilidad de
 escuchar cualquier conversaci≤n telef≤nica o, si no eras una persona de su  agrado, modificar el telΘfono de tu casa de tal manera que, cada vez que  levantaras el auricular, una grabadora pedφa que depositaras 25 centavos. 
 
 ┐C≤mo se forma un Hacker?
 
 Como Hacker, la carrera de Mitnick tiene sus inicios en 1980 cuando apenas  contaba 16 a±os y, obsesionado por las redes de computadoras, rompi≤ la  seguridad del sistema administrativo de su colegio, pero no para alterar sus  notas, lo hizo "solo para mirar". Su bautizo como infractor de la ley fue en  1981. Junto a dos amigos entro fφsicamente a las oficinas de COSMOS de  Pacific Bell. COSMOS (Computer System for Mainframe Operations) era una base  de datos utilizada por la mayor parte de las compa±φas telef≤nicas  norteamericanas para controlar el registro de llamadas. Una vez dentro de  las oficinas obtuvieron la lista de claves de seguridad, la combinaci≤n de  las puertas de acceso de varias sucursales y manuales del sistema COSMOS. La  informaci≤n robada tenia un valor equivalente a los 200 mil d≤lares. Fueron  delatados por la novia de uno de los amigos y debido a su minorφa de edad  una Corte Juvenil lo sentencio a tres meses de cßrcel y a un a±o bajo  libertad condicional.
 
 Luego de cumplido el periodo de tres meses el oficial custodio encargado de  su caso encontr≤ que su telΘfono fue desconectado y que en la compa±φa  telef≤nica no habφa ning·n registro de el.
 
 Sus objetivos iban creciendo a cada paso y en 1982 entro ilegalmente, vφa  modem, a la computadora del North American Air Defense Command en Colorado.  Antes de entrar altero el programa encargado de rastrear la procedencia de  las llamadas y desvi≤ el rastro de su llamada a otro lugar. El FBI, creyendo  que habφa hallado a Mitnick, allano la casa de unos inmigrantes que estaban  viendo televisi≤n.
 
 Un a±o mas tarde fue arrestado de nuevo cuando era estudiante de la  Universidad del Sur de California. En esta ocasi≤n entro ilegalmente a  ARPAnet (la predecesora de Internet) y trato de accesar a la computadora del  Pentßgono. Lo sentenciaron a seis meses de cßrcel en una prisi≤n juvenil en  California.
 
 En 1987, luego de tratar de poner su vida en orden, cayo ante la tentaci≤n y  fue acusado, en Santa Cruz California, de invadir el sistema de la compa±φa  Microcorp Systems. Lo sentenciaron a tres a±os de libertad condicional y  luego de la sentencia su expediente desapareci≤ de la computadora de la  policφa local.
 
 Luego busco trabajo en lo que mejor sabia hacer y solicito empleo en el  Security Pacific Bank como encargado de la seguridad de la red del banco. El  banco lo rechazo por sus antecedentes penales y Mitnick falsifico un balance  general del banco donde se mostraban perdidas por 400 millones de d≤lares y  trato de enviarlo por la red. Afortunadamente el administrador de la red  detuvo el balance antes de que viera la luz.
 
 Ese mismo a±o inicio el escßndalo que lo lanzo a la fama. Durante meses  observo secretamente el correo electr≤nico de los miembros del departamento  de seguridad de MCI Communications y Digital Equipment Corporation para  conocer como estaban protegidos las computadoras y el sistema telef≤nico de ambas compa±φas. 
 
 Luego de recoger suficiente informaci≤n se apodero de 16 c≤digos de seguridad  de MCI y junto a un amigo, Lenny DiCicco, entraron a la red del laboratorio  de investigaciones de Digital Corporation, conocida como Easynet. Ambos  Hackers querφan obtener una copia del prototipo del nuevo sistema operativo de seguridad de Digital llamado VMS. El personal de seguridad de Digital se  dio cuenta inmediatamente del ataque y dieron aviso al FBI, y comenzaron a  rastrear a los hackers.
 
 Mitnick fue un mal c≤mplice y, a pesar de que habφan trabajado juntos, trato de echarle toda la culpa a DiCicco haciendo llamadas an≤nimas al jefe de este que trabajaba en una compa±φa de software como tΘcnico de soporte. Lleno de rabia y frustraci≤n DiCicco le confeso todo a su jefe que los denuncio a Digital y al FBI.
 ...
 Adicional a la sentencia el fiscal obtuvo una orden de la corte que prohibφa a Mitnick el uso del telΘfono en la prisi≤n alegando que el prisionero podrφa obtener acceso a las computadoras a trabes de cualquier telΘfono. A petici≤n de Mitnick el juez lo autorizo a llamar ·nicamente a su abogado, a su esposa, a su madre y a su abuela y solo bajo supervisi≤n de un oficial de la prisi≤n.
 
 Este caso produjo revuelo en los Estados Unidos, no solo por el hecho delictivo sino por la tßctica que utilizo la defensa. Su abogado convenci≤ al juez que Mitnick sufrφa de una adicci≤n por las computadoras equivalente a la de un drogadicto, un alcoh≤lico o un apostador. Gracias a esta maniobra de la defensa Mitnick fue sentenciado a solo un a±o de prisi≤n y al salir de allφ debφa seguir un programa de seis meses para tratar su "adicci≤n a las computadoras". Durante su tratamiento le fue prohibido tocar una computadora
 o un modem y llego a perder mas de 45 kilos.
 
 Para 1991 ya era el Hacker que habia ocupado la primera plana del New York Times y uno de sus reporteros, John Markoff, decidi≤ escribir un libro de estilo Cyberpunk narrando las aventuras de Mitnick. Al parecer a Mitnick no le gusto el libro ya que luego de salir a la venta, la cuenta en Internet de Markoff fue invadida, cambiando su nivel de acceso, de manera de que cualquier persona en el mundo conectada a Internet podφa ver su correo electr≤nico.
 
 En 1992, y luego de concluir su programa, Mitnick comenz≤ a trabajar en una agencia de detectives. Pronto se descubri≤ un manejo ilegal en el uso de la base de datos y fue objeto de una investigaci≤n por parte del FBI quien determino que habia violado los tΘrminos de su libertad condicional. Allanaron su casa pero habia desaparecido sin dejar rastro alguno. Ahora Mitnick se habia convertido en un Hacker pr≤fugo.
 
 El fiscal no estaba tan equivocado cuando pidi≤ la restricci≤n del uso del telΘfono. TambiΘn en 1992, el Departamento de Vehφculos de California ofreci≤ una recompensa de 1 mill≤n de d≤lares a quien arrestara a Mitnick por haber tratado de obtener una licencia de conducir de manera fraudulenta, utilizando un c≤digo de acceso y enviando sus datos vφa fax.
 
 El Fin
 
 Luego de convertirse en pr≤fugo de la justicia cambio de tßctica y concluyo que la mejor manera de no ser rastreado era utilizando telΘfonos celulares. De esta manera podrφa cometer sus fechorφas y no estar atado a ning·n lugar fijo. Para ello necesitaba obtener programas que le permitieran moverse con la misma facilidad con que lo hacia en la red telef≤nica.
 
 Luego de varios intentos infructuosos, en cuanto a calidad de informaci≤n, se encontr≤ con la computadora de Tsutomu Shimomura la cual invadi≤ en la Navidad de 1994. Shimomura, fφsico computista y experto en sistemas de seguridad del San Diego Supercomputer Center, era ademßs un muy buen Hacker, pero era de los "chicos buenos", ya que cuando hallaba una falla de seguridad en alg·n sistema lo reportaba a las autoridades, no a otros Hackers.

 Shimomura noto que alguien habia invadido su computadora en su ausencia,  utilizando un mΘtodo de intrusi≤n muy sofisticado y que el nunca antes habia  visto. El intruso le habia robado su correo electr≤nico, software para el  control de telΘfonos celulares y varias herramientas de seguridad en Internet. Allφ comenz≤ la cuenta regresiva para Mitnick. Shimomura se propuso como orgullo personal atrapar al Hacker que habia invadido su privacidad.
 
 Hacia finales de enero de 1995, el software de Shimomura fue hallado en una cuenta en The Well, un proveedor de Internet en California. Mitnick habia creado una cuenta fantasma en ese proveedor y desde allφ utilizaba las herramientas de Shimomura para lanzar ataques hacia una docena de corporaciones de computadoras, entre ellas Motorola, Apple y Qualcomm.
...
"┐QuΘ? íEstable! ┐Estßs bien? íMi rat≤n bloqueado!, tengo mensajes sobre un fondo azul como: Error en VxDà" 

Efectivamente, durante la instalaci≤n de Windows, algunas VxD* no son instaladas (ígracias Microsoft!) (*VxD = controlador de dispositivo virtual). Aquφ tenemos una manera de resolver este problema: 

Hacer clic con el bot≤n derecho del rat≤n en Mi PC y seleccionar Propiedades. Ir a la pesta±a Administrador de Dispositivos, en esta ventana hacer doble-clic sobre Dispositivos de Sistema. La operaci≤n es un poco larga pero serß beneficiosa; ahora haciendo clic sobre cada dispositivo, pulsamos en Propiedades y elegimos la pesta±a Controlador/Detalles del archivo del controlador...  Si ves: C:\Windows\System
onfigmg.vxd quiere decir que estßs utilizando el controlador adecuado. Sin embargo si ves: C:\Windows\System\vmm32.vxd(configmg.vxd)  entonces es que Windows utiliza un controlador genΘrico para este dispostivo cuando podrφa utilizar el controlador adecuado pero íno estß instalado!

íAhora instalamos los VxD que faltan, son 7! 

Vcomm.vxd = controlador de comunicaci≤n. 
Vdd.vxd = controlador del dispositivo de vφdeo. 
Vflatd.vxd = dispositivo virtual del adaptador de vφdeo. 
Vdmad.vxd = controlador de acceso directo a memoria (DMA). 
Vmouse.vxd = controlador del rat≤n. 
Configmg.vxd = administrador de configuraci≤n de la placa base. 
Ntkern.vxd = controlador del n·cleo (kernel). 

Estos controladores se encuentran en los ficheros CAB del CD de Windows 98 (CAB 50 para Windows 98 SE y CAB 45 para Windows 98 versi≤n primera). Seleccionar los controladores que faltan y copiarlos en el directorio C:\Windows\System. Reiniciamos Windows y volvemos a echar un vistazo al Administrador de Dispositivos del Sistema, íahora Windows utiliza los controladores adecuados! 
íTenemos un Windows mßs rßpido y sobre todo mßs estable! 
    

í Todavφa mßs estable ! 
(o como forzar Windows a no utilizar 36 maneras de administrar el bus PCI) 

Hacer clic con el bot≤n derecho del rat≤n sobre Mi PC y seleccionar Propiedades, despuΘs sobre la pesta±a Administrador de Dispositivos\Dispositivos de Sistema\Bus PCI\Propiedades\Control de IRQ. Desactivar todas las opciones salvo: Obtener la tabla IRQ mediante la tabla de especificaciones MS. Esto forzarß Windows a utilizar s≤lo esta tabla. Como siempre, reiniciar el equipo tras esta modificaci≤n (de todas maneras Windows os pide que se haga).
    

í Un inicio mßs rßpido y mßs recursos del sistema ! 

┐Prßcticamente trabajas con Windows s≤lo? Entonces, aquφ encontrarßs como acelerar el arranque: 
Menu Inicio\Ejecutar à\msconfig y aceptar. 
En el programa de configuraci≤n del sistema desactivar las siguientes opciones: Procesar el archivo Config.sys, Procesar el archivo Autoexec.bat. Ahora seleccionar la pesta±a Inicio. Vamos a impedir que Windows cargue ciertos programas que consumen recursos del sistema y ralentizan su velocidad como el administrador de la tarjeta grßfica, el peque±o mensaje al comienzo del arranque para recordarnos de registrar los programas, la b·squeda acelerada de Microsoft Office, etc. Personalmente, yo he conservado solamente Task Monitor, asφ como LoadPowerProfile (los 2). Antes de estas modificaciones, esperaba alrededor de un minuto para poder utilizar Windows y me quedaban 86% de recursos disponibles. Ahora, íel arranque tarda menos de 20 segundos y tengo un 95% de recursos disponibles! 
...
Pibi es un gusano reportado el 13 de Septiembre del 2002 de alta propagaci≤n masiva en Internet a travΘs de mensajes de correo electr≤nico, vφa el canal de Chat IRC y de la popular red de archivos compartidos Kazaa. 
...
Pibi ha sido desarrollado en lenguaje Visual C++, tiene 30 KB de extensi≤n y estß comprimido con el utilitario UPX (Ultimate Packer for eXecutables): 
...
Al ser ejecutado el archivo infectado, el gusano se autocopia la carpeta C:WindowsSystem con el nombre de winsysnnn.exe (el valor nnn es un n·mero aleatorio de 3 dφgitos) y para activarse la pr≤xima vez que se inicie el sistema, genera la siguiente llave de registro: 

[HKCUSoftwareMicrosoftWindowsCurrentVersionRun] Windows = "task32 sys" 

El gusano intenta anular los procesos de ejecuci≤n de los programas que tengan la sub-cadena "AV" (mayormente relacionada a los software antivirus). Luego crea la siguiente llave de registro: 

[HKLMSoftwareRedCellinfected] "Yes" 

TambiΘn se autocopia a la carpeta C:WindowsSystem con el nombre "win32sysnnn.zip" (el valor nnn es un n·mero aleatorio) y si el compresor WinZip se encuentra instalado en el sistema, el gusano alterarß las instrucciones del script.ini, en caso que el usuario tenga instalado el software mIRC para Chat. 

Este script contiene las instrucciones para auto-enviar el gusano infectado, bajo formato .ZIP a otros usuarios que se encuentren conectados en una misma sesi≤n de Chat. 
...
Luego copia el c≤digo viral en formato de codificaci≤n Base64 en el archivo C:Msbootlog.sys, el cual usarß para crear los archivos anexados a los mensajes que serßn enviados a los buzones de correo hallados en los archivos con extensi≤n .HTM de la carpeta "Archivos Temporales de Internet". 

La codificaci≤n Base64 es un complejo dise±o de secuencias arbitrarias de octetos en un formato que no puede ser leφdo normalmente. Para este prop≤sito se emplea un sub-categorφa de 65 caracteres [A-Z a-z 0-9 + / =] de codificaci≤n ASCII (American Standard Code for Information Interchange), habilitando 6 bits para ser representados por caracteres imprimibles. 

La informaci≤n relacionada a la cuenta de correo y del servidor SMTP es capturada del registro, en caso contrario el gusano usa una direcci≤n codificada con su propio servidor SMTP: 

smtp.barrysworld.com, configurado para enviar mensajes cada 50 segundos. 

El gusano muestra la siguiente caja de dißlogo: 

"This program has performed sn illegal operation" 

Si la fecha es 15 de Septiembre, mostrarß una ventana: 

I-Worm/PiecebyPiece
"Cause nothing ever last forever we're like flowers in his vase, together.[...]" 

El SCRIPT.INI es un archivo capaz de afectar a todos los programas basados en las redes del IRC (Internet Chat Relay). Contiene comandos que permiten a otras personas controlar en forma remota las sesiones de IRC de un usuario infectado, observar las conversaciones que Θste mantiene y otras acciones adicionales, tales como terminar o cancelar una conversaci≤n o sesi≤n de Chat. 

Este script se puede reproducir auto-envißndose a travΘs de una transferencia DCC (Direct Control Command) y al hacerlo, toma ventaja de dos funcionalidades potencialmente peligrosas del mIRC: la configuraci≤n de recepci≤n automßtica DCC (auto-DCC-get) y la ejecuci≤n automßtica de cualquier archivo llamado SCRIPT.INI que se encuentre en el directorio de instalaci≤n de la aplicaci≤n. 
...
Interrumpir la sesi≤n de IRC cuando otra persona ejecute un comando determinado. 
Permitir el acceso total al sistema del usuario afectado, a travΘs de alg·n tipo de aplicaci≤n como el fserve, al enviar remotamente cualquier comando predefinido. 
Bloquear o alterar mensajes procedentes del o de los sistemas infectados. 
Enviarse en forma automßtica a otros usuarios.
...
Ednav.B, es un peligroso gusano polim≤rfico muy destructivo, reportado el 17 de Septiembre del 2002, que se propaga masivamente a travΘs de mensajes de correo, vφa MS Outlook con un archivo anexado, de nombre aleatorio con la extensi≤n .vbs y hace lo propio, vφa la popular red Kazaa. Borra archivos y llaves de registro de Windows. El contenido del mensaje simula ser una actualizaci≤n para servidores de correo. 

Este Visual Basic Script infecta los sistemas Windows 95/98/NT/Me/2000/XP, incluyendo los servidores NT/2000. 

Al ejecutar el archivo infectado, en forma inmediata, el gusano sobre-escribe los archivos con extensi≤n .VBS en todas las carpetas y sub-carpetas del sistema, dejßndolos inutilizables. 

Si el gusano no encuentra el Microsoft Windows Script Host en el sistema, no procederß a infectarlo. Cabe mencionar que para facilitar los procesos o automatizar las tareas la mayorφa de equipos lo tienen instalado. 

El gusano busca dentro de la carpeta C:\windir\ (por defecto es C:\Windows o C:\Winnt), la carpeta SamplesWSH. Si Θsta no existe, el gusano no podrß propagarse a travΘs de KaZaA. En caso de hallarla procederß a modificar la siguiente llave del registro de Windows, para que otros usuarios puedan descargar archivos infectados desde esta carpeta: 
...
Cada vez que el usuario ejecute un archivo Visual Basic Script infectado, el gusano procederß a auto-enviar masivamente mensajes de correo, haciendo uso de la Libreta de Direcciones de MS Outlook, con dicho archivo, previamente infectado. 

Si la fecha del sistema es 1o, 30 o 31 del mes actual, el gusano borrarß todos los archivos del Escritorio de Windows, los cuales tienen acceso directo para su ejecuci≤n, desde ese entorno grßfico. 

Este gusano polim≤rfico tambiΘn auto-envφa un mensaje a las siguientes direcciones de correo: 
...
Otra funci≤n del gusano muestra un falso mensaje para enga±ar al usuario: "The script demostrates how to access Excel using the windows Script host" 

Asimismo, crea un enlace a C:%windir%Notepad.exe en el Escritorio del sistema infectado. 

En los dφas 29, 30 o 32 esta variante borra una llave del registro de Windows, dejando inutilizable el sistema: 
...
Los payload de este nocivo gusano polim≤rfico son: 

Se propaga masivamente por correo tratando de saturar los servidores. Sobre-escribe archivos con extensi≤n .VBS, dejßndolos inutilizables. Intenta infectar a los usuarios de la red Kazaa. Borra llaves del registro de Windows en fechas determinadas. Provoca fallas en el sistema operativo o lo deja inutilizable.
...
Bueno pues por ultimo quiciera agradecerle a Keogh por permitirme publicar esta peque±a  
rese±a en su e-zine y por el apoyo que me a dado en la realizacion de TMU tambien quiero 
...
2. http://www.prehackers.com  Distribuidor de esta ezine, contiene uno de los mejores foros en espa±ol que existen, asφ como una biblioteca con gran numero de textos. Espa±ola
