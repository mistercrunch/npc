...
y mantenimiento  se  veia reducido  notablemente en ambientes peque±os y con
...
de la luz, es entonces cuando si estas dos se±ales se cruzan se producen las
...
detectarse una colision, el transceptor  esperaria  un  peque±o  periodo  de
...
7. Una LAN esta dise±ada para espacios relativamente peque±os, pero se puede
...
interesa el servidor, lo abrimos y nos vamos al Setup, hφ hay que configurar
...
telnet y conectamos a la maquina en el puerto que hemos se±alado en la orden
...
hacen falta  para  el  emech  por tftp, podemos  probar  a a±adirnos  en  el
...
nos da algun tipo de error podemos usar el Serv-u, un peque±o cliente de ftp
que configuraremos en  nuestro pc primeramente y lo subiremos, lo ejecutamos
desde la shell y listo tambien :D.
Pues ya estß,configuramos el emech en nuestro ordenata,si no sabemos miramos
...
A continuacion nos preguntara el tama±o de la particion que queremos crear,y
...
el tama±o de la particion Swap,esta particion es necesaria y muy util,ya que
...
poliglotas, elegiremos  el  espa±ol  vamos xD, el segundo  paso es elegir la
...
En el  siguiente paso  elegiremos el  teclado espa±ol y pulsaremos enter, el
...
paquetes, sera  adjudicar  una  contrase±a del  superusuario root, sera  una
contrase±a importante,por lo tanto tiene que ser dificil de adivinar,nada de
apellidos  o  el  nick, sino que  debe  contener  MAYUS/minus  y  caracteres
alf4num3r1c0s,la escribimos y yastap.
Despues nos  da la posibilidad de a±adir un usuario, es conveniente hacerlo,
...
other=/dev/hda1 <- Pues eso XD,la particion que se±ala a Windows
label=Windows
...
contrase±a...la IP  la pondremos  asignada  por el  servidor, y le meteremos
...
Solo a±adire que, en mi Slackware,  cuando voy a conectarme  al  irc con  el
BitchX, se pone de ident el user con el que estΘ,que en ocasiones sera root,
para que esto no ocurra nos vamos al /etc/inetd.conf, lo editamos con el joe
y a±adimos el comentario ( "#" ), a la siguiente linea:
...
de lo contrario carece de Θste,tecleamos "y".
...
Y ya esta todo configurado,escribimos "y" y walß. xD
...
Tecleamos make menuconfig y nos aparecera lo siguiente:
 ...
 Linux Kernel v2.4.14 Configuration
    ...
    features.  Press  &lt;Esc>&lt;Esc> to exit, <?> for Help.  Legend: [*] built-in  [ ] excluded
...
Todos esos menus son los que deberemos ir de uno en uno eligiendo lo que nos
conviene y lo que no,Si elegimos una opcion,lo podremos hacer de dos maneras
,tecleando Y,que lo incluira en el kernel,o bien tecleando M, que lo modula-
rizara, pudiendo cargarlo o  descargarlo cuando queramos, como es el caso de
los modulos de las tarjetas de sonido, etc. Para excluir  teclearemos N.Ar
ataker,xD,nos vamos al primer menu,que es Code maturity level options,entra-
mos pulsando enter y vemos lo que nos muestra :

 [1]   -Code maturity level-

[*] Prompt for development and/or incomplete code/drivers

Lo dejamos tal cual esta,Exit y al siguiente menu,Loadable module support:

 [2]    [*] Enable loadable module support
                  [*]   Set version information on all module symbols
                  [*]   Kernel module loader

Activamos la primera opcion con * y las dos que surgen derivadas de ella.
 ...
 [3]   -Processor type and features-
  ...
  Como seguramente tendremos un Pentium,los que nos pueden interesar aqui
son los siguientes :
...
Yo tengo un Pentium II,por lo tanto elegi esa opcion,hay algunas mas,asi que
elegir la vuestra y aseguraos de que es la correcta,si teneis alguna duda ya
sabeis, a los manuales o al folio ese  que os dan cuando la  tienda os clava
con la factura XDDDDDD.
...
Aqui activaremos el soporte para tarjetas PCI, y las derivadas que salen, si
tenemos una EISA la ponemos,en mi caso es no xD,tambien activamos el soporte
para Systcl y los 5 restantes que ahi se muestran.Si os fijais,hay un subdi-
rectorio, PCMCIA/CardBus support ,entramos y activamos estas dos opciones:
 ...
 Este apartdo lo dejamos tal cual,sin se±alar nada.
 ...
 [6]   -Parallel port support-
  ...
  Si tenemos una impresora y queremos que rule bajo  linux deberemos activar
esas opcines.
...
Como SEGURO que tenemos dispositivos "Plug & Play", no os olvideis de marcar
estos dos campos.
...
Floppy disk,os suena verdad? pues marcarlo xD.


 [9]   -Multi-device support (RAID and LVM)-


 Creo que ninguno de  nosotros tendra dispositivos RAIDS, asi que pasando de
 este menu
...
Este punto es importante ya que supongo que la mayoria querra utilizar linux
para conectarse a internet,asique se±alaremos los campos se±alados anterior-
mente,vamos a comentar algunos :

  [*] Network packet filtering (replaces ipchains)  <- Ipchains es
reemplazada por Iptables, asi que activaremos este menu y despues a±adiremos
Iptables.

  IP: Netfilter Configuration  <-   En este directorio encontramos la opcion
de activar el Iptables,asique activamos :

  < > Connection tracking (required for masq/NAT) <- Si usamos
ADSL,tendremos que hacer un NAT, aunque no se si rulara bajo esta version de
Slackware,solo he visto rular ADSL bien en la Essware nueva, y en Debian ha-
ciendo un NAT,pero ni zorra de como se configura ADSL :\.

  <*> IP tables support (required for filtering/masq/NAT) <-   De aqui hacia
abajo las se±alaremos todas  ya que las necesitaremos para  configurar Ipta-
bles con todas sus caracteristicas,tan solo no active las EXPERIMENTAL's xD.


 [11]  -Telephony Support-


 Aqui no activaremos nada,bueno  si alguno quiere llamar desde un tlf conec-
 tado a linux pos ke se las apa±e XD.
...
Seguramente tendremos un disco duro ATA 100 en mi caso de 7000 rpm's, asique
tendreis que activar esa opcion,a continuacion os vais al submenu que apare-
ce debajo,y son necesarias de activar las siguientes opciones :


  <*> Enhanced IDE/MFM/RLL disk/cdrom/tape/floppy support <- Pues
eso,soporte para cdroms... IDE.

  <*>   Include IDE/ATA-2 DISK support <- Soporte para hd's IDE/ATA
  ...
  Soporte para tarjetas PCMCIA IDE,cdroms,disketes...



  Hay tambien algunas marcadas por defecto,asi que las dejamos tal cual esta
  ,ya solo nos queda marcar si queremos emulacion para SCSI,etc.


 [13]   -SCSI support-

 Yo no dispongo de ningun dispositivo SCSI,ni sus correspondientes controla-
 dores... asi que no active nada,pero por defecto biene incorporado este so-
 porte.


 [14]   -Fusion MPT device support-

 Tal cual,sin se±alar nada

 [15]  -IEEE 1394 (FireWire) support-

 Nada,este tampoco nos interesa a ninguno
 ...
 Aqui activaremos el soporte para el protocolo PPP
( Point to Point Protocol ),la cosa quedara mas o menos asi :
 ...
 Daremos  soporte al PPP integrandolo en el kernel y  modularizaremos  todas
 las derivadas surgidas,si utilizamos una tarjeta de red y tal, que no es mi
 caso,iros al submenu PCMCIA network device support e indicar la vuestra.

 [18]   -Amateur Radio support-

 Me gusta mas la tipica radio del awelo, asique si quereis activarla pero ni
 zorra de como se rula XDDD.

 [19]  -IrDA (infrared) support-

 Si tenemos dispositivos IrDA de infrarojos lo marcaremos y si no pos no, xD

 [20]  -ISDN subsystem-

 Aqui no marque nada,no es necesario

 [21]  -Old CD-ROM drivers (not SCSI, not IDE)-

 Esto solo sera necesario si tenemos ordenadores muy antiguos tipo 386,486 y
similares,que no se si sera el caso xD.

 [22]  -Input core support-

 Aqui no marque nada,porque no uso dispositivos de juego como pueden ser
volantes o joysticks,si es vuestro caso lo marcamos.

 [23]  -Character devices-

 Este menu debe quedar mas o menos de la siguiente manera :
...
Hay  algunas marcadas por defecto, pero no os olvideis  si quereis impresora
marcar :

   <*> Parallel printer support

SI tenemos tarjeta grafica ATI, o Vodoo Banshee, pues ahi la he marcado yo,y
tambien para las tarjetas 3dfx.


 [24]  -Multimedia devices-

 Si queremos enchufar nuestra webcam marcaremos con *  el apartado Video for
Linux, nos iremos al submenu y seleccionaremos el tipo de webcam que tenemos

 [25]  - File systems-

 Este apartado tambien es importante,  porque a menudo nos sera util navegar
por la particion de windows para coger  algun fichero o lo que  necesitemos,
incluso es util cuando queremos formatear Windows,pasar toda la info a Linux
y  luego restablecerla despues de reinstalar, veamos  lo que deberemos sele-
ccionar para ello :
  ...
  ? ?         <*> DOS FAT fs support
  ? ?         <*>   MSDOS fs support
  ? ?         <*>     UMSDOS: Unix-like file system on top of standard MSDOS
fs
 ...
 Importantisimo activar  <*> DOS FAT fs support y <*>   VFAT (Windows-95) fs
support, lo demas como  se muestra en el ejemplo. Ahora nos vamos al submenu
Native Language Support  y marcamos el campo correspondiente y compatible al
nuestro :
 ...
 Si queremos tener sonido en nuestro Linux  es necesario configurar bien es-
 tos campos,yo por desgracia no tengo sonido en Linux,mi tarjeta es una Xwa-
 ve 4000,100 % compatible con sb16 segun el fabricante, pues...  ejem..  una
 mierda.

Se±alaremos lo siguiente :

<*> Sound card support

Y la tarjeta de que dispongamos,si os pasa como a mi,que teneis una compati-
ble,  tambien hay soporte para esas tarjetas, solo teneis que iros  al campo
OSS sound modules y se±alar :


&lt;M>     100% Sound Blaster compatibles (SB16/32/64, ESS, Jazz16) support

La modularizamos con M por si posteriormente cambiamos de tarjeta y queremos
cargar otra.

&lt;M>     Microsoft Sound System support

Tambien seria conveniente modularizarla.

 [31]  -USB support-

 Yo no dispongo de entrada USB,si no es vuestro caso, tambien biene este so-
 porte por defecto todo se±alado ;).

 [32] -Bluetooth support-

 Este campo tal como esta

 [33] -Kernel hacking-

 Si quereis hackearos el kernel .. xDDD na,como esta XDDDDDD.

Pues ya esta,aseguraos de que esta todo correcto,volver a echarle un vistazo
rapido por si se os ha escapado algo,guardar la configuracion en un file kon
el nombre del kernel ke tengais,2.4.x,lo ke sea.
Una vez guardado hay que compilarlo.
Poneis en consola lo siguiente :

make dep && make clean && make modules && make modules install && make
bzImage

Si todo va bien ya esta nuestro  sistema recompilado, ahora solo faltan unas
peque±as cosas por hacer,ke son las siguientes :

 1║ Copiar el el /usr/src/linux/arch/i386/bzImage en /boot
 2║ Copiar el /usr/src/linux/System.map en /boot tambien
 3║ A±adir una nueva entrada en el lilo para no chapar el anterior kernel


 1║ Pues eso,copiamos el bzImage en /boot con el nombre del kernel ke halla-
 mos configurado para luego entendernos, yo lo guarde como linux-2.4.14, asi
 que el directorio /boot keda asi :

bash-2.05# ls
System.map   boot-text.b  boot.b   config        linux2.4.14.old  message
boot-menu.b  boot.0300    chain.b  linux-2.4.14  map              os2_d.b


3║ Si os habeis fijado,en el ejemplo de Lilo que puse con anterioridad,al
final aparecia lo siguiente :

image=/boot/linux-2.4.14 <- Nueva imagen creada ( bzImage )
        label=Linuxnew
        read-only

Pues a±adis esa entrada a vuestro lilo.conf y lo ejekutais, o tambien,si se-
guis mi configuracion y guardais la bzImage como linux-2.4.14,os bastara con
copiar mi lilo.conf al vuestro,cambiar lo que dije de los /dev/'s y tal,eje-
kutar lilo y esperar que no de ningun error,si da algun fallo seguramente os
habreis  equivocado en los  parametros de los /dev/ y tal, asique echarle un
buen ojo antes de ejekutar lilo.

 ***NOTA FINAL***

   Esta configuracion de kernel es simple y solamente para un usuario normal,
segun  para que querais vuestro Linux cambiaran  muchos de los  aspectos men-
cionados. Pues nada,a disfrutar de Linux que son dos dias ;),si teneis alguna
duda podeis verme por ahi en el IRC, canal #linux_spain.
Como tambien se habran  preguntado muchos, no he hablado sobre como manejarse
entre el interprete de comandos bash, pero para eso esta (http://lucas.hispal
inux.org) y sus buenos documentos,os aconsejo leer bastante sobre linux antes
de poneroslo, porque de lo contrario os aburrireis a los dos dias y volvereis
a windows,no seras el primero,ni el ultimo,xD.O si quereis aprender por cojo-
nes, formatear windows y  durante una  temporada manejaros solo  con Linux, y
aprendes por webos,no soy ningun experto en Linux NI MUCHO MENOS, y no quiero
serlo,solo me gusta aprender cosas nuevas, ya sabeis, "renovarse o morir" XD.
Para donativos,mujeres bellas,criticas, bombas y antrax, rayika@killnet.org.

Links de interes :

http://lucas.hispalinux.es
http://www.netsearch-ezine.org
http://www.undersec.com
http://www.cybercursos.net
http://www.mundolinux.ne
http://127.0.0.1 <- xDD
http://www.linuxiso.org ( distros )
ftp.kando.hu ( distros )
ftp.funet.fi ( distros )
