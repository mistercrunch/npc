...
```
 ...
 |                                                                        |
 |  -+- Titulo_____: PERL A DOLOR 1¬ EDICION  [001.txt]                   |
...
- Hace cientos de a±os un noble caballero ansiaba tener la mejor espada, una
...
- Nosotros vamos a empezar a hacer nuestras espadas,  y dejarnos de usar las
  ...
  PERL A DOLOR 1¬ EDICION, si esta es la 1¬ parte de un  articulo que me  ha
  llevado su tiempo, espero que lo disfruteis, nunca  pense que iba a alcan-
  zar este tama±o, bueno que podeis encontrar en este articulo  guia  o casi
...
- Bueno voy a empezar por lo mas basico y facil, los comentarios o anotacio-
  ...
  posteriores implementaciones,  si dentro de un a±o vuelves a ver el codigo
...
- Si pusiesemos que empezara a  partir  del 2║ elemento y tomara 3  elemento
  saldria algo asi:
...
- Gracias a stat podemos saber las propiedades de un archivo -
  ...
  Ejemplo:
   ...
   print "$tambloqueIO\n"; #Tama±o preferido de bloque para las operaciones
                           #de entrada y salida "I/O"
   print "$IO\n"; #Tama±o real del blioeuq para todas las
...
- Para abrir  un  archivo  en modo  escritura  pero  a±adiendo  no  borrando
 ...
 **    Exponenciacion (el valor de la izq se eleva a la potencia de la derecha)
 ...
 í~    Coincidencia de patrones (comprueba si los elementos son falsos)
...
- 1║ veremos system,  nos ayudara a usar otros programas de manera facil,  y
...
- umm vamos a hacer un peque±o programeta para enviar unos cuantos pings:
...
- El otro dia en clase usando "net send" para enviar mensajes emergentes, un
  compa±ero no paraba de enviarme muchos mensajes seguidos tocando los  hue-
  vos, y se me paso por la cabeza una maldad,  para enviarle yo tambien unos
  pocos mensajes, vamos a crear un peque±o programa que automatice el envio,
...
- Si  queremos  que repita el n║ de elementos de un array o arreglo usaremos
  foreach:
...
* Para crear una subcadena a partir de una cadena usamos substr de esta  ma-
  nera:
   ...
   substr ($cadena,n║decaracteresquehayqueleer)
...
- Un dia quieres por ejemplo pasar esta ezine a formato html, co±o jodeeeeer
...
- En la 2¬ linea hay un die, que da el mensaje de como funciona el  programa
...
- Este tema es un poco mas chungo que todo lo que  hemos visto hasta  ahora,
  pero bueno a darle ca±a. Bueno un proceso es un programa que se ejecuta en
...
- Te estaras preguntado como manipular estos procesos, entre otras cosas me-
  diante se±ales, y cuales son estas se±ales??, estas se±ales tambien nos la
 ...
 CONT   #Esta se±al hace que se continue un proceso que esta detenido
 ...
 INT    #Se±al de interrupcion de un proceso ya sea por el usuario o por un programa
 ...
 KILL   #Se±al de muerte o finalizacion de un programa
 NUM33
 NUM34
 NUM35
 NUM36
 ...
 NUM61
 NUM62
 PIPE   #Se±al de fallo de canalizacion, execepcion de software se ha intentando de escribir a un pipe y este no dipone de lectores
 ...
 QUIT   #Se±al de finalizacion de un programa
 RTMAX
 RTMIN
 SEGV   #Se±al de fallo de segmentacion, se ha intentado acceder a una direccion en la memoria que no es valida
 STKFLT #Se±al de fallo en la pila, la pila se fue a carajo, el proceso termina
 ...
 TSTP   #Detiene un proceso por que se a recibido una se±al por el teclado
 TTIN   #Un proceso ejecutado en 2║ plano intenta leer el terminal de control
 TTOU   #Un proceso ejecutado en 2║ plano intenta escribir en el terminal de control
 UNUSED #Se±al no utilizada, el proceso termina
 URG    #Se±al de condicion de urgfencia de un socket
 USR1   #Esta se±al puede ser definida por una aplicacion o por el usuario, el proceso finaliza
 USR2   #Esta se±al puede ser definida por una aplicacion o por el usuario, el proceso finaliza
 VTALRM #Se±aliza que la alarma de intervalos expiro, el proceso finaliza
 WINCH  #Se±aliza que el tama±o de la ventana de un terminal ha cambiado
 XCPU   #Se±ala que el proceso ha excedido la cantida de tiempo de uso de la  CPU que podia utlizar, el proceso termia y graba imagen core dump  en memoria
 XFSZ   #Se±ala que el proceso intento acceder o modificar un archivo mas grande que el limite de tama±o de archivo del sistema, genera imagen core dump en memoria

* Como  se  juegan  con estas se±ales podemos ver un ejemplo en el  apartado
  siguiente de alarmas, usa esta sintaxis:


  $SIG{NOMBRE DE SE╤AL} = sub { print "Pepe\n"; }



* Cada vez que esa se±al surga imprimira esa subrutina pepe,  adelantemos un
...
- Como ignorar una se±al, sigue esta sintaxis $SIG{NOMBRE DE SE╤AL} = 'IGNORE';

- Como restablecer una se±al a su valor o salida original  $SIG{NOMBRE DE SE╤AL} = 'DEFAULT';


- Matando procesos con kill, la sintaxis es la siguiente:

kill('NOMBRE DE SE╤AL', IDENTIFICADOR DEL PROCESO );# o lo que es  lo  mismo
                                                    # en vez una coma usamos
                                                    # =>


   kill NOMBRE DE SE╤AL => IDENTIFICADOR DEL PROCESO ;

* Para grupos de procesos se pone un - delante

   kill NOMBRE DE SE╤AL => -GRUPO DE PROCESO ;
...
- Se±al de advertenvia _WARN_ , vamos  a usarla par imprimir un  mensaje  de
  advertencia:
...
- Ahora veremos la se±al die, evitaremos que ejecute una subrutina:
...
- Cuando se habla de truncar archivos, se habla de acortarlo pero tambien es
  posible aumentar su  tama±o llenando  de ceros el final del archivo, (buen
...
* umm quedaria asi algo mas bonito
 ...
 <++> trucarchi.pl
  ...
  #   D I S I D E N T S    #
  ##########################
  die "sintaxis: truncar.pl &lt;nombre de archivo> &lt;tama±o>\n"
...
- Ahora vamos a prepararnos para recinir conexiones:
  ...
  Ejemplo:
   ...
   $sock = new IO::Socket::INET (LocalHost => 'mihost';
                                 LocalPort => 1100,
                                 Proto     => 'tcp',
                                 Listen    =>  6,   #Tama±o de la cola para
...
# Uso:      disidentsmailbomb.pl [mail de mentira] [victim mail] [smtp server]  [n║ de mails] [+d | -d] #
#########################################################################################################
...
use Net::SMTP; #Modulo que necesitamos
die "sintaxis: $0 [mail de mentira] [mail de la victima] [servidor smtp] [n║ de mails] [+d | -d]
...
- Bueno sabiendo esto podemos ya crear nuestro propio monitor de puertos,  y
  si nos scanean  imprimir  un peque±o mensaje de bienvenida y unos cuantos,
         ...
         while(@ready = $select->can_read ) {
          ...
          if($conexion == $socket) {
           ...
           $nuevosocket = $socket->accept;
                     $select->add($nuevosocket); #a±ade el nuevo socket abierto
...
- Umm ahora a±adiremos lo de enviar mails , << y trabajara conjuntamente con
...
$socket = IO::Socket::INET->new( Proto     => 'tcp',
  ...
  ###################
         ...
         sub ayuda {
                     print "[puerto]:         n║ de puerto a monitorizar\n";
...
print " ################################################## \n";
print " #    °°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°  # \n";
print " #    °   DisidentsCGIScanner v.1.0 by DDiego  °  # \n";
print " #    °°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°  # \n";
print " ################################################## \n\n";
...
- guapoooo ehhhh, good ya tenemos nuestro scanner de cgis, ahora veamos como
  tiene que ser el archivo de la lista, seria un array, asi:
   ...
   @cgis=("....../autoexec.bat",
          "....../etc/master.passwd",
          "....../etc/passwd"); #Este es un ejemplo en peque±o, en la pagina
...
- Buff que putada, que nuestro scanner de cgis scanee a pelo, pero noooooooo
  locos porque el se±or DDiego va a ense±aros como vuestro scanner va  a po-
...
* Solo tenemos que a±adir la linea:   $ua->proxy(['http'], 'http://ipdelproxy:80/');
...
* Veamos como quedaria el programa "completo",  va  es un poco  apa±ao  pero
...
- Umm ahora que necesitamos escribir en un archivo, veamos como ;):
 ...
 SINTAXIS: sysread FILEHANDLE, SCALAR, LENGTH
  ...
  Ejemplo:
   ...
   syswrite(ARCHIVO, "DISIDENTS HACKER TEAM", 21); #se puede especificar el numero de bits a a±adir
...
* jeje hemos a±adido el texto, pero imaginemos que queremos a±adirlo en otra
  parte del archivo ;)
...
- La  funcion "time" simplemente nos indica el n║ de segundos que han pasado
...
- Para quienes quieren pasar su script a exe de forma rapida,aunque sin sen-
  tido teniendo un interprete, ya que crea un exe de un tama±o grandisimo e-
...
---[ DIRECCIONES INTERESANTES
   ...
   -[ CASTELLANO ]-
     ...
     Tutorial de PERL en espa±ol  : http://granavenida.com/perl/
     ...
     Recursos de perl             : http://dmoz.org/World/Espa±ol/Computadoras/Programaci≤n/Lenguajes/Perl/
     El CGI Hecho Realmente Fßcil : http://www.jmarshall.com/easy/cgi/spanish/cgi_footnotes.html
     ...
     Curso sobre programaci≤n Perl: http://cdec.unican.es/soporte/cursos/perl/Default.htm
...
- Los libros estan bien para consultar, pero cuestan una pasta de la hostia,
  yo conozco 4 libros en particular:
   ...
   - Titulo: Guia esencial Perl 5
     ...
     N║ de hojas: 206
     Nivel:Principiante
     Valoracion: Una peque±a guia interesante para programadores web, que quieren algo rapido
     y facil
     Precio: 1.496 pts


 NOTA: Me quedo con el primero ;) y el 3║ dan ganas, para cuando me toque la loteria
...
- Ummm en la proxima edicion espero tratar mucho mas a fondo la programacion
  ...
  la sacare o si podra tener el tama±o y la calidad de esta pero lo intenta-
