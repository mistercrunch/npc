...
|===========================================================================
|=~ 001 - Introduccion                                                     |
|=~ 010 - Estudio de la vφctima                                            |
|=~ 011 - Tomando apuntes                                                  |
|=~ 100 - Meditaci≤n y contemplaci≤n del C0D3                              |
...
Mmmmm, respiro aire fresco de un  nuevo e-zine, nueva informaci≤n a nuestras
mentes, a nuestros espφritus;  esta  info que,  nuestro team les trae, es un
caricia para sus neuronas  cansadas  y agoviadas de tanta propaganda y shows
de colores. Bienvenidos a un humilde artφculo en el que veremos mas de cerca
...
Estudiaremos el CrackMe programado por mi como desafio en la e-zine n·mero 2
ahora veremos mucho mas de cerca como funciona ,como crackearlo parcheßndolo
y tambiΘn lo mas interesante hacer  un  keygen para generar llaves correctas
...
Bueno, como comente en el apartado  anterior la vφctima se trata nada mas ni
nada menos que de nuestro querido CrackMe del n·mero anterior de esta ezine.

Una de las cosas mas interesantes para  saber de nuestra vφctima es, como lo
...
http://farrokhi.net/language ,  utiliza   una  peque±a  base  de  datos para
reconocer los compresores y lenguajes,  esta base de datos estß en constante
actualizaci≤n , y desde la pßgina se puede bajar para actualizar.
...
clickeamos en OK y listo DEDE se encargara del proceso, tan fßcil como eso.

Vamos a la secci≤n "Procedures"  y veremos dos eventos que estan programados
...
bot≤n, ese bot≤n 1, intuyo que puede  ser el boton "Registrar", y el segundo
parece ser que cada  vez  que  presionamos  un  boton  dentro de los editbox
calcula algo o genera alguna acci≤n, yo soy el programador de este crackme y
se que hace, pero no se los dirΘ ;).
...
* El evento Click del bot≤n 1 contiene lo siguiente:
...
* Possible String Reference to: 'Θá#√ δπ_^[σ]├'
|
...
* Possible String Reference to: 'C≤digo Incorrecto!!!'
|
...
* Possible String Reference to: '_^[σ]├'
|
...
Ustedes se preguntarßn con que fin  peguΘ este texto aquφ?, ok, la respuesta 
es, que a medida que explique como  funciona el crackme , sacare el texto de
aqui e irΘ  mostrando  que  significa  cada  una  de  las  instrucciones que
componen estos  dos  eventos,  estos  dos  eventos son lo mßs importante del 
programa, si aunque no  lo  crean , este  c≤digo  es lo que hace que estemos 
...
Estamos en la  parte  clave  de   este   artφculo  donde  trato  de explicar
bßsicamente que hace el crackme.  Dividamos el c≤digo en partes para que sea
mßs leible y  encontraremos  lo  siguiente  al  principio  de ambos eventos,
...
┐ que sugiere esto ? 
Perfecto, antes de nada darΘ un poco de teoria sobre las llamadas subrutinas
...
la ejecuci≤n  es  totalmente  normal  a  una  le  sucede  la siguiente y asi 
sucesivamente, pero cuando se llega a un llamado a proceso o funci≤n, sucede
algo que en ASM lo llamamos CALL .  Si,  CALL es una instrucci≤n del ASM, se 
...
analogφa con una de ASM, pero lo que  varφa es la preparaci≤n interna de los
...
MOV cx,ax	; como queremos que la funci≤n devuelva en cx el
...
correcto, sino hiciΘramos eso,  el programa seguirφa con un valor incorrecto
en cx, y si luego cx se usara  para  otra  cosa suponiendo el valor antiguo,
el programa puede llegar a hacer cualquier cosa. Por eso tambiΘn, despuΘs de
la llamada lo recuperamos de la pila.

El secreto de CALL y RET:
-*-*-*-*-*-*-*-*-*-*-*-*	

Mas allß de todo esto, el par de instrucciones CALL y RET, hacen otras cosas
a nivel interno en  los  registros  de  la PC. Cuando se realiza un CALL, el 
registro IP ( es  un  registro  que   lleva  la  direcci≤n  de  la siguiente 
...
Ahora si estamos listos  para  explicar el trozo de c≤digo mencionado antes,
...
los valores correctos. Como podrßn ver los eventos no son mßs que subrutinas
que en el programa principal se llaman a travΘs de CALLs. ;)


Ya que hemos aclarado algunas cosas bßsicas con respecto al manejo de subru-
...
004516EC   8945FC                 mov     [ebp-$04], eax
004516EF   85DB                   test    ebx, ebx
004516F1   0F84F9000000           jz      004517F0       <---- como podrßn 
                                                               ver hay che-
                                                               queos
004516F7   8B07                   mov     eax, [edi]
004516F9   B90A000000             mov     ecx, $0000000A
...
ok, ahora las demßs lineas las iremos analizando paso a paso, los saltos son 
...
00451722   FF248529174500         jmp     dword ptr [$451729+eax*4]<-- aqui sal-
 ...
 [X] esta es una llamada que hice mientras leiamos el  c0d3,  como  prodrßn 
...
i≤n, me refiero de nros enteros a letras.

00451722   FF248529174500         jmp     dword ptr [$451729+eax*4] 

salta como dije antes, a un lugar rarisimo, que  no estß bien  desensamblado 
por DeDe. Entonces usemos el lφder en desensambladores, el mejor del mundo , 
el IDA, me arrodillo ante Θl, es el desensamblador mßs popular entre los cr-
ackers, testers, y programadores, es una herramienta   realmente potente, ya 
explicarΘ como usarla y porque lo digo, ahora  solamente miremos los resulta-
...
usa la pila tambiΘn (POP y PUSH), en cambio IDA,  nos dice  en  que secci≤n 
del archivo esta el c≤digo (CODE), y que puede ser esto!, aqui dice, que es
...
... CASE!!!! si se±or en pascal y/o delphi es usado el CASE, osea que esta-
mos ante, un CASE cuando se hace la conversi≤n se chequea algun valor en lo 
...
(lo siento por los DeDe users no lo verßn) ;)
...
no nos parece familiar???? piensen... si! es la conversi≤n de nuevo quiere 
decir que todo este embrollo es algo asφ:
...
veamos, vamos a la instrucci≤n, que habφamos visto antes:
...
bucle de X valores seg·n la cantidad de valores que  chequemos  dentro  del 
CASE y luego si una condici≤n se cumple sale del bucle y contin·a su camino. 
:)
...
bien, podemos decir que div 10 saca un dφgito a un n·mero empezando por la 
derecha de Θste.
...
Bueno, una posible explicaci≤n es, si tenemos que chequear por  una  "tabla
de valores" , que no es nada mßs ni nada menos que un CASE con valores asφ:
...
Tenemos que una variable depende del dφgito que hayamos  "desprendido"  del 
n·mero introducido por el usuario, chequearemos en un CASE, y lo sumaremos, 
y asφ convertiremos un n·mero en string.

Ahora, posiblemente los dos IDIV se hagan porque queremos "desprender" un 
dφgito, pero no queremos que el n·mero continue como antes, sino que saque 
realmente el dφgito, y que si antes era 156, ahora sea 15.
...
(Integer DIVision), entonces asφ haciendo dos divisiones y moviendo los re-
sultados a distintas variables, se obtiene en una de ellas, el n·mero orig-
inal con el dφgito realmente desprendido, ┐sorprendente no?
...
|           or: system.@LStrCat;
|
00451759   E86E2DFBFF  call  004044CC<--- estos son los case's que habφamos
...
004517E3   E8E42CFBFF     call  004044CC
004517E8   85DB           test  ebx, ebx
004517EA   0F8507FFFFFF   jnz   004516F7<---aquφ chequeamos si es 0 ebx  o 
...
0045181D   742F           jz      0045184E<--- si no es asφ, saltamos...sal-
							tamos a esta parte del c≤digo	 -\
0045181F   8D55EC         lea     edx, [ebp-$14]			        |
											        |
* Reference to control TForm1.Edit2 : TEdit				        |
|										              |
...
00451828   E8C3DCFDFF   call    0042F4F0			                    |
0045182D   837DEC00     cmp     dword ptr [ebp-$14], +$00                 |
00451831   741B         jz      0045184E <---aquφ chequeamos otra vez	  |
							si es disntinto de vacφo        |
00451833   6A30                   push    $30					  |
											        |
* Possible String Reference to: 'Information'					  |
|										              |
...
-+-+-+-	Luego de esto se va a la rutina de finalizaci≤n.-+-+-+-
				|
											|
* Reference to: controls.TWinControl.GetHandle(TWinControl):Windows.HWND;|
|           or: controls.TWinControl.GetHandle(TWinControl):Windows.HWND;|
|									|
00451841   E81E43FEFF             call    00435B64	|
00451846   50                     push    eax<-------/  (entonces el n·mero es
									   incorrecto)
...
* Possible String Reference to: 'C≤digo Incorrecto!!!'
|
00451855   6838194500             push    $00451938	<--- aqui en esa direcci≤n se encuentra ese texto.
0045185A   8BC6                   mov     eax, esi
...
Puff... por fin, hemos terminado, vieron lo que un CASE lleva en lφneas de 
ASM?, es totalmente perjudicial para su PC xD. Pero ojo,  hemos  terminado 
una de las dos rutinas... ┐como? si se±or, falta la rutina de keydown. :)


Ahora el evento KeyDown, este se genera cada vez, que una tecla es presiona
da dentro de un EditBox, y empieza asφ:
...
Bueno, explicarΘ esa parte que dije, eso es claro que hay un bucle, que mie
...
ermina, el salto JL genera la repetici≤n del FOR, el bucle.

Ahora harΘ una analogφa, de c≤digo ASM a c≤digo de ALTO NIVEL, como DELPHI,
C o alg·n otro:	
...
Ahφ vemos que hace exactamente el C0d3, y como esta organizado  dentro  del 
...
Hemos visto la parte mas importante del c0d3, el click del bot≤n  registrar
y el editbox, tenemos que, ordenarnos y pensar en hacer la keygen o por  lo
menos un pseudoc≤digo que incite a hacerla. Vemos que, cada vez que presio-
namos una tecla en la edit box, se va ejecutando una rutina que  genera  el
c≤digo correcto, y luego lo que hace es compararlo con el introducido, y de
ahφ saca el mensaje de correcto! o incorrecto!.Lo importante es haber ente-
ndido la generaci≤n del c≤digo, por lo visto hasta ahora, el c≤digo se gen-
...
Muy bien, el proceso  de  creaci≤n   de  una  keygen  en base a un programa,
...
Las formas que explicarΘ dependen  de  como estΘ implementada la protecci≤n,
pero bßsicamente tocare dos caminos  y algunas variaciones que ya veremos en
mßs detalles.

Primero empezarΘ por el mßs fßcil , resulta que tenemos un programa que esta
...
n·mero de serie vßlido el programa debe chequear con el n·mero vßlido. Salvo
...
un  serial  diferente   porque   sino   conseguirφan  el  programa  completo
introduciendo el mismo serial, que es el  mismo de todos; eso no es factible
como podran ver. Entonces  la  conclusi≤n  es, por cada usuario un n·mero de
serie distinto, ahora, como  consigue  el  programa esos n·meros de serie??,
tiene una base de  datos  de  5000  terabytes??? , NO!!!! , los genera en el
instante , si , cuando ingresas  el  nombre de usuario , genera el n·mero de
serie correspondiente para ese usuario, y lo testea luego con el introducido
por supuesto, el que el program genera no se muestra, sino sabrφas el n·mero
que te corresponde. ;)

Una de las  tΘcnicas  del  cracking  es  SERIAL  FISHING , y  esto es pescar
...
el c≤digo hasta encontrar en que parte  de la memoria se almacen≤ el serial,
entonces, ya tenemos el serial vßlido para el nombre de usuario introducido,
fßcil no?; pero eso no  tiene  nada  que  ver con las keygens, ese es mΘtodo
fßcil, y limitado al nombre de  usuario  de  ese momento, lo que es muy pero
muy ·til, es generar un programa que  genere esos n·meros de serie, seg·n el
nombre de usuario, y la forma fßcil de  hacerlo, es encontrando la rutina de
generaci≤n del c≤digo y luego simplemente copiando toda la rutina de cßlculo
(algunas suelen ser muy tontas), modificarla , para que se pueda compilar, y
listo!, keygen.

O sea, sabiendo donde  esta  la  rutina, pero  no estudiando la ecuaci≤n que
calcula el serial, ya podemos hacer el keygen con unas pocas modificaciones,
agregßndole etiquetas, variables mas leibles que las direcciones de memoria,
etc. Generalmente se generan en win32asm , es un lenguaje perfecto para este
tipo de cosas, fßcil de  implementar  (casi como el visual c++) no necesitas
traducir la rutina del  programa  a  otro lenguaje de alto nivel, con lo que
conlleva el  entendimiento  de  patrones de c≤digo y demßs, y encima de todo
eso, genera c≤digo ultra rßpido y optimizado.

Las rutinas de generacion  de  codigo  pueden  ser muy variadas, tenemos las
otra gran variante, las famosa s rutinas  en  las que debes hacer el cßlculo
inverso para poder generar  el  c≤digo  correcto, porque es asi?, justamente
porque muchas veces sacan un valor  inicial y luego hace cßlculos en los que
se incluyen el nombre de  usuario  o  solo algunas letras de Θl, luego hasta
...
sabiendo que hace el c0d3 y  entender  bien ASM se puede siempre comprender,
 llevarß mas o menos tiempo ,  mas  o  menos  esfuerzo, todo eso depende del
reverser (nosotros!!! :)) tanto como de la rutina a analizar.

El segundo camino es la forma de hacer  el  camino inverso al cßlculo que se
realiza, por ejemplo:
...
contendra por cada  iteraci≤n  un  numero  de  la llave que es una llave que
...
Estas estrategias de "kaygenear" un programa, depende de la protecci≤n como,
dije antes.

A lo nuestro:

Yo, aquφ no harΘ  el  c≤digo  compilable  ni  con  masm , ni nasm, ni ning·n
compilador, porque podrφa  llevar  mas  tiempo para debugearlo y demas, solo
expondrΘ el c0d3 una vez mßs y con  etiquetas, un poco mas "civilizado" para
...
Esta es una aproximaci≤n del keygen que se podrφa programar para este 
crackme que he programado como desafφo.
...
visto tambiΘn diferencias  de  desensamblado  entre desensambladores famosos
como el DeDe y el invencible IDA; pero  no dejamos de mirar el aspecto de la
ingenierφa inversa los cuales mis  artφculos van dedicados mayormente a este
tema. TambiΘn pasamos por la  programaci≤n en general, l≤gica de condiciones
