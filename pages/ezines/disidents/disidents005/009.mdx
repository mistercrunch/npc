...
(el troyano) estß intentando acceder a la red como servidor, lo cual 
...
hago que el firewall no se de cuenta de que se estß ejecutando un archivo 
como servidor? bien ... muy sencillo, íaprovecharnos de los servicios que 
estßn corriendo en la maquina victima!
...
firewall) seria con un script para la aplicaci≤n.

┐Que son los scripts de aplicaciones? pues son "peque±os programas" que van 
ligados a la aplicaci≤n "principal", la cual los ejecuta y los emplea para 
...
Puesto que este texto estß orientado principalmente a win32, veamos una de 
...
protocolo de chat) que corre bajo win32, y que permite la creaci≤n de script's 
...
de programaci≤n mIRC Scripting (documentacion sobre mIRC Scripting en 
...
- sockopen : con esta funcion podremos abrir un socket (conexi≤n)
...
Ahora vamos a preparar un peque±o script que segun sea ejecutado en la maquina 
...
.sockwrite -nt conexion-smtp El directorio donde la victima instal≤ su MIRC es: $mircdir
...
Ahora, analizemos ese peque±o c≤digo linea por linea :-)
...
if ( $sockerr < 0 ) { echo $null } --> Esta linea indica que si la conexi≤n 
...
mßs sobre SMTP, RFC o google.com)
...
.sockclose conexion-smtp --> Por ultimo, cerramos nuestra conexi≤n 
(conexion-smtp) al servidor SMTP.

NOTA: Para que esto funcione, o bien necesitamos un servidor smtp con 
relay abierto (que permita enviar correo a cualquier direcci≤n) o bien 
...
pepito@terra.es, tu servidor smtp serß smtp.terra.es. Si tu cuenta es 
loleilo@eremas.com, tu servidor smtp serß stmp.eresmas.com y asφ, fijandonos 
...
autorizado para actuar como servidor, si no lo estß, el firewall pedirß 
autorizaci≤n, y a no ser que la victima sea avispada, casi seguro que 
autorizarß al MIRC. Todo depende del nivel de conocimientos y de la 
...
Pero hemos hablado de troyanos, tambiΘn se podria emplear un virus/gusano, 
...
veces habreis entrado a un canal, y os habrß aparecido seg·n entrasteis,
...
nos abre un query diciendo que visitemos una pßgina porno, entonces nosotros 
visitamos la web y aqui es donde empieza todo el matute.

NOTA: El virus solo infecta a los mIRC.

1- En la pßgina web que visitamos, la que nos infecta, reside un codigo 
   javascript/activeX que nos copia en nuestro disco duro un archivo llamado 
   rol.vbs (un visual basic script).

2- Ahora el javascript/activeX de la pßgina, nos ejecuta el rol.vbs.
...
NOTA: El nombre del archivo virus server.ini estß creado intencionadamente 
      asφ, para que se confunda con el servers.ini que trae el mIRC por 
      defecto.
...
veremos un puerto "extra±o" abierto. Otro problema es tambien el firewall,
...
Voy a ejemplificarlo con un peque±o codigo en mIRC Scripting (tambien poco
...
Ahora, solo seria trabajarse mas el codigo, a±adir mas funciones, una 
...
"extra±a" se esta abriendo a una maquina remota, y desde ahi podriamos
