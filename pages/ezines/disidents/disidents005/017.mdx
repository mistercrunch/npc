...
V≤y, en este articulo voy a hablar sobre Windows NT, sus bugs y las asa±as de mi amigo
Bill.
...
Donde 020829 es el a±o/mes/dia.
...
Code-Red-Scanner : Scanner de rangos dise±ado por eEye que busca servidores con bug
...
Y nos dara una pantalla quΘ dice asφ :
 
 No se encuentra el archivo c:\inetpub\wwwroot\cualquiercosa.idq

Y c≤mo ven, ahφ esta el path de la web.
...
informaci≤n de los archivos y algunas lineas de codigo de fuente .

Si la informacion es sencible, como contrase±as, que guard≤ ASP interior en el archivo
...
Con esto no podremos modificar absolutamente nada, solo podremos obtener informaci≤n 
...
Establecer una contrase±a cuando instalan servidores SQL !! 
...
ASP (Active Server Pages) es un lenguaje de programaci≤n como PHP, generalmente usado
en plataformas Windows y vale destacar que el lenjuage ASP es muy poderoso pero hoy en
dia yo elijiria PHP.
ASP tiene muchos bugs, a continuaci≤n resumire algunos.
...
Este es un bug que afecta a los IIS 4, el bug consiste en agregar un '.' (sφ, un punto)
al final de la direcci≤n. Nos podriamos descargar todo el codigo en ASP de la web.
...
con los servidores de informaci≤n, tales como servidores HTTP o Web. Un programa CGI,
por otro lado es ejecutado en tiempo real, asφ que puede generar informaci≤n dinßmica.
...
Los exploits, generalmente, estan programados en C ≤ Perl. 
...
Bueno, sin mas quΘ decir, espero quΘ hayan aprendido el peligro quΘ se corre al usar
sistemas NT.
Quiero aclarar quΘ muchos bugs quedaron en el tintero, pero mas adelante los voy a publicar, solo puse los mßs conocidos.

Sφ te leiste todo el documento tendras ganas de explorar y ver c≤mo actua esto, asφ quΘ aca teneis unos servidores vulnerables :

 www.ucu.edu.uy <- Universidad Catolica de Uruguay, fue avisado al administrador el  19/9, hoy estßmos en octubre y todavia persiste el bug.

 www.feapsmadrid.org <- Web de ayuda a discapacitados, fue avisado al administrador el  1/9 y todavia persiste el bug.

 213.97.100.202 <- Web marroquφ, fue avisado al administrador pero parece quΘ no  entiende castellano xDDD.
