...
-------------------------------------------------------------------------------------------------
	...
	Hemos empezado este peque±o cursillo de c en win, nos iremos metiendo mas y mas y mas, 
...
proceso se trabajo 6 a±os y en 1989 fue establecido el estandar, bueno menos rollo y a darle 
ca±a.
...
Una peque±a explicacion de los operadores logicos  
...
--    decremento (a±ade 1 a el valor del operando)
...
main()
{
 ...
 printf("Aqui no llega ni de co±a\n");
}
...
-Vemos como al se±alar el 0 sale la opcion default
...
esto es una peque±a introducion con el uso surgiran algunas preguntas y os afianzareis en su uso
...
La sintaxis de esta funcion es int printf( const char *formato[,argumetos1...]); , La funci≤n 
printf retorna el n·mero de caracteres transmitidos, o un valor negativo si se produce
...
void  main(void)
{
   ...
   printf("Hola %s. Tienes %d a±os.", nombre, edad );
...
La sintaxis de esta funcion es int puts(const char *cadena); Esta funci≤n escribe la cadena 
apuntado por cadena y a±ade un carßcter de lφnea nueva a la salida. El carßcter nulo final 
no es escrito. Devuelve 0 si hay algun error y un numero mayor de 0 si todo ha salido bien el 
valor que devuelve en realidad es EOF es una macro  Una expresi≤n entera constante y negativa 
que es retornada por varias funciones para indicar un final de fichero (End Of File), esto es, 
no hay mßs datos de entrada de un stream.
...
La sintaxis es char *strpbrk(const char *s1, const char *s2); Localiza la primera aparici≤n de la
cadena apuntada por s1 de cualquier carßcter de la cadena apuntada por s2. Como valor de retorno 
...
Rompe la cadena s1 en segmentos o t≤kens. Esta ruptura destruye s1, en el proceso. La forma de
romper la cadena depende de la secuencia de caracteres de la cadena s2. Estos caracteres se 
denominan [caracteres] delimitadores. La funci≤n recorrerß la cadena en busca de alguno de los 
delimitadores de la cadena s2. Cuando lo encuentre, el proceso se detiene, ya que tiene un token. 
Posteriores llamadas a strtok romperßn la cadena s1 en otros t≤kens. Estas llamadas pueden tener
...
(incluyendo el carßcter nulo) a la cadena apuntada por s1, como valor de retorno devuelve la 
funcion copiada si ocurre algo "extra±o" su comportamiento no esta definido 
...
A±ade una copia de la cadena apuntada por s2 (incluyendo el carßcter nulo) al final de la cadena 
apuntada por s1. El carßcter inicial de s2 sobrescribe el carßcter nulo al final de s1. 
...
A±ade no mßs de n caracteres (un carßcter nulo y los demßs caracteres siguientes no son a±adidos)
de la cadena apuntada por s2 al final de la cadena apuntada por s1. El carßcter inicial de s2 
sobrescribe el carßcter nulo al final de s1. El carßcter nulo siempre es a±adido al resultado
como valor de retorno copia al final de la cadena los caracteres a±adidos en caso contrario el 
...
Localiza la ·ltima aparici≤n de c (convertido a unsigned char) en la cadena apuntada por s 
(incluyendo el carßcter nulo). 
...
Localiza la primera aparici≤n de c (convertido a unsigned char) en la cadena apuntada por s 
(incluyendo el carßcter nulo). 
...
Compara no mßs de n caracteres (caracteres posteriores al carßcter nulo no se tienen en cuenta) 
...
main()
     {
      ...
      scanf("%s",&texto);
      tamano=strlen(texto);
      printf("La cadena %s tiene un tama±o de %d caracteres\n",texto,tamano);
     }    
...
La cadena DDiego tiene un tama▒o de 6 caracteres
...
Copia no mßs de n caracteres (caracteres posteriores al carßcter nulo no son copiados) de la 
...
Localiza la primera aparici≤n en la cadena apuntada por s1 de la secuencia de caracteres
(excluyendo el carßcter nulo) en la cadena apuntada por s2. 
Valor de retorno:
