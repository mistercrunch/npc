...
1║  Coger un soplete y darle al hd tipo MA en el equipo A, despues ya
puedes tirarlo a la papelera de reciclaje, tranquilo nadie podra ver esas
cosas que tienes tantas ganas de borrar

2║ Una forma un poco rollo pero efectiva es llenar tu disco  duro,
...
6 veces, como para que te puedan leer algo despues de ese trabajo
     ...
     (((((((((((((((((((((((((((
3║ Una forma mßs comoda y rapida es usando el PGP (Pretty Good Privacy),
el programa mßs popular de encriptaci≤n y creaci≤n de llaves p·blicas
y privadas para seguridad de aplicaciones informßticas, escrito por
...
Funcionamiento:
Puedes borrar desde el exporador de win2, simplemente se±alando el/los
...
un algoritmo de borrado seguro basßndose en que dicho algoritmo
efectua principalmente un procedimiento de sobreescritura casi 40 veces,
haciendo un flush de la cachΘ de disco despuΘs de cada una de ellas,
ademßs trunca el fichero a borrar y lo renombra aleatoriamente antes de
efectuar el unlink(), de forma que para un potencial atacante sea mßs difφcil
obtener cualquier informaci≤n del archivo una vez borrado.Este programa incluye
smem (borrado seguro de datos en memoria RAM), sfill (borrado seguro de
datos en el espacio disponible de un disco) y por ·ltimo sswap (borrado
seguro de datos en el ßrea de swap de Linux). Menuda limpieza...
...
1║ Lo desempaquetamos, recuerda que tar es una utilidad de archivado no un compresor tipo
gunzip
...
2║ Lo instalamos 1║  ponemos make
...
3¬  Ahora voy a crear un simple archivo y borrarlo con el s
...
datos con seguridad, sobreescribiendo el fichero m·ltiples veces con varios
...
con ficheros hay que recordar que las fechas de creaci≤n de ficheros,
permisos, etc., no serßn borrados, de modo que aseg·rate de borrar el
...
lo a±adiremos o haremos referencia a el
...
como se utiliza; este programa tiene una interfaz grßfica bastante amigable,
no tiene ninguna complicacion. Seleccionas los archivos y borras, el programa
no tiene mßs opciones que borrar; hay otro programa pero mßs cutre que solo
...
que es usado para emular la memoria RAM; entre ellos OS/2 y win2, aquφ os
...
que se llama BCWipe y esta en (http://www.jetico.sci.fi). Tiene tambiΘn
opciones
...
- Existen formas de recuperar la informaci≤n del disco mediante sofisticados
instrumentos como microscopios electr≤nicos de barrido o de efecto t·nel.
