---
title: "6-troyanos_en_internet"
ezine: "disidents"
---

# 6-troyanos_en_internet
...
reproducirse. Son programas de administraci≤n remota de sistemas, que constan
...
Ademßs, una vez ejecutado el servidor, queda instalado en el sistema, ya que
a±ade una entrada  en el registro del sistema o se introduce en los ficheros
de inicio  (win.ini, autoexec.bat etc).


┐C≤mo puedo saber si tengo un troyano?
...
Nos aparecerß algo similar a esto:
...
Active Connections
  ...
  UDP    212.60.120.90:138      *:*
	...
	En la primera columna (Proto) aparecen los tipos de conexi≤n, TCP y UDP son
los  servicios que utiliza Internet para establecer la comunicaci≤n.
La siguiente columna (Local Address o Direcci≤n local ) nos dice nuestra
direcci≤n IP que, en este caso, es la  212.60.120.80 en Internet y la
127.0.0.1, direcci≤n que siempre indica que se trata  de una mßquina local.
DespuΘs, separado por dos puntos, nos encontramos el puerto  que tenemos
abierto.
En la siguiente columna, (Foreign Address o Direcci≤n remota) se encuentra la
direcci≤n IP y  el puerto de la mßquina con la que estamos manteniendo la
comunicaci≤n. En este caso, la IP 209.235.102.9 corresponde a
www.raza-mexicana.org y el puerto 80, al servicio HTTP del servidor Web, lo
que significa que, con nuestro navegador, estamos visualizando la pßgina web
de RazaMexicana.
La ·ltima columna (State o Estado) nos indica el estado de la conexi≤n:
establecida, a la escucha, etc.
En la columna  Local Address o Direcci≤n local es donde tenemos que mirar los
puertos que tenemos abiertos. Seguidamente vemos una lista de los puertos que
abren los troyanos mßs conocidos:
...
1024  
TCP  
Latinus 1.0 - 1.1 beta,á NetSpy 0.6.98 Build A
...
la columna Foreign Address o Direcci≤n remota nos la IP de la mßquina del  
atacante cuando se conecta con nosotros.


┐Como puedo evitar que me metan un troyano?
Hay 4 vφas principalmente por las que nos pueden ?colar? el server de un 
troyano:
a) Por el correo
b) En el IRC, por DCC
...
En el caso del correo, se recomienda no abrir ning·n archivo adjunto que 
...
nuestro antivirus es capaz de examinar ese tipo de archivos). Por precauci≤n, 
es conveniente, si utilizamos el outlook desactivar la opci≤n ?descargar 
automßticamente el mensaje al verlo en el panel de vista previa? (en el men· 
herramientas - opciones, pesta±a leer), ya que algunos virus y troyanos pueden 
activarse solamente con abrir el correo.
En el IRC, no debemos jamßs aceptar envφos por DCC de ninguna persona que no 
sea de nuestra total confianza y, aun siΘndolo, asegurarnos bien de que el 
envφo procede efectivamente de Θsa persona y de que no se trata de alg·n virus
de los que se autoenvφan. Como en el caso del correo, conviene igualmente 
examinar los archivos con un buen antivirus antes de abrirlos. Y, por 
supuesto, tener desactivada en nuestro script  la opci≤n de autoaceptar DCC ( 
en DCC ? options on send request: se±alar "ignore all" (ignorar todas) o "show 
get dialog" (mostrar dißlogo), pero nunca "auto get file" and/or "minimize"). 
Asimismo, en File ? Options ? DCC ? Folders ? ?ignore all files except? 
conviene tenerlo se±alado, y no incluir extensiones ejecutables (al final del
documento se incluye una lista de extensiones ejecutables que debemos evitar 
aceptar). TambiΘn resulta conveniente, en la secci≤n de protecciones de 
nuestro script tener se±alado ?rechazar troyanos? o ?rechazar exes?. 
Un error muy com·n en el IRC es la creencia de que instalar un firewall nos 
...
4- Tengo un troyano ┐C≤mo puedo eliminarlo?
La mayorφa de los antivirus detectan y eliminan los troyanos mßs conocidos. 
(Un buen antivirus online que se actualiza a diario lo tenemos en  
http://www.hispasec.com/housecall) TambiΘn antitroyanos como The Cleaner 
(http://www.moosoft.com/download.php)

Para eliminar algunos de los troyanos mas extendidos tenemos tambiΘn 
antitroyanos especφficos; vamos a ver como detectar y eliminar algunos de
ellos ?a mano?
...
servidor de Back Orifice en nuestra mßquina.

El ejecutable del servidor del BO ocupa 124.928 bytes; Al ser ejecutado, se 
borra de forma automßtica y crea una copia de si mismo en el subdirectorio
windows/system como ½.exe╗ y un archivo ½windll.dll╗ de 8.192 bytes. Por 
·ltimo se mete en el registro para ejecutarse cada vez que se inicie Windows.
...
Un antitroyano especφfico para el BO es el BO detect; lo tenemos en 
http://www.cbsoftsolutions.com/


NETBUS:

Es fßcil de detectar la presencia de NetBus, ya que este programa emplea los 
...
. TambiΘn se puede ejecutar el comando telnet y establecer una conexi≤n a 
½localhost╗ (el propio ordenador) en el puerto 12345. Si Netbus se encuentra 
instalado se mostrarß una cadena indicando su presencia.
...
el parßmetro ½/remove╗. Otra forma de eliminar el servidor de nuestro sistema
es utilizar nosotros mismos la aplicaci≤n cliente de Netbus y conectar con 
½localhost╗ . En las opciones del programa elegir ½Server Admin╗ y despuΘs la 
opci≤n ½remove Server╗.


DEEPTHROAT

DeepThroat es mucho menos conocido que los dos anteriores, aunque sus efectos 
son similares, por lo que tambiΘn hemos de prestarle atenci≤n. En este caso 
...
una vez instalado en una mßquina. Es decir, no crea nuevos archivos, ni se 
...
completa hacia el servidor, bastarß con borrar esta clave y tras ello 
localizar el programa en la ruta que se indicaba y eliminarlo del disco duro.


SUBSEVEN (SUB7)

Uno de los troyanos mßs peligrosos, emplea los puertos 1243, 6712 y 6713 en su 
primera versi≤n; sin embargo actualmete existen versiones que utilizan otros 
puertos. Un antitroyano especifico lo podemos encontrar en la siguiente 
direcci≤n: ftp://ftp.europe.f-secure.com/anti-virus/tools/s7disinf.reg Una vez 
...
no se muestre la extensi≤n en windows, aunque si se muestre en el cuadro de 
dißlogo de inicio de transferencia).
...
Quiero hacer constar aquφ mi agradecimiento a mis compa±eros y amigos leon01 y 
