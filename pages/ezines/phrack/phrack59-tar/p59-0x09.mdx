...
-------[ 4. Exploitation conditions
 ...
 We can also return into send() because send() linux implementation use
 sys_socketcall and takes only one parameter on the stack, the parameter
 array absolute address .

000cbb70 <__libc_send>:
   cbb70:       89 da                   mov    %ebx,%edx
   cbb72:       b8 66 00 00 00          mov    $0x66,%eax
   cbb77:       bb 09 00 00 00          mov    $0x9,%ebx
   cbb7c:       8d 4c 24 04             lea    0x4(%esp,1),%ecx
   cbb80:       cd 80                   int    $0x80

 Moreover, send() has a very good error handling code so we will not
 crash the process if we try to read some unmapped process area .
 From the send(3) manual page :
 ...
 address, but these attacks would be quite hard to perform since we
 would have to control many of the vulnerable function parameters .
...
-------[ 6. References
 ...
 [8] Bypassing PaX ASLR using format bugs		anonymous
     (not widely released but outdated now)

 [9] Exploiting format bugs				scut
     http://team-teso.net/articles/formatstring/

 [10] The ELFsh project					devhell labs
