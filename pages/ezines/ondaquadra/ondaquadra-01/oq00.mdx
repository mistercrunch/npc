...
--EDITORIALE--
Comincia cos∞ la storia, spero lunga, di questa nuova e-zine.
Che cos'Φ OndaQuadra, Φ una domanda che non ci Φ ancora pervenuta, soltanto perchΦ 
non Φ ancora uscito nessun numero della rivista, ma che sicuramente arriverebbe, 
se non mi mettessi a spiegarlo qui.
OndaQuadra Φ nata da una chiacchierata in chat, in una serata come tante altre, si 
stava parlando appunto di mettere in piedi una rivista eletronica, pi∙ semplice di 
quelle che ci giα sono in giro, ma con uno scopo finale, FAR CAPIRE A CHI LEGGE 
COME UTILIZZARE TALE PROGRAMMA E COME FARE QUESTA O QUELLA COSA.
Sicuramente il cammino di questa rivista e lo scopo cambierα con il tempo, non ci 
...
diciamo che il punto di partenza della rivista Φ stato questo.
...
per farci capire se quello che stiamo facendo merita e se Φ il caso di proseguire.
Oltre a questo potrete fare domande, a partire dal prossimo numero ci sarα un "angolo 
...
Si tratteranno svariati argomenti, come si pu≥ vedere dal sommario pi∙ in basso, gli 
articoli sono divisi per argomento, tra un articolo e l'altro ci sarα un cosiddetto 
...
di questo primo numero, invitandoli a continuare cos∞, e volevo anche invitare 
tutti i lettori della rivista ( ce ne saranno? hehehe!  :)) che si sentono in grado 
di insegnare qualcosa, a non esitare e ad inviarci il loro articolo, che, se sarα 
reputato idoneo, sarα pubblicato sul prossimo numero.
...
Eccoci qua a parlare di un programmino per Windows che pu≥ rendersi
utile per numerosi scopi. 
Il programma in questione Φ Sockscap32. Potete scaricarlo da siti
...
client per i servizi pi∙ disparati. 
Innanzitutto chiariamo cos'Φ un socks: in breve Φ il servizio che
...
Area Network) ed essa Φ connessa a internet tramite un computer che
...
Armandoci della santa pazienza e di uno scanner (o pi∙ velocemente
cercando sui siti che contengono liste di proxy, socks e wingate) ci
capiterα di trovare un socks che si lascia attraversare, non solo da
...
rete. CioΦ da pacchetti generati da NOI. Quindi in parole povere il
socks farα da intermediario fra noi e il server al quale ci
...
qui tutto il procedimento...) io andr≥ a dialogare direttamente con
...
scritta da un'altra persona. Per≥, se fate una prova, vi accorgerete
che questo non basta a garantirvi l'anonimitα, perchΦ comunque nelle
intestazioni della e-mail rimarrα impresso sempre e comunque il
...
Apriamo il Sockscap, clicchiamo sul men∙ "file" e quindi su "setup".
Comparirα la schermata dove inserire i dati del socks (possibilmente
...
usare. In questo caso sarα: c:\windows\telnet.exe
Cliccate su ok e vedrete che il nome del client (Telnet in questo
esempio) comparirα nel campo delle applicazioni. Cliccate ora su "Run
...
(ripeto che non elencher≥ qui le fasi di spedizione tramite server
smtp di una mail, perchΦ sono ampiamente trattate su quasi tutti i
manuali per hacking). Quando la mail vi arriverα andate a vedere le
intestazioni (Headers) dell'e-mail e noterete che nel campo
"Received", al posto del vostro IP, ci sarα l'IP del socks da voi
usato. Bello, no? La cosa carina di Sockscap32 Φ che lo si pu≥ usare
...
sferrare un attacco, ecc... Oppure un esempio pi∙ "terra-terra" pu≥
...
uno scanning da windows tramite socks. S∞, avete capito bene: anche
se da windows non si pu≥ attuare uno stealth scanning (come si pu≥
fare su linux con nmap) Φ comunque possibile con questa tecnica
effettuarne uno abbastanza sicuro. Infatti lascerete, anche in questo
caso, l'IP del socks anzichΦ il vostro nei logs che registrano il
vostro tentativo di scansione. Quindi un margine di anonimitα
l'avrete guadagnato. Per fare ci≥ inserite il vostro scanner
preferito (io ho usato pi∙ volte Superscan) nel campo delle
...
accortezza Φ usare un socks abbastanza veloce e abbassare la velocitα
dello scanner, cos∞ da non farvi cadere la connessione con il socks
stesso.
Insomma c'Φ da divertirsi. 
...
ricordato che i Socks loggano, cioΦ tengono traccia dei pacchetti che
...
Autore:  -=AndreaGeddon=- [(-UIC-)]  (e chi senn≥!)
Logo:
...
Puff puff... mi Φ sempre piaciuto disegnare in ascii... ma non sono molto bravo.
Oki inziamo.
...
-------------------------------------------------------------------------------------------------------+
 -o OVERVIEW                                                                                           |
-------------------------------------------------------------------------------------------------------+
Ho trovato questo bellissimo programma, che Φ una sorta di ProcDump ma pi∙ evoluto e con molte pi∙
...
--+-- INTRO -- Informazioni sull'uso del programma e sua reperibilitα, accenni al formato PE.
  |
...
Executable) della piattaforma Win32, ma Φ anche un potente Memory Tool che permette di dumpare i
...
piacimento. Questi programmi si autodecriptano all'esecuzione, quindi si troveranno in memoria giα belli
che decriptati, ed Φ la che arriviamo noi col PEditor: non dobbiamo fare altro che prendere il file
dalla memoria e salvarlo su disco, per ottenere una copia del file eseguibile giα decriptata e 
funzionante e pronta per essere manipolata da noi. Peccato che in realtα non Φ mai cos∞ semplice, visto
che spesso c'Φ bisogno di rimettere a posto alcune cosette per far partire il programma. Il PEditor se
...
per la quale Φ stato scritto il software. Quella di cui ci interessemo qui Φ la struttra del PE, il
COFF (Common Object File Format) degli eseguibili standard della piattaforma win32. Tale struttura ci
servirα anche per capire come il programma viene caricato in memoria. Vediamola in breve:
...
dalle stesse proprietα. Di solito ci sono delle sezioni standard, tipo:
...
e altre un p≥ meno comuni. Dare uno sguardo alle sezioni Φ sempre utile, non solo per farci un'idea del
programma, ma anche per ottenere preziose informazioni: insomma, se trovate la sezione .Shrink non avete
difficoltα a capire che avete a che fare con uno Shrinker :-).
Ora abbiamo una idea di come Φ fatto un file eseguibile PE, vediamo brevemente come viene caricato,
...
inizio per la mappatura Φ di 00401000 per un eseguibile (cioΦ la sua image-base + CodeBase, vedi dopo).
...
giα decriptato e funzionante. L'idea Φ quindi quella di copiare il contenuto della memoria (relativo al
file eseguibile decriptato) e salvarlo su disco. Otterremmo cos∞ senza sforzo il nostro eseguibile
decriptato. Come immaginate la cosa non Φ cos∞ semplice! Ci sono vari problemi che si incontrano nel
...
paginazione per gestire la memoria, cioΦ le sezioni del file eseguibile vengono mappate a pagine di 4kb.
Il problema Φ che per risparmiare memoria non sempre sono presenti TUTTE le pagine del programma che
...
non funzia, quindi dobbiamo andare a vedere cosa c'Φ che non va, e qui succedono i casini, perchΦ
bisogna vedere come hanno implementato i programmatori il sistema di Crypting: ad esempio una cosa molto
comune Φ la manomissione della Import Table. Addirittura si potrebbero anche incontrare problemi quando
si va a fare il PAGEIN. Ecco perchΦ andremo a studiarci il PEditor: conoscendo a fondo tutte ci≥ che ci
...
RVA = Relative Virtual Address, cioΦ indirizzo RELATIVO all'image base (del tipo 00001234)
VA = Virtual Address, cioΦ indirizzo virtuale che troviamo in memoria (del tipo 00401234)
...
attraverso il FLC. In questo testo per comoditα ho scelto programmi in cui l'rva coincide sempre con
...
-- Entry Point --
Questo valore ci dα l'indirizzo fisico dell'Entry Point, cioΦ dell'indirizzo da cui il programma inizia
l'esecuzione. Nel mio caso Φ 00055C20. Ho detto indirizzo FISICO in quanto questo valore corrisponde
alla locazione nel file eseguibile, cioΦ al 55C20 = 351264-esimo byte ci sarα il punto dove il programma
deve iniziare l'esecuzione. L'indirizzo di memoria corrispondente sarα ottenuto sommando 55C20 al valore
datoci dall'Image Base (per≥ non Φ sempre cos∞), di cui parleremo fra poco.

-- Image Base --
Nel mio caso vale 00400000 ma praticamente quasi tutti gli eseguibili hanno questo valore. L'image base
non Φ altro che il valore da cui viene iniziata la mappatura del file eseguibile, cioΦ Φ l'indirizzo a
...
Appunto questo valore ve lo dα l'image base. Tale valore cambia a seconda del formato dell'eseguibile,
cioΦ se EXE o DLL... etc. L'image base per le DLL per esempio Φ pi∙ vario, ma generalmente l'indirizzo Φ
del tipo X0000000, per i tipi SYS in genere Φ 00010000 etc.

-- Base of Code --
Qui trovo il valore 00001000. Anche questo Φ quasi sempre uguale per tutti gli eseguibili. La sezione di
codice di un eseguibile inizia all'indirizzo 00401000, che appunto Φ dato da Image Base + Base of Code.

-- Base of Data --
68000. Questo valore invece cambia di volta in volta. Infatti Φ dato dalla grandezza delle sezioni di
...
-- Size of Image --
La grandezza dell'immagine, cioΦ la grandezza che verrα mappata in memoria. Nel mio caso Φ FA000, che
corrisponde a 1'024'000. Il size fisico dell'eseguibile Φ invece di 983'040 bytes. Questa differenza Φ
...
le sezioni). Se questo valore Φ = a 1000 avremo un allineamento perfetto tra Virtual Offset e Raw
Offset, cioΦ VA - Image Base = Offset fisico (perchΦ la mappatura delle sezioni in memoria inizia da
00401000!). Se invece il size Φ diverso da 1000, allora per calcolarci l'offset fisico dobbiamo usare
...
valore. Per esempio, se una sezione Φ grande 1234, in memoria non verrα mappata uno spazio di 1234 ma
...
-- File Alignment --
Come prima, solo che questo allineamento Φ relativo al file fisico. Anche nel file fisico le sezioni
vengono scritte a multipli di questo valore (che di solito Φ 200, ma varia parecchio). Ovviamente se
mettiamo un valore di allineamento troppo grande, sprecheremo dello spazio nel file fisico.

-- SubSystem --
Il sistema richiesto dal programma. Nel mio caso 0002. Questo campo pu≥ avere i seguenti valori:
0000h __Unknown
0001h __Native
0002h __Windows GUI
...
-- Machine Type --
Il tipo di macchina per cui Φ stato scritto il sw. Nel mio caso 14C. I valori di questo campo sono:
0000h __sconosciuto
014Ch __80386
014Dh __80486
014Eh __80586
...
quindi il programma in esame girerα su tutte le macchine 386+.
...
L'optional header Φ quello contiene le informazioni di Base of code, Size of Image etc. Questo campo vi
dice il suo size (nota che l'optional header per≥ non contiene solo i campi che vediamo dal PEditor)

-- Characteristcs --
Contiene vari flag, ad esempio per specificare se il PE considerato Φ una DLL o un EXE.

-------------------------------------------------------------------------------------------------------+
 -o LA SEZIONE DELLE SEZIONI                                                                           |
-------------------------------------------------------------------------------------------------------+
Nella schermata iniziale c'Φ un pulsante etichettato "Sections". Premetelo e vi aprirα la finestra con
...
delle varie sezioni. Di solito iniziano con un ".", ma non Φ sempre cos∞. Il nome delle sezioni spesso
Φ anche relativo al contenuto: "text" indica una sezione di codice (anche .CODE), .data o simili indicano
sezioni di dati, .rsrc indica la sezione delle risorse (bitmap, icone, dialogs etc...), .reloc indica la
sezione delle Rilocazioni (Φ questa la traduzione di Relocations?). Qui Φ il caso di spendere due parole
sulle rilocazioni. Premetto che in un file eseguibile questa sezione Φ completamente inutili, visto che
nel 99% dei casi un eseguibile non subisce MAI una rilocazione. Ma cos'Φ una rilocazione? E' un fenomeno
che troviamo spesso nelle DLL. Quando un processo carica una dll, il codice della dll viene mappato su
un certo indirizzo xxx. Se per≥ il programma carica diverse dll, si troverα a mappare di nuovo al tale
indirizzo xxx, dove Φ giα presente una dll. Quindi il sistema operativo deve caricare questa dll in un
...
stringa sarα 00401234. Quando la dll verrα mappata, all'indirizzo 00401234 troverete la stringa. Se per≥ 
la dll Φ stata rilocata, anche questo indirizzo verrα cambiato per puntare alla stringa mappata nel nuovo
spazio yyy. Bene, la sezione .reloc contiene tutti questi indirizzi fissi, cos∞ quando la dll viene
caricata, il sistema operativo controlla questi indirizzi; se sono giα occupati in memoria (da una dll
...
-- Virtual Offset --
Indica l'RVA a partire dal quale sarα mappata in memoria la sezione. Per la sezione .text il valore Φ di
1000 e non zero. PerchΦ? PerchΦ questo 1000 Φ il famoso "Base of code". Quindi la prima sezione (.text)
partirα dal VA 00401000. Adesso ritorniamo al discorso degli allineamenti. Per la sezione .text io parto
da 00401000 e mappo 322B bytes. Quindi essendo le sezioni contigue, dovrei aspettarmi la prossima
sezione mappata da 00401000 + 322B = 0040422B. Il valore di allineamento delle sezioni, per≥, qui Φ di
1000, quindi la prossima sezione comincerα dal primo multiplo di 1000 disponibile dopo 0040422B, cioΦ
al VA 00405000 (RVA = 5000). E cos∞ via tutte le sezioni. L'ultima sezione comincia all'RVA 9000 ed Φ
lunga 430 bytes. Quindi in tutto l'ultimo byte sarα il 9430░. Sempre per l'allineamento, la fine di
questa sezione va a finire a A000, che Φ il valore del Size Of Image.

-- Raw Size --
La grandezza che le sezioni occupano nel file fisico. Anche questo valore Φ correlato ad un allineamento,
quello dato da File Alignment, che di solito Φ pari a 200. In effetti tutti i size sono multipli di 200.
La prima sezione ad esempio abbiamo visto che VIRTUALMENTE Φ grande 322B. Il primo multiplo di 200
disponibile dopo tale valore Φ 3400, quello del raw size. Lo spazio che va da 322B a 3400 viene riempito
con taanti zeri (469 decimale per l'esattezza). Bene, immaginate che questo si ripete per tutte le sezioni
di tutti gli eseguibili (e tutti i file con formato PE) che avete sull'HD! Giα! Avete un hard disk pieno
di inutili zeri! Che ci volete fare...

-- Raw Offset --
L'indirizzo fisico dal quale Φ mappata la sezione nel file. Nel nostro caso per la sezione .text abbiamo
un offset di 400 e non di zero. 400 infatti Φ il valore che ritroviamo in "Size of Headers". Quindi gli
header MZ+PE sono grandi 400 bytes, e da qui inizieranno le sezioni. La raw size di .text Φ 3400, quindi
la prossima sezione sarα a 3400+400 = 3800. Come vedete qui tutto combacia con l'allineamento a 200.
...
080000000h  La sezione non Φ paginabile
100000000h  La sezione Φ condivisa
200000000h  Sezione eseguibile
400000000h  Sezione in lettura
800000000h  Sezione in scrittura
e possono essere combinati. Ad esempio la sezione .text ha il valore 60000020, che Φ una combinazione di
000000020 + 40000000 + 20000000, cioΦ Sezione di codice, eseguibile e leggibile. La sezione .data,
invece, Φ una combinazione di 80000000 + 40000000 + 00000040, cioΦ dati inizializzati, lettura e 
scrittura. Questi attributi sono molto importanti perchΦ ci danno un idea precisa delle sezioni che
abbiamo davanti.
...
>>Edit Section
Vi aprirα un nuovo DialogBox con tutti i campi della sezione, e ve li potrete modificare a vostro
...
Offset sia Virtuali che Raw. Prima ce lo facevamo a parte col procdump, ora con il PEditor Φ automatico!
Questa operazione si chiama "Riallineamento del PE", e a questo punto dovrebbe esservi chiaro perchΦ
dobbiamo farla: dopo aver dumpato un programma Φ probabile che le sezioni contengano dei dati non giusti
...
-- Tasks --
L'elenco dei task in esecuzione. Per ogni task avete i dati, cioΦ Process ID, Image Base, Size of Image
...
>> load into PEditor: attenzione per gli assuefatti al ProcDump! Questa opzione carica solo il file exe
   relativo al processo, e non i dati relativi al processo stesso! CioΦ, in procdump voi con process
...
-- Section Split --
Dumpa in automatico tutte le sezioni separate, ognuna in un file diverso che verrα messo nella dir
corrente. Attenzione perchΦ non vi viene chiesta nessuna conferma!
...
ripristinare all'indirizzo al posto del CC! Non Φ stramitico ??!!

-- FLC --
Sta per File Location Calculator. Praticamente vi richiede un valore che pu≥ essere uno dei seguenti
campi: VA, RVA, Offset esadecimale, Offset decimale; tale valore verrα convertito negli altri campi
e vi vengono pure indicati i primi bytes che si trovano all'offset in quel file.

-- Checksum --
Calcola il checksum corretto del file e vi dice quale Φ invece il checksum attualmente memorizzato nel
file (di solito 0). C'Φ anche la possibilitα di correggereil checksum in automatico tramite l'apposito
tasto "correct it".
...
dumpato avremo bisogno di ricreargli la IT. Questa cosa si pu≥ fare in diversi modi, e il Rebuilder Φ
uno di questi! La comoditα del rebuilder Φ che lo fa in automatico, per≥ non sempre riesce (anzi, io
finora non sono mai riuscito a ricostruire una IT semplicemente col rebuilder). Cmq vediamo come funzia:
  ...
  spazio. La sezione Relocation negli eseguibili, come abbiamo visto, Φ praticamente inutile, visto che
  ...
  Notate che la riduzione di spazio ottenuta con il riallineamento non Φ sempre significativa.
  ...
  Force functionsearching: Forza la ricerca nell'intero file delle stringhe delle funzioni importate 
     anche se c'Φ giα l'OriginalFirstThunk.
     Nota-- Il discorso del table rebuilding Φ abbastanza complesso, qui ho dato solo la semplice 
...
ricordo che se possibile le IT Φ meglio ricavarle in fase di dumping, o con il ProcDump (Rebuild New IT)
o ve la dumpate a mano. E' possibile che vi ritroverete ad aver aggiustato una IT, e scoprirete che il
file dumpato e riparato funzionerα solo sul vostro PC. Anche questo spiacevole inconveniente verrα
...
-------------------------------------------------------------------------------------------------------+
Nella schermata principale avete il pulsante "Directory". Se ci cliccate verrα aperto un nuovo dialog
pieno di nuove funzioni e valori da commentare (gasp! ma quando finisce 'sto testo??). Questa sarα la
parte pi∙ difficile e importante, visto che parleremo della Import/Export Table e dei problemi connessi.
Cominciamo dal trafiletto di dwords: per ogni campo abbiamo sia RVA che SIZE. RVA ci dα l'indirizzo di
mappatura dell'oggetto in considerazione, SIZE ci dα la grandezza dell'oggetto stesso. In questa ultima
versione del PEditor hanno anche aggiunto i tips, tenete il mouse su un campo e vi dirα la posizione
...
dove e_lfanew Φ un membro della struttura IMAGE_DOS_HEADER, membro  che contiene il puntatore all'header
PE. 80h Φ la posizione del puntatore all'IT all'interno della struttura Directory, e related to .rdata
...
(nota che sono riportati nella notazione intel, quindi ad esempio la seconda dword non Φ E4EEFD38 ma 
 il suo contrario 38FDEEE4)
questi sono i 329 Bytes della Export Table (149h, il Size). Come vedete la seconda metα Φ piena di 
quelli che sembrano i nomi delle funzioni. Okei, diamo un significato a questa manciata di bytes.
La Export Table Φ strutturata nel modo seguente:
...
bene, abbiamo tutti i dati per interpretare la nostra ET. Partiamo dalla Directory Table, che Φ formata
...
------------------------------------+-----------------------------+
  ()- Export Flags: attualmente questo valore non Φ mai usato, sta sempre settato a zero.
  ...
  ()- Name RVA: punta alla stringa che rappresenta il nome della DLL
       Nel nostro caso punta a 0002FD0A, che Φ l'indirizzo della stringa wz32.dll (contare per credere).
  ()- Ordinal Base: indica il numero di partenza della EAT (export address table), normalmente Φ a 1.
  ()- # EAT Entries: il numero delle funzioni esportate (nel nostro caso 0D = 13dec).
  ()- # Name Ptrs: indica il numero di funzioni nella Name Ptr Table, cioΦ abbiamo 13 nomi di funzioni.
  ...
  ()- Name ptr table RVA: indica la struttura che contiene i nomi delle funzioni importate.
       Nel nostro caso punta a 0002FCBC, che Φ la seguente struttura:
       02FCBC                                            13 FD 02 00   ................
       02FCC0  22 FD 02 00 - 32 FD 02 00 - 37 FD 02 00 - 42 FD 02 00   ................
       02FCD0  52 FD 02 00 - 58 FD 02 00 - 63 FD 02 00 - 67 FD 02 00   ................
       02FCE0  70 FD 02 00 - 81 FD 02 00 - 90 FD 02 00 - 99 FD 02 00   ................
       Anche questi sono puntatori (sempre RVA) ai nomi delle funzioni esportate, prendiamo la prima
       dword: 0002FD13 punta alla stringa "DragAppendFile" che Φ il nome della prima funzione esportata,
       la seconda dword punta a 0002FD22, cioΦ la stringa "DragCreateFiles", e cos∞ via per tutte e 13
  ...
  ()- Ordinal Table RVA: indica la struttura degli Ordinali (li vedremo fra poco).
       ...
       fatto di questi ordinali, ora Φ il momento di fare luce! Le funzioni possono essere esportate
       tramite i nomi, come in questo caso, o anche soltanto tramite i loro numeri ordinali, senza dover
       usare i nomi. Sicuramente vi sarα capitato di debuggare qualche programma che nella sua Import
       Table non ha nomi di funzioni carini tipo "CheckTheSerial" o "CheckIfCdIsCopied", ma avrete solo
       quei noiosissimi "Ordinal 0160h" e cos∞ via. Questo Φ dovuto proprio al fatto che il programma fa
...
le macchinine (o con le bambole), perchΦ stiamo per affrontare l'argomento pi∙ ostico di tutto questo
testo, e francamente anche il pi∙ importante, quello che vi ritroverete spesso a dover fronteggiare
quando ingaggerete battaglie disperate con crypters assurdi. Cmq fin qui Φ stato tutto facile, giusto?
Vedrete che alla fine se spiegato bene anche questo argomento diventerα facile come gli altri.
Tanto per cominciare mi serve una cavia adatta: qual'Φ un file con una IT semplice che abbiamo tutti?
il Notepad.exe! Andiamo nella Directory Table e per la IT troviamo:
Import table:   RVA: 00006000       SIZE: 8C
quindi abbiamo indirizzo e grandezza. Ora 6000 Φ un RVA, quindi col FLC andiamocelo a convertire in raw
...
()- Import Flags
Questo dovrebbe rientrare nel campo "characteristics", ma in realtα la dword Φ una union tra il valore
...
E le Characteristics? La union Φ come una struttura, solo che i membri di tale struttura possono essere
...
In realtα questi indirizzi sono dei puntatori ad altre strutture, cioΦ:
...
()- Forwarder Chain
Nel nostro caso Φ settato a -1, e quindi non ci sono forwarders. Cos'Φ un forwarder? Una dll pu≥
esportare un simbolo che non Φ definito nella dll stessa, ma Φ importato da un'altra dll. In tal caso
il simbolo si dice Forwarded. Ora sorge il problema che nel FirstThunk di un Forwarded noi abbiamo
l'entry point della funzione, per≥ il loader non ha modo di verificare se questo entry point Φ corretto
...
nella lista dei thunks, in cui la funzione alla posizione indicizzata Φ una funzione esportata in
forward, e il FirstThunk a tale posizione Φ l'indice della prossima funzione importata. La catena
...
nel file (non serve la conversione in raw offset perchΦ come abbiamo visto l'allineamento Φ perfetto) e
ci troviamo la stringa:
0000658A  53 48 45 4C 4C 33 32 2E 64 6C 6C 00   SHELL32.dll

()- Import Address Table RVA
Abbiamo il valore 000063F0. Questo valore punta ad un array di dwords, l'ultima delle quali Φ nulla ed
...
abbiamo giα visto, era SHELL32.DLL, ora passiamo al secondo. Andiamo subito alla terza dword, otteniamo
...
questi sono tutti i puntatori a strutture HINT, cioΦ strutture formate come giα visto da una dword che
...
funzioni Φ effettivamente uguale al numero degli indirizzi della IAT.
...
ci resta 00006358. Cosa c'Φ a 6358 nel file? Il FirstThunk di DeleteFileA. Ora l'ultimo passo: andiamo
in sice e settiamo un bel bpx DeleteFileA. Provate ad aprire un file di testo con il notepad, e il sice
popperα nel modulo KERNEL32 all'indirizzo BFFA19E5. Ecco quindi che abbiamo raggiunto il codice della
funzione DeleteFileA.
Visto che anche l'IT Φ facile ora?

-- Recourse --
Pare che abbiano sbagliato la sintassi sul PEditor: questo Φ il campo Resource. Nelle risorse troviamo
...
Nel nostro caso Name Entry Φ pari a 0, e ID entry Φ 7. Detto cos∞ non Φ molto chiaro, spiegheremo tutto
meglio fra poco.

()- Resource Dir Entries
In seguito alla directory c'Φ un array variabile di elementi che fanno parte della directory entries.
...
identificabili tramite interi a 32 bit. La struttura di un direcotry entry Φ fatta nel modo seguente:
         NAME RVA/INTEGER ID           DWORD
    E | DATA ENTRY RVA/SUBDIR RVA      DWORD
dove la prima dword Φ una union tra i campi NAME o INTEGER: pu≥ contenere o l'rva che punta al nome
della risorsa (rva a 31 bit per≥) o un intero a 32 bit che indica l'identificativo della risorsa.
La seconda dword invece Φ determinata dal primo bit pi∙ a sinistra (E):
...
Resource Dir Entries (siccome ID entry Φ sette, prendiamo sette strutture da 2 dwords):
...
indicano l'offset dei dati della risorsa: 00000338. Ora questo non Φ un indirizzo vero e proprio, ma Φ
...
queste sono tutte le strutture puntate dalle resource dir entries. Ognuna di queste strutture Φ formata
...
00000000    RESERVED
 ...
 . Size
  Φ la grandezza dei dati della risorsa
 . CodePage
...
-- Basereloce --
Sta per base relocation. Abbiamo giα parlato delle rilocazioni, abbiamo detto che le variabili statiche,
le stringhe etc venivano caricate ad un diverso indirizzo da quello specificato in ImageBase. Il campo
basereloc ci dα un puntatore (alla sezione .reloc) alla struttura che contiene le informazioni per
...
la prima dword (page rva) Φ 00001000, ed ci dα l'rva che serve per individuare dove andare ad effettuare
...
in cui Type indica il tipo di rilocazione a seconda del suo valore:
 ...
 . 2 = Low
   Come prima, l'offset punta alla parte bassa (16 bit a destra) di un indirizzo a 32 bit, ed Φ l∞ che va
...
cioΦ la rilocazione inizierα all'rva 00001000 e sarα lunga 0268h byte. La lunghezza per≥ comprende anche
...
-- Debug --
Anche questo campo ci dα l'rva di una struttura. Questa contiene info per il debugging, la descriver≥
fra poco.
...
-- Tls Table --
Ci dα l'indirizzo del Thread Local Storage. Ne parleremo fra poco.

-- Load Config --
sconosciuto

-- Bound Import --
Questo per rispondere a WebCide che mi ha fracassato di domande l'altro giorno in chat :-P. Abbiamo giα
...
funzione in una dll sarα sempre lo stesso, quindi possiamo mettere nei FirstThunk direttamente gli entry
point, in modo che il loader non perda tempo a metterceli per noi. Questo processo si chiama "binding".
Ovviamente se abbiamo i FirstThunk giα settati sugli Entry point delle funzioni, ma per un qualsiasi
...
Abbiamo giα trattato la IAT nella sezione della IT, cmq questo valore Φ superfluo, in quanto giα
...
struttura che giα abbiamo commentato, nella destra avete le funzioni ordinate per indice, con i relativi
rva e nome.
...
nome (che giα abbiamo descritto in dettaglio).
Il riquadro superiore merita un attimino di attenzione: nella descrizione dei moduli compaiono le voci
corrispondendi a FirstThunk, OriginalFirstThunk etc. che ho giα descritto.

** RESOURCES **
Beh, avete il visualizzatore ad albero delle risorse, avete anche il visualizzatore delle icone! Mitico!
A destra avete le info sui valori che ho giα discusso prima (name entries, etc..), e per ogni risorsa
avete in basso a destra la locazione (rva) e il suo size. La prima risorsa Φ un icona, ed Φ la classica
icona del notepad (come giα visto prima; se osservate attentamente il blocco ascii relativo all'icona,
vi sembrerα di intravedere il disegnino del notepad! infatti Φ cos∞).

** DEBUG **
Per vedere qualcosa in questo campo dovete creare e compilare un eseguibile con le info di debug (il
visual c++ lo fa in automatico). Debug quindi vi aprirα un dialog con le informazioni delle strutture di
debug. Una struttura di debug Φ formata nel seguente modo:
               CHARACTERISTICS
               TIME/DATE STAMP         
      MAJOR VERSION  |  MINOR VERSION  
                 ...
                 DATA RVA              
                 DATA SEEK             
dove: Characteristics Φ sempre a zero,
      TimeDate Stamp Φ la data e ora della creazione delle info di debug
      Maj/Min Version Φ la versione delle info di debug
      Debug Type Φ il formato delle info di debug, che pu≥ essere in simboli COFF, FPO, o CodeView
      Data size Φ il numero di bytes dei dati di debug (escluse le debug directory)
      Data rva Φ appunto l'rva dei dati di debug
      Data seek Φ il seek value dall'inizio del file fino ai dati di debug
...
** TLS **
Acronimo di Thread Local Storage. Questa struttura non la descriver≥, anche perchΦ il suo funzionamento
non mi Φ ancora molto chiaro, e ci≥ Φ dovuto al fatto che non riesco a trovare un eseguibile che abbia
una TLS. La TLS contiene dei dati che verranno usati dai thread. In particolare la struttura contiene
anche un array di rva dei Callback, cioΦ un array di funzioni che devono essere richiamate prima e dopo
la creazione del thread.

-------------------------------------------------------------------------------------------------------+
 -o NOTE                                                                                               |
-------------------------------------------------------------------------------------------------------+
Nella descrizione della IT ho usato il modello classico, che Φ quello della struttura:
...
la differenza Φ data dal fatto che alcuni compilatori usano la seconda struttura per costruire la IT.
...
prima dword conterrα l'rva per i nomi delle funzioni, se la struttura rientra nel secondo caso allora la
prima dword sarα settata a zero. Praticamente Φ proprio questa union che determina il tipo di struttra
della IT.

Quando ho parlato del Rebuilder ho detto che potreste ritrovarvi ad aver ricostruito la IT di un prog,
per≥ scoprirete che tale prog funziona solo sul vostro pc. Il problema Φ legato ai Thunks. Infatti nei
...
IT dopo questo processo, vi ritroverete una IT che non Φ l'originale, ma il file funziona in quanto ha
giα prepatchati gli entry point delle funzioni. Pu≥ capitare che non tutte le dll carichino il loro
codice sempre allo stesso indirizzo (per vari motivi). Nello stesso pc al 99% delle volte gli indirizzi
sono sempre quelli, ecco perchΦ sul vostro pc funziona, per≥ su un altro pc gli indirizzi probabilmente
saranno diversi, e quindi il file non funzionerα. Voi direte: ma abbiamo sempre gli OriginalFirstThunks,
...
testo (e che si sono incazzati perchΦ non li avevo salutati :-P)
...
www.uic-spippolatori.com  -  la UIC (universitα italiana crackers)
...
pochi ma buoni"
Ricordo con molto piacere la , come potrei definirla , strepitosa , nonchΦ
...
VabbΦ tralasciando la miseria in questione , c'Φ da dire che lo SMAU lo
...
VabbΦ perdonato ! Ti posso capire eri con la ... morosa :PPPP
...
E vabbΦ che cosa ci si puo' fare ?
...
Questo Φ un semplice programmino scritto in visual basic, per
ottenere un Host
Name da un IP Address.
Il codice riportato qui sotto Φ stato preso in visione dal sito
VBnet Network.
...
Torno a dire che questo codice non Φ mio, ma Φ di proprietα di Randy
Birch, non
voglio prendere i meriti di nessuno, questo Φ suo, e suo rimane.
...
ricerca e immettere come parole chiave æcracking tutorialsÆ, ne 
appariranno decine di migliaia).

Questo eÆ il primo di una serie di ætuteÆ che spero sia un poÆ piuÆ 
...
costituisce un grosso problema al giorno dÆoggi, ma volete mettere la 
cara vecchia lingua di papaÆ Dante? ;) ) oppure peggio ancora sono di 
entrambi i tipi visti sopra, insieme.

Ora la mia pretesa non eÆ quella di scrivere un vero e proprio tute con 
tutte le possibili strade che si possono intraprendere cercando di 
craccare un programma (sarebbe unÆopera troppo vasta, e le mie 
...
reverse engeneering, e percheÆ no, darvi qualche diritta per rendere 
piuÆ mirato il vostro æattaccoÆ ad un programma.

Un solo avviso, non sperate di trovare æla luce alla fine del tunnelÆ 
...
e con i piuÆ svariati tipi di applicazioni, in modo da ampliare la 
vostra esperienza e conoscenza sullÆargomento. Quindi se avete 
allÆattivo giaÆ una media conoscenza dei meccanismi di protezione e 
dellÆuso dei debugger, potete risparmiarvi di leggere, oppure se 
...
Bene, passiamo ora alla sezione æSrumenti necessariÆ, innanzi tutto vi 
serve un debugger che funzioni in (come si dice in gergo) æreal modeÆ, 
...
windows 9X). Un altro strumento utile eÆ il WinDASM, questÆultimo eÆ 
presente dappertutto in rete in versione demo ma attenzione alle 
limitazioni di questÆultima che sul piuÆ bello del tracing ti blocca 
dicendoti che il numero di operazioni della versione demo eÆ limitato, 
quindi se ne avete la possibilitaÆ procuratevi la versione integrale 
(che tra lÆaltro consente di salvare il file disassemblato come testo 
...
trovo decisamente inefficace (comunque, come si dice, æde gustibus...Æ), 
avete anche bisogno di un editor in grado di modificare gli eseguibili 
per æpatchareÆ i programmi che dovrete craccare, di questi strumenti ce 
ne sono centinaia, io ne ho installati un paio, dei due trovo molto 
funzionale l'æUltra Edit 32Æscritto da 'D. Mead' (www.ultraedit.com)
...
Bene, bene, benissimo cominciamo col vedere come eÆ possibile 
migliorare i tools che abbiamo a disposizione.

Il SoftICE eÆ un debugger che, funzionando in modalitaÆ æreal timeÆ 
deve essere caricato in memoria prima del sistema operativo, quindi, 
dopo lÆinstallazione, allÆinterno del file æAutoexec.batÆ saraÆ 
presente la riga di comando che avvia lÆICE, quindi a meno che non 
usate il computer solo ed esclusivamente per fare i crackers, potrebbe 
tornarvi utile un menuÆ in fase di avvio del sistema che vi permetta di 
scegliere se avviare o meno il SoftICE.

Se non avete dimestichezza con la programmazione mediante file di 
æBatchÆ vi riporto le righe che avviano questo menu:
...
Questo menu, (che potete tranquillamente ætagliareÆ ed æincollareÆ nel 
file autoexec.bat al posto della sola riga di comando 
æC:\SIW95\WINICE.EXEÆ che viene inserita dal setup del SoftICE, 
...
SoftICE, se scegliete æSÆ lo avvia (sono un genio :p ), se scegliete æNÆ 
no, mentre se non rispondete nulla, dopo cinque secondi di attesa, il 
menu sceglie automaticamente di non avviare lÆICE (scelta di default: 
No).

A questo punto eÆ possibile andare a modificare la configurazione del 
SoftICE, o aprendo lÆapplicazione æSymbol loaderÆ presente nel 
pacchetto e andando ad intervenire sulle voci presenti allÆinterno 
delle opzioni in æEditÆ e poi in æSoftICE initialization settingsÆ, 
oppure andando a modificare il file (contenuto nella directory dove 
lÆICE eÆ stato installato, solitamente [DRIVE]:\Sicew95\) winice.dat

Questo file contiene tutte le informazioni che il SoftICE utilizzeraÆ 
allÆavvio e durante il suo uso.

Di seguito vi riporto la copia del mio file, commentandovi i punti 
salienti. Questo file eÆ gia ottimizzato secondo le mie necessitaÆ, se 
queste dovessero coincidere con le vostre, saraÆ sufficiente ætagliareÆ 
il testo e incollarlo in un file del blocco note di windows che andraÆ 
...
Le prime righe (quelle fino alla stringa æINIT=Æ vanno lasciate come 
sono se avete un processore pentium (o compatibile) o successivo, (se 
cioÆ non eÆ vero, ad esempio se avete un 80486, semplicemente settate 
su OFF la voce æPENTIUMÆ) le uniche stringhe che ci interessano sono: 
quella che æsettaÆ il æLOWERCASEÆ su on o off: impostandola su on si 
...
del computer æPHYSMBÆ dove il numero alla destra deve essere posto 
uguale al quantitativo in megabyte della memoria presente sulla 
macchina.

Per quanto riguarda la stringa æINIT=Æ questa eÆ quella responsabile 
della æpersonalizzazioneÆ della finestra dellÆICE.

Vediamo come si interpretano i comandi in essa contenuti: 

Faults [off | on] questo comando se settato su off, impedisce allÆICE 
di intervenire, nel caso si verifichino errori del kernel del sistema 
operativo, di pagina non valida, ecc. e poicheÆ non eÆ raro che questo 
accada, conviene impostarlo su off, se non volete resettare il sistema 
ogni qual volta il kernel di windows decida che qualche applicazione æha 
compiuto una operazione non valida e verraÆ terminata...Æ (piuÆ tecnico: 
nel caso di ægeneral protection faultsÆ).

'X' questo comando riporta il controllo al sistema operativo.

NOTA: tutti i parametri che vengono inseriti nella stringa æINITÆ prima 
...
Set font [1 | 2 | 3] Seleziona il tipo di carattere che verraÆ 
utilizzato dallÆICE (la 2 eÆ quella che a me piace di piuÆ)
...
scegliere il numero di righe che piuÆ preferite.
...
UnÆocchiata alle funzioni assegnate ai tasti æF8Æ, æF10Æ, æF11Æ:

F8 permette di seguire un æsingle steppingÆ entrando nelle procedure 
chiamate dalle istruzioni æCALLÆ

F10 æsingle steppingÆ come sopra ma con la differenza che le æCALLÆ 
...
eseguite in una sorta di æbackgroundÆ e lÆICE si riporta 
...
una Call (ne vedremo un utilizzo pratico piuÆ avanti).

Le altre linee presenti nel file servono allÆICE per caricare (e 
...
che costituiscono lÆinterfaccia grafica del sistema operativo e che 
permettono a questÆultimo di interagire con programmi ed utente 
mediante le finestre, la pressione dei pulsanti, lÆuso del mouse, ecc. 
ecc. (e che permettono di æsproteggereÆ i programmi ;) !!); quindi, a 
...
lÆunica accortezza, di cambiare il percorso (path) mettendo quello 
corrente del vostro sistema operativo (ad esempio, se avete due HD e il 
windows eÆ installato su quello chiamato D, dovete cambiare la lettera 
C in D, nel percorso di ogni singola funzione, nel file winice.dat, 
altrimenti lÆICE non troveraÆ un kakkio!).

Aggiungo anche un breve æQuick Command ReferenceÆ sui comandi che 
...
Bpx : (breakpoint on execution) significa che lÆICE deve intervenire e 
fermare lÆesecuzione del programma nel caso si verifichi una data 
condizione specificata dopo il comando (i dettagli dopo, questo eÆ un 
æQuick ReferenceÆ ;) )
...
una serie di indirizzi (su un ærangeÆ di memoria) 
...
che controllano, lÆaccesso alle periferiche)

Bmsg : (breakpoint on message) fa intervenire lÆICE su un messaggio (lo 
...
Gli æhandleÆ sono, in parole povere, dei numeri che identificano una 
data finestra in un programma, ogni finestra (dalla nag screen alla 
æmainÆ o principale del programma ) eÆ identificata univocamente da un 
...
bd* : Disabilita tutti i bp, il simbolo æstarÆ pu≥ essere sostituito 
con il numero identificativo del breakpoint 

be* : Abilita tutti i breakpoints il resto Φ uguale a "bd" 

bl : Lista tutti i breakpoint che stiamo usando ed il loro numero 
identificativo, se il break eÆ preceduto da un æ*Æ allora vuol dire che 
eÆ disabilitato 
...
vedere quale valore eÆ contenuto in un dato registro, ecc. 

d [indirizzo] : permette di avviare il ædumpingÆ esadecimale di una 
data locazione di memoria, per mostrarci cosa essa contiene 
(chiaramente in codice esadecimale)

a [indirizzo] : ci permette di modificare ci≥ che si trova ad un dato 
...
lo scrivo semplicemente perchΦ ... ehmmm ... e perchΦ lo scrivo ?
VabbΦ lasciamo perdere ! Diciamo che voglio mettere i puntini sulle I !!
Il punto e' questo !!!! Tutti tentano , cercano , ci provano , insomma molta
gente prima o dopo si chiede che cos'Φ l'hacking !
...
pensano la stessa cosa e proprio perchΦ e' una cosa che non va bene
...
"bΦ gli hacker sono persone che non fanno altro che penetrare i sistemi
informatici altrui e distruggerli ! Gli hacker sono persone di sinistra che
vorrebbero che tutto fosse libero e gratuito !"
AlchΦ io e jey ci prendiamo in query chiedendoci che stronzate stesse tirando
...
Insomma che male c'Φ se tutto cio' che si fa  lo si fa per cercare di capire
come funzionano le cose sulla rete ?
Si magari si puo' anche smanettare con qualche smurf o qualche trojan , pero'
anche in questo caso , che cosa c'Φ di male se lo scopo e' verificare come e'
stato codato ?
...
Si certo perchΦ questo e' uno dei client SMTP che ci sono al mondo !
...
consideri di una certa schiera politica solo perchΦ smanetto un pochino , mi
