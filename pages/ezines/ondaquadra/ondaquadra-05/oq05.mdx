...
| Breakpoint 1 at 0x40080d76: file printf.c, line 30.                      |
|                                                                          |
| Breakpoint 1, printf (format=0xbfffea60 "<Ω ┐<Ω ┐=Ω ┐=Ω ┐>Ω ┐>Ω ┐?Ω ┐?   |
| Ω ┐%x\n%x\n%x\n%x\n%x\n%x\n%x\n%8x%n%8x%n%8x%n%8x%n\n") at printf.c:30   |
| 30      printf.c: File o directory inesistente.                          |
|                                                                          |
|                                                                          |
...
| La volta scorsa abbiamo spiegato come Φ strutturato il costrutto IF,     |
...
| - Conclusioni :)                                                         |
|                                                                          |
| Questo e' l'indice dell'intera "opera", pero' siccome Φ molto            |
...
| Dunque.. Cos'e' la questa Crittografia? Bene.. la crittografia Φ la      |
...
| Per non parlare degli Sniffer (di cui parliamo più avanti) !! :D         |
...
| asimmetrico).... ma di questo ne parleremo più avanti! :)                |
| Forse potreste trovare esagerate tutte ste preoccupazioni ma.. come si   |
| dice? "Fidarsi e' bene ma non fidarsi Φ meglio!! :D"..                   |
...
| sicuro sara' anche la quantitα di tempo che serve per renderlo           |
...
| Questo e' pressochΦ la struttura generale di un algoritmo (Schema di     |
| Feistel).                                                                |
...
| questa scelta e' appunto che la sua struttura Φ ben definita e cio'      |
...
| (Di seguito vi propporrò un breve commento su ogninuo di questi          |
...
| andare su google e provare a cercarli visto che è pieno... nel caso non  |
| riusciste a trovare nessun sorgente o altro mandatemi una mail vedrò se  |
...
| Il Triplo-DES e' una variazione del DES: proprio perchΦ cifra il         |
...
|                                                                          |
| Questo e' un sommario di pressochΦ tutti (o quasi) gli algoritmi a       |
...
| bello no? :D Beh, purtroppo c'e' un problema: la chiave pubblica Φ       |
...
| messaggio, decifrarlo tramite la sua chiave privata (puo' farlo, perchè  |
...
| più bassi.... quindi tutto cio' e' possibile!                            |
...
| Come e' anche evidente dai nomi degli elementi,la chiave pubblica Φ      |
...
+--------------------------------------------------------------------------+
|                                                                          |
| Disclaimerz: Non mi assumo nessuna responsabilitα di quello ke sta       |
| scritto qui sotto, tutto quello scritto nella guida non Φ mai stata      |
| scritta di mio pugno ma Φ solo la traduzione pi∙ fedele possibile della  |
...
| Se tutto quello che Φ scritto di seguito in qualche modo vi offende      |
...
| Crackare un programma Windows spesso Φ molto pi∙ semplice di un          |
| programma che gira in DOS. In Windows, Φ difficile trovare qualcosa da   |
| qualcuno che cerchi realmente le informazioni, finchΘ le funzioni sono   |
| in possesso di Windows.                                                  |
| Il primo (e anche l'ultimo) programma di cui tu hai bisogno Φ            |
...
| Questa Φ probabilmente la via pi∙ facile per fare pratica, prendere un   |
| programma Shareware e provare a registrarlo.                             |
| 3.1. Task Look 3.00 - Una semplice registrazione numerica                |
| l'unica protezione                                                       |
| Questa Φ una semplice protezione, un solo codice, che non cambia.        |
...
| Hai controllato?...                                                      |
| Ok, ora tu sai che Φ una applicazione di Windows 95 a 32-bit, e che tu   |
...
| stringhe a un-byte o a un      doppio-byte. Il codice a doppio-byte Φ    |
...
| Look Φ un 32-bit cos∞ metti un breakpoint su GetWindowTextA. Se questo   |
...
| e ti comparirα subito una stupida finestra di messaggio che ti dirα che  |
...
|  :be0                                                                    |
| La prima linea nella finestra del codice Φ come questa:                  |
...
| Φ inserito con una funzione. Steppiamo attraverso il programma una       |
...
|   JNZ   00402BC0  ; Salta se non Φ zero                                  |
| Per me, questa sembra direttamente come una stringa che controlla la     |
| funzione. Questa funzione cos∞:                                          |
|  *Inserisci 2 stringhe, ritorna zero se Φ uguale, altrimenti non-zero    |
| E perchΘ il programma dovrebbe controllare una stringa con una           |
| inserita? Per vedere se era valida! (Come probabilmente hai giα          |
| immaginato) Allora, com'Φ il nascondiglio dietro all'indirizzo           |
...
|  :? 0-FFFFFF64                                                           |
| Il numero 100000000 Φ troppo grande per SoftICE, ma da lo stesso         |
| risultato in un altro modo.                                              |
| E ora... Φ ora di guardare cosa si nasconde dietro EBD-9C... cos∞:       |
|  :d ebd-9c                                                               |
| La finestra dei dati ora mostrerα una lunga fila di numeri - il codice!  |
...
|   ADD  ESP, 08     ; Cancella le informazioni salvate                    |
|   TEST EAX, EAX    ; Controlla la funzione di ritorno                    |
|   JNZ  00402BBF    ; Salta se non Φ zero                                 |
| E cosa trovi all'indirizzo EBP-68? Allora... un altro codice             |
| di registrazione!                                                        |
|  :d ebp-68                                                               |
| Questo Φ tutto... Spero che tutto funzioni!                              |
| 3.1. Command Line 95 - Facile registrazione nome/codice, e inserimento   |
| di un keymaker                                                           |
| Questo Φ un semplice programma, con un codice di calcolazione            |
| molto facile.                                                            |
...
| numero qualunque (un numero intero Φ il codice pi∙ usato), io ho         |
| scritto questo, e ho premuto OK...                                       |
|  Name: ED!SON '96                                                        |
|  Code: 12345                                                             |
| Il programma si Φ fermato a GetDlgItemTextA. Proprio come con TaskLook,  |
...
|   CALL  [USER32!GetDlgItemTextA]                                         |
| Io numero 40A680 per noi Φ interessante guardarlo; allora controlliamo   |
| questo indirizzo:                                                        |
|  :d 40a680                                                               |
...
| GetDlgItemItem Φ simile a GetDlgItemTextA, ma ritorna ad un numero       |
...
| questa descrizione molto semplice di come il codice Φ calcolato:         |
...
| box" Φ che tutti i parametri sono stati convertiti in maiuscolo, cos∞    |
...
| Se la chiami da un programma C, la chiamata sarα come questa:            |
...
| la funzione dopo controlla come Φ posizionata nello stack e lo usa per   |
| farci qualsiasi cosa Φ stato pensato.                                    |
...
| da exe per far girare il programma. Questo Φ anche un motivo del perchΘ  |
...
| soluzione Φ un decompilatore. C'Φ un decompilatore per Visual Basic      |
|  2 & 3, creato da qualcuno chiamato DoDi. E' shareware ed Φ disponibile  |
| in rete. (Vedi Appendice C) In windows 95, ci sono applicazione per      |
| versioni di Visual Basic a 32-bit, e per loro non c'Φ decompilatore che  |
...
| La traduzione della guida Φ finita, ricordo che tutto quello scritto     |
...
| Master^Shadow, Gogetto, al Que anke se Φ sempre [zzz], Tux, Kill3xxx,    |
...
|                                                                          |
| Chiunque va in un  tribunale troverα una scritta che dice: "La legge Θ   |
| uguale per tutti"  mi chiedo quindi perchΘ in mesi si sta facendo una    |
...
| la mostruositα del terrorismo? Sia ben chiaro  quello che Θ accaduto     |
| l'11 settembre Θ stato un atto di una mostruositα inaudita che non pu≥   |
...
| definiscono bestiale Θ incivile. Veniamo dunque al perchΘ tutti i paesi  |
| si sono accomunati ad una logica sola di fare guerra al nemico della     |
| civiltα, radunando le truppe di aria, di mare e' di terra? Io credo che  |
| questa logica fa parte della nostra distorsione a vedere il male negli   |
| altri e non vederlo in noi stessi, poichΘ noi riteniamo che quello che   |
| stiamo facendo Θ giusto e sacrosanto perchΘ la ragione, e la civiltα     |
| stanno dalla nostra parte. In nome di questa ragione e civiltα noi       |
| usiamo la forza perchΘ se non la usassimo ci sentiremmo pi∙ deboli nei   |
...
| nostra incapacitα a risolvere i nostri antichi problemi di sangue, con   |
...
| nell'autodifesa non c'Θ omicidio ma c'Φ giustizia. Ma ritorniamo al      |
...
| imputati hanno diritto di difendersi, poichΘ Θ scritto nel codice penale |
...
| Quello che Θ accaduto l'11 settembre naturalmente ha fatto orrore a      |
| tutti quelli che hanno una coscienza, e un cuore, ma proprio perchΘ chi  |
| ha una coscienza e un cuore credo si debba fare un salto di qualitα      |
...
| paura e dalla rabbia, la ragione e la civiltα non centrano nulla. Noi    |
...
| Una civiltα non pu≥ mettersi sullo stesso piano di chi uccide una        |
| persona o settemila persone anche perchΘ le forze in campo sono di gran  |
| lunga sproporzionate, giacchΘ un elefante non pu≥ farsi intimidire da un |
| ratto anche se questo e portatore della peste pi∙ nera.                  |
| Quello che sta avvenendo e davvero poco intelligente per gente che       |
| dovrebbero governare questo pianeta, perchΘ come si pu≥ pensare di       |
| distruggere il terrorismo quando per 50 anni abbiamo avuto sotto gli     |
| occhi un conflitto che non si Θ ancora risolto perchΘ Ebrei e            |
...
| morti ambo le parte e non ha risolto nulla: il terrorismo non Θ morto    |
| anzi Θ arrivato fino alle torri di New York. Qualcuno dirα che non sono  |
| la stessa cosa, sta di fatto che quel conflitto in medio oriente e il    |
| vulcano dove si alimentano tutti i terrorismi poichΘ dove non c'Φ        |
...
| chiedersi perchΘ siamo arrivati a questo orrore? E giusto e sacrosanto   |
| piangere e indignarsi con rabbia, ma se noi non facciamo una             |
| autocritica profonda poichΘ quello che ci distingue dalla barbarie dei   |
...
| trascinare dalla paura, e con la paura non si pu≥ ragionare bene perchΘ  |
...
| sta facendo un grossissimo errore perchΘ si sta facendo il gioco dei     |
| terroristi perchΘ i terroristi sapevano benissimo che quando avrebbero   |
...
| deplorevoli da chi ha subito la violenza, ma una civiltα deve essere una |
| civiltα in tutto e deve prendersi le proprie responsabilitα impopolari   |
| di fronte a atti tremendi che la vogliono distruggerla .                 |
| La via che stiamo percorrendo non Θ una via di civiltα, poichΘ se        |
...
| Porgere l'altra guancia non e un atto di vigliaccheria poichΘ una        |
| civiltα che possiede i mezzi di distruzione di massa deve anche capire   |
...
| ha bandito per sempre perchΘ una barbarie come un omicidio  non pu≥      |
| essere giudicato con altra barbarie come la pena di morte.               |
| Quello che Θ successo l'11 settembre non Θ diverso dalla violenza che    |
| ogni giorno dilaga in tutto il mondo, non sono i numeri ha farci         |
| inorridire poichΘ una morte orribile di una donna fatta a pezzi da un    |
...
| processata, giudicata e condannata con i metodi della civiltα del        |
| diritto e della giustizia. Io credo che  se vogliamo veramente debellare |
| la violenza, dobbiamo andare in profonditα e capirne le cause, poichΘ se |
...
| poichΘ per distruggere un tumore bisogna sradicarlo e ucciderlo perchΘ   |
| solo nella sua morte il paziente pu≥ vivere. Questa logica non Θ diversa |
| dalla guerra, poichΘ se c'Φ un nemico bisogna distruggerlo per non       |
...
| strumenti della lotta ai tumori poichΘ avendo paura del tumore noi       |
| vogliamo distruggerlo con tutti i mezzi repressivi senza per≥ riflettere |
| del perchΘ c'Φ venuto un tumore?                                         |
...
| crediamo che prima o poi questo tipo di intervento darα il risultato che |
...
| ladro, lo stupratore, l'assassino, ecc. ecc. perchΘ se andasse in        |
| profonditα scoprirebbe il marcio che non vuol portare a galla perchΘ si  |
...
| governo di criminali, poichΘ tutto viene giustificato al proprio         |
...
| criminali perchΘ continuiamo a fregarcene della terra, poichΘ nessuno    |
| sente l'atrocitα che si sta facendo contro la stessa casa dove abitiamo  |
| tutti.                                                                   |
| Noi esseri umani stiamo distruggendo questo pianeta, e nessuno piange    |
| o inorridisce, poichΘ siamo incapaci di sentire di vedere al di lα di    |
| noi stessi, perchΘ noi sentiamo la nostra sofferenza e non sentiamo la   |
| sofferenza degli altri. Quello che sta accadendo non Θ nato dal caso, e  |
| frutto di quella sofferenza che per troppo tempo non abbiamo sentito,    |
| era li davanti a noi, e noi non sentivamo perchΘ avevamo altre cose      |
...
| angosciati, e diciamo a noi stessi; perchΘ? perchΘ ci odiano tanto?      |
...
| economico sociale perchΘ pochi barbari ci hanno attaccato nel cuore del  |
| nostro impero civile che credevamo invincibile. Il fattore umano a       |
| prevalso su quello tecnologico poichΘ a nessuno sarebbe venuto in mente  |
...
| quella dell'autoritα politica.                                           |
| Adesso non c'Φ pi∙ sicurezza su niente perchΘ il nemico della civiltα    |
| potrebbe colpire in ogni momento e il  suo messaggio di morte Θ stato    |
| esplicito, cos∞ tutti o quasi tutti si aggrappano a quelle autoritα che  |
| prima criticavano o applaudivano, perchΘ adesso la prioritα assoluta e   |
| distruggere il terrore con ogni mezzo, militare ed economico, e devo     |
| dire che ci stiamo riuscendo alla grande, perchΘ i dispacci dal fronte   |
| del pentagono dicono che tutto sta procedendo secondo i piani che le     |
| autoritα costituite hanno messo in atto per far fronte a questa nuova    |
| emergenza planetaria.                                                    |
| Noi cittadini, dobbiamo cos∞ stare alle loro parole che sarα fatto       |
| quel che sarα fatto per il bene della nostra civiltα e quella musulmana. |
| Dicono quindi che non sarα una guerra di religione ma solo una guerra    |
...
| santa.                                                                   |
| A noi questa cosa potrα sembrare assurda e persino ridicola visto che    |
| siamo entrati nel terzo millennio, poichΘ per noi che viviamo nel        |
| benessere e opulenza quel pazzo che Θ comparso sui teleschermi di tutto  |
| il mondo va solo annientato perchΘ il suo e solo un delirio              |
| farneticante.                                                            |
...
| ma parla seduto per terra mettendosi al loro livello di povertα          |
...
| intelligenti non ne abbiamo colto l'importanza, poichΘ stiamo            |
| combattendo un nemico che ci conosce bene giacchΘ lui ha vissuto tra noi |
| e si serve dei nostri stessi strumenti per farci la guerra.              |
| Dobbiamo capire che Bin Laden non Θ uno stupido ne un pazzo, perchΘ      |
| se fosse pazzo dovrebbe essere pi∙ curato che giustiziato, ma siccome i  |
| confini della normalitα e della pazzia sono sottili, quello che lui sta  |
| facendo e il modo estremistico di difendere un mondo in cui lui crede    |
| ciecamente. Nella sua logica politico religiosa c'Φ un fanatismo dettato |
...
| questo per noi Θ inaccettabile Θ incomprensibile ma bisogna entrare      |
...
| il  mezzo pi∙ efficace per sconvolgere i piani di Bin Laden poichΘ solo  |
...
| sapeva di ricevere poichΘ siamo riusciti a farci odiare ancora una volta |
| perchΘ a noi interessa Caino vivo o morto a costo di far scoppiare la    |
| fine del mondo.                                                          |
| con cuore                                                                |
|                                                            BRUNO FRANCHI |
|                                                                          |
...
| caratteristica. La veridicitα, o meno, degli eventi sarα confermata da   |
...
| arte  ha gia' regalato all'essere umano una dignitα  che non ha eguali   |
...
| coscienza Φ re nel guidare l'uomo nel  fine e nei valori in cui crede.   |
| Questo ha rivestito, riveste e rivestira' sempre un ruolo centrale,      |
| correlato alla storia dell'umanitα.                                      |
|                                                                          |
|                                                                          |
|                                                        di Virgilio Violo |
...
|  4 $strip($1-)  }                                                        |
|  ;se il primo termine e' "Non" e il terzo Φ "nessun" azzera la           |
...
|  ;se il primo segmento e' uguale a "*" o a "*D" e la dialog Φ presente   |
...
|  ;se il primo termine e' numerico e il terzo Φ "Mittente:" controlla se  |
...
| possono usare pressochΦ indifferentemente due funzioni: la WriteConsole  |
| e la WriteFile                                                           |
...
| );                                                                       |
|                                                                          |
| Come potete notare... le due dichiarazioni sono pressochΦ identiche!     |
| L'unica cosa che cambia (non a caso) Φ l'ultimo parametro che per la     |
| WriteConsole non Φ usato (va messo a NULL), mentre per la WriteFile Φ    |
...
| cambia Φ il nome della funzione. L'unica differenza stando alla SDK Φ    |
...
| pressochΦ niente. Ma ora analizziamo pi∙ in dettaglio la prima funzione  |
| (il cui significato dei parametri Φ lo stesso anche per la               |
| seconda funzione):                                                       |
...
| come paramentri (cioΦ 5 variabili da 4 bytes l'una). Il valore di        |
| ritorno Φ una DWORD ritornata nel registro a 32bit eax, come per tutte   |
| le altre funzioni dell'API di Windows. Il valore di ritorno della        |
| funzione Φ 0 se la funzione fallisce (cioΦ qualcosa va storto, cioΦ      |
| normalmente per avergli passato dei parametri sbagliati), diverso da     |
| zero se la funzione ha successo. Il primo parametro Φ uno handle al      |
| buffer di output della console. Uno handle non Φ altro che un numero a   |
...
| dato che noi vogliamo scrivere qualcosa, ci interesserα sapere l'handle  |
| per lo standard output.                                                  |
| Il secondo parametro Φ un puntatore alla stringa che vogliamo scrivere   |
| nella console. Il puntatore in questo caso non Φ altro che l'indirizzo   |
...
| funzione andrα avanti a scrivere i bytes negli indirizzi successivi di   |
| memoria (che non Φ proprio quello che vogliamo :), se indichiamo invece  |
| un numero minore della lunghezza della nostra stringa saranno scritti    |
| solo i primi N caratteri della nostra stringa, se mettiamo 0 non verrα   |
| scritto assolutamente nulla :)                                           |
| Il quarto parametro Φ un puntatore ad un buffer che conterrα, quando la  |
| funzione ritorna, il numero di caratteri effettivamente scritti in       |
| console. Normalmente il numero scritto in questo buffer sarα uguale a    |
...
| al posto dell'indirizzo del buffer. (Su Windows2000, questo non dα       |
| alcun problema, ma su Win95 mi Φ stato riferito che fa crashare per≥ il  |
...
| Per leggere dati dalla console, il procedimento Φ molto simile, basterα  |
...
| );                                                                       |
| come potete notare, la struttura delle due funzioni Φ praticamente la    |
...
| abbastanza chiaro, ed Φ molto simile a quello delle due funzioni         |
| precedenti: il primo parametro Φ l'handle allo standard input, che si    |
| ottiene dalla funzione GetStdHanle passando a questa come parametro non  |
| STD_OUTPUT_HANDLE, ma STD_INPUT_HANDLE. Il secondo parametro Φ un        |
| puntatore ad un buffer che sarα usato per contenere i caratteri letti    |
...
| buffer fosse minore del numero di caratteri letti dalla console, c'Φ il  |
...
| puntatore al buffer di 4 byte che conterrα il numero di cara             |
...
| GetStdHandle(STD_OUTPUT_HANDLE). Il secondo parametro Φ una WORD (2      |
...
| Queste ultime due costanti praticamente impostano l'intensitα del        |
...
| console del DOS. Il numero di colori ottenibili Φ quindi 16. Lo stesso   |
| discorso vale per le costanti che definiscono il colore di sfondo.       |
| Altro esempio: FOREGROUND_GREEN (or) BACKGROUND_RED farα s∞ che i        |
...
| format.com nessuno si accorgerα di niente :). L'impostazione DOS         |
| predefinita Φ FOREGROUND_RED FOREGROUND_GREEN FOREGROUND_BLUE FOREGROU   |
| ALTRE UTILI FUNZIONI                                                     |
| Sotto Windows, avete la possibilitα, quando lanciate il vostro           |
| programma, di cambiare il titolo (il caption) della finestra che         |
| contiene la console (ovviamente se non siete in modalitα tutto           |
| schermo). Per fare questo, Φ sufficiente che usiate la funzione          |
| SetConsoleTitle, definita come segue:                                    |
| BOOL SetConsoleTitle(                                                    |
|   LPCTSTR lpConsoleTitle   // address of new title                       |
| );                                                                       |
| l'unico parametro Φ ovviamente il puntatore al buffer che contiene il    |
| nuovo titolo (la fine della stringa deve essere determinata da un        |
| carattere nullo). Se avete necessitα di leggere quale Φ il caption       |
...
| );                                                                       |
| il primo parametro Φ il puntatore al buffer che conterrα il caption      |
| corrente della finestra, il secondo Φ la dimensione di tale buffer.      |
| Ovviamente il buffer deve essere sufficientemente grande per contenere   |
| quello che potrα essere il caption della finestra della console,         |
| altrimenti nel buffer vi beccherete solo i primi nSize caratteri (-1     |
| che Φ il terminatore).                                                   |
| ESEMPIO                                                                  |
...
| darα problemi, ma visto che NT usa UNICODE nativamente e voi usate       |
| stringhe ANSI, il programma probabilmente crasherα (almeno a me succede  |
...
| e poi vi chiama lui la W, ma Φ solo una mia ipotesi).  Ah, ovviamente    |
