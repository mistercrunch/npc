...
```
    ...
    ▄▀▀▀▀▀▀▀▀▀▀▀▄▄▄▄▄▄▄▄▄▄▄▀█ Opposition  Journal █▀▄▄▄▄▄▄▄▄▄▄▄▀▀▀▀▀▀▀▀▀▀▀▄
   ▐█                        ▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀                        █▌
    █       Volume: [ 1]          Month: [ Jan]           File:  [13]     █
   ▐█       Issue:  [10]          Year:  [2001]           Total: [20]     █▌
    ▀▄▄▄▄▄▄▄▄▄▄▄           ▄▄▄▄▄▄▄▄▄█▀▀▀▀█▄▄▄▄▄▄▄▄▄▄           ▄▄▄▄▄▄▄▄▄▄▄▀
                ▀▀▀▀▀▀▀▀▀▀▀                         ▀▀▀▀▀▀▀▀▀▀▀

D|amond's list of 8086 processor (non extended  non 32bit) registers
8086 PROCESSOR
-  typ b  nm     lnm                  desc
...
Besides the extra bits in the flags register, the 80286 also has five additional 
registers  used by an operating system to support memory management and
multiple processes: 

the machine  status word (msw), the global descriptor table register (gdtr),
the local descriptor table  register (ldtr), the interrupt descriptor table
register (idtr) and the task register (tr).


Discordia: After segements of memory and before. -pika 
hail discordia
EOF
```
