...
************************EDICION NAVIDE╤A************************
Vamos todavia se acerca La navidad y el A±o nuevo vamos carajo
...
Bun0 espero ke esten pasando un buen fin de a±o y unas buenas fiestas junto a los ke kieren
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 de  usuario  (Login)  y contraseña (Password)  por lo que la entrada  a  ese
...
Shell: /bin/john
 ...
 las contraseñas se encuentran 'sombreadas'.
 Si las contraseñas se encuentra sombreada,las podras encontrar en el archivo
 ...
 se  pueden  des-encriptar.  Lo que Unix hace es obtener  la  contraseña  del
 usuario,la encripta y la compara con la que ya esta encriptada, si coinciden
 entonces se le permite el acceso.

 Para poder obtener las contraseñas es necesario tener un archivo con palabras
 y usar un programa para que encripte las palabras del archivo  y las compare
 con las contraseñas encriptadas,si coinciden te avisa que palabra fue la que
 coincidio con la contraseña encriptada.
...
(he copiado las palabras que son mas usadas en las contraseñas).
...
gente los posibles ataques, pero no ense±an a como defenderse de ellos, a lo mejor, un dφa de estos me apetece y hare un
...
estan corriendo en ellos y versi≤n. 
...
los resultados. Uso: 
    ...
    NAT [-o ficheroLOG] [-u ficheroLogin] [-p ficheropass] ip ≤ rango de ips 
...
Usando el Frontpage se puede modificar una page, los pasos que siguen para modificar una pßgina son los siguientes: 
...
Y os saldra una cosa asi, si lo habeis echo bien claro: 
 ...
 A±adido nuevo trabajo con trabajo ID = 0 
...
compa±ia telefonica, lo que paso es que no cayeron en la cuenta de que todo
...
Bien, una vez que los phreakers conocieron los secretos la compa±ia telefonica
...
simple (pero efectivo), mandaban una se±al lo suficientemente corta para que
...
Nuestro colega no tendra mas que apretar un botoncito para mandar una se±al y
...
imitaba el sonido que hacen las monedas al caer, haciendo creer a la compa±ia
...
a±os y por azar silbo por el auricular de su telefono cuando escuchaba un
...
habilidad para silbar 2600 Hz y ya con 20 a±os era capaz de llamar, producir
...
ense±o a manejarlas, lo que le hizo muy popular. Una vez que Draper fue a
...
par de veces al a±o como mucho, debido a las dificultades que conllevaba esto.
...
los a±os 70. Dentro de la carcel se vio obligado a transmitir sus
...
resultado es evidente. Draper ense±o a los reclusos de la prision en la que se
...
simplemente que con el avance de las tecnologías tambien avanzaron ellos.
...
Ficheros:
  ...
  (Cuando pongo dos directorios significa que el fichero puede estar en cualquiera de esos
     ...
     utmp --> Guarda un registro (log) de los usuarios que estßn utilizando el
     sistema mientras estßn conectados a Θl. 
     ...
     lastlog --> Guarda un log del momento exacto en que un usuario entr≤
     por ·ltima vez. 
...
En algunos sistemas el fichero acct se puede llamar pacct Comandos: 
     ...
     who --> Permite saber quiΘn estß conectado al sistema en el momento en
     que ejecutamos el comando. 

     finger --> Lo mismo que el comando who, con el a±adido de que se
     puede  aplicar a otras m quinas. Es decir, podemos saber queé usuarios
     estΘn conectados a una determinada m quina en el momento en que
...
Los comandos who, finger, users y rusers toman la informaci≤n que sacan en pantalla del
fichero utmp. 

     last --> Permite saber cuando fuΘé la ·ltima vez que se conect≤ un
     usuario. 

El comando last toma la informaci≤n que saca en pantalla del fichero wtmp. 

     ps --> Permite saber queé procesos estßn siendo ejecutados por el sistema
     y que usuarios los ejecutan. 

El comando ps ofrece una informaci≤n mucho mßs completa de quiΘn estß utilizando el
sistema puesto que un usuario que no aparezca en los ficheros utmp o wtmp puede tener
procesos ejecutßndose, por lo que el comando ps ofrece la informaci≤n de quiΘn estß
ejecutando dichos procesos. En contrapartida, la informaci≤n que ofrece el comando ps es
mßs complicada de interpretar que la informaci≤n ofrecida por el resto de comandos. 

     accton --> Activa un proceso llamado accounting, que es el que
     proporciona informaci≤n al fichero acct. 

     lastcomm --> Permite saber quΘ comandos han ejecutado los usuarios. 

     acctcom --> Igual que lastcomm pero exclusivamente para Unix del tipo
     SYSTEM V. 

Los comandos lastcomm y acctcom toman la informaci≤n que sacan por pantalla  del fichero
...
Ficheros utmp y wtmp: 
 ...
 2 -Los ficheros utmp y wtmp no son ficheros de texto, es decir, no se 
     ...
     llamados zappers (debido a que el programa mßs famoso de este tipo se 
...
Fichero acct: 
 ...
 Cuando el accounting estß activado (es decir, cuando el sistema recoge 
 ...
 del todo, aunque sí se pueden reducir a una mínima informaci≤n de nuestra 
 presencia en el sistema. 

 1 -LO PRIMERO que hacemos nada mßs entrar en el sistema es copiar el 
 ...
 2 -Dejamos el fichero acct a cero bytes. Como antes, esto es bastante 
     sospechoso para un administrador, ademßs, algunos sistemas reaccionan 
     mal y paran el proceso de accounting, para no levantar sospechas habría 
     que reactivarlo con el comando accton. 

Problema: Bastante sospechoso. El propio comando accton quedaría registrado como
...
Problema: La ejecución del programa editor que borra nuestras huellas quedaría registrado
...
Syslog es una aplicaci≤n que viene con el sistema operativo Unix. El sistema
...
Para conseguir esto hay que configurar varias cosas. 
     ...
     A -Decidir quΘ programas, procesos y aplicaciones pueden generar 
         ...
         auth --> mensajes relativos a procesos de autentificaci≤n 
     ...
     B -Decidir quΘ tipos de mensajes pueden generar cada uno de esos 
         ...
         warning --> avisos. 
         notice --> cuando se da una condici≤n que no constituye un error .
         info --> mensajes informativos. 

         etc... 

     C -Decidir a quΘ ficheros van a parar dichos mensajes dependiendo del 
         tipo al que pertenezca el mensaje correspondiente. 

Syslog cumple su funci≤n mediante el syslogd (syslog daemon o en      castellano el
demonio syslog). 

NOTA:Un demonio (o daemon) es un proceso que no tiene propietario (es decir, no es
ejecutado por ning·n usuario en particular) y que se estß ejecutando permanentemente. 

El syslogd lee su configuraci≤n del fichero /etc/syslog.conf. Dicho fichero contiene
la configuraci≤n relativa a quΘ eventos del sistema son registrados y en que ficheros
son registrados. Los ficheros a los cuales se mandan los registros (logs) pueden
estar situados en la misma mßquina en la que estamos trabajando o en otra
mßquina de la red. 

C≤mo borrar las huellas relativas al syslog: 

Bien, nuestras andanzas por el sistema cuando hemos accedido a Θl y cuando nos
...
guardan los registros del syslog sí se pueden editar con un editor de textos. 

Para poder borrar estas huellas necesitamos tener privilegios de root,
naturalmente. Bastarß con examinar el fichero /etc/syslog.conf para saber los
ficheros que guardan los registros del syslog. DespuΘs miraremos cada uno de esos
ficheros comprobando que no hay ning·n mensaje relativo a nuestra intrusi≤n en el
...
coincida la fecha del ·ltimo mensaje (despuΘs de haber borrado nuestras huellas)
con la fecha del fichero. Si no lo hacemos así, alg·n administrador demasiado
suspicaz puede comprobar que las fechas no coinciden y deducir que alguien ha
modificado el fichero (esta es una      precauci≤n extrema pero la recomiendo por
experiencia). Si es necesario,      y SOLO si es necesario, habría que cambiar la
fecha de los directorios en los que estΘn incluídos los ficheros que guardan los logs.
...
root, etc. Es decir, cuanto mßs sigilosos seamos a la hora de hacernos root y menos
ruido armemos mßs posibilidades tendremos de no aparecer en este tipo de logs. 

2 - TCP-Wrapper 

Se trata de una aplicaci≤n que proporciona una serie de mecanismos para el
registro (logging) y filtro (filtering) de aquellos servicios invocados o llamados a
travΘs del inetd (internet daemon). Con esta herramienta el administrador posee un
control absoluto de las conexiones hacia y desde su mßquina. 

Puede, entre otras muchas cosas, filtrar un servicio de internet como por ejemplo el
telnet, ftp, etc de forma que nadie pueda conectarse al sistema desde otra mßquina
o puede especificar una lista de mßquinas que sí pueden conectarse (y las demßs no
podrßn). Ademßs, el administrador es informado en todo momento y con todo lujo
de detalles de las conexiones que se han hecho desde su mßquina y hacia su
mßquina con cualquiera de los diferentes servicios de internet (telnet, ftp, finger,
etc...) 

Como en el syslog, para borrar nuestras huellas del tcp-wrapper, tendremos que
buscar posibles huellas mirando el archivo de configuraci≤n (alojado NORMALMENTE
en el directorio /etc), borrar dichas huellas y cambiar las fechas de los ficheros
correspondientes. 

Bien, hasta aquí un resumen sobre c≤mo borrar las huellas. Como vereis me he
extendido un poco mßs porque me parece importante que la gente adquiera
conciencia de que tan importante o mßs que controlar el sistema (convertirse  en
root) es saber ocultarse en Θl (aunque es una opini≤n personal). 
...
configuraci≤n es posible preparar un shell script (el equivalente a los ficheros batch
en MS-DOS, aunque la programaci≤n en shell es infinitamente mßs potente :-) ) que
haga todo el trabajo por nosotros en cuesti≤n de borrar las huellas. Dicho script lo
podemos dejar bien camuflado en el sistema para que la pr≤xima vez que entremos
...
entrado, etc..) ahorrßndonos todo el trabajo pesado. 

Para terminar con lo de borrar las huellas, s≤lo advertir que aunque seamos
perfectamente invisibles en el sistema, cualquier usuario que estΘé conectado al
mismo tiempo que nosotros podría detectarnos viendo el terminal por el que hemos
entrado (el fichero /dev/ correspondiente a nuestro terminal tendría como
propietario (owner) al usuario con el que hemos entrado en el sistema, y la fecha
del fichero /dev/ correspondiente al terminal tambiΘn nos delataría). Para evitar
esto tendríamos que cambiar de owner el fichero correspondiente al terminal
...
De todas formas, esto ·ltimo, junto con lo de cambiar de fecha ciertos ficheros de
logs, son medidas quizß extremas, pero vuelvo a insistir que son muy
recomendables. 

Por ·ltimo, la cuesti≤n de ocultar o camuflar procesos mientras los estamos
ejecutando es otra cuesti≤n que se tratar  en otro mensaje si teneis la paciencia de
seguir. :-) 

Ya hemos visto de forma resumida y sin detallar algunas tΘcnicas sobre c≤mo
conseguir acceso, conseguir privilegios y borrar nuestras huellas. Vamos a ver el
·ltimo paso, c≤mo conseguir acceso a otros ordenadores una vez controlado el host
que hayamos hackeado (es decir, despuΘs de asegurarnos que hemos borrado
absolutamente todas nuestras huellas y de implantar alg·n sushi u otro mΘtodo an
logo para conseguir privilegios de root). 

Una vez controlado el host que teníamos como objetivo, podemos hacer todo lo que
...
Es en este punto donde adquiere importancia la "filosofφa" del hacker. La diferencia
...
da±arlo o corromperlo y un hacker accede al sistema simplemente para conseguir
informaci≤n o por pura curiosidad, pero nunca corromper ni borrar  ning·n fichero
del sistema, sigue el lema (aunque tampoco de forma radical, es decir, sin
tomßrselo al pie de la letra) de "se ve pero no se toca". A esto ·ltimo hay que hacer
una excepci≤n , naturalmente. Los ·nicos ficheros que el hacker modificar o borrar
serßn los ficheros relativos a los logs que haya podido dejar en el sistema. Por
supuesto que esto es una situaci≤n ideal y no realista, en la prßctica un hacker
puede que realize otras acciones en el sistema que puedan modificar ficheros ya
existentes, pero siempre procurarß que los cambios sean mínimos. 
