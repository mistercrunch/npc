...
°ñ║┤              ░ñ°╕            .$2*EST*z$.             ░ñ°╕               `░ñ°
°ñ║┤              ░ñ°╕           x*#"~```"!Rn╖$           ░ñ°╕               `░ñ°
°ñ║┤  ELECTRON    ░ñ°╕          z!~           ~@!         ░ñ°╕   ELECTRON    `░ñ°
°ñ║┤              ░ñ°╕         xZ~             `Eo        ░ñ°╕               `░ñ°
°ñ║┤  SECURITY    ░ñ°╕        $? ^%$%╖τ╖#~~╖^]τ*`$        ░ñ°╕   SECURITY    `░ñ°
°ñ║┤              ░ñ°╕        $EL"╖╖$╖$%2~$~ÇP*:$╕        ░ñ°╕               `░ñ°
°ñ║┤    TEAM      ░ñ°╕        ?B                          ░ñ°╕     TEAM      `░ñ°
°ñ║┤              ░ñ°╕         `?2L                       ░ñ°╕               `░ñ°
°ñ║┤              ░ñ°╕          `#$m.......u@#~           ░ñ°╕               `░ñ°
...
°ñ║┤ñ░`░ñ°╕╕,°ñ░`┤░ñ°╕╕,°ñ░`░ñ°╕╕,°ñ░`░ñ°╕╕,°ñ░`░ñ°╕╕,°ñ░`░ñ°╕╕,°ñ░``░ñ°╕,°ñ░`░ñ°
°ñ║┤                                                                         `░ñ°
°ñ║┤                         ELECTRON SECURITY TEAM                          `░ñ°
°ñ║┤                                                                         `░ñ°
°ñ║┤                                 # 03                                    `░ñ°
°ñ║┤                                                                         `░ñ°
°ñ║┤              Revista Electronica CHILENA, del UnderGround               `░ñ°
°ñ║┤                                                                         `░ñ°
°ñ║┤                          Informatico Mundial                            `░ñ°
°ñ║┤                                                                         `░ñ°
°ñ║┤ñ░`░ñ°╕╕,°ñ░`┤░ñ°╕╕,°ñ░`░ñ°╕╕,| 07/03/01 |░`░ñ°╕╕,°ñ░`░ñ°╕╕,°ñ░``░ñ°╕,°ñ░`░ñ°
...
en el 32║ juzgado del crimen de Santiago...  luego de que la  Subsecretaria de
...
Capa Inalßmbrica de Seguridadd de Transporte
============================================

La Capa Inalßmbrica de Seguridad de Transporte (en adelante WTLS),  constituye
una capa modular, esta  depende  del  nivel de  seguridad  requerido  por  una
determinada aplicaci≤n,proporciona a las capas de nivel  superior  de  WAP  de
una interfaz de servicio de transporte seguro,que lo resguarde de una interfaz
de transporte inferior

El objetivo es proporcionar privacidad, integridad  de datos y autentificaci≤n
...
participantes, s≤lo puede ser invocada cuando existe previamente una  conexi≤n
segura entre las direcciones de transporte de los dos participantes

SEC-Create: Se utiliza para iniciar el establecimiento de una conexi≤n segura

SEC-Exchange: Esta primitiva se utiliza en la creaci≤n de una conexi≤n  segura
si el servidor desea utilizar autentificaci≤n de clave  p·blica o  intercambio
de claves con el cliente

SEC-Terminate: Esta primitiva se utiliza para finalizar la conexi≤n.


Protocolo Inalßmbrico de Datagramas 
===================================
...
Transacci≤n y Sesi≤n), comunicßndose de forma transparente sobre  uno  de  los
servicios disponibles.

Este protocolo ofrece servicios a los  protocolos  superiores, segmentaci≤n  y
re-ensamblado opcional y  detecci≤n  de  errores  opcional, de  forma  que  se
...
datagramas. No requiere que exista una conexi≤n para establecerse

T-DError: Esta primitiva se utiliza para proporcionar  informaci≤n  a la  capa
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^
Bueno, lo primero para instalar cualquier distribuci≤n, es seleccionar el modo 
de partida en la BIOS (para entrar a la BIOS, lo mßs comñun es  apretar _supr_ 
mientras el equipo parte). Nos movemos en los men·es hasta encontar  la  parte
...
Despues  seleccionamos  _salir_guardando_los_cambios_ y   el   equipo  deberφa
reiniciar.

2. Iniciando La Instalaci≤n:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
**NOTA1: Recuerda incertar el CDROM de Debian  en  la unidad, para  que  parta
desde el CD el equipo.

Bueno, si todo ha ido bien en la BIOS, el computador deberφa iniciar  desde el 
CDROM con un mensaje de  bienvenida  a  "Debian 2.2 GNU/Linux"  y  nos  pedirß 
apretar _enter_,al hacerlo deberß partir el kernel... mostrarß algo como esto:

Loading root.bin.........
loading linux............

DespuΘs de cargar el kernel saldrß un mensaje  de  bienvenida, apretamos  _ok_

2.1 Selecci≤n del teclado:
^^^^^^^^^^^^^^^^^^^^^^^^^^
Aquφ buscamos nuestro teclado dentro de la lista:
...
cfdisk para crear la particion donde instalar Debian), aparecerß una  lista de 
Pasos a seguir. Es _conveniente_ leer los pasos a seguir, recuerda que  Debian
no es como la mayorφa de las distribuciones _Next_ ;), sino  mßs  bien  es  al
estilo _sabes_REALMENTE_lo_que_estas_haciendo_??_ (si al mßs  puro  estilo  de
...
Si continuamos el orden que nos pone Debian,lo mßs probable es que no entremos 
a cada una de estas categorφas (obvio ;), asi  que  no  te  preocupes  por  no 
"Configure PCMCIA support" ;)

2.2 Seleccionando las particiones:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Si ya habφas hecho las particiones con anterioridad  (partition magic o fips), 
tendrßs que seleccionar cual quieres que sea tu particion raiz (/), y  despuΘs 
seleccionar otras _posibles_ particiones con su punto de montaje por  ejemplo,
esta es mi distribuci≤n:
...
particiones primarias;y 5 y 6 son particiones l≤gicas. Cada una  esta  con  su 
punto de montaje.

2.3 Install Operating System:
^^^^^^^^^^^^^^^^^^^^^^^^^^^
Aquφ tenemos que seleccionar el medio por el cual  instalaremos  la  base  del 
sistema:

CDROM ==>Es lo que tenemos que seleccionar ;)
/dev/fd0 =\---Esta opci≤n es para los que quieran instalar Debian por 
/dev/fd1 =/---Diskettes (si es que alguien lo hace a·n ;)


DespuΘs de seleccionar el CDROM, nos preguntarß (en caso de  tener  mßs  de un 
CDROM) que lector  queremos  ocupar. Luego  de  eso  nos  preguntarß  el  PATH 
(directorio) donde estßn los paquetes. Dejamos por defecto la que sale:
...
IPv4 ==>Si quieres enmascarar una red o algo asφ, mira esto(ipip,ipmasq)
IPv6 ==>S≤lo para personas que quieran experimentar con IPv6
MISC ==>Todo lo que no entra en las otras categorφas(Sonido,lp,etc)
NET ==>Soporte para tarjetas de red y ppp,slip,plip
SCSI ==>Si tienes adaptadores SCSI o un ZIP, mira aquφ
VIDEO ==>Todo lo que es Video4Linux

**NOTA3:Si estas compartiendo disco duro con wilson(guin2), deberßs elegir los 
...
2.5.1 Hostname:Ponemos el nombre que nuestro PC tendrß en una RED (ej:microsoft)
^^^^^^^^^^^^^^^
2.5.2 Network Interface:
^^^^^^^^^^^^^^^^^^^^^^^^
gre0
eth0 ==>Esta es al que mßs se ocupa en las redes Ethernet o Fast Ethernet
ipddp0
Seleccionamos la nuestra (eth0)

2.5.3 DHCP or BOOTP: Aqui nos pregunta si en nuestra red existe un servidor de 
^^^^^^^^^^^^^^^^^^^^ DHCP/BOOTP, si es asφ le respondemos _yes_ y  tratarß  de
buscar al servidor (con un 99% de seguridad lo encontrarß =). Al apretar _yes_
hace un auto-test del server, quien  le  designa IP, Netmask,etc. Si  Marcaste
_yes_, tienes que pasar a la seccion (2.7)

**NOTA5:Si Trabajas con Cable modem(vtr,metropolis) o adsl con IP dinßmica (si 
no tienes idea que es una IP Dinßmica, tienes una de esas ;),tienes que marcar 
_yes_
Si elegiste _no_ en la parte anterior, pasarßs a el resto de las preguntas

2.5.4 Choose a IP adress: Por defecto la Direccion de una Red privada clase  C
^^^^^^^^^^^^^^^^^^^^^^^^^ es: 192.168.x.x 

2.5.5 Netmask: Para una red de clase C, sin subredes la mßscara es 255.255.255.0 
^^^^^^^^^^^^^^
...
2.8.1 Select Timezone: Ubicamos nuestro Paφs y  apretamos  _enter_  y  con tab 
^^^^^^^^^^^^^^^^^^^^^^ seleccionamos una de las opciones  que  entrege el paφs
(ej: Chile-->(1)Continental(2)Insular)

2.8.2 Timezone Configuration: Seleccionamos si queremos  dejar  nuestro  reloj
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ con GMT.

**NOTA6:Si tienes mßs  de  2  sistemas  operativos  corriendo (Linux y wilson) 
deberφas decir que _no_
...
a  re-instalar(wilson), temdrßs que tener un Diskett con el LILO cargado, para 
poder bootear nuestro Linux otra vez =)

2.9 Make a Boot Flopy: Aqui podemos crear un Disco de Arranque, para caso de 
^^^^^^^^^^^^^^^^^^^^^^ _desastres_

DespuΘs de Crear el disco, nos pedirß reiniciar el  computador. Es  una  buena 
opci≤n volver a entrar a la BIOS y seleccionar que parta desde  el  disco duro 
primero(C only - C,A,CDROM - etc).

Ahora ya puedes empezar a trabajar en Debian =), y no tomo mßs de 15 minutos. 
Claro que un sistema asφ, no es  mucho lo  que  nos ofrece =), asi  que  sigue 
leyendo mßs ;)

3. Debian System Configuration:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Al reiniciar el equipo, no arrancarß wilson (si es que lo tienes instalado =( ), 
debido a que debemos configurar el LILO (vi /etc/lilo.conf =). Ya al cargar el 
kernel  con  nuestros  flamantes modulos elegidos =), Debian  nos  llenarß  de 
preguntas =)

3.1 Shall I enable md5 passwords?: Contestaremos _yes_   si   nuestro  sistema 
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ estarß  aislado  de  una  red  guindows. Si
...
(john rulez). En todo caso, yo siempre contestarφa que si =)
...
^^^^^^^^^^^^^ root (como programar o compilar =). Nos pedirß un Username y  una
password para el nuevo usuario.

3.5 Remove PCMCIA: Contestaremos que _si_
^^^^^^^^^^^^^^^^^^

3.6 Do you want to use a ppp connection to install the system?: Yo contestarφa 
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ que _no_, a lo
...
3.7: How do you want choose additional softaware to install?: Aquφ     ponemos 
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ como   vamos   a
seleccionar los paquetes adicionales que vamos a instalar:

Simple ==>Es la que vas a seleccionar, sino no estarφas leyendo
Advanced ==>mmm, no creo que todavia quieras enfrentarte a dselect
Deberφa aparecer algo como esto:
...
[*] Debug 
[*] Devel Common ==>Desarrollo Com·n
[*] Dial up ==>Una Herramienta para conectarse a Inet
[ ] Dial up ISDN
[ ] DNS Server
[*] Fortran ==>S≤lo para ver lo que se hacia años atrßs ;)
[*] Games ==>Let's Fun
[ ] Gnome Apps
[ ] Gnome Desktop
[ ] Gnome Games
...
[ ] Japanese
[ ] Laptop
[*] Newbie Help ==>Yo que tu lo instalarφa ;)
[ ] News Server
...
[ ] Polish
[*] Python ==\
[*] Python Bundle ===\-->Un Potente lenguaje matemßtico
[*] Python Dev ===/-->Muy de Moda Hoy en dφa
[*] Python Web ==/
[*] Samba ==>Para Compartir Recursos con maquinas wilson
[*] Science ==>Herramientas cientφficas
[ ] Sgml
[*] Spanish ==>S≤lo ponlo ;)
[*] Tcltk 
...
[*] X Window System Core ==/-->Servidor Grßfico y eso

Las que estßn con [*] son las que yo usualmente Instalo,te recomiendo Instalar 
los: xxxx Dev, ya que traen librerias que necesitaras para  compilar programas 
despuΘs (ej:ncurses para programas en la consola,etc).

4 EL ataque de preguntas:
^^^^^^^^^^^^^^^^^^^^^^^^^

4.1 AnXious: Tratarß de configurar tu servidor grßfico ;)
^^^^^^^^^^^^
...
[ ] ==>muchos mßs
[*] xterm
Yo elijo s≤lo esas dos, cuesti≤n personal =P
...
Por posibles problemas de seguridad (xploits), en un sistema en red se deberφa 
...
**NOTA11:Cuando se instale wvdial, pon que no se  active, ya  que  asi  podrßs 
trabajar con pppconfig y pon.

5 Despues de la instalaci≤n:
...
"upgredear", configurar,( y mßs) paquetes.

**NOTA13: apt-Setup nos permite seleccionar de donde leerß  los paquetes para
...
y buscamos las siguientes lφneas:
...
6 Inet:
^^^^^^^
6.1 pppconfig: es una poderosa herramienta que nos permitirß conectarnos a Inet 
^^^^^^^^^^^^^^ por ppp. Segir las instrucciones.

**NOTA15:En la seccion DNS, fijarse en elegir Static DNS!!

6.1 Conectandose: La herramienta bßsica para esto es pon.
^^^^^^^^^^^^^^^^^ pon  _nombre_que_le_dimos_a_nuestro_proveedor_de_Inet   (por
defecto provider) plog muestra como esta nuestra conecci≤n poff desconecta  de
Inet

7 Otros: Aunque no son  cosas  especφficas  de  Debian, nunca  estan  demas =P
^^^^^^^^

7.1 Configurar el teclado en español para el entorno grßfico:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
yo lo hago asφ:

polux:~# vi /etc/X11/XF86Config

y busco la siguiente lφnea:
...
7.2 Configurar el manejador Grßfico por defecto
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
th3nc0d3r@polux:~$echo "exec manejador_grßfico">~/.xinitrc
ej:
...
Cualquier duda escribeme tambiΘn.
...
/etc/passwd o si estßn  shadowed  en  el  fichero /etc/shadow (por lo general,
podrφa ser tambien por ejemplo /etc/master.passwd).

El fichero  /etc/passwd  es  un  archivo  compuesto  por  lφneas, donde  cada 
lφnea representa un registro, compuesto de siete campos  separados  entre  sφ 
por ' : '.

Por ejemplo una lφnea podrφa ser asφ:

Little95:E6r68fgtl13mz:0:0:root:/root:/bin/bash

1║ campo('little95'): es el nombre de usuario.

2║ campo('E6r68fgtl13mz'): es el password encriptado, donde 'E6',representa la
semilla(como se le llama en criptologia, lo que es una clave en la que se basa
para encriptar el password) y el resto es el password encriptado (todo esto sΘ
vera un poco mas adelante).

Si en vez de aparecer eso aparece un ' * ' o una ' X ' en todos los campos  de
password del fichero,quiere decir que los password estßn shadowed. Si al final
del password encriptado hay una ' , ' y dos cifras mas,estas corresponden a la
fecha de expiraci≤n del password.

3║ campo(' 0 '): es el UID(User ID). UNIX no identifica a los usuarios por  su 
nombre(o nick) sino que por un numero que se almacena en este campo.
Este valor puede variar entre 0 y 60000, por lo general se usan n·meros  entre
100 y 60000 para usuarios normales y el ' 0 '  corresponde  al  super  usuario
(root).

4║ campo(' 0 '): es el GID(Group ID). Es la identificaci≤n del  grupo  al  que 
pertenece al  usuario.  Tambien  varia  entre  0  y  60000 y el ' 0 '  tambiΘn 
pertenece al grupo del root y tienen  los  mismos  privilegios  para  aquellos 
elementos que pertenezcan a su dominio.

5║ campo(' root '):este es un campo reservado para un comentario o informaci≤n
extra del usuario, como podrφa ser por ejemplo el nombre completo o algo asφ.

6║ campo(' /root '): es el directorio home de ese usuario,o sea, su directorio
de trabajo.

7║ campo(' /bin/bash '): es la shell (interprete de comandos) que  utiliza  el 
usuario. Estas sirven tambiΘn para restringir el uso  de  algunos  comandos  a 
algunos usuarios en especial, por ejemplo un usuario an≤nimo,  por lo  general
tendrß una shell muy restringida que le permitirß ejecutar muy pocos comandos.


Las contraseñas en UNIX se encriptan usando una funci≤n del kernel (n·cleo del
sistema), crypt(3). Esta funci≤n es un algoritmo  basado  en  el  estßndar  de
encriptaci≤n  de  datos (DES),  desarrollado  por  el  Instituto  Nacional  de
Estßndares y Tecnologφa(NIST).
...
repitiΘndose este proceso 25 veces. Una vez finalizado se obtiene un resultado
...
un valor de 12 bits, que se utilizan para alterar el resultado de  la funci≤n,
lo que hace que un password se pueda variar entre 0 y 4095,  por lo  que  para
cada password hay 4069 formas distintas de encriptaci≤n  (mas adelante veremos
un poco mejor la funci≤n crypt(3), junto con unos ejemplos).

Por ejemplo el programa /bin/passwd, que se usa pata cambiar la contraseña  de
un determinado usuario, calcula la semilla de 12 bits,basßndose en la hora del
sistema.
...
con el password  del  fichero. Este  mΘtodo  es  muy  lento  y  puede  incluso 
llegar a ser muy difφcil averiguar la clave.

Los  crackeadores  de  password  que  utilizan  este  sistema  son  fßcilmente
diseñables, incluso se pueden conseguir uno en 35 lφneas aprox.  ┐Por  quΘ  no
desencriptarlos? Porque la  funci≤n  crypt(3) es  una  funci≤n  muy  fßcil  de
ejecutar de ida pero extremadamente difφcil de  ejecutar  de  vuelta,  o  sea,
revertirla, lo que todavφa no se ha logrado.

El fichero /etc/shadow puede tener una  forma  un  poco  distinta (el  fichero 
/etc/passwd siempre existe y solo varia si esta el password  encriptado ahφ  o 
no). La forma seria mas o menos asφ:


Usuario : pass : ultima vez que cambio pass : dφas que deben pasar  para poder
cambiar pass : dφas despuΘs de los que debe cambiar pass : dφas  antes  de  la
expiraci≤n de la cuenta en que el usuario  debe  ser  advertido :  dφa  de  la
expiraci≤n de la cuenta : dφas  que  lleva  deshabilitada  la  cuenta : bloque
reservado

Claro que en la realidad si cave en una lφnea, como por ejemplo:

z0rbas:E6r68fgtl13mz:9753:0:1000::::

La estructura en c de una lφnea del fichero shadow seria:
...
sptime sp_min;            /* mφnimo de dφas antes del cambio */
sptime sp_max;            /* dφas a los que debe ser cambiado el password */
sptime sp_warn;           /* dφas antes de la advertencia de la expiraci≤n */
sptime sp_intact;         /* numero de dφas a los que expira */
sptime sp_expire;         /* dφas que lleva expirada la cuenta */
...
La semilla en la funci≤n puede variar entre a-z,A-Z y 0-9 y con la combinaci≤n
...
El retorno de la funci≤n,apunta a una cadena de 13 caracteres ASCII imprimible
(incluida la semilla). El retorno apunta a una variable estßtica, la  cual  es
sobrescrita en cada llamada.

El espacio de la clave consiste en 5**56(7,2*10**16) posibles valores, lo  que 
hace demasiado difφcil de encontrar la combinaci≤n.
...
password(/etc/passwd) para la autentificaci≤n, tambiΘn chequea si el 
usuario es valido. 
Retornos de la funci≤n:
...
Static int login(usuario, passwd, msg, largomsg)
       	...
       	Char *passwd;
        	Char **msg;
 	Int *largomsg;            // definici≤n de los argumentos de la funci≤n.
{
	struct passwd *pw;  // define puntero a la estructura del registro
	char *epasswd;        // puntero a lo que serß la pass encriptada
...
registro del usuario correspondiente, usando la funci≤n getpwnam() y se 
compara al tiro con NULL, si es verdadero, quiere decir que el usuario no se 
encontr≤ y el login se rechaza. */
...
/* si son distintos los dos string( epasswd, el password encriptado(instrucci≤n 
...
/* calcula la terminal de logeo y crea un log, luego retorna la funci≤n 
diciendo que el logeo ha sido exitoso */
}

-------------------- fin auth.c ------------------------

Este programa no soporta password shadowed, pero es fßcilmente 
...
/* este programa chequea un usuario y password, basado en el fichero de 
password(/etc/passwd) y soporta shadowed tambiΘn para la autentificaci≤n,
tambiΘn chequea si el usuario es valido. 
Retornos de la funci≤n:
UPAP_AUTHNAK : logeo fallido.
UPAP_AUTHACK : logeo exitoso.
En algunos casos la variable msg apunta a un mensaje en especial. 
(TambiΘn traducido por z0rbas)   */
...
En este archivo se corrigi≤ el que se aceptara el logeo  sin  password, porque
sino seria muy fßcil  para  usuarios  conseguir  una  shell  por  el  servidor 
de ppp. Para compilar este archivo se necesita editar  el  archivo  'Makefile' 
de la siguiente manera(agregar):
LIBS = -lshadow
Y se modifica la lφnea que dice:
COMPILE_FLAGS = -I.. -D_linux_=1 -DGIDSET_TYPE=gid_t
Por:
COMPILE_FLAGS = -I.. -D_linux_=1 -DGIDSET_TYPE=gid_t -DUSE_SHADOW

Toda la informaci≤n contenida en  este  texto  esta  escrita  solo  con  fines 
...
iniciar alguno de los programas de aplicacion, cuyas instrucciones empezarßn a
...
Los buses mas comunes en las computadoras son:
    ...
    una direcci≤n de la memoria para posteriormente leer los datos k contiene.
...
Asi es como encontramos:
    ...
    programas que se quieren ejecuta, asφ como los datos, los calculos
    ...
    interna.
		 ...
		 Memoria RAM : (Random access memory), aqui se puede grabar y leer
             ...
             dependiendo de la posicion que ocupan en el mapa de memoria.  Asφ
...
Este controlador puede considerarse como un ayudante de la unidad central. Asφ
...
anduvo alargando, ya pal proximo numero creo  que estarφa tirando  lo que son:
...
tipo fue...
<< íPero si nadie les pidio que lo hicieran!, la forma en  que lo hacen no  es
...
Sobre el caso the Eye ya he dado mi opini≤n en  varias  oportunidades  pero  a 
...
informaci≤n, que albergan informaci≤n relevante para el pais.  Ya  no se trata
...
por un tema muy justificado (yo creackarφa sitios que  albergaran  pornografia
...
no se da±a la imagen de la empresa o corporacion y no le quitas  su  fuente de
...
Debido a la existencia de m·ltiples vulnerabilidades en  sus  productos, Cisco
ofrece actualizaciones gratuitas de los sistemas operativos IOS  y  CatOS  que
controlan sus routers y  switches.  Se  han  descubierto  un  gran  n·mero  de
vulnerabilidades diferentes entre sφ aunque  relacionadas  que  afectan  a  la
creaci≤n  y  exposici≤n  de  cadenas  comunitarias. Estos  fallos  pueden  ser
empleados para conseguir acceso no autorizado y  visualizar  o  modificar  los
dispositivos afectados.  S≤lo son vulnerables  los  productos  Cisco  con  las
...
GSR. DistributedDirector.  Para determinar la versi≤n  del  sistema  operativo 
que se  ejecuta  en  cada  dispositivo  es  necesario  acceder  al  sistema  e
introducir el comando "show version". El software se identificarß a si  mismo,
en la segunda lφnea se mostrarß el n·mero de versi≤n del sistema, por ejemplo: 
...
y de la versi≤n instalada de IOS serß  necesario  instalar  una  actualizaci≤n
diferente, por lo que recomendamos acudir  a  la  pßgina  de  Cisco  donde  se 
precisan todos los detalles sobre las  actualizaciones  y  contramedidas  para
evitar el problema en la direcci≤n:
...
que se tratan las URLs con una construcci≤n  y  longitud  especφficas.  Si  se
envφan m·ltiples peticiones de dicha URL especφficamente diseñada podrß causar
un error en la asignaci≤n de memoria que provocarß un  fallo  en  el  servicio
IIS.   Esta  vulnerabilidad  de  denegaci≤n  de  servicios  tambiΘn  afecta  a
...
y una parte del c≤digo que conforma el servidor de correo es el  afectado  por
el problema.  Los dos  programas  contienen  el  mismo  error, que  puede  ser 
explotado de la misma forma, pero el efecto en ambos casos serß la  caφda  del
servicio IIS. En ning·n caso el servidor Exchange sufrirß  por  un  ataque  de 
este tipo, por lo que los  usuarios  podrßn  seguir  usando  sus  clientes  de 
correo de forma habitual. Tan s≤lo se ve  afectado  el  servicio  IIS, por  lo 
que en tal caso si se  verßn  afectados  los usuarios  que  hagan  uso  de  un 
cliente de correo basado en web.  Como  el  problema  ocurre  en  dos  m≤dulos 
...
en ning·n caso podrß permitir a un  atacante  la  consecuci≤n  de  privilegios
administrativos o cualquier tipo de control sobre el servidor. Por otra  parte
los servicios afectados se reinician de forma automßtica en caso de fallo, por
lo que los sistemas afectados podrßn reanudar su funcionamiento de forma  casi
inmediata.
...
estoy en tercer año de la carrera, la forma en que me  introducφ  en  el  tema 
...
suerte no naci allφ, ja ja ja ;-) hasta la proxima.
...
Hola.
Soy General Failure. Hace un tiempo les enviΘ un artφculo, y me gustarφa saber
si les interes≤ o no, te pido que me lo digas.  Se llama  "Trucos  y  consejos
practicos para Windows,Internet Explorer,Html y Javascript". Me gustarφa saber
si lo pondrßn en la pr≤xima edici≤n de la ezine, o si no les gust≤, o  lo  que
sea.

Ahora les mando este logo que hice, es una reproducci≤n en ascii del logo  que
sale en la pagina, espero tambiΘn que les guste y que lo pongan en la ezine.

Bueno, saludos a todos y cuenten conmigo si  desean  artφculos  sobre  html  y
javascripts.
...
Gracias por responder y gracias por lo de señor. En efecto todos acß  tiene un
host, lo que pasa es que me integrΘ hace pocos dφas y me estaban  configurando
una cuenta y todo eso, cuestyi≤n que ya estß lista.

Nos vemos y que estΘn bien.
...
Sigan adelante con su pßgina, MUY BUENA!!!!

"La informaci≤n debe ser libre" 
...
trate de temas en general sobr el mundo de la  computaci≤n,  soporte  tΘcnico,
driver, novedades, noticias en general.  T estare enviando la direccion  de la
misma el proximo mes. Estßs de acuerdo?  Helberth A. Correa   	
...
programas  grßficos  bastante,  sobre  todo  los  productos  macromedia  como:
...
bueno entonces íímanos a la obra!!.
...
contenido, no?. harΘ algunos diseños y los subirΘ a  mi  web  para  que puedas
opinar al respecto....trabajarΘ sobre lo que  ya  hay, es  decir  contenidos y
menus.

Tambien me gustaria poder hacer algo en flash, una presentacion o algo asφ. 

Todo esto espero tenerlo dentro de la  proxima  semana... ojalß  nos  podamos
...
°ñ║┤ñ░`░ñ°╕╕,°ñ░`┤░ñ°╕╕,°ñ░`░ñ°╕╕,°ñ░`░ñ°╕╕,°ñ░`░ñ°╕╕,°ñ░`░ñ°╕╕,°ñ░``░ñ°╕,°ñ░`░ñ°
°ñ║┤                                                                         `░ñ°
°ñ║┤         Nuestra tercera entrega llega a su final, esperamos que la      `░ñ°
°ñ║┤         revista sea de gran utilidad para sus estudios y  para  su      `░ñ°
°ñ║┤         aprendisaje.Mandamos fuerza y apoyo a 350cc (FYE) por todo      `░ñ°
°ñ║┤         todo lo que estan  pasando...  esperamos  verlos  a  todos      `░ñ°
°ñ║┤         nuevamente reunidos y ya con su 5║ entrega en nuestros  HD      `░ñ°
°ñ║┤                  Eso seria todo señores! chao a todos!                  `░ñ°
°ñ║┤                               -[ EST ]-                                 `░ñ°
°ñ║┤                                                                         `░ñ°
°ñ║┤ñ░`░ñ°╕╕,°ñ░`┤░ñ°╕╕,°ñ░`░ñ°╕╕,| 07/03/01 |░`░ñ°╕╕,°ñ░`░ñ°╕╕,°ñ░``░ñ°╕,°ñ░`░ñ°
