...
________________________________________________________________________
	  ...
	  Si se±ores , este es el primer E-Zine Chileno desde akφ
    ...
    ææææ  A Caso es esto íííRevoluci≤n!!! ..... Emociones Clandestinas .
...
☻      µµµµµµµµµµµµµµµ°ñINDICEñ°µµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµµ
	  ...
	  Bueno aki estamos comenzando el primer (si no es el ultimo)
        ...
        en la tierra . Nuestra labor sera imponer en este paφs una
...
a.- Definici≤n de computφn : Ser enajenado , que vive gracias a la
...
o como funciona la tecnologφa , si no se preocupa de aprovecharla en
...
tecnologφa o software si ni siquiera saben ocuparla , y lo que es
...
informaci≤n que salgan en nuestras pßginas son con fines educativos
...
:o) .-- si quieres aportar algo , bienvenida serß tu colaboraci≤n para
...
>>>>>> Despues del boom que a tenido internet , por la cual los
      ...
      , cargo fijo mensual = trßfico ilimitado local .. el cuento
      serφa otro . Estamos Condenados al Subdesarrollo , a ser un paφs
...
>>>>> Se ha hablado en el paφs que los indices de delincuencia han
      ...
      no hacen una dieta parlamentaria y lo que ahorren de ahφ
      ...
      sueldos se pagarφan con esa estupida inversi≤n .. al final 
      ese dinero es de todos los chilenos y los se±ores politiqueros
      la gastan para sus lujos  ..!!!ES EL COLMO ííí ... a todo esto
...
>>>>  íííí ATENCION , INFORMACION DE SUMA IMPORTANCIA !!!!
...
Hackers : Son aquellos genios de la informßtica que se dedican a 
estudiar y conocer todos los sitemas informßticos que puedan , solo por
...
seguridad que tienen etc....(yo quiero ser como ellos alg·n dφa)
...
Lamers : Son aquellos (muchos) chikos que andan por ahφ en irc ,
...
una vez al dφa su pantalla se ponφa azul con alg·n error medio extra±o
que le obligaba a resetear su equipo , cual era el problema . Yo le
respondφ que podφan ser varios pero que lo chequearφa ; el usuario me
contesto con tono burlezco .- pero como tantas cosas pueden ser??? a lo
cual repuse .--------"QUE LE VAMOS A HACER ES GUINDOWS" ..........
Recuerdan que hace algunos dφas a don Billy se le colgo el guindoze 98
en una conferencia jajaj , (para que vea don tito la mugre de software
que saca al mercado), pero ya habφa pasasdo antes en una demostraci≤n 
...
mortales puedan utilizar ... UNIX Y LINUX NO SON FACILES ; costaría
...
que podemos manejar a nuestro antojo (bueno no es tan asφ yo nunca he modificado un paquete a mano)
...
desde guindoze serφa de la sgte forma .-
...
algunas tΘcnicas BASICAS de hack ademas de visualizar los problemas y
barreras que se nos iran presentando . Este texto es especial para los
novatillos que andan por ahφ en la red preguntando como se hackea , pero
...
root       pts/1        May 24 12:30    (root@victima.com)
...
root   Epidemia          p1      31      wed 14:16   epidemia@victima.com 
...
que si ... vamos entonces a tratar de cambiar los derechos de este archivo .
  ...
  ************************************************************************
  ...
  forma absoluta hemos de seguir la siguiente Sintaxis: 'chmod XYZ archivo'.
        X representa al dueño del archivo, Y al grupo y Z al resto de
...
el archivo solo lo podφa leer el systema , cosa que solamente el root podφa
...
Probemos la siguiente tΘcnica , trataremos de copiar el archivo a otro
a ver que pasa .
...
Esta tΘcnica es mas que basica pero funciona , se trata de utilizar las cuentas default
...
o no y te desconectar  después de tres malos intentos. También mantiene un
...
entre los dem s sistemas, para acceder a esta ayuda sólo escribe HELP en el
prompt.
...
DEC-10
~~~~~~
Este es el equipo de cómputo m s nuevo que existe dentro de la linea de DEC's.
...
de obtener información sobre las personas en línea antes de entrar a él usando
el comando systat. Si ves una cuenta que lea [234,1001] BOB JONES, sería
...
Este sistema como antes había dicho, permite intentos sin limite y adem s te
...
corría en PDP-11's, pero ahora corre en otras minis. Para identificar uno de
...
que recibir s sera FAMILY: (presiona enter) depués llegar s a otro prompt
...
DECSERVER
~~~~~~~~~
Las DECs son una serie de computadoras conectadas entre sí para formar una
sóla unidad de procesamiento, la funcionalidad de estos sistemas es altamente
...
cualquier cosa, lo mejor ser  presionar algo nada sospechoso como `c' o algún
número. De ahí te presentar  con el prompt `local>'. De ahí debes ejecutar
...
comandos como MODEM o DIAL que permiten hacer uso de un módem para llamadas
externas que te permitir n marcar a BBS internacionales a expensas de la
compañía que opera el DEC.
...
nuestro correo es proyecto_r@bigfoot.com


EL PERRO CAZADOR DE MONOS *********************************************

  Habφa un tipo que querφa recolectar monos para un circo. 
  Le indican que en ┴frica habφa un tipo que agarraba monos 
  vivos. 

  Llega al ┴frica el comerciante y contacta al tipo que 
  agarraba los monos vivos. El dφa de buscar los monos el 
  recolector de monos le dice que hay que llevar un perro 
  mastφn cazador y una escopeta. El tipo le dice que el perro 
  si pero la escopeta no, que Θl quiere los monos vivos. No se 
  ...
  Asφ ocurre con muchos monitos. Al caer, el perro le hacia el 
  amor. 
  ...
  ┐Que nace de un papß argentino y una mamß empleada de servicio? 
  ...
  Fijßte chΘ, c≤mo es cierto que Dios es muy humilde, seg·n 
  dice las Escrituras. Mirß que naci≤ en BelΘn pudiendo haberlo 
  hecho en Buenos Aires. 

  -------------------- 
  - Papß, Papß, cuando yo sea grande quiero ser como vos. 
  ┐Y para quΘ hijo? 
  - Pues para tener un hijo como yo !!! 

  -------------------- 
  Habφa un argentino tan, pero tan presumido, que para su 
  ...
  Yo, t·, Θl, nosotros, vosotros, ellos. 

  -------------------- 
  Dos argentinos se encuentran. 
  DespuΘs de los saludos habituales uno pregunta: 
  - ┐c≤mo van tus cosas? 
  - Muy bien, pero muy bien chΘ, ┐o querΘs que te cuente la 
  verdad? 

  -------------------- 
  Es verdad que Dios es argentino? 
  No, todos saben que naci≤ en BelΘn... 
  Pero eso sφ, tiene su oficina en Buenos Aires. 
  ...
  - ChΘ, ┐les decimos que somos argentinos? 
  - No, que se jodan. 
  ...
  Al otro dφa el mudo esta puntual, entra y el medico le dice 

  - Bien, ahora haga todo lo que le digo. Quφtese toda la 
  ropa. 
  ...
  1.- MODESTO 
  Yo nacφ en el seno de una familia pobre y sufrφ penurias 
  ...
  sirva de precedente. Yo dirΘ cuanto quiero cobrar y el color 
  del Ferrari. 
  ...
  envißrselo en CDROM. Puede usted llamarme al m≤vil (celular) 
  o dejar el mensaje a mi secretaria. 

  5.- CIBERPLASTA 
  Si visita mi pßgina web podrß ver mi direcci≤n de correo y 
  mi n·mero de abonado del beeper. TambiΘn me conecto al canal 
  irc #curriculum de 8 a 10, y he creado un foro de discusi≤n, 
  desde el que podrß acceder a un viaje virtual en el que 
  conocerß a mi novia, a mi perro y varios trucos de hacker. 


  6.- ARTHUR ANDERSEN 
  Soy master MBA, NBA y del Universo. Me ha bastado un vistazo 
  a su empresa para darme cuenta de que tiene problemas. Yo 
  podrφa ayudarle por una m≤dica cantidad. Pero como usted no 
  tiene ni idea le cobrarΘ el doble. 
  ...
  En el verano realicΘ un curso de 3 horas y cuarto de 
  duraci≤n sobre windows 3.11 avanzado. Asimismo se programar 
  en Word y dibujar con el scandisk, asφ como modelado 
  avanzado con Excel. Pero en lo que realmente soy experto es 
  en el retoque fotogrßfico, empleo indistintamente el Winzip 
  y el Java. Tengo un primo en MΘrida que tiene Internet. 

  ┐A cußl de ellos contratamos? 
...
┐POR QUE EL POLLO CRUZ╙ LA CARRETERA? ************************
  ...
  -PROFESOR DE PRIMARIA: Porque querφa llegar al otro lado. 
  ...
  que lanzßsemos 50 toneladas de gas nervioso estuvo 
  plenamente justificado. 
  -RONALD REAGAN: Se me olvid≤. 
  -CAPITAN KIRK: Para ir donde ning·n pollo ha llegado jamas. 
  -HIPOCRATES: Debido a un exceso de humores en su pßncreas. 
  ...
  su estrategia de distribuci≤n fφsica y procesos de 
  implantaci≤n. Empleando el mΘtodo de integraci≤n avφcola 
  (MIA), Andersen ayudo al pollo a emplear sus habilidades, 
  metodologφa, conocimiento, capital y experiencias para 
  alinear a la gente, procesos y tecnologφa del pollo en el 
  ...
  abordaron durante dos dφas una serie de reuniones al objeto 
  de apalancar su capital personal de conocimiento, tanto 
  explφcito como implφcito, y de permitirles obtener sinergias 
  entre si para conseguir las metas implφcitas de la entrega y 
  dise±o exitoso e implementando un marco de valores de 
  empresa a travΘs del continuo de procesos avφcolas. Las 
  reuniones se mantuvieron en un entorno que reproducφa un 
  parque, posibilitando y creando un entorno de alto impacto 
  que estaba estratΘgicamente situado, centrado en la 
  ...
  serßn libres de cruzar la carretera sin que sus motivos se 
  cuestionen. 
  ...
  ┐A quiΘn le importa por que? El fin de cruzar la carretera 
  justifica cualquier motivo. 
  -FREUD: El hecho de que estΘs preocupado porque el pollo 
  ...
  tienen una disposici≤n genΘtica a cruzar carreteras. 
...
vendieron mßs computadores que televisores, en el que 
arrancar a los hijos del computador es mßs difφcil que 
obligarlos a tomarse una sopa de arvejas, es importante 
saber si la fiebre informßtica le ha alterado a uno el 
comportamiento normal. 


Por esto he decidido adaptar un test que circula por 
Internet con el cual usted puede deducir si estß pasando 
...
7. ┐Cuando el mΘdico le dice que esa gripe que tiene es por 
un virus que anda circulando, lo primero que piensa es en 
cußl serß el mejor programa antivirus para hacerle frente? 

8. ┐Sus amigos, haciΘndole un chiste, le cuentan a todo el 
mundo que su nombre es... 'Bon', 'Web-bon'? 

9. ┐Sin darse cuenta, anota los valores de sus cheques en 
binario? 

10. ┐Cuando estß buscando la frase apropiada para proponerle 
...
13. ┐Cuando le solicitan su telΘfono, ha suministrado en su 
lugar su direcci≤n IP? 

14. ┐Estß usted pensando en crear un sitio en Internet para 
su mascota preferida? 

15. ┐Tiene usted hijos que se llaman Yahoo, Eudora o 
Altavista? 

No se debe calificar ninguna de las respuestas 'Sφ' o 'No' a 
las preguntas de este cuestionario. Con que solo haya 
contestado 'Sφ' a una de ellas, es muestra de que usted estß 
en problemas! 
...
Hola chikos aki vamos a mostrar una forma fßcil y simple de robar nicks
...
tu ingenio ..., bueno akφ esta el resultado de tus labores :-) .
...
&lt;Mensajes> si quieres ingresar a la lista de nicks de dalnet , escribe
...
alg·n op que caera fßcil ;y si tienes suerte te calzas algun sop e incluso 
el founder , te das op y listo .....

Otros metodos de recibir claves y pass en forma facil serφa ponerte un 
...
serφa inteligente registrar algun nick parecido a este como nickser etc..
...
Asi recolectarφas gran cantidad de claves de nicks en caso de alguna 
emergencia :-) .----


Otra forma entretenida de darte op serφa a travez del DCC , la forma es
la siguiente ..
...
caφdo bien y creo que se merece un OP , se lo puedes dar ya que estoy super 
...
correo , asφ que si ya estabas inscrito te recomiendo que lo hagas de nuevo .RARETRIP
desaparece de la ecena durante alg·n tiempo pero GuyBrush , Azumlord y Cytorax , estßn
...
SET : Deberφa estar en circulaci≤n el numero 15 de SET Saqueadores Edici≤n TΘcnica , para el
dφa 15 de Junio .. esperemos que se asφ para disfrutar de este fantastico magazine ..--
...
a crear un programa que mande el kilobyte de informaci≤n al comando hello y asφ poder hacer un verdadero
programa de mail an≤nimo ... de hecho podrφa intervenir alg·n programa existente ....si lo logran 
...
y demases , asφ que espero que se animen  . La idea es compartir conocimientos
y no dejar la informaci≤n para algunos pocos , como dicen por ahφ la informaci≤n
debe ser libre .Espero disculpen todas las faltas de potografφa.. perd≤n de 
ortografφa , deben entender en las circunstancias que escribimos este E-Zine .
Aceptamos cualquier tipo de crφticas , simpre que estas sean constructivas .
No nos queremos poner al nivel de grandes E-Zine por que no lo estamos "aun" :-)
Cuando copiemos algun texto o listado , simpre dirß de donde lo sacamos
no vamos a plagiar textos anteriores y si se parece a alguno serß mera 
coincidencia .
No queremos ser como algunos E-Zine que sacan su primer numero y ahφ quedan 
...
difφcil hacer el numero Dos pero ya tenemos algo no desespereφs el siguiente
numero estarß listo esperamos que en menos de 2 meses con cueva .,.
...
ISLATORTUGA
FER             Por ser como eres . Gracias a ti , cuando tenφas viva el jam≤n y
