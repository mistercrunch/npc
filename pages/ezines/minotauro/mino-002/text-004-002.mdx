...
-------------------------------------------------------------------------------
   Lo que acá tratamos de hacer, es enumerar TODOS los posibles flags del
Thunderbyte, cosa de que no les quede la duda si su virus va a hacer saltar una
alarma bajo una circunstancia específica. En definitiva, este texto es el mismo
que el del número anterior, pero ahora mas completo.


Flag "B" - Back to entry point
------------------------------
   Este es el que indica cuando un prog. en algún lugar de todo su código, re_
   gresa al Entry Point, osea al principio... Para un COM sería un salto al
...
-------------------------------
   ...
   dan ? Bueno, la forma clásica sería ésta:
...
OBTENER_DIRECCION:
   ...
   Y la forma anti-tb sería simulando ese POP:
...
--------------------------------
   Este es el culpable de la detección de muchos de los virus...en programación
   convencional no es de lo mas utilizado :-)
   Originalmente lo que hacíamos era: leer los 2 primeros bytes del archivo, 
   tirarlos a un búffer, y comparar ese búffer con 'ZM' (ojo! con ZM y no con
   MZ debido al almacenamiento inverso de palabras).
   Por ejemplo (Suponiendo que ya tenemos los 2 primeros bytes en un búffer...)
   ...
   Pero otra forma de hacer esto, es por ejemplo, comparar byte x byte:
	...
	CMP	BYTE PTR [BUFFER], 'M'		;<--- Ojo! Acá no lo comparamos
						; con ZM porque estamos compa_
						; rando byte x byte, no todo el
						; word junto, entonces primero
						; lo comparo con M y después
						; con Z.
	JNE	NO_ES_EXE
	CMP	BYTE PTR [BUFFER+1], 'Z'
	JNE	NO_ES_EXE
	...					;<--- Si llegó hasta acá, es
	...					; porque es un EXE.
	...

Flag "S" - Search for executables
---------------------------------
   Este es el que te deschava la rutina de búsqueda de ejecutables... para un
   no_residente, es impresindible, y por lo tanto impresindible su ocultamien_
   to. Lo que originalmente hacíamos era buscar *.COM ó *.EXE en algún directo_
   rio, pero lo que podemos hacer es, en vez de buscar los *.COM ó *.EXE, bus_
   car los "*.C?M" ó "*.E?E", y después verificar si la letra del medio es una
   "O" ó una "E" respectivamente... por ejemplo:
   (Suponiendo que el nombre del file ya lo tenemos en una variable DTA_AREA)

;Verificar si es realmente un .COM y que no sea un .CAM o algo así...
;Para eso, muevo el puntero hasta el comienzo de la extensión (buscando el pun_
;to en el filename) y lo incremento en 1, para que apunte a la letra del medio
;de la extensión... que es la que nos interesa.
;Ahora busco el punto:

	MOV	CX, 13d				;13d es lo máximo que puede me_
						;dir el filename.
...
;El punto lo tiene que encontrar sí o sí, entonces incremento el puntero para
;que apunte al caracter del medio de la extensión:
...
-------------------------------
   ...
   sible. Osea, un archivo que fue creado en el año 2094 por ejemplo. Esto apa_
   rece generalmente en los archivos infectados por un stealh, y hace esto para
   comprobar infección, ya que si tiene que abrir todos los archivos para veri_
   ficar si ya están infectados, sería el virus mas lento del mundo. La forma
   tradicional es sumarle 100 o 200 al año del archivo, cosa de que si se mira
   la fecha con un dir, o con otra cosa, muestre la misma fecha (que realmente
   es la misma, pero 100 años mas adelante).
   Ejemplo:
...
-------------------------------
   Bueno, el TB avisa sobre este flag cuando encuentra que obtenemos y/ó setea_
   mos los vectores de interrupción. Una forma de cagarlo es la típica. En vez
   ...
   tar el VECTOR. El TB avisa que es código residente únicamente con interrup_
   ciónes críticas, como por ejemplo la 13h ó la 21h. Osea que lo que podríamos
   hacer, es, en vez de decirle que obtenga un vector N, que obtenga un vector
   N-1 y después incrementarle 1.. Es solo un ejemplo :
...
--------------------------------------------------
   Este flag se dispara en el momento que el TB se da cuenta de la típica com_
   paración de verificar si es el servicio de ejecución de un file (4bh) una
   ...
   sea tan evidente para el TB que estamos comparando el servicio :
		...
		CMP	AL, 4BH		; dificar ningún registro.
		XCHG	AH, AL		;

Flag "c" - No checksum data
---------------------------
   Este salta cuando el TB no encuentra el ANTI-VIR.DAT, que es un archivito
   que crea en cada directorio, donde guarda información crítica de cada pro_
   grama, como por ejemplo los primeros bytes de un COM y su tamaño ó el Entry
   Point de un EXE y otras boludeces, además de guardar el CRC del archivo ori_
   ginal. En caso de que un archivo quede infectado, y siga existiendo el ANTI-
   VIR.DAT, el TB se da cuenta que el archivo fué modificado y larga el cartel
...
--------------------------------------------------
   ...
   basura, es generalmente lo que queda luego de una encripción... Al encrip_
   tar un virus, el código que queda (obviamente no ejecutable) puede ser toma_
   do como basura para el TB. Entonces el TB interpreta esa basura como un po_
   sible resultado de una encripción, y dispara ese flag.
   Una posible solución es que el resultado de la encriptación no sea mayor que
   la instrucción mas alta. Una forma de lograr esto es rotando 1 bit, o ha_
   ciendo alguna operacion que no se sarpe..
   Otra forma es haciendo un falso exit antes del código encriptado...Con esto
...
DECRYPT:	...		;<───── Rutina de desencripción
	JMP	TE_CAGUE	;<───── Salto para no ejecutar la salida al DOS
		MOV	AH, 4Ch	;<─┬─── Código que no se ejecuta nunca
		INT	21h	;<─┘
TE_CAGUE:
		...		;<───── Continuación de la rutina de desencrip.
		¿╞A¼		;<───── Virus encriptado (basura)

   Ahora bien, como se tardo DEMASIADO en lanzar el segundo número de la revis_
   ta, ahora el TB se da cuenta del JMP que está antes de la salida al DOS.
   ...
   quedó toda una noche sin dormir y sacó la nueva versión ANTI-TB 6.20 :) es
   ...
   destruya los registros de clave y demás. El código quedaría de la siguiente
   manera :
...
DECRYPT:	...		;<───── Rutina de desencripción
	...
	JMP	TE_CAGUE	;<───── Salto para no ejecutar la salida al DOS
		...
		INT	21h	;<─┘
TE_CAGUE:
		MOV	AH, XXX	;<───── LLamada al pedo, pero que no estropée
		INT	XX	;	nada.
		...		;<───── Continuación de la rutina de desencrip.
...
-------------------------
   ...
   que el archivo está comprimido y que puede ser que en su interior contenga
   un virus.

Flag "F" - Suspicious file access
---------------------------------
   Este flag salta cuando el TB encuentra que el programa tiene código capaz de
   modificar los atributos de un file.. Se puede cagar de la misma forma que la
   mayoría: En vez de MOVer el valor del servicio en AH, PUSHearlo y POPearlo.

Flag "?" - Inconsistent EXE header
----------------------------------
   El TB nos avisa de este flag, cuando encuentra que en el header EXE hay in_
   formación incongruente.. Un ejemplo es que el header, en el campo donde
   guarda el tamaño dle file, indica un valor, pero el file realmente ocupa
   mas.. Esto podría ser causado por un programa que usa overlays, o por un vi_
   rus que calcula mal el nuevo tamaño.
...
------------------------
   ...
   1) Que el Stack Segment (SS) y el Code Segment (CS) apuntan al mismo
      segmento. Cosa de lo mas normal, porque lo típico es poner al SS unos by_
   ...
   2) Que el Stack Pointer (SP) sea impar (odd stack).. esto es debido al di_
      seño del micro, el cual no esta preparado para situaciones de, por ejem_
      lo, un desbordamiento de segmento con un SP impar.. en este caso se col_
      garía la máquina.. Esta bien, se puede usar un SP impar, pero no es lo
   ...
   El virus tiene una longitud constante, y como la longitud de separación
