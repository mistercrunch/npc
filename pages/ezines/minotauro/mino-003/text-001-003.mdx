...
;cada vez que se ejecuta un file infectado.. No tiene búsqueda transversal de
;archivos, por lo que su difusión se limita un poco.. Su rango de infeccion se
...
;del calculo del offset ß. Una vez que sabe donde esta parado el virus, hace
...
;ción 4eh (Buscar la primera entrada) ó 4fh (Buscar la próxima entrada), se nos
...
;lizados por el programa anfitrión, osea que al ser sobreescritos con los datos
...
;Aca se verifica si el mes de ejecución del file es Diciembre.. Esto se hace
...
;ciembre :-).. si es el mes correcto, me fijo si es fin de año (31 de Dic.),
;comparando DL con 31.. Donde DL contiene el dia actual.. En caso de que sea
;la fecha deseada, se ejecuta el playload, ó carga ó 'detonación' del virus.
;Como el 786 v1.01 es un virus benévolo, solo mostrara un mensaje y no dejara
...
;Con este código estoy restituyendo los primeros bytes del programa original,
...
;bytes del programa con un 'ΘXX' donde el Θ es el ASCII del jump, y XX la di_
...
;y saltar al offset 100h (De donde se carga todo el código ejecutable de TODOS
;los COM).
;------------------------------------------------------------------------------
...
;en el directorio y no busca mas.. sino, es que encontró otro file y vuelve a
