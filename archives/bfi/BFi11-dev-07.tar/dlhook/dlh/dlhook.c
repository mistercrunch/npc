/*
 *  dlhook.c <hook on datalink layer...>
 *
...
#ifdef DEBUG
#  define dbg(fmt, args...) \
	printk(KERN_DEBUG "dlhook: %s: " fmt, __FUNCTION__ , ##args)
#else
#  define dbg(fmt, args...)
#endif
...
(struct sock *sk, struct tpacket_req *req, int closing);
#endif
...
void packet_sock_destruct (struct sock *sk)
{
	...
	if (!sk->dead) {
		dbg("attempt to release alive packet socket: %p\n", sk);
		return;
	}
	...
	dbg("packet socket %p is free, %d are alive\n", 
...
static int packet_sendmsg_spkt
(struct socket *sock, struct msghdr *msg, int len, struct scm_cookie *scm)
{
...
(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt)
{
	...
	u_int32_t flow = (ntohl(*(u_int32_t *)hdr) & 0x000fffff);

	dbg("src %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", dip(hdr->saddr));
	dbg("dst %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", dip(hdr->daddr));
	dbg("len: %u  hoplimit: %u  traffic_class: %u  flowlabel: %u\n", 
			(ntohs(hdr->payload_len) + HDR_LEN), 
			hdr->hop_limit,
			(ntohl(*(u_int32_t *)hdr) & 0x0ff00000) >> 20,
			(ntohl(*(u_int32_t *)hdr) & 0x000fffff));

	/** with flowlabel > 100: drop packet **/
        if (flow > 100)
                goto drop;
#endif
	dbg("packet recieved...\n");
...
(struct sk_buff *skb, struct net_device *dev,  struct packet_type *pt)
{
	...
	if (macoff + snaplen > po->frame_size) {
		if (po->copy_thresh && atomic_read(&sk->rmem_alloc) + 
				skb->truesize < (unsigned)sk->rcvbuf) {
			if (skb_shared(skb)) {
...
static int packet_do_bind 
(struct sock *sk, struct net_device *dev, int protocol)
{
	lock_sock(sk);
...
static int packet_create (struct socket *sock, int protocol)
{
	...
	dbg("sock_init_data(sock, sk) done...\n");
	...
	dbg("sk->num %u, prot_hook initialized...\n", protocol);
	...
	if (protocol) {
		sk->protinfo.af_packet->prot_hook.type = protocol;
		dev_add_pack(&sk->protinfo.af_packet->prot_hook);
		dbg("dev_add_pack(*prot_hook*) done...\n");
		sock_hold(sk);
		sk->protinfo.af_packet->running = 1;
	}
...
(struct socket *sock, struct msghdr *msg, int l, int f, struct scm_cookie *scm)
{
	...
	err = -EINVAL;
	if (f & ~(MSG_PEEK|MSG_DONTWAIT|MSG_TRUNC))
		goto out;
	...
	skb = skb_recv_datagram(sk, f, f & MSG_DONTWAIT, &err);
	...
	err = (f & MSG_TRUNC) ? skb->len : copied;
...
#ifdef CONFIG_SOCK_PACKET
static int packet_getname_spkt
(struct socket *sock, struct sockaddr *uaddr, int *uaddr_len, int peer)
{
	struct net_device *dev;
	struct sock *sk	= sock->sk;
...
#ifdef CONFIG_PACKET_MULTICAST
static void packet_dev_mc
(struct net_device *dev, struct packet_mclist *i, int what)
{
	switch (i->type) {
...
static int packet_mc_drop (struct sock *sk, struct packet_mreq *mreq)
{
	...
	for (
		mlp = &sk->protinfo.af_packet->mclist; 
		(ml = *mlp) != NULL;
		mlp=&ml->next
	) {
...
(struct socket *sock, int level, int optname, char *optval, int optlen)
{
	struct sock *sk = sock->sk;
	int ret;
...
static int pkt_notifier(struct notifier_block *this, unsigned long msg, void *data)
{
	struct sock *sk;
	struct packet_opt *po;
...
(struct sock *sk, struct tpacket_req *req, int closing)
{
	...
	if (req->tp_block_nr) {
		...
		io_vec = kmalloc(req->tp_frame_nr*sizeof(struct tpacket_hdr*), 
								GFP_KERNEL);
		if (io_vec == NULL)
			goto out_free_pgvec;
...
static struct notifier_block packet_netdev_notifier = {
	notifier_call:	pkt_notifier,
};

#ifdef CONFIG_PROC_FS
static int packet_read_proc
...
struct proc_dir_entry *fproc (void)
{
...
{
	npf->create = save_packet_create;
	dbg("resetted original packet_create func...\n");

#if defined CONFIG_PROC_FS && defined (LINUX_24)
	if (p) {
		dbg("p->read_proc %p...\n", p->read_proc);
		p->read_proc = save_read_proc;
		p->data = NULL;
...
static int packet_init (void)
{
	...
	save_packet_create = npf->create;
	dbg("saved original packet_create func...\n");
	npf->create = packet_create;
	dbg("setted our packet_create func...\n");
	...
	if ((p = fproc())) {
		...
		save_read_proc = p->read_proc;
		dbg("p->read_proc %p...\n", p->read_proc);
                dbg("old_read_proc %p...\n", save_read_proc);
		dbg("saved old_read_proc...\n");
		p->read_proc = packet_read_proc;
